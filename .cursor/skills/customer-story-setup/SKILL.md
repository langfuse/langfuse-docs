...
1. **YAML frontmatter** at top: `title`, `date`, `description`, `ogImage`, `tag: customer-story`, `author` (display string if needed), `customerLogo`, `customerLogoDark`, optional quote fields, `showInCustomerIndex`.
2. **Imports**: `BlogHeader`, `CustomerQuote` (if quote), `ImpactChart` (if impact section), `CustomerStoryCTA`; use `Frame` for images (global in MDX).
...
1. **`content/customers/meta.json`**  
   ...
   - **Order matters** for layout: the Users grid and homepage carousel are sorted by **`sortCustomerStoriesByMetaOrder`** in `lib/sortCustomerStoriesByMeta.ts`, which reads this list. Place the slug where the card should appear (e.g. last in the list = bottom-right in a 2-column grid).

2. **Authors**  
   - If new author: add to `data/authors.json`** (key must match `BlogHeader` `authors`).

...
Deprecated. Endpoints will be sunset on Langfuse Cloud on <V4CutoverDate />; self-hosted will return 404 when calling these endpoints on v4 in `events_only` mode. The [migration guide](/faq/all/deprecated-api-migration) is the canonical per-endpoint mapping with parameter tables and before/after examples; it also documents the [deprecated endpoints](/faq/all/deprecated-api-migration#endpoints) themselves.

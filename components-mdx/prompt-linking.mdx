...
Use `propagate_attributes(prompt=prompt)` to link a prompt version to all generations created within its context. This works for generations created with the Langfuse SDK and for generations emitted by third-party OpenTelemetry instrumentation. This option is available in Python SDK 4.14.0 and later.

**Langfuse SDK generations**
...
The same pattern links generations created by instrumentation libraries where you do not create the generation through the Langfuse SDK yourself. For example, with the [LiteLLM OpenTelemetry integration](/integrations/frameworks/litellm-sdk):
...
This also works with the OpenAI Agents SDK and OpenInference instrumentations exported through the Langfuse Python SDK. Only generation observations are linked to the prompt. If a generation sets its own prompt explicitly, that prompt takes precedence.

Alternatively, set the prompt on a specific generation with one of the following Langfuse Python SDK methods. For more information, see the [SDK documentation](/docs/observability/sdk/python/instrumentation).

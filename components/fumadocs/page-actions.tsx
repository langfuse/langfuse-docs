// Forked from fumadocs-ui page-actions (16.15.x) solely to add the Docs MCP
// server item. Native ViewOptionsPopover has no extra-items API.
// Remove this fork and import from fumadocs-ui again if upstream adds a slot
// for custom actions, or if we drop the MCP callout.
...
export function ViewOptionsPopover({
  ...
  const items = useMemo(() => {
    ...
    return [
      ...
      {
        title: "Install Docs MCP server",
        href: withBasePath("/docs/docs-mcp"),
        icon: <IconMCP />,
      },
    ].filter((v) => !!v);

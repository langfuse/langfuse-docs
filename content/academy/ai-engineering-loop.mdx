...
Capture the full path of a request, including prompts, retrieved context, tool calls, outputs, latency, and cost. Tracing is the raw record of what your system actually did. [→ Read more](/academy/tracing/overview)

### Monitor [#monitor]

Track how the system behaves over time and surface the traces that deserve attention. Monitoring turns a stream of raw data into an ongoing understanding of how the system evolves. [→ Read more](/academy/monitoring/overview)
...
Turn real scenarios surfaced through monitoring into repeatable test cases. Instead of testing against a handful of hand-picked examples, you build a set that reflects how the system actually gets used. [→ Read more](/academy/datasets/overview)

### Experiment [#experiment]

Change one variable at a time — a prompt, a model, a retrieval strategy — and compare it against a stable baseline. That way you know what actually improved instead of guessing. [→ Read more](/academy/experiments/overview)

### Evaluate [#evaluate]

Decide whether results are good enough to ship using manual review, code-based checks, or LLM judges. Evaluation is how you turn a comparison into a decision. [→ Read more](/academy/evaluate/overview)
...
[→ Start with Tracing](/academy/tracing/overview)

...
That goal defines the dataset boundary and job: end-to-end datasets use the payload the application receives, while step-level datasets use the structured state a step sees in production. Exploration datasets can change while you learn; regression, CI, and benchmark datasets should stay stable and get stricter review.

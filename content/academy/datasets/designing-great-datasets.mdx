...
- the datasets in the [Langfuse demo project](/docs/demo) and in our [Academy examples](/academy/examples) to get an idea of a dataset scope
...
At this stage, you only need to decide how each dataset item will be reviewed: by an automated evaluator, manual annotation, or both. For the deeper work of designing the evaluator itself, see the [Academy page on evaluation](/academy/evaluate).

If you cannot yet name the evaluator or review rubric that will score each row, go for a review workflow first and use [annotation queues](/docs/evaluation/evaluation-methods/annotation-queues) to label examples manually.
...
<Callout type="info">
Once you have decided on an item schema, enforce it so your team can effectively collaborate on adding items [in the right structure](/docs/evaluation/experiments/datasets#schema-enforcement).
</Callout>
...
Once the goal, distribution, evaluation method, and schema are concrete, start turning selected source examples into [dataset items](/docs/evaluation/experiments/datasets). Use the sources you inspected earlier, and add them in a format that matches the defined schema.
...
Do not wait until the dataset feels complete. Run the first coherent version in an [experiment](/docs/evaluation/core-concepts#experiments) before adding more rows.
...
- **Bad-trace expansion:** add a reviewed dataset item whenever you find a serious production failure. This works well once the system is live and you can continuously mine traces. For scoring, use automated evaluators such as [LLM-as-a-judge](/docs/evaluation/evaluation-methods/llm-as-a-judge) or [code evaluators](/docs/evaluation/evaluation-methods/code-evaluators), or send examples to [annotation queues](/docs/evaluation/evaluation-methods/annotation-queues) for manual review.
...
- Anything that is additional information, notes, comments belongs into metdata. Not into expected outputs.

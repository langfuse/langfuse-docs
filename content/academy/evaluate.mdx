...
todo: most production applications will require continuous labeling by human experts, this is not a one-and-done
...
Both code-based and LLM-as-a-judge evaluators can be either reference-based or reference-free. Whether you need a reference depends on the evaluator — see [different ways to use the expected output field](/academy/datasets#different-ways-to-use-the-expected-output-field) in the datasets section for more detail.

todo: this is a circular refernce where both pages do not exp explain it; give list of examples here; share advantage of reference-free that it can be applied on unseen production data for monitoring and does not just work in offline experiments; example: same-language is reference-free, correctness is reference based
...
todo: add link to error analyis blog here
...
todo: delete table, this is a repetition of the above, make this section about having evaluators that have a specific goal each, and then combining them to have a view on overall quality
...
todo: put this at the top which helps us explain the process once, currently we explain/mention error analysis 3 times on this page, I want this to be more concise by introducing it top-down

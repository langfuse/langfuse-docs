...
3. Set up an automated evaluator only when you need to test that failure mode repeatedly across many inputs or over time. The deeper dive on [writing good evaluators](/academy/evaluate/writing-evaluators) covers how to build one you can trust.
...
<ManualGuideList
  ...
  guides={[
    ...
    },
    {
      ...
      topic: "Evaluate and improve structured extraction",
      lede: "Score document extraction per field with a labeled dataset and catch regressions that pooled accuracy hides.",
    },
    {

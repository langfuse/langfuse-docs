...
3. Set up an automated evaluator only when you need to test that failure mode repeatedly across many inputs or over time. The deeper dive on [writing good evaluators](/academy/evaluate/writing-evaluators) covers how to build one you can trust. When you are ready to score live production traces in Langfuse, follow the [online evaluation get started](/docs/evaluation/get-started/online).

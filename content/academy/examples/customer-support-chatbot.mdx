...
<TraceViewDiagram
  ariaLabel="The observations inside a draft-support-reply trace"
  rows={[
    {
      kind: "trace",
...
In this phase, the agent drafts a reply for every incoming ticket, and a support team member can use it and edit it before sending. We can use the difference between the agent's draft and what the team member actually sends as an [outcome signal](/academy/monitoring/capturing-signals#outcome-signals), the most reliable kind of [user signal](/academy/monitoring/capturing-signals).

A second, [explicit rating](/academy/monitoring/capturing-signals#explicit-ratings) signal is a thumbs up or thumbs down button on the draft, where the team can also leave written feedback.
...
The bot now answers customers directly, and the signal that drove phase 2 disappears: nobody edits the reply before the customer sees it. Instead, we introduce new [conversation and outcome signals](/academy/monitoring/capturing-signals#four-kinds) that will help us learn and improve over time.
...
Customers rarely rate their support chat, so the main focus is on [conversation signals](/academy/monitoring/capturing-signals#conversation-signals), with a one-question rating kept in place for the few who do:
  ...
  evaluators={[
    ...
    },
    {
      ...
      details: (
        <>
          A form of{" "}
          <a href="/academy/monitoring/capturing-signals#conversation-signals">implicit feedback</a>: a
          customer repeating themself means the previous answer didn't land.
    ...
    },
    {
      ...
      type: "Human",
      check: "One-question rating when the conversation closes",
      returns: "binary, on the session",
      details: (
        <>
          Collected in the chat widget when the conversation closes: a
          single thumbs up or thumbs down, with an optional free-text comment.
          The comment is stored on the score, so the customer's own wording is
          there when the conversation gets reviewed. Attached to the session
          rather than a trace, since the customer is rating the conversation
          and not one specific reply.
        </>
      ),
    },
  ]}
...
The strongest signals in this phase come from the ticketing system rather than from the chat: [outcome signals](/academy/monitoring/capturing-signals#outcome-signals) that show what happened after the conversation ended. They arrive days later, so they are written back to the session by its ID:

<EvaluatorBlock
  evaluators={[
    {
      name: "ticket_reopened",
      type: "Ticketing event",
      check:
        "The customer contacts support again about the same issue within seven days",
      returns: "binary, on the session",
      details: (
        <>
          No evaluator needed: the ticketing system writes the score when a
          ticket is reopened or a new ticket references the same issue. A
          conversation that ended as <code>resolved</code> and comes back was
          not resolved. This is the phase 3 counterpart of{" "}
          <code>edit_type</code>: the environment's verdict on the reply,
          arriving late but hard to argue with.
        </>
      ),
    },
    {
      name: "handoff_resolution",
      type: "LLM-as-a-judge",
      check:
        "Classifies what the human did after a handoff: confirmed the bot's answer, corrected it, or handled a request the bot could not",
      returns: "categorical, on the session",
      details: (
        <>
          Runs only on handed-off conversations, once the human has replied.{" "}
          <code>corrected</code> marks the bot failures worth opening one by
          one. <code>confirmed</code> means the customer wanted a human rather
          than a better answer, which asks for a different fix than a wrong
          reply.
        </>
      ),
    },
...
Two things to look out for when [reading these signals](/academy/monitoring/capturing-signals#how-to-interpret-signals). Handoff and repeat rates differ by ticket category and by customer cohort, so compare a category against its own history rather than reading one overall rate. And customers who give up silently never ask for a human, so they show up as `abandoned` as the `session_outcome` score.
...
With these signals, there is a good setup to continuously improve the agent: [alerts](/docs/observability/features/alerts) on handoff and reopen spikes and a filtered view of negatively scored conversations surface the bad ones, which get improved and become dataset items, to test against structurally. The team can also use this setup to safely try out a newer/cheaper/faster model, and make an informed decision on whether to deploy it or not.

...
| **Variable**           | The configuration you're changing — model, prompt, context, tool access, or agent architecture. See [On variables](#on-variables) below. |
...
It often helps to change only one variable at a time. However as variables are interacting some configurations require changing multiple variables at once.
...
Don't set up the full evaluation pipeline before running anything. A few examples side traces side by side will teach you more in the first hour than a week of infrastructure work. Many of the early issues can be fixed by manually iterating back and forth based on traces the system produces for your inputs.

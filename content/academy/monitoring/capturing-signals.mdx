...
Unlike traditional online evaluation, capturing signals does not directly evaluate the quality of the agent's work. Instead, it captures things a user did or said that indicate whether the user perceives the agent's work as good or bad: a thumbs down, a retry, a request for a human, etc.

<Callout type="info">
**Signals measure perceived quality at the moment of use.**
</Callout>
...
![Comparison of evaluation methods by scalability and value of discovering new cases](/images/blog/trace-evaluation-methods/scalability-discovery-tradeoff.png)

## Four kinds of signals [#four-kinds]

So what kinds of signals are we talking about? We can divide user feedback signals into four kinds: explicit ratings, behavioral signals, conversation signals, and outcome signals.
...
<Callout type="info">
Separate steering from correction. In this [music DJ example](/academy/examples/music-streaming-dj), "play something calmer" is normal use of the feature and "I said calmer" is an indication the agent didn't deliver what the user wanted; only the latter is an actual failure.
</Callout>
...
They are also often the most complex to implement.
...
Once relevant signals are implemented, you can look at your traces as labeled for that signal: labeled negative, labeled positive, or unlabeled.
...
Signals can be very useful, but you should also watch out for inherent biases. Often, you won't get a signal from the entire population of users. You can split these kinds of biases into:
...
Signals are slightly different from explicit metrics and quality evaluators. Don't replace your [goal metrics](/academy/evaluate/choosing-what-to-evaluate) purely with signals. As for what to optimize for, prefer signals closest to what the product is for. Otherwise you might start optimizing for behavior you don't want.
...
The [Langfuse Agent Skill](/docs/api-and-data-platform/features/agent-skill) can inspect your application and existing scores, propose the signals worth capturing, and help implement the ones you choose.

<AgentPromptCallout
  prompt={`I want to capture useful user feedback signals for my LLM application.
Install the Langfuse skill (https://github.com/langfuse/skills/tree/main/skills/langfuse), help me identify what user signals I should capture.`}
  lede="Paste this prompt into your coding agent. The Langfuse skill inspects your application and proposes signals grounded in the workflow your users already follow."
/>

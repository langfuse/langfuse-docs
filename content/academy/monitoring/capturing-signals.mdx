...
Once relevant signals are implemented, every trace, or every session for signals that describe a whole conversation, sits in one of three groups: labeled negative, labeled positive, or unlabeled, with unlabeled being the large majority.

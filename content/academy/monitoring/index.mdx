...
## Metrics and signals [#metrics-and-signals]

It helps to separate monitoring into two distinct activities, because they answer different questions.

**[Aggregate metrics tracking](/docs/metrics/overview)** tells you whether things are getting better or worse over time. Cost, latency, evaluation scores — these become trends you can watch and reason about. Did that prompt change last Tuesday improve anything? Is quality drifting as usage grows?
...
Beyond the built-in fields, you add evaluation — user feedback (explicit ratings or implicit signals like session abandonment), human annotation, and automated evaluator scores — by annotating traces manually or running evaluators where they apply. That data feeds into aggregate charts for tracking trends over time, and into signal rules so individual traces surface when something crosses a threshold you care about.

### Explicit and implicit user feedback [#user-feedback]

[User feedback](/docs/observability/features/user-feedback) is one of the richest sources of signal, but it comes in two forms with different tradeoffs.
...
_A customer support chatbot embedded in a SaaS company's help center. Users can rate the conversation after it ends, and can request a human handoff at any point during the chat._
...
_An invoice-processing tool that extracts structured fields (vendor, total, due date) from uploaded PDFs. Each extraction is reviewed by a human in a downstream approval step before being written to the accounting system._
...
- [LLM-as-a-judge](/docs/evaluation/evaluation-methods/llm-as-a-judge) (for quality signals or behavioral patterns like user disagreement)
- [Code evaluators](/docs/evaluation/evaluation-methods/code-evaluators) (for precise checks on observations or experiments, not traces, like whether the response contains a certain word or exceeds a length limit).

...
seoTitle: "Trace LLM Apps with the Langfuse Decorator"
date: 2024/04/24
...
<Callout type="info" emoji="💡">
  _**Note:** This blog post references our Python SDK v2, including the
  `from langfuse.decorators import observe` import path, which no longer
  exists. In the current OpenTelemetry-based Python SDK the decorator is
  imported with `from langfuse import observe`. Please follow the current
  [Python SDK docs](/docs/observability/sdk/overview) and the [v2 to v3
  upgrade path](/docs/observability/sdk/upgrade-path/python-v2-to-v3) instead
  of the code samples below._
</Callout>
...
<Video
  src="https://static.langfuse.com/docs-videos/decorator-complex-trace.mp4"
  gifStyle
  aspectRatio={16 / 9.93}
/>

...
Export the values of your free-text score from the Langfuse annotation job (you can [query scores via the Langfuse API](https://langfuse.com/docs/api)). You can use an LLM to perform an initial clustering of these notes into related themes. Review and manually refine the LLM's output to ensure the categories are distinct, comprehensive, and accurately reflect your application's specific issues.
...
Langfuse has a built-in [online evaluation](/docs/evaluation/evaluation-methods/llm-as-a-judge) feature that runs LLM judges automatically on new traces, worth checking before writing anything custom.

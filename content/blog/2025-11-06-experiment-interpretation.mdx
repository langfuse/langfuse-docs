...
For the current Langfuse v4 workflow, see [Compare experiments](/docs/evaluation/experiments/compare-experiments).
...
Evaluators score the quality of the task's output for a single item. They receive the input, output, and optional expected output, and produce scores (e.g., `relevance`, `correctness`). Often if expected output is provided evaluators compare actual and expected output. See [Evaluation methods](/docs/evaluation/overview) for choosing evaluators.
...
2. **Inspection:** Open the filtered cases and compare their outputs and score explanations.

This filtered list is the work queue.
...
<Cards num={1}>
  ...
  <Card title="Error Analysis" href="/academy/monitoring/error-analysis" icon={<FileCode />} arrow />
</Cards>

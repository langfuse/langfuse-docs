...
seoTitle: "LLM Evaluation: Methods and Best Practices"
date: 2025/11/12
...
| Typical tools | Experiments, CI gates                                     | Sampled LLM-as-a-judge, code evaluators, user feedback, alerts |
...
Offline gates cannot catch what production sends you, so the last step is watching your evaluation scores continuously. In Langfuse, [score analytics](/docs/evaluation/scores/score-analytics) and [custom dashboards](/docs/metrics/features/custom-dashboards) trend every score over time, and [alerts](/docs/observability/features/alerts) fire a notification through Slack, webhooks, or GitHub Actions when a metric leaves the range you expect, for example when average cost per trace crosses a ceiling or a quality score falls below your bar.

As of July 2026, dashboards and alerts also support boolean scores: the average of a boolean score is the share of `true` results, so you can chart a hallucination-detection rate or a policy-check pass rate and alert when it crosses a threshold.

→ **[Alerts](/docs/observability/features/alerts)**
...
<Details id="full-trace-evals">
<Summary>How do I evaluate a full agent workflow instead of a single LLM call?</Summary>

Target the root observation of the workflow: with current Langfuse SDKs, the root observation records the overall input and output of an application or agent invocation, so an observation-level evaluator on the root sees the end-to-end result. If the judge needs intermediate context (for example, which documents were retrieved), write that context onto the root observation from your application code. Trace-level LLM-as-a-judge evaluators continue producing results on Langfuse Cloud until <V4CutoverDate />; on self-hosted, they will stop producing results once you run the v4 server in `events_only` mode. Upgrade to observation-level evaluators before the cut-off to avoid a gap in your evaluation results. Observation-level evaluators complete in seconds instead of minutes, and there is a [migration guide](/faq/all/llm-as-a-judge-migration) for moving over.

</Details>

<Details id="evals-without-rerun">
<Summary>Can I run evaluations on production traces without re-running the LLM?</Summary>
...
<Details id="judge-cost">
<Summary>How much does LLM-as-a-judge cost, and how do I keep it under control?</Summary>
...
<Details id="judge-model">
<Summary>Which model should I use as a judge, and how do I configure it?</Summary>
...
<Details id="session-evals">
<Summary>Can I evaluate whole sessions or multi-turn conversations?</Summary>
...
<Details id="pairwise-evals">
<Summary>Can I run pairwise comparisons between two prompt or model variants?</Summary>
...
<Details id="ci-gating">
<Summary>How do I block a deployment when evaluation scores regress?</Summary>
...
<Details id="evals-as-code">
<Summary>Can I manage evaluators as code in Git?</Summary>

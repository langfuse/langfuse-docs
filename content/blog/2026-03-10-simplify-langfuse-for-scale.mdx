...
For you, this means a single table where every LLM call, tool execution, and agent step is directly queryable — no more opening traces one by one to find what you need.
...
<Frame className="flex-1">
  ![Langfuse gets Faster](/images/blog/2026-03-10-simplify-langfuse-for-scale/og.jpg)
</Frame>

With this change, we are moving to an observation-centric data model based on a new, wide, and (mostly) immutable ClickHouse table.
This eliminates joins and deduplication at read-time and optimizes for the most performant ClickHouse access patterns.
Initial table loads for large amounts of data go from seconds to milliseconds.
Dashboard load times for large projects improve by at least 10x for longer durations.
...
Two years ago, LLM API calls had high latencies and users wanted to see their traces in our UI as early as possible.
...
As OpenTelemetry observations are immutable, all observations ingested into Langfuse are immutable.
...
While growing 19x in terms of data processed through the year, our ClickHouse node-sizes grew 15 times.
Still, some queries on longer durations produced out of memory terminations.
Eventually, we knew that adding more compute would not be a practical strategy to handle the growth.
...
In addition, scaling linearly with throughput is a nice property compared to fixed infrastructure components for deployments that vary widely in traffic.

The first win on cost reduction came with OpenTelemetry.
Because observations are immutable, we could write an entire batch as one object and fetch it as one \- no merging required.
...
<Frame className="flex-1">
  ![Scaling Milestones](/images/blog/2026-03-10-simplify-langfuse-for-scale/milestones.png)
</Frame>

### Experiments with AggregatingMergeTrees

Before deciding to denormalize trace attributes to observations, we tried to cut S3 costs and improve performance while keeping the two-table structure.
...
With thousands of observations in a single agentic trace, an observations-first view is more useful: find your most expensive interaction, drill into a specific agent, or find failed tool invocations.
...
The broader observability community has been converging on this approach for years.
Charity Majors describes it as [Observability 2.0](https://charity.wtf/tag/observability-2-0/): instead of scattering telemetry across separate metrics, logs, and trace stores, you keep one unified source of truth — arbitrarily-wide structured events in columnar storage, queryable by any dimension.
Ivan Burmistrov's widely circulated essay ["All you need is Wide Events"](https://isburmistrov.substack.com/p/all-you-need-is-wide-events-not-metrics) makes a similar case: the separation into three pillars is artificial — what matters is a single table of richly attributed events that you can slice and dice without predetermined aggregations.
Uber's engineering team arrived at the same conclusion when they [rebuilt their logging platform on ClickHouse](https://www.uber.com/en-DE/blog/logging/), moving from Elasticsearch to a schema-flexible columnar model where wide, structured events with 40+ fields are the primary unit of analysis.

As part of ClickHouse, Langfuse is uniquely positioned to take this further.
The ClickHouse team has written about [the convergence of observability and business analytics](https://clickhouse.com/blog/ai-redrawing-database-market#a_unified_data_platform) — the insight that API calls are purchases, errors are failed transactions, and separating operational data from business data is an outdated convention.
With Langfuse running natively on ClickHouse Cloud, the same infrastructure that powers your LLM observability can query alongside your business data: correlate model costs with revenue, join agent performance with user engagement, or build dashboards that span both operational and product metrics — all without moving data between systems.

For Langfuse users, this means the observations table is now the primary workspace.
Filters, sorting, and [saved views](/changelog/2025-05-20-save-table-views) replace the old traces-table browsing.
...
But discovery starts with the flat table: filter by model, cost, latency, user, session, or error status to find the observations that matter, then drill into the trace for context.

### What this means for how you use Langfuse

This is not a Langfuse-specific pattern — the broader observability community has converged on observations-first exploration as the default, from [Observability 2.0](https://charity.wtf/tag/observability-2-0/) to [Wide Events](https://isburmistrov.substack.com/p/all-you-need-is-wide-events-not-metrics) to [Uber's ClickHouse-based logging platform](https://www.uber.com/en-DE/blog/logging/). You are learning a portable mental model, not a proprietary one.

For users upgrading to the new experience, here is the practical shift:

- **The observations table is your starting point.** Instead of scanning a list of traces, use filters to surface the operations you care about — expensive LLM calls, errors, slow retrievals. Each row is one operation with all its context (input, output, cost, latency, metadata) directly available.
- **Saved views replace the traces table.** Create a saved view that filters for root observations to get the familiar one-row-per-trace view. Then create additional views for your most common debugging workflows. Most teams find they need 2-3 views to feel productive.
- **`trace_id` is a filter, not a destination.** When you want to see all operations in a single request, filter by `trace_id`. When you want to see all operations for a user, filter by `user_id`. They work the same way — `trace_id` is no longer special.
- **The trace waterfall is a drill-down, not the entry point.** Click any observation row to open its full trace context. The flat table is for discovery; the waterfall is for investigation.
...
Based on observed data distributions, this is the ideal tradeoff of pushing most trace attributes to respective observations while making data available quickly in the new UI.
For newly incoming observations, the full data is propagated, but past observations remain as they are.

<Frame className="flex-1">
  ![Dual Write](/images/blog/2026-03-10-simplify-langfuse-for-scale/dual_write.png)
</Frame>
...
This was due to our individual rows being huge with multiple Megabytes of strings and maps.

...
With this change, we are moving to an observation-centric (spans) data model based on a new, wide, and (mostly) immutable ClickHouse table.
...
Datadog's [Trace Explorer](https://docs.datadoghq.com/tracing/trace_explorer/) defaults to a flat list of spans, using faceted filters and saved views as the primary navigation mechanism.
Honeycomb built their entire platform around the idea that [structured events are the basis of observability](https://www.honeycomb.io/blog/structured-events-basis-observability) — one span equals one event, and traces are just a visualization you stitch together from events sharing a trace ID.
...
This is not a Langfuse-specific pattern — Datadog, Honeycomb, and the broader observability community have converged on spans-first exploration as the default. You are learning a portable mental model, not a proprietary one.
...
- **Saved views replace the traces table.** Create a saved view that filters for root spans to get the familiar one-row-per-trace view. Then create additional views for your most common debugging workflows. Most teams find they need 2-3 views to feel productive.
...
Based on observed data distributions, this is the ideal tradeoff of pushing most trace attributes to respective spans while making data available quickly in the new UI.

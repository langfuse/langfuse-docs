...
seoTitle: "An Agent Skill That Synthesizes Feedback"
date: 2026/03/13
...
**1. Tracing Reliability & Data Loss** _(Severity: High)_
...
**3. Token Count & Cost Calculation Errors** _(Severity: High)_
...
**7. SDK Upgrade Path Pain** _(Severity: Medium)_
...
**Langfuse v4 preview toggle causes traces to disappear or queries to slow down**
Multiple users reported that enabling the new Langfuse v4 preview experience caused traces to stop appearing entirely or dashboard queries to take longer. One user had to switch back to the legacy implementation to get traces flowing again.
...
- Sources:
  ...
  - [Circleback: Langfuse Friday Demo](#) - Team acknowledged the Langfuse v4 preview launched this week; lazy loading fixes shipped or nearly shipped
...
The second version flagged a six-month-old GitHub issue as a current bug because it had a recent comment. The comment was just someone confirming a workaround. For any agent working with time-scoped data: don't just check _when_ something was updated, check _what_ the update actually says.

### Category names carry implicit assumptions

The third version introduced a "Quick Fixes" category, but AI doesn't know how quick a fix will be. We renamed it to reflect what we _do_ know: whether the path forward is clear or needs discussion. Watch for labels that embed assumptions the model can't verify.

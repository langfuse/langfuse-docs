...
flowchart LR
  subgraph Harness["Repo-owned agent harness"]
    ...
    Skills["Skills<br/>weekly production review<br/>cloud cost analysis<br/>Datadog recipes<br/>security review"]
    MCP["MCP servers<br/>and plugins"]
    Output["Fixed output tables<br/>incidents, bugs, pages, cost, security"]
    Router --> Skills
    Skills --> MCP
    Skills --> Output
  end
  ...
  subgraph ProdData["Production data"]
    DD["Datadog EU/US"]
    MB["Metabase<br/>cost marts"]
    INC["incident.io"]
    PYLON["Pylon"]
    CH["ClickHouse query log<br/>planned MCP"]
  end
  ...
  subgraph Workflow["Workflow tools"]
    ...
    Docs["Langfuse docs MCP"]
    DeepSec["DeepSec daily scan"]
  end
  ...
  MCP --> Docs
  Skills --> DeepSec
  Output --> Review["Engineering review<br/>sync, routing, follow-ups"]
```

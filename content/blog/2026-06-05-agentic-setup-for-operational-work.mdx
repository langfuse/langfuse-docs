...
1. The weekly production review surfaces important production signals to act on.
...
4. Datadog investigations help us debug production telemetry across environments.
...
We set up a system with four layers. First, we made the relevant data queryable. For example, we built pipelines that move AWS cost data into BigQuery, join it with business data in dbt, and surface it in Metabase. Second, we exposed these sources to the agent harness through MCP. Third, we developed and iterated on repo-owned skills. Fourth, each skill defines a stable output table.
...
<FileTree>
  <FileTree.Folder name=".agents" defaultOpen>
    ...
    <FileTree.Folder name="skills" defaultOpen>
      ...
      <FileTree.Folder name="datadog-query-recipes" defaultOpen>
        <FileTree.File name="SKILL.md" />
      </FileTree.Folder>
    </FileTree.Folder>
  </FileTree.Folder>
...
flowchart LR
  subgraph Harness["Repo-owned agent harness"]
    Router["AGENTS.md"]
    Skills["<u>Skills</u><br/>production review<br/>cost analysis<br/>Datadog debugging<br/>security review"]
    MCP["MCP servers"]
...
The agent produces tables for monitor alerts, errored traces, and error logs from PagerDuty. It also produces a table for bug-labeled Linear tickets and their status, plus any public incident we published. This gives us one weekly pass over the signals that may need action: bug fixes to prioritize, incidents to discuss with the team, and Datadog errors that should become follow-up issues.

We iterated heavily on this skill to get the output right. The agent needs to query each data source in the right way, deduplicate issues across tools, and keep regions distinct. It also needs to preserve links between sources, for example incident.io or Datadog links already attached to Linear tickets.

### Cloud cost analysis depends on warehouse data

Agents need data access before they need better prompts. For cost analysis, we export AWS CUR reports into BigQuery, expose the resulting marts through Metabase, and make the relevant tables accessible through MCP.
...
Before assigning work, the agent checks Linear for existing issues so the same vulnerability does not get reported every day. We then review the list and let the agent create Linear tickets for the relevant findings. Linear Intelligence assigns those tickets to the right engineer.
...
We review skill output like product behavior: whether the agent found enough evidence, kept formatting stable, and stopped at the right point. Each miss becomes a workflow change.
...
The next step is to connect agents directly to ClickHouse query-log data through MCP. We already tag ClickHouse queries with dimensions such as project, API route, feature area, query name, and accessed tables. Today, dashboards use these tags to show live resource consumption for each ClickHouse cluster. Next, we want agents to use the same data to enrich the production review and cost reports. This should let us allocate ClickHouse cost to specific features or projects and find access patterns that caused API performance issues.

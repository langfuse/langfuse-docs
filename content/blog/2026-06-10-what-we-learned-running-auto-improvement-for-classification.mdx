---
title: "97% on train, 82% on test: validation and test data matter more than the loop"
date: 2026/06/10
description: "Fable 5 had just come out, so we used it to run a prompt-improvement loop end to end. The loop worked. What we learned was why validation and test datasets are what make the result mean anything."
tag: engineering
author: Annabell
---
...
We had just argued in [AI is eating the AI engineering loop](/blog/2026-06-09-ai-is-eating-ai-engineering) that agents can run more of the loop now, but only if the loop itself is set up well. When Fable 5 came out and was promoted for loop-shaped work, it felt like a good moment to test that on a simple benchmark.
...
If you only take one thing from this experiment, take this: **validation and test datasets are what make a loop result trustworthy.**
...
That last part matters most. The agent will get to your target. Make sure it is the right one, and make sure your validation and test datasets are set up to tell you whether it actually improved.

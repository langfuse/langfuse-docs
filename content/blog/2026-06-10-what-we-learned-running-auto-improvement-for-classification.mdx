...
3. Write a short qualitative annotation on every error: what went wrong, what likely pulled the model to the wrong label. These were posted as comments on the Langfuse trace.
4. Form a hypothesis and revise only the prompt, published as a new version to Langfuse.
...
- **Human-owned:** the target function, including the held-out data nobody optimizes against, dataset composition, when to restart with different constraints, and when to stop

...
Loop-shaped workflows are having a moment. More teams are using agents to run a task, inspect failures, revise, and run again. Primitives like `/goal` in Claude Code and Outcomes in Claude Managed Agent make that hill-climbing pattern easy to apply to a concrete benchmark.

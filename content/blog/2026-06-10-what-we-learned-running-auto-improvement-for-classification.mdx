...
Loop-shaped workflows are having a moment. Peter Steinberger even turned it into a monthly ritual: reminding people to stop prompting coding agents and start designing loops that prompt their agents. The design is the task, the environment, the eval, the exit condition. Then you hand it to an agent in `/goal` mode and let it run.

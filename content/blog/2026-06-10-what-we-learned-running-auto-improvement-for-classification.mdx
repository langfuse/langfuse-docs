...
Loop-shaped workflows are having momentum. Boris Cherny recently put it bluntly: much of his job is "to design loops."
...
Primitives like `/goal` in Claude Code and Outcomes in Claude Managed Agent make that pattern easy to apply to a concrete benchmark.
...
Here is what we gave it:

- **Task:** classify arXiv papers into one of 10 categories
- **Stack:** a train/test split in [Langfuse Datasets](/docs/evaluation/experiments/datasets) and a prompt in [Prompt Management](/docs/prompt-management/get-started)
- **Goal:** hit 95% train accuracy or stop after 15 runs, then run the held-out test set once

<Callout type="info">
Train accuracy moved from 78% to 97% in four runs. The held-out test set barely moved, and 11 test errors were shared across every prompt variant.

That shifted the question from "how do we run auto-improvement loops better?" to "what would the dataset have needed to contain for auto improvement to work better?"
</Callout>

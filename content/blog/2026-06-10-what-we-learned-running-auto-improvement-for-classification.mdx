---
title: "97% on train, 82% on test: Claude Fable 5 ran the loop. We should have used a validation set."
date: 2026/06/10
description: "Fable 5 had just come out, so we used it to run a prompt-improvement loop end to end. The mechanics worked. The failure was our train-only selection setup."
tag: engineering
author: Annabell
---
...
We had just shared our take on [AI is eating the AI engineering loop](/blog/2026-06-09-ai-is-eating-ai-engineering): agents are getting good enough to run more of the loop, but that only matters if the loop itself is set up well. Then Claude Fable 5 came out and was explicitly promoted for loop-shaped work, which made it a good moment to try this again on a simple setup.
...
Based on what we learned from [running autoresearch on the Langfuse skill in March](/blog/2026-03-24-optimizing-ai-skill-with-autoresearch), we picked a task with one of the cleanest target functions in AI engineering: **exact-match accuracy against gold labels**. The interesting question was not whether Fable could operate the loop. It could. The more interesting failure was ours: we never set up a proper validation split, so the loop optimized against the train set and called it progress.
...
It hit the target in 4 runs. That part worked. The failure came later: we let the loop hill-climb on the train split, and it overfitted.

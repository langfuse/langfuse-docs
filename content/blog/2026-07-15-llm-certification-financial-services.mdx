...
Every model call is traced in Langfuse and scored by the evaluators. Running the same dataset against multiple models gives you a side-by-side comparison in the Langfuse UI; the portal screenshot further down shows the same comparison, with `claude-sonnet-4-6` and its finance-expert prompt variant as separate rows.
...
Two things change for agents. First, each agent runs as a multi-step task per dataset item, with one nested span per step, so a reviewer can open any item in Langfuse and see which tool the agent invoked and what it returned. Second, instead of a single accuracy threshold, each agent has its own **multi-dimensional gate**: every dimension must pass at once or the run fails.

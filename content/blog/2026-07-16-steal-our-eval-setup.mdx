...
The advice you hear and that we also support, is that your evals need to be custom, built for your specific use case. That is mostly true. What gets less airtime is that within a given use case type, the approach is often surprisingly comparable and copyable.

One of the most established things people build is a chatbot. It is the OG use case, the thing we will tell our grandchildren about, and still what most people have in mind when they hear "AI". So why would everyone building a chatbot reinvent the eval wheel? You can copy from someone else's chatbot.

For example, from ours.

We wanted to share exactly how we built the observability and eval setup, plus the continuous improvement loop, for our own docs chatbot. You do not have to take our word for any of it: you can click through the live setup yourself.
...
What you are looking for is the user's question, the retrieval step showing which docs got pulled in, the tool calls, and the generation step with the final answer. If something is wrong with the answer, the trace tells you whether the problem was bad retrieval or bad generation on top of good retrieval. Those are two very different fixes.
...
You could review every conversation by hand. We decided to evaluate where we need to look and call that monitoring. A few things reliably mark a chatbot conversation as worth a closer look:
...
## Annotation queues, and why you still look yourself
...
This dataset with its evaluators can then be used to run [experiments](/docs/evaluation/experiments/experiments-via-sdk) and understand the quality of your system. We frequently run experiments on newly released models, after changes to the docs, and whenever we add new edge cases to the dataset.

That dataset doubles as your regression suite. We run it as part of [CI](/docs/evaluation/experiments/experiments-ci-cd), so a prompt or model change gets checked against real failure cases before it ships, not after.

## The failure mode that is actually yours

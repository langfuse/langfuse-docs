...
Switch between Jev and the previous GPT-4o-mini classifier. Jev answers a Choice question and returns a probability for each label. GPT-4o-mini returns a label, a short explanation, and the phrases it used. The [example project](/docs/demo#sentiment) shows the Jev classifier only.
...
<Callout type="info">
  [Sign up for Langfuse Cloud](https://cloud.langfuse.com/auth/sign-up) (free,
  no credit card) to get view-only access to the public example project and
  inspect the traces your classifications create.
</Callout>

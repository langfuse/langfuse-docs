...
import { SentimentClassifierCompare } from "@/components/sentimentClassifier/compare";
...
[TypeSafe](https://typesafe.ai) launched [Jev](https://typesafe.ai/blog/introducing-system-one-models-and-jev) mid-September 2026. It makes decisions, cannot write a single sentence, and is 20 to 200x faster and 40 to 400x cheaper than frontier models according to TypeSafe.
...
## Try a Jev-based sentiment classifier [#try-a-jev-based-sentiment-classifier]

Run Jev and GPT-5.6 Luna (high reasoning) on the same text. Add up to four classifications — sentiment, urgency, intent, and next action. Jev answers them in one pass; Luna makes a parallel call per classification, so its cost scales with the count. Jev returns all answers together; each Luna call appears as it finishes. Latency and estimated cost are compared below.

<div className="not-prose my-6">
  <SentimentClassifierCompare />
</div>

## The shape of a Jev request [#the-shape-of-a-jev-request]

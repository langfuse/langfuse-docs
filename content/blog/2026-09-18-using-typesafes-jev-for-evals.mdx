...
Classification is one of the jobs Jev is built for. The Langfuse [example project](/docs/demo#sentiment) includes a live sentiment classifier that asks Jev a Choice question (`positive` / `negative` / `neutral`), returns the full probability distribution, and traces every call.

Paste a review below, or open the same demo on the [example project page](/docs/demo#sentiment).

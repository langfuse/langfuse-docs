...
Run Jev and the previous GPT-4o-mini classifier on the same text at once. Jev returns a Choice probability for each label. GPT-4o-mini returns a label, a short explanation, and the phrases it used. The [example project](/docs/demo#sentiment) shows the Jev classifier only.

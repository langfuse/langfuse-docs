...
<BlogHeader
  ...
  authors={["annabellschafer", "hassiebpakzad"]}
/>
...
Jev is [TypeSafe](https://typesafe.ai)'s System One model. It does not generate text. You send it a state and typed questions, and it returns typed answers with probabilities. It handles narrow, typed questions over unstructured data and is best suited for quick decisions that are repeated, high volume, and the possible answers are known before the call.

Jev answers three question types, which TypeSafe calls primitives:
...
- **[Noul](https://docs.typesafe.ai/primitives/noul)** returns the probability that a statement is true. It carries no separate confidence field, so code that reads `answer.confidence` on everything will break on Nouls.
...
Click **New decision model evaluator**. Langfuse groups System One models like Jev under the **Decision model** evaluator type, so it can support similar models from other providers as they ship.

### Define your question and choose your question type

You can add multiple questions against the same state. The editor offers Jev's three primitives: **Choice**, **Score**, and **Yes / no**, which is a Noul.
...
For a Noul, open **Refine what yes and no mean** to describe what counts as true and false. For a choice, name each option and the condition that makes it the right verdict.

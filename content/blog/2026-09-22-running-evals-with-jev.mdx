...
Jev is TypeSafe's System One model, part of a new category that prioritizes performance on one dimension over general ability of the model. It can be seen as a "zero-shot classifier", also called decision model, which is the name we use in Langfuse. As founder [Diogo Almeida](https://www.latent.space/p/jev) said on the latest Latent Space episode, they optimize for "Intelligence per Dollar" and design the model with fast, structured, machine-native decision-making inside software in mind.

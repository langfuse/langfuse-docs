...
Both follow-ups announced with this release have shipped:

- Fetch a dataset at a specific version timestamp with the `version` parameter in the API and SDKs ([docs](/docs/evaluation/experiments/datasets#versioning))
- Run experiments on a specific dataset version, including pinned versions in CI with `dataset_version` ([docs](/docs/evaluation/experiments/datasets#run-experiments-on-versioned-datasets))

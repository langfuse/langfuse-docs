...
3. Use `fields` to request only the data you need, for example `GET /api/public/v2/observations?fields=core,basic&limit=100` or `langfuse.api.observations.get_many(fields="core,basic,usage")`.
4. Update your limits and parsing assumptions: default limit is `50`, max is `1,000`, and `parseIoAsJson` defaults to `false`.
5. Check SDK compatibility before migrating. Prefer the latest SDKs, but use at least Langfuse server `v3.133.0` for Observations, JS/TS SDK `v4.5.1`, and Python SDK `v3.11.1`. In Python SDK `v4+` and JS/TS SDK `v5+`, v2 is the default under `langfuse.api.observations`. Self-hosted users should stay on the legacy v1 namespaces for now.

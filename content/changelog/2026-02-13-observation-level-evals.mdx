...
seoTitle: "Evaluate Individual Operations with a Judge"
...
<Callout type="info">
Existing trace-level evaluators continue to work on Langfuse Cloud until <V4CutoverDate />; on self-hosted, they will stop producing results once you run the v4 server in `events_only` mode. For users with existing evaluations, see the [upgrade guide](/faq/all/llm-as-a-judge-migration) to transition to observation-level for faster execution.
</Callout>

...
import { Book, FileCode, Server } from "lucide-react";

<ChangelogHeader />

You can now create code evaluators in Langfuse to score observations and experiments with deterministic Python or TypeScript logic. Use them for exact checks such as JSON parseability, schema validation, exact match, required tool arguments, latency and cost thresholds, or custom business rules.
...
<Cards num={3}>
  ...
  <Card
    ...
    href="/self-hosting/configuration/code-based-evaluators"
    icon={<Server />}
    arrow
  />
</Cards>

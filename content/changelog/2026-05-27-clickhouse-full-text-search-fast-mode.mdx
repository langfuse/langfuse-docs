...
We are also expanding the [Observations API v2](/docs/api-and-data-platform/features/public-api#v2) with a new `matches` operator for token-based search on `input`, `output`, and string metadata filters.
...
Pass the JSON array as the URL-encoded `filter` query parameter on `GET /api/public/v2/observations`. See the [Observations API v2 docs](/docs/api-and-data-platform/features/public-api#v2) and [API reference](https://api.reference.langfuse.com/#tag/observations/GET/api/public/v2/observations) for the full filter schema.

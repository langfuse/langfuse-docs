...
<Callout type="info">
  ...
  (`input_price`, `output_price`, `total_price`) — they are computed in an
  enrichment step that Parquet exports skip. Choose a text format if you need
  them, or compute prices downstream from `usage_details` and your model
  definitions. Trace and score exports are identical across all file types. See
  the
  [Export Field Reference](/docs/api-and-data-platform/features/blob-storage-export-fields#parquet-exports)
  for details.
</Callout>

...
One dropdown switches what the strip plots. You can view **Cost**, **Latency**, or **Tokens** on their own, or all three at once in **Split** mode. Each metric uses the aggregate that fits it: cost as max or total, latency as max, p95, or average. The bars are time buckets across your full range, so a quiet stretch reads as a flat baseline rather than empty space.

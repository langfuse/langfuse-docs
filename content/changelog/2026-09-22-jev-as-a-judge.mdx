...
description: Use TypeSafe's Jev System One model as a judge in Langfuse evaluators. Ask typed questions about every observation and get calibrated scores at a fraction of the cost and latency of an LLM judge.
author: Hassieb
...
- **Cheaper.** Jev is priced by input tokens only, at $0.042 per million. A judgment over a typical conversation costs a fraction of a cent, so you can score more observations instead of sampling.
...
| **Noul** (**Yes / no** in the UI) | The probability that a statement is true               | Numeric (`P(true)` from 0 to 1) | Out-of-scope requests, PII, policy violations, user disagreement |
...
In the Langfuse UI, Jev evaluators are called **decision-model evaluators**. The evaluator type is named generically so that similar models from other providers can be added later. Jev is the first one we support.

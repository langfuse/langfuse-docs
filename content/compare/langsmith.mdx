...
**Choose Langfuse** if the quality layer should outlive your framework choice. LangChain sells the runtime; Langfuse sells only the layer around it, so nothing in the product pulls you toward one agent stack. Self-hosting is MIT-licensed on every plan rather than an Enterprise contract, and one meter covers unlimited users, so reviewers and product managers cost nothing to add. Proven at 21 of the Fortune 50, and on the [worked example](#worked-example) roughly 6x cheaper than LangSmith Plus at list prices, with 3 years of history against a 180-day ceiling. You accept that Langfuse does not run your agents; it traces whichever runtime you pick.

**Choose LangSmith** if you are all in on LangChain and LangGraph and want one vendor to deploy the agent, observe it, and propose fixes from a single managed platform. You accept lock-in: a seat fee on every invited user, traces in a closed store with bulk export gated to Enterprise, a 180-day SaaS retention ceiling, and self-hosting only on an Enterprise contract.
...
### Does LangSmith offer a BAA for HIPAA? [#does-langsmith-offer-a-baa]

...
This guide outlines the key differences between **[Langfuse](/)** and **[LangSmith](https://www.langchain.com/langsmith)** to help engineering teams choose the right AI engineering platform. All LangSmith facts were checked against public LangChain sources in September 2026.

## TL;DR

- **Full-stack open source.** Langfuse is [MIT-licensed](#open-source-and-distribution), identical on Cloud, self-hosted, and air-gapped, with Enterprise SSO and organization RBAC in the free distribution. LangSmith is proprietary: self-hosting, custom SSO, and RBAC are Enterprise-only, and even self-hosted Engine [sends trace and code content to LangChain-managed inference](#evaluation-and-experiments).
- **Independent of the runtime.** Langfuse ingests [OpenTelemetry and 100+ frameworks](#integrations) and sells no runtime, gateway, or framework, so traces, evaluators, datasets, and prompts survive a change of agent stack. LangSmith is structurally integrated with LangChain and LangGraph and meters Deployment, Engine, Fleet, and the Gateway as separate lines on top.
- **~6x cheaper on the worked example, with no seat fees.** 500k traces/month with 5 users is $621 on Langfuse Pro where [LangSmith Plus is $3,895](#worked-example): $39 per invited seat, per-trace usage, and a retention upgrade that online evaluators trigger by default. Langfuse Pro keeps 3 years of history; LangSmith SaaS caps retention at 180 days, Enterprise included.
- **Open data, no lock-in.** Langfuse runs on ClickHouse, [open source and in production for a decade](#storage), and gets data out through [high-volume APIs, scheduled exports, and standard SQL when you self-host](#open-platform). LangSmith Cloud moved traces to SmithDB, a proprietary engine launched in May 2026, and gates bulk export to Enterprise.
- **Same-day switch.** [Repoint your OpenTelemetry exporter or swap the LangChain environment variables](#switching-from-langsmith) for the Langfuse callback handler; datasets copy over via SDK. Most teams see first traces in Langfuse the same day.

**Choose Langfuse** if the quality layer should outlive your framework choice. Self-hosting is MIT-licensed on every plan rather than an Enterprise contract, and one meter covers unlimited users, so reviewers and product managers cost nothing to add. Evaluator scores bill as regular units with no separate evaluator fee. On the [worked example](#worked-example) roughly 6x cheaper than LangSmith Plus at list prices, with 3 years of history against a 180-day ceiling. Proven at 21 of the Fortune 50. You accept that Langfuse does not run your agents; it traces whichever runtime you pick.

**Choose LangSmith** if you are all in on LangChain and LangGraph and want a managed SaaS covering observability, evals, and agent deployment infrastructure in one closed platform. You accept lock-in: a seat fee on every invited user, traces in a closed store with bulk export gated to Enterprise, a 180-day SaaS retention ceiling, and self-hosting only on an Enterprise contract.
...
Langfuse works directly with ClickHouse's core team as [part of ClickHouse](/blog/joining-clickhouse). Every LLM call, tool execution, and agent step is written once to a wide, immutable observations table. Details are in [Simplifying Langfuse for Scale](/blog/2026-03-10-simplify-langfuse-for-scale).
...
|                            | Langfuse                                                                                                                              | LangSmith                                                                                              |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Data store                 | **ClickHouse**, [launched 2016 (OSS)](https://clickhouse.com/blog/open-source-10), Apache 2.0                                         | **SmithDB**, [launched 2026](https://www.langchain.com/blog/introducing-smithdb), proprietary / closed |
| Maturity                   | ~10 years open source; [2,000+ contributors](https://clickhouse.com/blog/open-source-10)                                              | First public release May 13, 2026                                                                      |
...
| Full-text search           | [Yes](/docs/observability/features/full-text-search) on ClickHouse                                                                    | Yes (SmithDB on Cloud, Quickwit on self-hosted)                                                        |
| High-volume reads          | [Observations API v2](/docs/api-and-data-platform/features/public-api#v2) and [Metrics API v2](/docs/metrics/features/metrics-api#v2) | SmithDB (Cloud), Managed Bulk Data Export (Enterprise-only)                                            |
...
<Callout type="info">
  **Run your own numbers.** The [Langfuse vs LangSmith pricing
  model](/pricing-comparison-sheet) is an editable Google Sheet with every list
  price in this section. Set traces, spans per trace, scores, seats, and
  retention to see where the costs cross over.
</Callout>

Langfuse prices observability, evaluation, prompt management, and metrics as one platform: traces, observations, and scores meter as units, with no seat fees. LangSmith meters seats, traces, and a retention upgrade, plus separate lines for Deployment, Engine, Fleet, Sandboxes, the LLM Gateway, and per-evaluation charges for Tuned Evaluators on [their pricing page](https://www.langchain.com/pricing). The worked example covers observability seats and traces only.
...
| Retention  | 30 days Hobby, 90 days Core, **3 years** Pro and Enterprise                    | 14 days base, 180 days extended ([SaaS maximum since September 14, 2026](https://docs.langchain.com/langsmith/usage-and-billing)) |
...
Both platforms accept OpenTelemetry traces and instrument non-LangChain applications. LangSmith's advantage is integration with LangChain and LangGraph through one environment variable.

The deeper difference is what each vendor sells. LangChain sells the agent stack around LangSmith: Deployment hosts the runtime and Managed Deep Agents, Studio inspects and replays LangGraph state, fleet builds agents without code, the LLM Gateway (beta) routes model calls, and Engine watches production traces and proposes fixes. Each is metered as [its own line on the pricing page](https://www.langchain.com/pricing). That is LangSmith's strength and its tradeoff: unusually deep integration with one vendor's stack, at the cost of neutrality.

Langfuse takes the opposite position on purpose: MIT-licensed, with neither a runtime to sell nor a framework to protect. It [integrates with 100+ frameworks](/integrations) and model providers (OpenAI, Anthropic, Vercel AI SDK, LangChain, LlamaIndex, LiteLLM, and others) or your own harness, traces land in an open ClickHouse database you can operate, and no seat fees gate who can look at them. If you change agent vendors later, traces, evaluators, datasets, and prompts stay. The suite-adjacent pieces exist in the open ecosystem too: the ClickHouse-built [Agentic Data Stack](/integrations/clickhouse/agentic-data-stack) pairs LibreChat's chat UI with MCP tools and Langfuse tracing in one Docker Compose deployment, [independent gateways](/resources/engineering/llm-gateway) cover model routing today, and a [Langfuse Gateway is on the public roadmap](/docs/roadmap#roadmap).
...
Both platforms cover offline and online evaluation. Langfuse runs LLM-as-a-judge and deterministic code evaluators inside the platform and gates CI/CD pipelines on experiment results. Evaluator scores bill as regular units with no separate evaluator fee, evaluators run the same on Cloud and self-hosted, and online scoring does not auto-promote traces into a paid retention tier the way [LangSmith online evaluators do by default](https://docs.langchain.com/langsmith/usage-and-billing#data-retention-auto-upgrades) (LangSmith lets you opt out per evaluator).

LangSmith pairs its evals with two managed, separately metered services. [Tuned Evaluators](https://www.langchain.com/blog/introducing-langsmith-tuned-evaluators-starting-with-perceived-error) (August 2026) attach LangChain-trained judges to tracing projects, starting with Perceived Error; they are in public beta on Plus and Cloud Enterprise, US Cloud only, each successful evaluation bills 0.01 LCU at $1.50/LCU, and the scored trace auto-upgrades to paid extended retention. LangSmith Engine clusters production failures on a 6-hour schedule and is also metered in LCUs; the [August 2026 Engine release](https://www.langchain.com/blog/new-in-langsmith-engine-2x-better-issue-detection) reports over 2x better issue detection on LangChain's internal benchmarks, self-hosted availability, and a Reduced Analysis mode to contain LCU cost. Engine runs only on LangChain-managed inference: no bring-your-own-key, US-only, and [even self-hosted Engine sends trace and code content to LangChain's cloud](https://docs.langchain.com/langsmith/engine-self-hosted); air-gapped installs cannot run it. Langfuse instead gives you [evaluators](/docs/evaluation/core-concepts#evaluators-and-rules) on your own model keys, [alerts](/docs/observability/features/alerts) to Slack or webhooks, an [Assistant](/docs/langfuse-assistant) that clusters failures on demand, and an [MCP server](/docs/observability/features/agentic-access) so the coding agent you already use ships the fix.
...
| Experiments                     | [UI](/docs/evaluation/experiments/experiments-via-ui) + [SDK](/docs/evaluation/experiments/experiments-via-sdk), versioned datasets, [baseline comparison with row-level regressions](/docs/evaluation/experiments/compare-experiments) | Yes, plus [repetitions](https://docs.langchain.com/langsmith/repetition) per example |
...
| Production insights | [Dashboards](/docs/metrics/features/custom-dashboards), [Pulse](/changelog/2026-07-28-langfuse-pulse), [filter search bar](/docs/observability/features/filter-search-bar) | Trace UI filters; [Insights](https://www.langchain.com/blog/interrupt-2026-overview) (Plus and Enterprise) |
...
| Data export             | [Blob storage export](/docs/api-and-data-platform/features/export-to-blob-storage) to S3, GCS, Azure (JSONL/Parquet); Pro with Teams add-on, Enterprise, self-hosted | [Bulk export](https://docs.langchain.com/langsmith/data-export) Enterprise-only for accounts created after August 3, 2026 |
...
| In-product AI assistant | [Langfuse Assistant](/docs/langfuse-assistant): all Cloud plans, self-hosted public beta                                                                             | Engine: scheduled issue detection and proposed fixes, LCU-metered, LangChain-managed inference only                       |
...
| BAA            | **From Pro plan** in the [HIPAA region](/security/hipaa)                                           | Not published; HIPAA compliant per [docs](https://docs.langchain.com/langsmith/regions-faq) |
...
Choose **Langfuse** if you want **an independent, open-source platform that adapts to your stack**: OpenTelemetry-based instrumentation for any framework, MIT self-hosting with SSO and org RBAC included, LLM-as-a-judge and code evaluators with CI/CD gates on the same traces, observability data in an open database you can query directly, and unit pricing with no seat fees.

Choose **LangSmith** if you are committed to LangChain and LangGraph and want **one vendor to host the agent runtime, observe it, and propose fixes**: zero-config instrumentation and managed deployment for their frameworks, Fleet for no-code agents, and Engine's scheduled issue detection with proposed fixes.
...
- **Retention is its own meter.** LangSmith's included window is 14 days, keeping traces longer is a per-trace upgrade, and [online evaluators and automation rules promote traces into the paid tier by default](#pricing). Retention is $1,250 of the worked example's monthly LangSmith total, and since September 14, 2026 the SaaS maximum is 180 days, on Enterprise too. Langfuse Pro and Enterprise retain traces for 3 years with no per-trace retention charge.
- **Compliance routes through sales.** Self-hosting, custom SSO, and role-based access control are [Enterprise-only on LangSmith](https://www.langchain.com/pricing), so teams that must run in their own infrastructure cannot evaluate on the free or Plus tiers. Langfuse self-hosts [MIT-licensed on every tier](#open-source-and-distribution), and the same product runs on Cloud, so teams start in their own infrastructure while legal reviews and move later without a migration. A [BAA is available from the Pro plan](#enterprise-security).
- **Framework coupling.** Platform teams serving heterogeneous stacks don't want the observability layer tied to one agent framework. LangSmith works with other frameworks through SDK wrappers and OpenTelemetry, but it is structurally integrated with LangChain and LangGraph, and [Deployment, Engine, Fleet, and the Gateway](#integrations) are priced separately on top.
...
- **Cloud stores traces in a closed engine launched in 2026.** [SmithDB](https://www.langchain.com/blog/introducing-smithdb) is proprietary, launched in May 2026. Langfuse Cloud and self-hosted stay on the same [ClickHouse](#storage) architecture, open source since 2016.
...
<Details id="is-langfuse-the-best-langsmith-alternative">
<Summary>Is Langfuse the best LangSmith alternative?</Summary>
...
<Details id="does-langfuse-cover-the-same-product-areas">
<Summary>Does Langfuse cover the same product areas as LangSmith?</Summary>
...
<Details id="how-does-langsmith-pricing-compare-to-langfuse">
<Summary>How does LangSmith pricing compare to Langfuse?</Summary>

LangSmith Plus is $39/seat/month with 10,000 base traces included, plus per-trace usage and optional retention upgrades (as of September 2026). Langfuse Cloud Hobby includes 50,000 units/month, Core starts at $29/month, and the MIT distribution is free to self-host. On the rates in our [editable public model](/pricing-comparison-sheet), 500k traces / 10 spans / 1 score / 5 users / 12-month retention is **$621** Langfuse Pro vs **$3,895** LangSmith Plus (the LangSmith figure uses the extended-retention rate; LangSmith SaaS caps retention at 180 days, below the example's 12 months). We bill every span; they bill the trace, so span depth changes the crossover. Details in the [pricing section](#pricing).

</Details>

<Details id="does-langfuse-have-alerting">
<Summary>Does Langfuse have production alerting?</Summary>
...
<Details id="does-langfuse-run-code-evaluators">
<Summary>Does Langfuse run deterministic evaluators on production traffic?</Summary>
...
<Details id="is-langsmith-open-source">
<Summary>Is LangSmith open source?</Summary>

No. The LangSmith platform (backend, UI, storage, including [SmithDB](https://www.langchain.com/blog/introducing-smithdb), launched 2026) is closed source. LangChain, LangGraph, and the LangSmith client SDKs are MIT open source. Langfuse is MIT and [self-hostable](/self-hosting) on [ClickHouse](#storage), open source since 2016.

</Details>

<Details id="does-langsmith-require-langchain">
...
<Details id="can-i-self-host-langsmith">
<Summary>Can I self-host LangSmith?</Summary>

On the Enterprise plan. As of LangSmith Self-Hosted v0.16, SmithDB, Engine, and Sandboxes are available to self-hosted customers, with SmithDB in public beta behind a waitlist. The LLM Gateway is [not in the v0.16 stable release](https://docs.langchain.com/langsmith/llm-gateway); self-hosted access is by request. Tuned Evaluators are [US Cloud only](https://www.langchain.com/pricing) and do not run in self-hosted or BYOC deployments.

</Details>

<Details id="does-langsmith-offer-a-baa">
<Summary>Does LangSmith offer a BAA for HIPAA?</Summary>

LangSmith states that it is [HIPAA compliant](https://docs.langchain.com/langsmith/regions-faq) but does not publish which plan includes a Business Associate Agreement; ask their sales team. Langfuse signs a BAA from the Pro plan ($199/month) in its dedicated [HIPAA region](/security/hipaa).

</Details>

<Details id="how-do-i-migrate-from-langsmith">
<Summary>How do I migrate from LangSmith to Langfuse?</Summary>

Follow the [step-by-step migration guide](/resources/engineering/migrate-from-langsmith). LangSmith's OpenTelemetry mode can fan traces out to Langfuse and LangSmith in parallel while you verify, LangChain and LangGraph apps add the [Langfuse callback handler](/integrations/frameworks/langchain), and datasets and Prompt Hub prompts copy over via the SDK. Most teams see first traces the same day. See [Switching from LangSmith](#switching-from-langsmith).

</Details>

<Details id="what-is-smithdb">
<Summary>What is SmithDB?</Summary>

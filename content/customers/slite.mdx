...
<div className="corner-box-corners not-prose relative my-8 border border-line-structure bg-surface-1 px-6 py-8 text-text-primary md:px-12 md:py-10">
  ...
  <ol className="m-0 mt-7 list-none space-y-6 p-0">
    <li className="grid grid-cols-[auto_1fr] items-start gap-4 md:gap-6">
      ...
      <p className="m-0 text-sm leading-6">Slite is turning its AI knowledge base into an agent that keeps a company's docs current on its own, proofreading, moving, and archiving documents without being asked.</p>
    </li>
    <li className="grid grid-cols-[auto_1fr] items-start gap-4 md:gap-6">
      ...
      <p className="m-0 text-sm leading-6">Langfuse tracing gives them deep insight into what their agents are doing, which is essential as agentic capabilities get more complex.</p>
    </li>
    <li className="grid grid-cols-[auto_1fr] items-start gap-4 md:gap-6">
      ...
      <p className="m-0 text-sm leading-6">A ~100-item Langfuse dataset, graded by an LLM-as-a-judge on expected behavior rather than exact answers, replaced a manually run spreadsheet and took experiments from once a quarter to about once a week.</p>
    </li>
    <li className="grid grid-cols-[auto_1fr] items-start gap-4 md:gap-6">
      ...
      <p className="m-0 text-sm leading-6">As the agent gets more autonomy over the workspace, Langfuse gives both developers and a background review agent the visibility to expand scope without breaking what already works.</p>
    </li>
  </ol>
</div>
...
Sustaining quality while scope keeps expanding is key: "having this test
suite running constantly on the CI every time we touch it... makes sure that we
maintain the quality and we don't break it as we are making big changes." With
new models shipping monthly and the agent's footprint only growing, Charley
expects Slite's Langfuse value to keep increasing right alongside it: more
traces stored, more retrospectives run, and more datasets to constantly evaluate
whatever comes next.

...
The endpoint responds with `Trace deleted successfully` as soon as the deletion has been queued, not when the data is gone. The trace and its observations and scores can remain visible in the UI and API for a few minutes until the background job has run; see [limitations](#trace-deletion-limitations) for timing and how to verify.
...
Project deletion is asynchronous, and the time it takes scales with the size of the
project.

<Callout type="warning">
  Deleting a project is irreversible and all data will be removed. Be cautious
  when executing this action. For very large projects, deletion can take multiple
  days. If deletion is slower than you expect, please reach out to
  [support](/support).
</Callout>
...
Only organization owners can delete an organization, and only once every project in it has been deleted or transferred to another organization. Delete the projects first, then the organization.
...
Two errors can block the deletion:

- `Please delete or transfer all projects before deleting the organization.` — at least one project still exists in the organization.
- `Deletion of your projects is still being processed, please try deleting the organization later` — you already deleted the remaining projects, but their data is still being purged in the background. Project deletion is asynchronous and takes longer for projects with more data; wait and retry. The error is not a permissions problem.

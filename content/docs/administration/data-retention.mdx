...
Retention is a per-project setting. There is no organization- or team-level retention policy: to apply the same window across many projects, set it on each project, in the UI or via the Organization Management API. On self-hosted instances, `LANGFUSE_INIT_PROJECT_RETENTION` sets the retention of the project created at startup.
...
- The source link points to a deleted trace. You can no longer inspect the original trace's [observations](/docs/observability/data-model#observations-traces-and-sessions), timing, or other details through that link.

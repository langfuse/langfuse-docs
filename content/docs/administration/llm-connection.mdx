...
- Google Vertex AI
- Amazon Bedrock
- TypeSafe (experimental): Jev decision models for [Jev as a judge](/docs/evaluation/evaluation-methods/jev-as-a-judge) evaluators only. Jev does not generate text, so TypeSafe connections are not available in the Playground, prompt experiments, or LLM-as-a-Judge.

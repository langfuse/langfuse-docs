...
Each scope is `resource:action`. `read` grants viewing; `CUD` (create, update, delete) or `CRUD` grants changes. The project-level `Admin` role has every project scope of `Owner` except `project:delete`. A few practical consequences of the tables above:
...
Actions such as `traces:read`, `traces:create`, `scores:read`, `scores:create`, `sessions:read`, `metrics:read`, and `media:create` are not listed in the tables because no UI role holds them: they are granted to [API keys](#api-keys) only, and are the actions the public API checks.

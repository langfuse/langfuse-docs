...
| `input_price`             | string or null             | Per-unit input price from the matched model definition. Decimal string.          | Null if no model definition matched. Cast to numeric in your pipeline. Omitted from [Parquet exports](#parquet-exports).                                                |
| `output_price`            | string or null             | Per-unit output price from the matched model definition. Decimal string.         | Null if no model definition matched. Cast to numeric in your pipeline. Omitted from [Parquet exports](#parquet-exports).                                                |
| `total_price`             | string or null             | Per-unit total price from the matched model definition. Decimal string.          | Null if no model definition matched. Used for models with a flat per-call price. Omitted from [Parquet exports](#parquet-exports).                                      |
...
Parquet exports stream data directly from the query engine and skip the enrichment step that resolves per-unit model prices. As a result, the following **observation** columns are omitted from Parquet exports (both `observations_v2/` and legacy `observations/`):

- `input_price`
- `output_price`
- `total_price`

These columns are present in CSV, JSON, and JSONL exports. Trace and score exports contain the same fields in all file types. If you depend on per-observation pricing, choose a text format or compute prices downstream from `usage_details` and your model definitions.

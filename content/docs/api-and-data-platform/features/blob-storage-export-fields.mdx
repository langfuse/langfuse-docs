...
<Callout type="info">
  The columns below are grouped into eleven togglable [field groups](/docs/api-and-data-platform/features/export-to-blob-storage#export-field-groups). New integrations export all groups by default. The `core` group is always included; the others can be deselected per integration to shrink files or omit sensitive columns. Field groups do not apply to the legacy `traces/` and `observations/` files.
</Callout>

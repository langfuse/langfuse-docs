...
<Callout type="info">
  The columns below are grouped into eleven field groups [field groups](/docs/api-and-data-platform/features/export-to-blob-storage#export-field-groups), ten of which are toggleable. New integrations export all groups by default. The `core` group is always included; the others can be deselected per integration to shrink files or omit sensitive columns. Field groups do not apply to the legacy `traces/` and `observations/` files.
</Callout>

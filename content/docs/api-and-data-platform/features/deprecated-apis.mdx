...
The APIs documented on this page are deprecated. They no longer receive fixes — only security patches — and no new functionality; new capabilities ship in the current API. See [Migration of deprecated APIs](/docs/deprecated-api-migration).

</Callout>

## Deprecated endpoints [#endpoints]

The endpoints below are deprecated. Their parameters are documented in the linked API reference; follow the replacement link to migrate. All paths are relative to `/api/public`.
...
The v1 reads `GET /scores` and `GET /scores/{scoreId}` are not in the API reference — they predate non-numeric scores (see the [announcement](/changelog/2024-07-11-non-numeric-scores-api)) and were superseded by the v2 reads. Score **writes** via `POST /scores` are not deprecated.

### Dataset runs [#dataset-runs-v1]

Unlike the other reads on this page, the dataset run reads are not gated by `events_only` mode and keep working on self-hosted v4.

### Ingestion [#ingestion-v1]

On self-hosted v4 in `events_only` mode, `POST /ingestion` still returns `207` but rejects most event types with a `400` item error; `score-create`, `sdk-log`, and `dataset-run-item-create` continue to work. The older synchronous write endpoints `POST /traces`, `/spans`, `/generations`, and `/events` are not in the API reference and also stop working after the cutover to `events_only`.

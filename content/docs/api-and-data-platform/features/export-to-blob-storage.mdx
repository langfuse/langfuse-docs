...
<Callout type="info">
  For the legacy `observations` file, selecting `trace_context` has no effect: tags, release, and trace name live on the trace rather than the observation, so they are exported in the separate [`traces` file](/docs/api-and-data-platform/features/blob-storage-export-fields#traces) instead (join on `trace_id`).
</Callout>

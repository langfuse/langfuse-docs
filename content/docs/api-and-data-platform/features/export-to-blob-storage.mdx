...
<Callout type="warning">
  ...
  Langfuse Cloud, remaining legacy exports switch at the v4
  cutover on <V4CutoverDate />. On self-hosted v4, the legacy source stops
...
Enriched export sources are unavailable in `legacy` mode or when the Langfuse v4 preview is disabled. Switch the integration before the server's [`events_only` cutover](/self-hosting/upgrade/upgrade-guides/upgrade-v3-to-v4#cutover).

...
You can schedule exports to Blob Storage, e.g. S3, GCS, or Azure Blob Storage. Exports contain enriched observations (each observation row includes its trace attributes) and `scores`; every run writes to `observations_v2/`, `scores/`, and `manifests/` under `{prefix}{project-id}/` in your bucket. This is also the recommended path to [export Langfuse data to ClickHouse](/docs/api-and-data-platform/features/export-to-warehouse).

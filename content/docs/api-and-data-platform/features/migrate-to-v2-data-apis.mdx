...
description: Migrate deprecated Langfuse data retrieval APIs to Observations API v2 and Metrics API v2.
---
...
The legacy data retrieval endpoints `GET /api/public/traces`, `GET /api/public/traces/{traceId}`, `GET /api/public/observations`, `GET /api/public/observations/{observationId}`, and `GET /api/public/metrics` are deprecated. They remain available only for backward compatibility.
...
| SDK | Deprecated or transitional usage | Recommended usage |
| --- | --- | --- |
...
Legacy namespaces remain available for temporary compatibility only:
...
Trace-centric read endpoints are deprecated for extraction workflows. Query observations directly and use `traceId` to group rows that belong to the same trace.
...
Observations API v2 and Metrics API v2 are available on Langfuse Cloud. For self-hosted deployments, keep existing compatibility calls in place until v2 support is available, and avoid building new integrations on deprecated endpoints.

...
If `fields` is not specified, `core` and `basic` field groups are returned by default. Fields from groups you do not request are **absent** from the response — not `null`. Check for key presence rather than null-checks when parsing partial responses.

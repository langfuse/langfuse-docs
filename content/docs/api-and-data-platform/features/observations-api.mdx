...
If `fields` is not specified, `core` and `basic` field groups are returned by default. Fields from groups you do not request are **absent** from the response — not `null` — with one exception: `modelId`, `inputPrice`, `outputPrice`, and `totalPrice` are always present and set to `null` when the `model` field group is not selected.

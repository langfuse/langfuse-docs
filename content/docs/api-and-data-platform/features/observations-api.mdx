...
| `io`            | input, output                                                                                                          |
| `metadata`      | metadata                                                                                                               |
| `model`         | providedModelName, internalModelId, modelParameters, inputPrice, outputPrice, totalPrice                               |
...
If `fields` is not specified, `core` and `basic` field groups are returned by default. Fields from groups you do not request are **absent** from the response — not `null`. The following fields are an exception and are always present (as `null` when no value is available): `modelId`, `inputPrice`, `outputPrice`, `totalPrice`. Note: `modelId` is the user-provided model identifier and is distinct from `internalModelId` (the Langfuse model definition UUID, returned within the `model` group).

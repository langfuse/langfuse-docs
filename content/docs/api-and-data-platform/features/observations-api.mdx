...
The legacy read endpoints `GET /api/public/traces`, `GET /api/public/traces/{traceId}`, `GET /api/public/observations`, and `GET /api/public/observations/{observationId}` are deprecated. Use Observations API v2 for row-level data extraction. See [Upgrade to v2 data APIs](/docs/api-and-data-platform/features/migrate-to-v2-data-apis) for migration examples.
...
### Upgrade from deprecated trace and observation reads
...
The following read endpoints are deprecated and retained only for backward compatibility:

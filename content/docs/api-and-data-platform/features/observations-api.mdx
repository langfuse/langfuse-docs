...
The older read endpoints `GET /api/public/traces`, `GET /api/public/traces/{traceId}`, `GET /api/public/observations`, and `GET /api/public/observations/{observationId}` remain available, but are not recommended as the default for new data extraction workflows because they are less performant at scale. Use Observations API v2 for row-level data extraction. See [Upgrade to v2 data APIs](/docs/api-and-data-platform/features/migrate-to-v2-data-apis) for migration examples.
...
| ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| List traces with `GET /api/public/traces`                                 | Use `GET /api/public/v2/observations` and group rows by `traceId`                                                       |
| Fetch one trace with `GET /api/public/traces/{traceId}`                   | Use `GET /api/public/v2/observations?traceId=<traceId>`                                                                 |
...
Use `fields` to request only the field groups you need, and use `cursor` pagination for large result sets.
...
## Older read APIs [#older-read-apis]

The following read endpoints remain available, but are not recommended as the default for new data extraction workflows because they are less performant at scale:

| Existing endpoint                              | Recommended replacement                                                                                             |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `GET /api/public/traces`                       | `GET /api/public/v2/observations` or `GET /api/public/v2/metrics`                                                   |
| `GET /api/public/traces/{traceId}`             | `GET /api/public/v2/observations?traceId=<traceId>`                                                                 |
| `GET /api/public/observations`                 | `GET /api/public/v2/observations`                                                                                   |
| `GET /api/public/observations/{observationId}` | `GET /api/public/v2/observations?filter=[{"type":"string","column":"id","operator":"=","value":"<observationId>"}]` |

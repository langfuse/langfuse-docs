...
The deprecated `GET /api/public/traces` and `GET /api/public/observations` endpoints are documented in the [Deprecated APIs reference](/docs/api-and-data-platform/features/deprecated-apis); to move off them, see [Migration of deprecated APIs](/docs/deprecated-api-migration).
...
The [migration guide](/docs/deprecated-api-migration) maps every deprecated read endpoint (`/api/public/traces`, `/api/public/observations`, `/api/public/sessions`, ...) to its v2 replacement, with parameter mappings and before/after examples. Always include `fromStartTime` and `toStartTime` to keep each request bounded.

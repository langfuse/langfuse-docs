...
- OpenAPI spec: https://cloud.langfuse.com/generated/api/openapi.yml (organization API: https://cloud.langfuse.com/generated/organizations-api/openapi.yml)
- Self-hosted: every instance running Langfuse v4.36.0 or later serves its own spec at `/api/openapi.yaml` and an interactive reference at `/api/docs`
...
| To retrieve...                                                                                                         | Use                                                     |
| ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| Row-level [observations](/docs/observability/data-model#observations-traces-and-sessions) (spans, generations, events) | [Observations API v2](#v2)                              |
...
Retrieve [observation](/docs/observability/data-model#observations-traces-and-sessions) data (spans, generations, events) for custom workflows, evaluation pipelines, and analytics. For aggregated metrics (total cost, token counts, trace volumes grouped by user, model, or time period), use the [Metrics API](/docs/metrics/features/metrics-api) instead of fetching and aggregating raw rows yourself.
...
#### Query parameters and filters [#observations-filters]

Requests need an API key with read access to traces. Bound every request with `fromStartTime` and `toStartTime`. The fixed parameters cover the common cases:

| Parameter             | Type                         | Matches                                                                                                     |
| --------------------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `fromStartTime`       | ISO 8601 datetime            | `startTime >=` value                                                                                        |
| `toStartTime`         | ISO 8601 datetime            | `startTime <` value                                                                                         |
| `traceId`             | string                       | Observations of one trace                                                                                   |
| `name`                | string                       | Exact observation name                                                                                      |
...
| `userId`, `sessionId` | string                       | Exact match on the trace's user or session                                                                  |
| `environment`         | string, repeatable           | Any of the given environments                                                                               |
| `version`             | string                       | Exact match                                                                                                 |
| `parentObservationId` | string                       | Physical parent; an empty value matches observations without a parent                                       |
| `isRootObservation`   | `true` / `false`             | Logical root status, see [above](#logical-root-observations)                                                |
| `fields`              | comma-separated field groups | Which [field groups](#field-groups) to return                                                               |
| `expandMetadata`      | comma-separated keys         | Metadata keys to return untruncated (values are otherwise cut at 200 characters)                            |
| `limit`, `cursor`     | int (≤ 1,000), string        | [Pagination](#observations-pagination)                                                                      |

Everything else — tags, cost, latency, token counts, model, prompt, input/output text, metadata by key, negations, and multi-value matches — goes through the `filter` parameter: a URL-encoded JSON array of conditions with the same columns and operators the filter bar in the Langfuse UI uses. When `filter` is present it takes precedence over the fixed parameters above.

Each condition is `{"type", "column", "operator", "value"}` (plus `"key"` for metadata). Columns include `id`, `type`, `name`, `traceId`, `traceName`, `startTime`, `endTime`, `environment`, `level`, `statusMessage`, `version`, `userId`, `sessionId`, `isRootObservation`, `tags`, `latency`, `timeToFirstToken`, `tokensPerSecond`, `inputTokens`, `outputTokens`, `totalTokens`, `inputCost`, `outputCost`, `totalCost`, `model`, `promptName`, `promptVersion`, `input`, `output`, and `metadata`. The operators depend on the condition type:

| `type`                          | Operators                                                       | Typical columns                                        |
| ------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------ |
| `string`                        | `=`, `contains`, `does not contain`, `starts with`, `ends with` | `name`, `traceName`, `userId`, `sessionId`, `model`, … |
| `stringOptions`                 | `any of`, `none of`                                             | `type`, `level`, `environment`, `promptName`           |
| `arrayOptions`                  | `any of`, `none of`, `all of`                                   | `tags`                                                 |
| `number`                        | `=`, `>`, `<`, `>=`, `<=`                                       | `latency`, `totalCost`, `totalTokens`, …               |
| `datetime`                      | `>`, `<`, `>=`, `<=`                                            | `startTime`, `endTime`                                 |
| `boolean`                       | `=`, `<>`                                                       | `isRootObservation`                                    |
| `stringObject` / `numberObject` | as `string` / `number`, with `key` naming the metadata field    | `metadata`                                             |
| `null`                          | `is null`, `is not null`                                        | any nullable column                                    |

`input` and `output` conditions support `=` or `matches` (full-text search); substring operators such as `contains`, `starts with`, and `ends with` are rejected for these columns. Each request that filters these fields must include at least one `=` or `matches` condition. There is no fixed `tags` parameter: filter tags with `{"type":"arrayOptions","column":"tags","operator":"any of","value":["prod"]}`. A filter-bar expression such as _name does not contain "healthcheck"_ translates to:
...
The complete list of columns and operators per type is in the [API reference](https://api.reference.langfuse.com/#tag/observationsv2/GET/api/public/v2/observations) under the `filter` parameter.

...
for the recommended migration path from older trace, observation, and metrics reads.
...
The older data retrieval endpoints `GET /api/public/traces`, `GET /api/public/traces/{traceId}`, `GET /api/public/observations`, `GET /api/public/observations/{observationId}`, and `GET /api/public/metrics` remain available, but are not recommended as the default for new extraction workflows because they are less performant at scale. See [Upgrade to v2 data APIs](/docs/api-and-data-platform/features/migrate-to-v2-data-apis) for migration examples.

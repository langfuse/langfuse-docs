...
For new data extraction workflows, use Observations API v2 or Metrics API v2. Existing `GET /api/public/traces`, `GET /api/public/traces/{traceId}`, `GET /api/public/observations`, `GET /api/public/observations/{observationId}`, and `GET /api/public/metrics` integrations remain available, but they are no longer the default path in these docs.
...
graph LR
    ...
    LF --> API["Public API<br/>(resource management)"]

...
The legacy data retrieval endpoints `GET /api/public/traces`, `GET /api/public/traces/{traceId}`, `GET /api/public/observations`, `GET /api/public/observations/{observationId}`, and `GET /api/public/metrics` are deprecated. New data extraction workflows should use Observations API v2 or Metrics API v2.
...
<Cards num={3}>
  ...
  />
  <Card
    title="Upgrade from deprecated APIs"

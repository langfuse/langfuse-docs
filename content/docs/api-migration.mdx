...
Still need the legacy endpoints? They are documented in the [Legacy v3 API reference](/docs/api-and-data-platform/features/legacy-v3-api).
...
In `legacy` and `dual` write modes, the legacy write path keeps working; it breaks after the cutover to `events_only`. See the [v3 to v4 upgrade guide](/self-hosting/upgrade/upgrade-guides/upgrade-v3-to-v4).

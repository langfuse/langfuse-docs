...
[Langfuse v4](/docs/v4) is generally available on Langfuse Cloud and self-hosted. On Cloud, where v4 has been rolling out in preview since March 2026, Langfuse will switch to v4 as the only experience, at which point remaining legacy APIs and ingestion paths are also removed (cutover date will follow). Self-hosted deployments can [upgrade to v4](/self-hosting/upgrade/upgrade-guides/upgrade-v3-to-v4) now.

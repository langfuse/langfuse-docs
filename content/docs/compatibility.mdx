...
Langfuse Cloud currently runs v3 with all [v4 features available in preview](/docs/v4) and switches to v4 only at the end of October 2026. The v4 release for self-hosted deployments launches by August 1, 2026.
...
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Langfuse Cloud**                 | Always runs the latest Langfuse version; server versions are managed for you. Only your SDK version and the API endpoints you call matter; the self-hosted server minimums on this page do not apply. Breaking removals happen on published dates (see [deprecations](/self-hosting/upgrade/deprecations)).                                                                                                                       |
...
| **Deprecated**  | Still works but is superseded; it will be removed on the dates listed on the [deprecations page](/self-hosting/upgrade/deprecations). Migrate to the replacement. |
...
| Public API & querying (`api.*`) | Use the [legacy APIs](/docs/api-and-data-platform/features/public-api) `api.legacy.observations_v1` / `api.legacy.metrics_v1` | Supported (v1 APIs are deprecated until Nov 30, 2026) |
...
| Public API & querying (`api.*`) | Use the [legacy APIs](/docs/api-and-data-platform/features/public-api) `api.legacy.observationsV1` / `api.legacy.metricsV1` | Supported (v1 APIs are deprecated until Nov 30, 2026) |
...
Deprecated. Trace, span, and generation events via the legacy batch ingestion API are not supported on the v4 data model: on Langfuse Cloud they keep working until **November 30, 2026** (see [deprecations](/self-hosting/upgrade/deprecations)), on self-hosted Langfuse v4 they are rejected.

</CompatDetail>
<CompatDetail name="v1 read APIs">

Deprecated. Removed on Langfuse v4 (Langfuse Cloud: **November 30, 2026**, see [deprecations](/self-hosting/upgrade/deprecations)). The full list and replacements:
...
| `/api/public/sessions`<br />`/api/public/sessions/{id}`                                                            | `GET /api/public/v2/observations?sessionId={id}&fromStartTime={datetime}&toStartTime={datetime}`                              |
...
GA. Requires Langfuse v4. See the [Observations API v2](/docs/api-and-data-platform/features/observations-api) and [Metrics API v2](/docs/metrics/features/metrics-api) docs.
...
Deprecated. Migrate to [observation-level evaluators](/faq/all/llm-as-a-judge-migration). Trace-level evaluators are not supported on the v4 data model; on Langfuse Cloud they keep running until **November 30, 2026** (see [deprecations](/self-hosting/upgrade/deprecations)).

</CompatDetail>

<CompatDetail name="Legacy export source (traces and observations)">

Deprecated. Exports built on the legacy "traces and observations" source are replaced by the enriched observations source (v4 data model). On Langfuse Cloud, projects created on or after 2026-05-20 cannot select legacy sources, new legacy export integrations cannot be created since 2026-06-22, and remaining legacy exports are switched to the enriched source automatically on **November 30, 2026** (see [deprecations](/self-hosting/upgrade/deprecations)).
...
Python SDK v2 and JS/TS SDK v3 (and older) send traces via the legacy batch ingestion API, which is removed on Langfuse Cloud on **November 30, 2026**. Upgrade to the GA SDK majors before then; see the [deprecations page](/self-hosting/upgrade/deprecations) and the [SDK upgrade paths](/docs/observability/sdk/upgrade-path).

...
| Public API & querying (`api.*`) | Use the [legacy APIs](/docs/api-and-data-platform/features/public-api) `api.legacy.observations_v1` / `api.legacy.metrics_v1` | Supported (v1 APIs are deprecated until Oct 31, 2026) |
...
| Public API & querying (`api.*`) | Supported                                         | v1 APIs are deprecated until Oct 31, 2026         |
...
| -------------------------------- | ------------------------------------------------- | ------------------------------------------------- | ------------------------------------------------- |
| Tracing (legacy batch ingestion) | Supported                                         | Supported                                         | Stops Oct 31, 2026                                |
...
| Public API & querying (`api.*`)  | Supported                                         | Supported                                         | v1 APIs are deprecated until Oct 31, 2026         |
...
| Public API & querying (`api.*`) | Use the [legacy APIs](/docs/api-and-data-platform/features/public-api) `api.legacy.observationsV1` / `api.legacy.metricsV1` | Supported (v1 APIs are deprecated until Oct 31, 2026) |
...
Deprecated. Trace, span, and generation events via the legacy batch ingestion API are not supported on the v4 data model: on Langfuse Cloud they keep working until **October 31, 2026**, on self-hosted Langfuse v4 they are rejected.

</CompatDetail>
<CompatDetail name="v1 read APIs">

Deprecated. Removed on Langfuse v4 (Langfuse Cloud: **October 31, 2026**). The full list and replacements:
...
Deprecated. Migrate to [observation-level evaluators](/faq/all/llm-as-a-judge-migration). Trace-level evaluators are not supported on the v4 data model; on Langfuse Cloud they keep running until **October 31, 2026**.

</CompatDetail>

<CompatDetail name="Legacy export source (traces and observations)">

Deprecated. Exports built on the legacy "traces and observations" source are replaced by the enriched observations source (v4 data model). On Langfuse Cloud, projects created on or after 2026-05-20 cannot select legacy sources, new legacy export integrations cannot be created since 2026-06-22, and remaining legacy exports are switched to the enriched source automatically on **October 31, 2026**.
...
Python SDK v2 and JS/TS SDK v3 (and older) send traces via the legacy batch ingestion API, which is removed on Langfuse Cloud on **October 31, 2026**. Upgrade to the GA SDK majors before then; see the [SDK upgrade paths](/docs/observability/sdk/upgrade-path).

...
| -------------------------------------------------------------------------------------------------- | ---------------------- |
| `page`                                                                                             | `cursor`               |
| `GET /scores/{scoreId}`, `GET /v2/scores/{scoreId}`                                                | `id=<scoreId>` filter  |
| `value` + `stringValue`                                                                            | Single typed `value`   |
| `operator` + `value`                                                                               | `valueMin`, `valueMax` |
| `datasetRunId`                                                                                     | `experimentId`         |
| `userId`, `traceTags`                                                                              | Removed                |
| `fromTimestamp`, `toTimestamp`, `name`, `source`, `dataType`, `environment`, `configId`, `queueId` | Unchanged              |
...
- **Cursor pagination.** `cursor` comes from the previous response's `meta`; `limit` max 100. There is no get-by-id endpoint in v3; fetch single scores via the `id` filter.
- **Filters.** `name`, `source`, `dataType`, `environment`, `configId`, and `queueId` accept comma-separated lists (OR within a parameter, AND across parameters). `valueMin`/`valueMax` are inclusive bounds and require `dataType=NUMERIC`. `userId` and `traceTags` are removed; for trace-level score questions, use the [Metrics API v2](/docs/metrics/features/metrics-api#v2) score views. Use at most one of `traceId`, `sessionId`, and `experimentId`; `observationId` requires `traceId` alongside it (observation IDs are scoped to a trace).
...
For other languages, send experiment traces to [`POST /otel/v1/traces`](https://api.reference.langfuse.com/#tag/opentelemetry/POST/api/public/otel/v1/traces) and apply the [required experiment attributes](/integrations/native/opentelemetry#experiments-ingesting-experiment-spans).

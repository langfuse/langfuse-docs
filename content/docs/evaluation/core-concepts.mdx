...
- [Score live production traces](/docs/evaluation/get-started/online) with an evaluator and a rule
...
- [Use Jev as a judge](/docs/evaluation/evaluation-methods/jev-as-a-judge) for typed verdicts at a fraction of the cost of an LLM judge
...
| [Jev as a judge](/docs/evaluation/evaluation-methods/jev-as-a-judge)       | Ask TypeSafe's Jev decision model typed questions and get calibrated answers | Typed verdicts at volume (classification, rubric levels, yes/no flags) at low cost and latency |
...
Follow [Evaluate Production Traffic](/docs/evaluation/get-started/online) to create an evaluator and attach a rule in the UI.
...
### Run an evaluator on historical observations [#backfill-evaluator-scores]

When you save an evaluator, enable **Also run on past observations** to score existing observations that match the rule. Choose a preset or custom range of up to six months and evaluate up to 25,000 observations. Check the estimated LLM cost before starting the evaluation.

For a one-off evaluation of observations selected from the traces table, use [batch evaluation](#batch-evaluation) instead.

### Monitor evaluator results [#monitor-evaluator-results]

Use [custom dashboards](/docs/metrics/features/custom-dashboards) to track scores and application performance over time. To get notified when an evaluator's score or cost crosses a threshold, open the evaluator and click **Add alert**.

You can create these prefilled alerts:

- **Score threshold:** Numeric and Boolean outputs use the average score over one day by default. Categorical outputs use the score count over one day. For code evaluators, Langfuse uses the first declared numeric, Boolean, or categorical score output. The shortcut is unavailable when the first output is a text score.
- **Cost threshold:** Available for individual LLM-as-a-Judge evaluators. On the **Evaluators** page, click **Add alert** to create an alert for the combined cost of all evaluators.

Langfuse prefills the metric, evaluator filters, and tags and excludes evaluator test runs. Review the threshold, time window, notifications, and other settings before saving.

The **Add alert** menu also lists alerts connected to the evaluator with their current severity, trigger condition, and last-fired time. It shows up to 20 alerts. Select an alert to open it, or click **See all alerts** to open the alerts table filtered to that evaluator. See the [alerts guide](/docs/observability/features/alerts#alert-setup) for the full setup flow.
...
See the [experiments data model](/docs/evaluation/experiments/data-model) for these objects.
...
You can **run experiments programmatically using the Langfuse SDK**. This gives you full control over the task, evaluation logic, and more. Pass evaluator functions to the runner so they receive each item's expected output. [Learn more about running experiments via SDK](/docs/evaluation/experiments/experiments-via-sdk).

You can **run experiments directly from the Langfuse interface** by selecting a dataset and prompt version. This is useful for quick iterations on prompts without writing code. Select Langfuse-managed evaluators on the run if you want them to see expected output. [Learn more about running experiments via UI](/docs/evaluation/experiments/experiments-via-ui).

If you already emit traces through OpenTelemetry and are not using the Python or JS/TS SDK, **attach experiment attributes to those spans** so Langfuse groups them as an experiment run. Score that pipeline yourself. [Learn more about experiments via OpenTelemetry](/docs/evaluation/experiments/experiments-via-opentelemetry).

A **remote webhook** on a dataset only starts your endpoint. It does not attach Langfuse-managed evaluators. Score in the webhook, or see [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments).

...
## Review experiment answers with a QA team [#review-experiment-answers]
...
1. Run your application on the test cases, or [publish saved check results](/resources/engineering/evaluate-existing-application#saved-check-results). Include the question, relevant source material, reference answer, and case ID in the recorded input or metadata so reviewers have the evidence they need. Only upload data that belongs in this project.
2. [Compare the experiment runs](/docs/evaluation/experiments/compare-experiments) and open the items that need review. Add the relevant **observations** to the queue through the **Annotate** dropdown. For version comparisons, include both versions and identify the baseline and candidate in the queue description.

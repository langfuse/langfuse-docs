...
<Callout type="info">
  This feature requires [Fast Preview mode](/docs/v4) to be enabled. Fast
  Preview is in open beta and currently available on Langfuse Cloud
  only—enable it in the bottom left of the UI to get started.
</Callout>

Code-based evaluators run custom Python or TypeScript logic in Langfuse and return one or more [scores](/docs/evaluation/scores/overview). Use them for deterministic, objective checks where code is more reliable than a model-based judgment.

Common examples include exact match checks, regex validation, JSON parseability, schema validation, keyword checks, tool-call checks, latency or cost thresholds, and custom business rules.
...
## How to use code-based evaluators?

Code-based evaluators can run on two types of data: **Observations** (individual operations from live production traffic) or **Experiments** (controlled test datasets). Your choice depends on whether you're testing in development or
...
- **Deterministic production monitoring**: Check JSON validity, schema compliance, exact matches, latency thresholds, or business rules on live traffic.
- **Compositional evaluation**: Run different code-based evaluators on different operations within one trace.
...
- Monitor latency or cost thresholds for selected model calls
...
3. Select code-based evaluators to score the generated observations.
...
Go to the Evaluators page and create a new code-based evaluator.
...
| `data_type` / `dataType` | Optional for numeric and categorical scores, required when returning boolean or text scores. Supported values are [`NUMERIC`, `CATEGORICAL`, `BOOLEAN`, and `TEXT`](/docs/evaluation/scores/overview#score-types). |
...
## Debug code-based evaluator executions

Every code-based evaluator execution creates a trace, giving you complete visibility into the evaluation process. This lets you inspect the selected inputs and outputs, experiment context, runtime latency, returned scores, logs, and errors.

You can show code-based evaluator execution traces by filtering for the environment `langfuse-code-eval` in the tracing table:

<Frame fullWidth>
  ![Tracing table filtered to code-based evaluator
  executions](/images/docs/evaluation/eval-execution-traces-tracing-table.png)
</Frame>

<Details>
<Summary>Code-based evaluator execution status</Summary>
...
Code-based evaluators are intended for compact, deterministic checks that can run quickly and safely for many observations.

<Callout type="info">
  Need a specific third-party library or network access for code-based
...
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Languages         | Write evaluators in Python or TypeScript.                                                                                                                                                                                                                  |
...
No. Code-based evaluators currently support standard libraries only. If your evaluation requires a third-party package, run that logic in your own infrastructure and ingest the result with [Scores via API/SDK](/docs/evaluation/evaluation-methods/scores-via-sdk).
...
<Details>
<Summary>Can I create code-based evaluators via API or SDK?</Summary>

Not yet. Create and manage code-based evaluators in the Langfuse UI. The public evaluator API currently remains scoped to LLM-as-a-Judge evaluators while the code-based evaluator contract is in Fast Preview.
...
<Details>
<Summary>Why can't I find code-based evaluator execution traces?</Summary>

Code-based evaluator executions use the internal environment `langfuse-code-eval`. Internal environments are hidden from the default tracing view, so filter the tracing table by `environment = langfuse-code-eval` or open the execution trace from the related score or evaluator log.

</Details>

<Details>
<Summary>How do I configure code-based evaluators on self-hosted Langfuse?</Summary>

Code-based evaluators currently require [Langfuse Fast Preview](/docs/v4), which is available on Langfuse Cloud. OSS and self-hosted deployments will receive support as soon as Langfuse v4 is supported outside Langfuse Cloud.

Follow the [Fast Preview docs](/docs/v4) and the dedicated [GitHub Discussion](https://github.com/orgs/langfuse/discussions/12518) for rollout updates.

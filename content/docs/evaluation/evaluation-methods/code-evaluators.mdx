...
Use [LLM-as-a-Judge](/docs/evaluation/evaluation-methods/llm-as-a-judge) instead when the evaluation needs semantic judgment, rubric-based reasoning, or subjective assessment such as helpfulness, tone, or answer quality. To score live production traces with an evaluator and a rule, start with [Evaluate Production Traffic](/docs/evaluation/get-started/online).
...
Each experiment run generates traces and observations that can be scored by your selected evaluators. The evaluator receives observation data plus experiment item context, such as expected output and item metadata, **only when you select it on a prompt experiment**.
...
3. For prompt experiments, select code evaluators so they receive experiment context. For webhook or SDK runs, score in your pipeline. See [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments).
...
| `ctx.experiment`                                                                                  | Present only when you select the evaluator on a [prompt experiment](/docs/evaluation/experiments/experiments-via-ui). Absent when a rule scores an observation, including observations from webhook or SDK experiments. |
...
`ctx.experiment` is present only when you select the evaluator on a [prompt experiment](/docs/evaluation/experiments/experiments-via-ui). It is `None` / `undefined` when a [rule](/docs/evaluation/core-concepts#evaluators-and-rules) scores an observation, even if that observation came from a webhook or SDK experiment. For those runs, score in your pipeline. See [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments).

For live observation evaluators, write your code so it handles `ctx.experiment` being `None` in Python or `undefined` in TypeScript.

...
### Add a TypeSafe connection [#add-a-typesafe-connection]

Go to **Settings → LLM Connections** in your project and add a connection with the adapter **typesafe**. Then choose the **upstream** that serves Jev and paste its API key:
...
Vercel AI Gateway and OpenRouter expose TypeSafe's API, so evaluators behave the same on every upstream. The connection offers the `jev-1.13.0` and `jev-latest` models by default. No base URL or additional headers are required. Switching an existing connection to another upstream requires the API key for the new upstream.
...
- **User Conversation Signal** (Yes / no × 7): score a chat for rephrases, corrections, human hand-off requests, repeats, error quotes, frustration, and success confirmation in one call. See the [blog post](/blog/2026-09-23-extract-signals-with-jev).
...
| Data handling           | State and questions are sent to TypeSafe's API, through Vercel AI Gateway or OpenRouter if you chose that upstream. Review the terms of your upstream and of [TypeSafe](https://typesafe.ai) before evaluating sensitive data, and use [masking](/docs/observability/features/masking) where needed. |
...
Create an account at [typesafe.ai](https://typesafe.ai) and generate a key in the [TypeSafe console](https://console.typesafe.ai/settings/keys). Access currently runs through a waitlist on TypeSafe's side. If you already use Vercel AI Gateway or OpenRouter, you do not need a TypeSafe account: pick that upstream in the connection and use its API key instead. See [Add a TypeSafe connection](#add-a-typesafe-connection).

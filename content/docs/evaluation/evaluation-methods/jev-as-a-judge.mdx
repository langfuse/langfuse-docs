...
description: Use TypeSafe's Jev System One model as a judge in Langfuse. Ask typed questions about each observation and get calibrated scores at a fraction of the cost and latency of an LLM judge.
---
...
<Callout type="info">
  ...
  The evaluator type is named generically so that similar models from other
  providers can be added later. Jev is currently the only supported model.
</Callout>
...
Jev answers three kinds of questions, which TypeSafe calls primitives. Each maps to one Langfuse score. In the evaluator editor, a Noul is labeled **Yes / no**.

| Question type                                                                 | What Jev returns                                                                                                          | Langfuse score                                                  | Example                                                                   |
| ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------- |
| **[Choice](https://docs.typesafe.ai/primitives/choice)**                      | One option out of a fixed set you define (2 to 255), with probability per option and a confidence value                   | Categorical score with the selected option as value             | "Which team should handle this ticket?" → `billing`, `technical`, `sales` |
| **[Score](https://docs.typesafe.ai/primitives/score)**                        | A position on ordered levels you describe (2 to 10), with the probability distribution over levels and a confidence value | Numeric score with the expected level, from `0` to `levels - 1` | "How frustrated is the customer?" → `0` calm ... `3` angry                |
| **[Noul](https://docs.typesafe.ai/primitives/noul)** (**Yes / no** in the UI) | The probability that a statement is true                                                                                  | Numeric score with `P(true)` between `0` and `1`                | "Does the message request a refund?" → `0.93`                             |
...
- **Noul** works well for guardrails and flags: out-of-scope request, PII in the output, user disagreement, policy violation. Optionally describe what counts as `true` and what counts as `false`. Because you get a probability instead of a hard label, you can pick a threshold that fits your risk tolerance, or use three bands: act, review, ignore.

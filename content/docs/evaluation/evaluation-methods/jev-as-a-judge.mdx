...
Define the state that Jev judges. Click **Add field** to add a state key, then map it to observation data: input, output, metadata, or tool calls. If you plan to use the evaluator in [prompt experiments](/docs/evaluation/experiments/experiments-via-ui), also map **Expected Output** and **Experiment Item Metadata**. Those fields stay empty on an observation-level rule, including rules that match webhook or SDK experiment traces. See [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments).
...
Yes. Jev evaluators run on incoming observations through rules, on historical observations through [batch evaluation](/docs/evaluation/core-concepts#batch-evaluation), and on [prompt experiments](/docs/evaluation/experiments/experiments-via-ui). Map **Expected Output** and **Experiment Item Metadata** into the state when the questions need them. Those mappings populate only on prompt experiments, not on rules that match webhook or SDK experiment traces. See [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments).

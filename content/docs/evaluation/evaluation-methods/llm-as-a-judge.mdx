...
A judge is a prompt you wrote, so check that it scores cases the way your reviewers would before you rely on it: [calibrate it against labeled examples](/guides/llm-as-a-judge-calibration-skill), or compare its scores with human annotation scores on the same data in [Score Analytics](/docs/evaluation/scores/score-analytics#human-vs-ai-annotation-agreement).
...
<Callout type="info">
  ...
  pipeline](/docs/evaluation/evaluation-methods/scores-via-sdk). Need a
  typed verdict (label, rubric level, or yes/no) on every observation at a
  fraction of the cost? Use [Jev as a
  judge](/docs/evaluation/evaluation-methods/jev-as-a-judge).
</Callout>
...
## Where evaluators run [#where-evaluators-run]

LLM-as-a-Judge evaluators are executed by the Langfuse **worker**: it picks up each matching observation from a queue, renders the evaluation prompt, calls the model through the project's [LLM connection](/docs/administration/llm-connection), and writes the resulting score back. Nothing runs inside your application or in the browser.

For self-hosted deployments this means the worker container — not the web container and not your instrumented application — needs network egress to the LLM provider or gateway configured in the LLM connection, and the [SSRF allowlist variables](/docs/administration/llm-connection#blocked-ip-address) apply to that call. Evaluator execution can be pinned to specific worker replicas with `QUEUE_CONSUMER_EVAL_EXECUTION_QUEUE_IS_ENABLED` and scaled with `LANGFUSE_EVAL_EXECUTION_WORKER_CONCURRENCY` (default `5`), but there is no option to run managed evaluators outside the worker. On Langfuse Cloud, evaluators run in Langfuse's infrastructure. [Code evaluators](/docs/evaluation/evaluation-methods/code-evaluators) use a separate worker queue and sandbox.
...
### "Model configuration not valid for evaluation" [#model-configuration-not-valid]

When you save an LLM-as-a-Judge evaluator, Langfuse sends one short test request to the selected model using the evaluator's structured output definition (the score type you chose). If that request fails, saving is blocked with `Model configuration not valid for evaluation.` followed by the error the provider returned. Typical causes:

- The model or the proxy in front of it does not support structured outputs (JSON schema). Provider errors that mention `response_format` usually point to this. Choose a model that supports structured output, or route to the provider directly instead of through a proxy that strips the parameter.
- The model name does not exist at the provider, or the API key or base URL of the [LLM Connection](/docs/administration/llm-connection) is wrong. The provider's error text names the exact problem.

Fix the connection or pick a different model, then save again. The same check runs when you change the model of an existing evaluator.
...
Research shows that strong LLM judges (such as GPT-5 class models) achieve 80-90% agreement with human evaluators on many quality dimensions, which is comparable to inter-annotator agreement between humans. Accuracy improves significantly with well-designed rubrics and clear evaluation criteria. For best results, [calibrate your LLM-as-a-Judge setup](/guides/llm-as-a-judge-calibration-skill) against a small set of human-annotated examples, and use [Score Analytics](/docs/evaluation/scores/score-analytics#human-vs-ai-annotation-agreement) to measure agreement between judge and human scores.
...
- [Score Analytics](/docs/evaluation/scores/score-analytics#human-vs-ai-annotation-agreement) measures agreement between judge scores and human annotation scores, for example with Cohen's Kappa and a confusion matrix.

...
- [LLM-as-a-judge for a customer support chatbot](/resources/engineering/llm-as-a-judge-customer-support-chatbot) is a worked setup: rubric, variable mapping, rule, and alerting for one concrete application.
- [LLM evals without glue code](/resources/engineering/llm-evals-without-glue-code) shows the managed path from a production trace to a dataset, a judge, and a CI gate.

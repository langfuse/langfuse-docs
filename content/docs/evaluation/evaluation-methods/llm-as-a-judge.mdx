...
Observation-level evaluators map variables from the matched observation. You can select its input, output, metadata, or tool calls.

Expected output and experiment item metadata are **not** available on this path. They are supplied only when you select the evaluator on a [prompt experiment](/docs/evaluation/experiments/experiments-via-ui). A [rule](/docs/evaluation/core-concepts#evaluators-and-rules) that matches traces from a webhook or SDK experiment still cannot load the linked dataset item, so any variable mapped to **Expected Output** stays empty. Score those runs in your pipeline instead. See [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments).
...
3. Select evaluators on the prompt experiment so they receive experiment context (expected output and item metadata)
4. Compare results across experiment runs to make data-driven decisions

Webhook and SDK experiments do not use this selection step. Score them in your pipeline. See [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments).
...
2. Write or edit the evaluation prompt with `{{variables}}` for the data the judge needs, such as `{{input}}`, `{{output}}`, or `{{expected_output}}`. Names like `{{ground_truth}}` are labels you choose, not reserved parameters.
...
Map each prompt variable by clicking the data you want to use. For online evaluation, you can select an observation's input, output, metadata, or tool calls. If you plan to use the evaluator in [prompt experiments](/docs/evaluation/experiments/experiments-via-ui), also map **Expected Output** and **Experiment Item Metadata**.

<Callout type="info">
  Mapping a variable to **Expected Output** does nothing on an observation-level
  rule, including rules that match webhook or SDK experiment traces. Score
  those runs in your pipeline. See [Evaluate externally run
  experiments](/docs/evaluation/experiments/evaluate-external-experiments).
</Callout>
...
- Continue without a rule. You can still use the evaluator for [batch evaluation](/docs/evaluation/core-concepts#batch-evaluation) or [prompt experiments](/docs/evaluation/experiments/experiments-via-ui). To score [webhook or SDK experiments](/docs/evaluation/experiments/evaluate-external-experiments), evaluate in your pipeline instead of adding a rule.
...
<Details>
<Summary>Can I map expected output on a rule that scores webhook experiments?</Summary>

No. Observation-level rules only see the observation's input, output, metadata, and tool calls. Expected output lives on the dataset item and is passed to the judge only when you select the evaluator on a [prompt experiment](/docs/evaluation/experiments/experiments-via-ui). For webhook or SDK runs, score in your pipeline. See [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments).

...
These dimensions overlap with the metric names used in evaluation libraries: faithfulness and groundedness measure whether the output is supported by the provided context, answer relevance (also called answer relevancy) measures whether the output addresses the question, and context relevance measures retrieval quality. Pick the managed evaluator for the matching quality dimension, or recreate a specific metric definition as a custom evaluator. We also publish judge-prompt recipes for [RAG faithfulness](/resources/engineering/rag-faithfulness-evaluation), [answer relevance](/resources/engineering/answer-relevance-evaluation), and [hallucination detection](/resources/engineering/hallucination-detection).
...
Every LLM-as-a-Judge evaluator execution creates a full trace, giving you complete visibility into the evaluation process. This allows you to debug prompt issues, inspect model responses, monitor token usage, and trace evaluation history. For a production runbook covering targeting, sampling, backfills, and the checklist for evaluators that produce no scores, see [Evaluator Operations](/docs/evaluation/evaluator-operations).

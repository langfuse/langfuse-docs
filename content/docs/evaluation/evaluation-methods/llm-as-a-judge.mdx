...
**Prerequisite**: Enable the **[Preview](/docs/v4)** (Fast Mode) toggle for the evaluator. To use the same evaluator on newly ingested data in real time, upgrade to Python SDK ≥ 4.7.0 or JS/TS SDK ≥ 5.4.0, or, if you ingest directly via OTEL, [set `x-langfuse-ingestion-version: 4` on your OTEL span exporter](/integrations/native/opentelemetry#real-time-ingestion).

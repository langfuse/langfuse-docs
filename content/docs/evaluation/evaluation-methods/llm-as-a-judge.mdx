...
If the mapped field holds a JSON object, you can optionally narrow it down with a **JsonPath** selector, for example `$.messages[-1:].content` to pass only the last message. Without a selector, the entire object is passed into the prompt as a JSON string. Selector behavior:

- A selector that matches exactly one value passes that value; a selector that matches several values (for example `$.messages[*].content`) passes an array of them.
- A selector that matches nothing leaves the variable empty.
- If the mapped field is a plain string or number rather than an object, the selector is skipped and the whole value is passed.
- Filter expressions (`[?(...)]`), script expressions (`[(...)]`), and negative indices such as `[-1]` are not supported. Use a slice like `[-1:]` instead of `[-1]`. The editor warns about these and blocks saving until they are removed.

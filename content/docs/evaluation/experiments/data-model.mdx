...
To **read** experiment runs, items, and scores after they exist, use the [Experiments API](/docs/api-and-data-platform/features/public-api#experiments). There is no public REST endpoint for creating new experiment runs; the legacy `POST /api/public/dataset-run-items` path is deprecated.

Langfuse-managed evaluators receive expected output only on [prompt experiments](/docs/evaluation/experiments/experiments-via-ui). SDK, webhook, and OpenTelemetry runs score in your pipeline. See [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments).
...
Experiment metadata and description describe the entire run, not individual
items. The Python and JS/TS Experiment runners propagate experiment metadata
to child observations, but set description directly on experiment-item root
observations without propagating it to children. Keep each field consistent
across the observations that carry it. For direct ingestion, follow the
[OpenTelemetry experiment attribute guide](/integrations/native/opentelemetry/experiments).
...
With Langfuse v4 and the current SDKs, [experiments on local data](/docs/evaluation/experiments/experiments-via-sdk) appear under **Experiments** without a hosted dataset. Each task execution also creates a trace for debugging. See [Compare experiments](/docs/evaluation/experiments/compare-experiments).

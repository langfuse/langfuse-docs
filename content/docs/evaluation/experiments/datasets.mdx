...
A dataset is a collection of inputs and expected outputs and is used to test your application. [UI-based](/docs/evaluation/experiments/experiments-via-ui), [SDK-based](/docs/evaluation/experiments/experiments-via-sdk), and [OpenTelemetry](/docs/evaluation/experiments/experiments-via-opentelemetry) experiments can all use Langfuse Datasets.
...
## Multi-modal dataset items [#multi-modal-dataset-items]
...
### Run experiments on versioned datasets [#run-experiments-on-versioned-datasets]
...
<Tab>
In the UI, use `+ Add to dataset` on any [observation](/docs/observability/data-model#observations-traces-and-sessions) of a production trace.
...
import { Table, WandSparkles, CodeXml, Database, Radio } from "lucide-react";
...
<Cards num={1}>
  ...
  <Card
    ...
    href="/docs/evaluation/experiments/experiments-via-opentelemetry"
    arrow
  />
</Cards>

...
def check_approved_baseline(result, *, dataset_version):
    baseline = json.loads(Path("experiments/approved-baseline.json").read_text())
    if (
        datetime.fromisoformat(baseline["dataset_version"].replace("Z", "+00:00"))
        != datetime.fromisoformat(dataset_version.replace("Z", "+00:00"))
...
Call `check_approved_baseline(result, dataset_version=version)` from your experiment entry point before returning `result`. Here, `version` is the ISO timestamp used to fetch the candidate dataset. Increment `EVALUATOR_VERSION` whenever the grader changes and review a new baseline before using it.

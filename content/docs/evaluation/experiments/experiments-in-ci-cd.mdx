...
description: Run Langfuse experiments in CI and gate changes from production.
---
...
1. Store your test cases in a [Langfuse dataset](/docs/evaluation/experiments/datasets).
...
3. Add [evaluators](/docs/evaluation/experiments/experiments-via-sdk#evaluators) to score the experiment results.
...
<Callout type="info">
  The GitHub Action requires Langfuse Python SDK v4.6.0 or newer, or Langfuse JS
  SDK v5.3.0 or newer.
</Callout>
...
| `should_skip_sdk_installation` | No       | Skip SDK installation when you manage the Python or Node environment yourself before this action. For TypeScript experiments, provide `tsx` yourself. Defaults to `false`. |
...
def test_geography_accuracy_passes(langfuse_client):
    ...
    avg_accuracy = next(
        evaluation.value for evaluation in result.run_evaluations
        if evaluation.name == "avg_accuracy"
    )

...
Each script must define an `experiment(context)` function that accepts a `context` parameter. The action creates a `RunnerContext` and passes it to this function.

...
sidebarTitle: Evaluate Your Application
...
The scripts create a new dataset, add two reviewed cases, and capture a UTC timestamp after the writes complete. Both runs fetch that exact version. In an existing project, reuse your dataset and store its approved version timestamp with your test configuration.

...
This guide walks you through a repeatable way to measure the quality of your LLM application. You can use this to catch regressions before shipping a new version, to optimize output quality, or to reduce the cost of your system. A more conceptual introduction to experiments can be found [here](/academy/experiments).
...
and use it to set up offline evaluation for this application
with Langfuse.
```
...
The experiment run sends each question to the model. The evaluator returns `1` when the model output exactly matches the expected site name and `0` otherwise.
...
- Learn how you can choose what to evaluate

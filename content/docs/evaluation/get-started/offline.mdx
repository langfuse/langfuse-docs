...
# Get Started with Evaluating your Application

This walkthrough is for [experiments via SDK](/docs/evaluation/experiments/experiments-via-sdk): a dataset of test cases, your application or agent code as the task, and evaluators that score every run. Use it to catch regressions before shipping, optimize output quality, or reduce cost.

You can also [run prompt experiments in the UI](/docs/evaluation/experiments/experiments-via-ui) when you only need to compare prompts or models on a dataset, [score traces in the UI](/docs/evaluation/evaluation-methods/scores-via-ui), or [review items in an annotation queue](/docs/evaluation/evaluation-methods/annotation-queues).

If you don't yet know what to evaluate, start with [Choosing what to evaluate](/academy/evaluate/choosing-what-to-evaluate). For how datasets and experiments fit together, see [Datasets](/academy/datasets) and [Experiments](/academy/experiments).
...
A dataset is a collection of test cases. Each item has an **input** and, optionally, an **expected output** that evaluators can compare against. How many items you add — and whether they are questions, tickets, tool traces, or something else — depends on your use case. A first dataset might have a handful of representative cases; later you can grow it to 10, 50, or more as you learn what fails.

The rest of this step is an **example** so you can run the walkthrough end to end: five questions about San Francisco tourist sites. You do not need five items, and they do not need to be questions.
...
Use the **SDK** when the experiment should execute your own application or agent code — any task you cannot express as a prompt and model in Langfuse. Use [prompt experiments in the UI](/docs/evaluation/experiments/experiments-via-ui) when you only want to compare prompt versions or models on a dataset.

You can still start an SDK experiment from the UI: configure a [webhook](/docs/evaluation/experiments/experiments-via-sdk#configure-webhook) that runs your experiment code, then trigger it with **Run → Custom Experiment** on the dataset.

This walkthrough runs the experiment in the SDK. In the example below, each dataset item is sent to the model. The evaluator returns `1` when the output exactly matches the expected site name and `0` otherwise.

...
Already have application code or test cases? Start with [Evaluate an existing application](/docs/evaluation/get-started/existing-application) to reuse your functions and graders, compare two versions, and add a CI gate. The walkthrough below creates a new model-backed example.

...
The example below uses a small `answer_question` / `answerQuestion` application function to keep the script runnable. In your project, import your existing application function in its place and keep the task as a thin adapter. Pass all inputs your application needs; evaluate the output from the same code path you use in production. The evaluator returns `1` when that output exactly matches the expected site name and `0` otherwise.
...
For a worked example that reuses deterministic checks, compares application versions, and adds a CI gate, see [Evaluate an existing application](/resources/engineering/evaluate-existing-application).

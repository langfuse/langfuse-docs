...
Use [Langfuse Cloud](/cloud) for hosted storage and reporting, or [self-host Langfuse](/self-hosting) to operate the platform yourself. Both accept experiments from your own runner. Self-hosting Langfuse does not make calls to an external model provider local; choose your model endpoint separately. Deterministic graders require no model-provider calls, but Langfuse storage and infrastructure still have their own [pricing](/pricing) and operational requirements.

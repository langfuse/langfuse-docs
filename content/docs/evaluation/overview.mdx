...
To catch regressions before they ship, run experiments in CI: the [`langfuse/experiment-action`](/docs/evaluation/experiments/experiments-ci-cd) GitHub Action runs your experiment on every pull request and fails the job when a result raises `RegressionError`. It works with Langfuse Cloud and self-hosted Langfuse.

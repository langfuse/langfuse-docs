...
<Callout type="info">
  **Self-hosted:** The Assistant requires instance-wide Amazon Bedrock and a
  worker queue consumer. We strongly recommend an AWS Lambda MicroVM sandbox.
  See [self-hosted setup](/self-hosting/configuration/in-app-agent).
</Callout>

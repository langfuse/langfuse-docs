...
description: Ask questions about your Langfuse project data in plain language from inside the Langfuse UI.
...
<Callout type="info">
  **Self-hosted:** Set `LANGFUSE_IN_APP_AGENT_ENABLED=true`, configure an
  instance-wide Langfuse AI model (Amazon Bedrock or Anthropic Messages), and
  enable the worker queue consumer. We strongly recommend an AWS Lambda MicroVM
  sandbox. The Assistant is in public beta, so expect its deployment and
  configuration surface to keep changing while it is. See [self-hosted
...
- On Langfuse Cloud, model requests use Langfuse-managed Amazon Bedrock in your [Langfuse data region](/security/data-regions). On self-hosted deployments, requests use the instance Langfuse AI model you configure.
...
- [Self-hosted Assistant setup](/self-hosting/configuration/langfuse-assistant)

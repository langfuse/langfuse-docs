...
4. Configure your widget:
   - **Data Source**: Choose from traces, observations, or scores (numeric, categorical, or boolean)
...
Sessions and users are not data sources of their own. To analyze at the session or user level, build the widget on **Traces** and use `sessionId` or `userId` as the dimension, or pick the unique-session or unique-user measure — for example "unique sessions per day" or "total cost by userId". The [Metrics API](/docs/metrics/features/metrics-api) exposes the same views, measures, and dimensions.

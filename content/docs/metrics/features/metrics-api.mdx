...
A trace should not submit more than one semantic root; multiple matching roots indicate an integration issue. The filter omits rootless traces, such as distributed traces whose root lives in another service that does not export to the same Langfuse project. For complete trace counts in projects that can contain rootless traces, use [Observations API v2](/docs/api-and-data-platform/features/observations-api#v2) and group by `traceId` client-side.

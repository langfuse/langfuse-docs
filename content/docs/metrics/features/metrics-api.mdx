...
The older `GET /api/public/metrics` endpoint remains available, but is not recommended as the default for new aggregate data extraction workflows because it is less performant at scale. Use Metrics API v2 for new reporting, billing, dashboard, and monitoring workflows. See [Upgrade to v2 data APIs](/docs/api-and-data-platform/features/migrate-to-v2-data-apis) for migration examples.

...
The legacy `GET /api/public/metrics` endpoint is deprecated. Use Metrics API v2 for new aggregate data extraction workflows. See [Upgrade to v2 data APIs](/docs/api-and-data-platform/features/migrate-to-v2-data-apis) for migration examples.
...
**Data availability:** Data from older SDKs (`langfuse-python` < `4.0.0`, `langfuse-js` < `5.0.0`) or direct OpenTelemetry exporters that do not send `x-langfuse-ingestion-version: 4` can be delayed on v2 endpoints. Upgrade to [Python SDK v4](/docs/observability/sdk/upgrade-path/python-v3-to-v4) or [JS/TS SDK v5](/docs/observability/sdk/upgrade-path/js-v4-to-v5), or set that header on your OTEL span exporter, to see new data in real time.
...
## Deprecated Metrics API v1 [#v1]

The v1 `GET /api/public/metrics` endpoint is deprecated and retained only for backward compatibility. Use [Metrics API v2](#v2) for new reporting, billing, dashboard, and monitoring workflows.

The deprecated v1 Metrics API supports querying across different views (traces, observations, scores) and allows you to:

...
**Self-hosted:** The v2 Metrics API requires [Langfuse v4](/docs/v4). Self-hosted v4 support is in progress — until it ships, use the [legacy Metrics API v1](#v1).
...
On Langfuse Cloud, the v2 Metrics API uses its own per-organization rate-limit bucket: 100 requests/day (Hobby), 100 requests/hour (Core), and 500 requests/hour (Pro/Enterprise, custom limits available). See the [API limits FAQ](/faq/all/api-limits) for details. Self-hosted instances have no enforced rate limits.

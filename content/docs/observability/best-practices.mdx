...
A tool call should nest under the `agent` or `span` [observation](/docs/observability/data-model#observations-traces-and-sessions) that orchestrates the step, as a sibling of the `generation` that requested it, so the tree shows which step each action belongs to instead of leaving tool calls dangling at the trace root.
...
Reasoning models produce **thinking** (also called **reasoning**) before they answer or call a tool. That text should appear on each `generation` in the tree, not only on the final action.

When a later tool call or answer is wrong, thinking is often the only record of why. A `refund_order` call can look valid while the thinking shows a bad assumption — for example, treating a repeated question as a duplicate charge. [Reasoning tokens](/docs/observability/features/token-and-cost-tracking#cost-inference-for-reasoning-models) on the generation tell you that the model thought, not what it thought.

Native [integrations](/integrations) usually capture thinking when the provider returns it. If you instrument manually, include it in the generation output so Langfuse can render it as a thinking block. Some providers hide the raw chain of thought and only return a summary when you opt in — request and store that summary if it is all you get.

### Is there noise you don't need?

Not every observation in the tree is useful for understanding what your application did. HTTP [observations](/docs/observability/data-model#observations-traces-and-sessions), database queries, and framework internals often add clutter without giving you meaningful insight. If you see observations like these polluting your trace tree, you can [filter them out](/faq/all/unwanted-http-database-spans).

<div className="max-w-[50%]">

![Noisy observations in a trace tree](/images/docs/faq/good-trace-noisy-spans.png)
...
- For a reasoning model: the prompt as input, and both the [thinking](#capture-thinking) and the final answer or tool call as output.

...
### Are LLM calls captured between tool calls? [#llm-calls-between-tool-calls]

You should see a <Fan size={16} className="text-muted-magenta inline" /> `generation` for each model invocation in an agent loop, interleaved with the <Wrench size={16} className="text-orange-600 inline" /> `tool` calls it requested. Avoid wrapping the whole loop in one parent generation that only records the final output.

Agent loops typically go: the model decides to call tools, the tools run, the model sees the results, and it either calls more tools or answers. Each of those model steps is a real LLM call that's already happening — capture it.

If you collapse them into a single generation, you lose the in-between:

- You can't see what the agent decided after each tool result.
- Tokens and [cost](/docs/observability/features/token-and-cost-tracking) are aggregated on one span, so you can't tell which step blew up the context window.
- Evaluators and dashboards that target generations can't score or filter individual model steps.

<div className="grid grid-cols-1 md:grid-cols-2 mt-4 gap-4">

<div>

**One generation wrapping the whole loop.** Tools nest under a single LLM call, so you only see aggregated cost and the final output.
...
**Each model invocation as its own generation.** Tools sit between generations, so you can see reasoning, tokens, and cost after every step.
...
Framework integrations usually emit one generation per model call. If you're instrumenting manually, start a new `generation` for every LLM round-trip and nest the tools it requested as siblings under the same `agent` or `span`.

...
<Callout type="info">
  ...
  **2** (Hobby), **20** (Core), **50** (Pro), and **100** (Enterprise).
  Self-hosted deployments running Langfuse v4 have no alert limit.
</Callout>
...
- Recovery (`WARNING | ALERT → OK`): always notifies, with the title `[OK] <alert name>` and a "recovered" message. Recovery notifications cannot be switched off on the alert itself; if an automation should only fire on breaches, add a `severity` condition to its trigger filter that excludes `OK`.

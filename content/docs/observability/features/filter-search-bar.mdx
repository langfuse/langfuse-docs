...
<Callout type="info">
  Ask AI is in beta. It is **off by default**; an organization owner or admin
  enables it in **Organization Settings → General → AI Features**. On Langfuse
  Cloud it uses Langfuse-managed Amazon Bedrock. On self-hosted deployments it
  uses the same instance Langfuse AI model as the Assistant. See [self-hosted
  Assistant setup](/self-hosting/configuration/langfuse-assistant) and [data privacy
  and security](/security/ai-features).
</Callout>

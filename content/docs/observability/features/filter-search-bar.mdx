...
<Callout type="info">
  Ask AI is in beta and **off by default**, until an organization owner or admin
  [enables it](/security/ai-features). On self-hosted deployments it uses the
  same instance Langfuse AI model as the Assistant, see [self-hosted Assistant

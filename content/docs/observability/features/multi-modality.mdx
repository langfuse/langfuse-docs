...
The maximum size of a single attachment is controlled by `LANGFUSE_S3_MEDIA_MAX_CONTENT_LENGTH` (in bytes, default `1000000000`, i.e. 1 GB); the media API rejects larger uploads with `File size must be less than … bytes`. Uploading files to dataset items through the browser is additionally capped at 100 MB.

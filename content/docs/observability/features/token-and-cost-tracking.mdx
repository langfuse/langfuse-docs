...
<Callout type="info">
  Self-hosting? You can turn model definitions off for the whole instance with
  [`LANGFUSE_MODEL_DEFINITIONS_ENABLED=false`](/self-hosting/configuration#model-definitions).
  Langfuse then infers nothing: no cost calculation, no tokenizer, and no
  Models settings. Usage and cost you send with your generations are still
...
- **Self-hosted instances can switch inference off.** If your instance runs with `LANGFUSE_MODEL_DEFINITIONS_ENABLED=false`, Langfuse records only the usage and cost you send, and the Models settings are hidden. See [configuration](/self-hosting/configuration#model-definitions).

...
Match patterns are anchored regular expressions evaluated by Postgres; the default definitions start with `(?i)` to match case-insensitively. For Amazon Bedrock, the default definitions match the model IDs and cross-region inference-profile IDs the SDKs usually report, such as `anthropic.claude-sonnet-4-20250514-v1:0` or `us.anthropic.claude-sonnet-4-20250514-v1:0` (`eu.`, `apac.`, `global.`, and the other regional prefixes work too). They do **not** match a full inference-profile ARN such as `arn:aws:bedrock:us-east-1:123456789012:inference-profile/us.anthropic.claude-sonnet-4-20250514-v1:0`, so generations that report the ARN as their `model` show usage but no cost. Either report the plain model or profile ID as the model name in your instrumentation, or add a custom model definition whose pattern accepts the ARN prefix, for example `(?i)^(arn:aws:bedrock:[a-z0-9-]+:\d{12}:inference-profile\/)?(us|eu|apac|global)\.anthropic\.claude-sonnet-4-20250514-v1:0$`.
...
#### How tier matching works [#how-tier-matching-works]

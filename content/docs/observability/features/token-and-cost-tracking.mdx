...
<Details id="pricing-tiers">
<Summary>How does Langfuse handle models with tiered pricing?</Summary>

Some model providers charge different rates depending on the number of input tokens used. For example, Anthropic's Claude Sonnet 4.5 and Google's Gemini 2.5 Pro apply higher pricing when more than 200K input tokens are used. Langfuse supports **pricing tiers** to calculate cost accurately for these context-dependent pricing structures.

...
export function register() {
  if (globalForLangfuse.langfuseTracingRegistered) return;
  globalForLangfuse.langfuseTracingRegistered = true;
...
Wrap each response in an observation, end it when the stream finishes, and return its trace ID as the message ID. `useChat` sends the chat's `id` with every request; it becomes the Langfuse session ID, so all messages of one chat are grouped into a session:
...
export async function POST(req: Request) {
  ...
  return startActiveObservation(
    ...
    async (span) => {
      ...
      return propagateAttributes({ sessionId: id }, async () => {

---
title: Web callbacks
description: Trigger a browser-side HTTP callback from a trace, observation, or session in Langfuse.
sidebarTitle: Web Callbacks
---

# Web callbacks

Web callbacks let project members trigger an HTTP `POST` request from a trace, observation, or session in Langfuse. Use them to connect debugging workflows to your own tools, for example opening an internal investigation workflow, notifying an external system, or sending a selected trace or session ID to a local helper service.

Unlike [prompt webhooks](/docs/prompt-management/features/webhooks-slack-integrations), web callbacks are not event-driven automations. A project member manually triggers them from a trace, observation, or session detail view.
...
Open a trace or observation and click the three-dot actions menu in the detail header. If an enabled endpoint exists, the menu shows `Call <callback name>`. On session detail pages, use the web callback button in the header.
...
- `traceId`: Trace ID for the selected trace or observation. `null` for session-level callbacks.
- `observationId`: `null` for trace-level and session-level callbacks, or the selected observation ID for observation-level callbacks.
- `sessionId`: Session ID when available. `null` if the selected trace or observation is not part of a session.

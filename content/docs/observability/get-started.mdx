...
Langfuse is open source. Send traces to [Langfuse Cloud](/cloud) (managed, free tier, nothing to run) or to a [self-hosted](/self-hosting) instance; the instrumentation code is the same for both.

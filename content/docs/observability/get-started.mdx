...
To send one span without an SDK, set your project credentials and run this
OTLP/HTTP JSON request:
  ...
  --data-binary @- <<JSON
{
  "resourceSpans": [{
    "resource": {
      "attributes": [{
        "key": "service.name",
        "value": { "stringValue": "curl-example" }
      }]
    },
    "scopeSpans": [{
...
Use the host for your [data region](/security/data-regions) or self-hosted
deployment. See the [OpenTelemetry property

...
Langfuse is open source. Use [Langfuse Cloud](/cloud) (hosted by Langfuse, free tier, no infrastructure to run) or [self-host](/self-hosting) it. The instrumentation code is the same for both.
...
Install the [Langfuse Agent Skill](https://github.com/langfuse/skills) to let your coding agent access all Langfuse features. If your agent supports MCP, also add the [Langfuse Docs MCP server](/docs/docs-mcp) (`https://langfuse.com/api/mcp`, no authentication) so it can look up current documentation while it works.
...
(Langfuse calls spans [observations](/docs/observability/data-model#observations-traces-and-sessions); `span` is also a specific observation type.)

<LangTabs items={["OpenAI SDK (Python)", "OpenAI SDK (JS/TS)", "Vercel AI SDK", "LangChain (Python)", "LangChain (JS/TS)", "Python SDK", "JS/TS SDK", "OpenTelemetry (API)", "More integrations"]}>
...
Send OTLP spans straight to the Langfuse API from an OTEL SDK, a collector,
or plain `curl`:
...
Point `LANGFUSE_HOST` at your [data region](/security/data-regions) or
self-hosted deployment. See the [OpenTelemetry property
mapping](/integrations/native/opentelemetry#property-mapping) to add model,
usage, user, session, and other Langfuse attributes.

<Cards num={1}>
  <Card
    icon={<BookOpen />}
    title="OTLP endpoint reference"
    href="https://api.reference.langfuse.com/#tag/opentelemetry/POST/api/public/otel/v1/traces"
    newWindow
    arrow
  />
</Cards>

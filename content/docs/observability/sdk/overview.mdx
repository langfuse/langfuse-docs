...
Upgrading from Python SDK v3 or JS/TS SDK v4? Follow the [upgrade paths](/docs/observability/sdk/upgrade-path) ([Python v3 → v4](/docs/observability/sdk/upgrade-path/python-v3-to-v4), [JS/TS v4 → v5](/docs/observability/sdk/upgrade-path/js-v4-to-v5)).

The Langfuse SDKs are the recommended way to create [custom observations and traces](/docs/observability/sdk/instrumentation#custom-instrumentation) and use the Langfuse [prompt-management](/docs/prompt-management/overview) and [evaluation](/docs/evaluation/overview) features. Both SDKs work with [Langfuse Cloud](/cloud) (hosted by Langfuse, free tier, no infrastructure to run) and [self-hosted Langfuse](/self-hosting) with the same code; only the credentials and base URL differ.

<Callout type="info">
  **Not using a Langfuse SDK?** Send traces via
  [OpenTelemetry](/integrations/native/opentelemetry), either directly or with an
  [auto-instrumentation
  library](/integrations/native/opentelemetry#instrumentation-libraries). On
  Langfuse Cloud, `POST /api/public/ingestion` stops accepting everything except
  scores on November 16, 2026. See the [migration
  guide](/integrations/native/opentelemetry/migration-to-v4) to move a custom
  integration to v4.
</Callout>
...
- [Legacy Python SDK v3 documentation](https://python-sdk-v3.docs-snapshot.langfuse.com/docs/observability/sdk/overview/)
- [Legacy TypeScript SDK v4 documentation](https://js-sdk-v4-docs-snapshot.langfuse.com/docs/observability/sdk/overview/)
...
Learn more in the [OpenTelemetry JS getting-started guide](https://opentelemetry.io/docs/languages/js/getting-started/nodejs/).
...
- **Robust context propagation** so nested [observations](/docs/observability/data-model#observations-traces-and-sessions) stay connected, even across async workloads.
...
## Browser and frontend applications [#browser]

The tracing SDKs are meant for server-side code: they authenticate with a secret key, which must never ship to a browser or mobile app. Instrument the backend that calls your LLM provider, and pass the resulting trace ID to the frontend when the UI needs to refer to it.

From the frontend itself, use [`@langfuse/browser`](/docs/evaluation/evaluation-methods/scores-via-sdk#browser-score-ingestion) to attach [user feedback](/docs/observability/features/user-feedback) and other client-side scores to that trace. It needs only the public key and sends scores directly to the ingestion API; it does not create traces or observations. To trace LLM calls that originate in the browser, route them through your backend or a proxy you control and trace there.
...
To use other Langfuse features, you can use the [public API](/docs/api-and-data-platform/features/public-api) to integrate Langfuse from any runtime. We also maintain a [list of community-maintained SDKs](https://github.com/langfuse/langfuse-examples?tab=readme-ov-file#community-maintained-sdks).

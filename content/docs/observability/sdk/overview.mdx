...
<Callout type="warning">
  **Writing traces with a custom HTTP client?** Send them as OTLP to `POST
  /api/public/otel/v1/traces`; do not send trace, span, or generation events to
  the legacy `POST /api/public/ingestion` endpoint. That endpoint remains
  available for supported event types such as scores, but it is not the
  supported trace and observation write path in Langfuse v4. Prefer the
  official SDKs when one is available; otherwise follow the [native
  OpenTelemetry guide](/integrations/native/opentelemetry) or the [custom
  ingestion migration
  guide](/integrations/native/opentelemetry/migration-to-v4).
</Callout>

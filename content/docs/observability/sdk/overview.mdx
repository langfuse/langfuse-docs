...
- [**Langfuse Trace**](/docs/observability/data-model#observations-and-traces): A Langfuse trace is the set of observations that share a `trace_id`, plus shared attributes such as `session_id` and `user_id`. It shares the same ID as the OTel trace. Trace attributes are set via specific OTel span attributes that are automatically propagated to every observation. Overall input and output belong on the root observation; trace-level input/output is deprecated in Langfuse v4.

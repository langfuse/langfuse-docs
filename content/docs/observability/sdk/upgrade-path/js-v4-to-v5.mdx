...
<Callout type="warning">
  ...
  `langfuse.api.legacy.metricsV1` instead. See the [compatibility
...
10. If you self-host Langfuse v3, use `api.legacy.observationsV1` and `api.legacy.metricsV1`; the default `api.observations` / `api.metrics` require Langfuse v4 (see the [compatibility overview](/docs/compatibility#sdk-server))

...
<Callout type="warning">
  ...
  `langfuse.api.legacy.metrics_v1` instead. See the [compatibility
  overview](/docs/compatibility#sdk-server).
</Callout>
...
13. If you self-host Langfuse v3, use `api.legacy.observations_v1` and `api.legacy.metrics_v1`; the default `api.observations` / `api.metrics` require Langfuse v4 (see the [compatibility overview](/docs/compatibility#sdk-server))

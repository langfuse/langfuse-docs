...
This page is part of [LLM prompt management](/docs/prompt-management).

[Langfuse Prompt Management](/docs/prompt-management) enables A/B testing by allowing you to label different versions of a prompt (e.g., `prod-a` and `prod-b`). Your application can randomly alternate between these versions, while Langfuse tracks performance metrics like response latency, cost, token usage, and evaluation metrics for each version.

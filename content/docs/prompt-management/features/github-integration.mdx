...
This sync is one-directional: Langfuse pushes an event to GitHub (a `repository_dispatch` for the GitHub action, or a webhook to your own endpoint) whenever a prompt version is created or a label changes. Langfuse never reads from your repository. To flow changes from Git back into Langfuse — for example to promote a reviewed prompt from a pull request — have your CI workflow call the [prompts API](/docs/api-and-data-platform/features/public-api) (`POST /api/public/v2/prompts`), as shown in the CI/CD section above.

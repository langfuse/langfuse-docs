...
After linking prompts and traces, navigating to a generation [observation](/docs/observability/data-model#observations-traces-and-sessions) in Langfuse will highlight the prompt that was used to generate the response. To access the metrics, navigate to your prompt and click on the `Metrics` tab.

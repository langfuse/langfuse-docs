...
This lets you deploy prompts safely without a code change. Promote a version to an environment by assigning a label, run experiments or [A/B tests](/docs/prompt-management/features/a-b-testing) with dedicated labels, and roll back by moving a label to a previous version. Your application fetches by label, so which version runs is a data change rather than a deploy.

## Implementation

### Versions & Labels [#versions--labels]

Use the Langfuse UI or SDKs to assign and update labels on a prompt version.

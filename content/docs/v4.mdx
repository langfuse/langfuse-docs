...
**What changed:** Langfuse is now observations-first. Every LLM call, tool execution, and agent step is a directly queryable row in the main observations table. Trace-level attributes (`user_id`, `session_id`, `metadata`, `tags`) now propagate to each observation. Input and output are no longer stored on traces directly.

**Why it's faster:** Query directly across all operations without expensive joins — charts, filters, and APIs stay fast at any scale, even as traces grow to hundreds or thousands of operations. See the [changelog entry](/changelog/2026-03-10-simplify-for-scale) for more background.
...
- **LLM-as-a-judge evals:** Migrate your evaluations to run at the observation level for faster execution — [migration guide](/faq/all/llm-as-a-judge-migration).

...
**What changed:** Langfuse is now observations-first. The main table is observations — every LLM call, tool execution, and agent step is a directly queryable row. Trace-level attributes (`user_id`, `session_id`, `metadata`, `tags`) now live on each observation rather than on a separate traces table.

**Why it changed:** As agentic workloads scale to hundreds or thousands of operations per trace, the old two-table model (traces + observations with joins) became a performance bottleneck. By denormalizing all attributes onto observations — the same approach used by other observability tools — Langfuse can run analytical queries without expensive joins, keeping charts, filters, and APIs fast at any scale. See the [changelog entry](/changelog/2026-03-10-simplify-for-scale) for more background.

**How to adopt:**
- **SDK upgrade:** Update to [Python SDK v4](/docs/observability/sdk/upgrade-path/python-v3-to-v4) or [JS/TS SDK v5](/docs/observability/sdk/upgrade-path/js-v4-to-v5). Replace `update_current_trace()` / `updateActiveTrace()` with `propagate_attributes()` to push `user_id`, `session_id`, etc. to all child observations.
- **Direct OTEL ingestion:** Set the `x-langfuse-ingestion-version: 4` header on your span exporter ([setup guide](/integrations/native/opentelemetry)).
- **UI opt-in:** Use the **Preview** toggle in the bottom-left of the Langfuse UI to enable the new experience. It's reversible. The legacy view is slower during this transition period — we run background backfills to keep both data models in sync.
- **Saved views:** Set up saved views on the new unified tracing table to surface the observations that matter most — [guide here](/faq/all/explore-observations-in-v4).

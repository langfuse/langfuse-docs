...
Langfuse is now observations-first — queries that used to require expensive joins now run instantly, even across traces with thousands of steps.

**What changed:** Every LLM call, tool, and agent step is a directly queryable row. Trace attributes (`user_id`, `session_id`, etc.) propagate to all child observations automatically. Input/output are no longer stored on traces.

**How to adopt:**
- **SDK:** Upgrade to [Python SDK v4](/docs/observability/sdk/upgrade-path/python-v3-to-v4) / [JS SDK v5](/docs/observability/sdk/upgrade-path/js-v4-to-v5). Replace `update_current_trace()` with `propagate_attributes()`.
- **OTEL:** Set `x-langfuse-ingestion-version: 4` on your span exporter. ([setup guide](/integrations/native/opentelemetry))
- **UI:** Enable the **Preview** toggle (bottom-left) to opt in. (Legacy view remains available.)
- **Saved views & evals:** Filter observations directly in the new unified table; migrate evals to the observation level. ([saved views guide](/faq/all/explore-observations-in-v4), [evals migration guide](/faq/all/llm-as-a-judge-migration))

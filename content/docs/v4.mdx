...
2. **Read APIs:** move reads from the v1 endpoints (`api.legacy.*` or direct REST calls) to the [Observations API v2](/docs/api-and-data-platform/features/observations-api) and [Metrics API v2](/docs/metrics/features/metrics-api). The [full endpoint list with replacements](/docs/compatibility#sdk-server) is in the compatibility matrix.
...
2. **Evaluators:** migrate trace-level LLM-as-a-judge evaluators to the [observation level](/faq/all/llm-as-a-judge-migration).
...
4. **Server:** upgrade from v3 to v4 following the [upgrade documentation](/self-hosting/upgrade). The v4 UI ships with the server; no toggle is needed.
5. **Read APIs:** move reads from the v1 endpoints (`api.legacy.*` or direct REST calls) to the [Observations API v2](/docs/api-and-data-platform/features/observations-api) and [Metrics API v2](/docs/metrics/features/metrics-api). The [full endpoint list with replacements](/docs/compatibility#sdk-server) is in the compatibility matrix.
...
1. **Ingestion:** everyone still sending trace, span, or generation events to the legacy `POST /api/public/ingestion` API must move to the [OpenTelemetry endpoint](/integrations/native/opentelemetry) `POST /api/public/otel/v1/traces`. Set the `x-langfuse-ingestion-version: 4` header on your span exporter for real-time data, and propagate trace attributes (`user_id`, `session_id`, ...) to all spans.
2. **Read APIs:** move reads from the v1 endpoints (`api.legacy.*` or direct REST calls) to the [Observations API v2](/docs/api-and-data-platform/features/observations-api) and [Metrics API v2](/docs/metrics/features/metrics-api). The [full endpoint list with replacements](/docs/compatibility#sdk-server) is in the compatibility matrix.
3. **Evaluators:** migrate trace-level LLM-as-a-judge evaluators to the [observation level](/faq/all/llm-as-a-judge-migration).
4. **Exports:** switch blob storage, PostHog, and Mixpanel integrations from the legacy "traces and observations" source to "enriched observations".

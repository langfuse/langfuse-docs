...
<Callout type="info">
V4 introduces a **single unified observations (spans) table** where all inputs, outputs, and context attributes live directly on observations. See the [guide on working with the observation-centric data model in v4](/faq/all/explore-observations-in-v4) for before/after workflows and saved view setup.
</Callout>
...
This approach aligns with how modern observability platforms like [Datadog](https://docs.datadoghq.com/tracing/trace_explorer/) and [Honeycomb](https://www.honeycomb.io/blog/structured-events-basis-observability) have evolved: a flat, filterable table of spans with rich attributes as the primary exploration surface, with trace waterfall views available as a drill-down for investigating individual requests.

<Callout type="info">
**Tip:** To recreate the old traces-table experience, filter for root spans (observations with no parent) and save it as a [saved view](/changelog/2025-05-20-save-table-views). We recommend this as one of your first saved views when getting started with v4.
</Callout>
...
|---|---|---|
| Traces | Root spans (no parent) | Recreates the old traces table — one row per request |

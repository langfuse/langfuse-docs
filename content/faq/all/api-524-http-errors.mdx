...
The older trace read endpoints remain available on Langfuse Cloud (see the [Legacy APIs reference](/docs/api-and-data-platform/features/legacy-apis) for their availability on self-hosted Langfuse v4), but are not recommended as the default for new data extraction workflows. They are particularly susceptible to 524 errors on large projects.

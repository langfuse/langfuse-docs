...
Langfuse v4 adopts an [OpenTelemetry-native data model](/docs/observability/data-model). In OTEL, there is no separate "trace" object — only observations arranged in a parent-child hierarchy. A trace is the set of observations that share a `trace_id`. An application entry point can be an outer root with no parent or an app root whose OpenTelemetry instrumentation or infrastructure parent was filtered from export by the SDK; the `Is Root Observation` filter includes both.

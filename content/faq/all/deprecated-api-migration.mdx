...
**Replacement:** [`GET /v2/observations`](/docs/api-and-data-platform/features/public-api#v2) ([reference](https://api.reference.langfuse.com/#tag/observationsv2/GET/api/public/v2/observations)).
...
**Replacement:** [`GET /v2/observations`](/docs/api-and-data-platform/features/public-api#v2), grouped by `traceId` on the client side.
...
**Replacement:** [`GET /v2/observations`](/docs/api-and-data-platform/features/public-api#v2) with a `filter` condition on the `sessionId` column, grouped by `sessionId` on the client side.
...
- **The `traces` view is removed.** In the `observations` view, measures such as `count`, `latency`, `totalCost`, and `totalTokens` are calculated over observation rows, not traces. For trace-level counts, filter on the v2-only `isRootObservation = true` dimension. See [Logical root observations](/docs/api-and-data-platform/features/public-api#logical-root-observations) for root semantics and trace-counting edge cases. For trace durations or raw per-trace data, use [Observations API v2](/docs/api-and-data-platform/features/public-api#v2) and group by `traceId` client-side.
...
- **No trace joins.** v3 queries scores directly and does not return trace fields (the v2 `trace` field group is gone). For row-level filtering by trace properties, query [Observations API v2](/docs/api-and-data-platform/features/public-api#v2) with the corresponding filters and `fields=core,trace_context`, collect the distinct trace IDs, then pass them to v3's comma-separated `traceId` filter.

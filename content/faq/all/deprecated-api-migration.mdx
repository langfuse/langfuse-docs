...
**Deprecated:** `POST /ingestion` (batched event ingestion) and the older synchronous endpoints `POST /traces`, `POST /spans`, `POST /generations`, `POST /events`. Langfuse Cloud serves these endpoints until <V4CutoverDate />; migrate to v4 before this date.

...
|---|---|---|
| Specific operations across traces | Not directly possible — had to open traces individually | Filter by observation name, type, model, or any attribute, then save as a [saved view](/changelog/2025-05-20-save-table-views) |
| Operations scoped to a user or session | Traces table → filter by user/session | Filter by `user_id` or `session_id` directly on the observations table |

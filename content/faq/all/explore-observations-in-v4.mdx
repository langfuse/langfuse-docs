...
sidebarTitle: Explore Observations in Langfuse v4
...
# Explore Observations in Langfuse v4
...
For conceptual background, see the [observation-centric data model](/docs/v4#under-the-hood). For broader release context, see the [Langfuse v4 docs](/docs/v4).
...
On Langfuse v4, those setups can lead to multi-minute delays before traces show up in the unified table.
...
Yes, for now. On Langfuse Cloud, you can switch the new experience off in the UI and return to the previous tracing experience. On self-hosted deployments, the `legacy` and `dual` [write modes](/self-hosting/upgrade/upgrade-guides/upgrade-v3-to-v4) retain the previous behavior while your migration is in flight.

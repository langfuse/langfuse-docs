...
In [Langfuse v4](/changelog/2026-08-13-langfuse-v4), every operation your application performs (LLM calls, tool executions, retrievals, agent steps) is one row in a single observations table. A trace is just the set of rows sharing a `trace_id`; the [data model docs](/docs/observability/data-model#observations-and-traces) explain the model.

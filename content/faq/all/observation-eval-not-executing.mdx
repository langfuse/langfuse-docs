...
If the evaluator **does run** but every score is `0` because expected output is blank, that is a different issue: rules cannot see dataset expected output. See [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments).

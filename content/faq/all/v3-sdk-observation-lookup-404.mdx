...
The v4 data model only has observations — traces don't exist as a separate entity, so a trace's metadata needs to be represented _as_ an observation. Langfuse does this by dual-writing a synthetic observation with id `t-<traceId>` whenever a trace comes in through the v3 ingestion path. That synthetic row lives in the events table (v4), not in the classic observations table (v3).

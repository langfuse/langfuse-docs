...
For background on why this matters, see the [Langfuse blog post](https://langfuse.com/blog/2025-08-29-error-analysis-to-evaluate-llm-applications).
...
The app was built by an adult child to help their father (HUAWEI Y5p, Android 10, Vodafone) get phone help without calling every time something went wrong. The bot speaks as if it were the child: warm, patient, non-technical. It can search the web for current info about the phone and carrier.
...
**No production data yet?** Run your app against representative synthetic inputs, capture the traces in Langfuse, and sample from those. See the Langfuse [datasets overview](https://langfuse.com/docs/datasets/overview).
...
2. **Set up evaluators** for categories that warrant them, starting with the highest-impact failure requiring judgment.

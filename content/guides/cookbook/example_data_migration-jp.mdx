...
description: Python SDK v4を使って、Langfuseプロジェクト間でオブザベーション、スコア、プロンプト、データセット、カスタムモデル、スコア設定を移行します。
category: Examples
---

# Langfuseプロジェクト間のデータ移行 (Python SDK v4)

このクックブックは[英語](/guides/cookbook/example_data_migration)でも利用できます。

このノートブックでは、**Langfuse Python SDK v4** を使って、あるLangfuseプロジェクトから別のLangfuseプロジェクトへデータを移行します。

<Callout type="warning">
データを移行先のLangfuseプロジェクトへ取り込むと、新しいインジェストイベントとして課金対象になります。つまり、移行するオブザベーション数に応じて Langfuse のインジェスト料金が発生します。
...
5. **スコア**
6. **データセット** (定義 + アイテム。移行後のトレース/オブザベーションに紐付けられます)

## 移行されないもの

Langfuse の公開APIでは現在、次のリソースをプログラムから作成することはできません: **LLM-as-a-Judge の評価設定**、**カスタムダッシュボード**、**ユーザー / RBAC / SSO**、**プロジェクトや組織の設定**。これらは手動で作り直すか、UI上のエクスポート機能を使って移してください。過去の **dataset run items** も移行対象外です (セクション6を参照)。

## 0. セットアップ

このノートブックには Python SDK 4.8.1 以降が必要です。それより前の 4.x リリースには v3 Scores API クライアントが含まれていません。
...
os.environ.setdefault("LANGFUSE_SOURCE_HOST", "https://cloud.langfuse.com");  # 例: EU クラウド
...
os.environ.setdefault("LANGFUSE_DEST_HOST", "https://jp.cloud.langfuse.com");  # 例: JP クラウド

# --- オブザベーション移行の時間フィルタ (任意, ISO 8601) ---
# 空にすると全トレースが対象になります。
os.environ.setdefault("LANGFUSE_MIGRATE_FROM_TIMESTAMP", "2026-03-01T00:00:00Z");  # 例: "2025-01-01T00:00:00Z"
os.environ.setdefault("LANGFUSE_MIGRATE_TO_TIMESTAMP", "2026-04-20T00:00:00Z");    # 例: "2025-02-01T00:00:00Z"
```
...
スコア設定は最初に移行しておく必要があります。こうすると、移行先で作成するスコアが ID で参照できるようになります。あとのセクションで参照をつなぎ直すために `config_id_map` を作っておきます。
...
> **セルフホスト環境の場合:** Observations API v2 には [Langfuse v4](https://langfuse.com/docs/v4) が必要です。セルフホスト向けにも v4 は一般提供されています。移行元がまだ Langfuse v3 の場合、v2 のエンドポイントは利用できません。その場合は `src.api.trace.list(page=..., from_timestamp=FROM_TS, to_timestamp=TO_TS)` でトレースを列挙し、各トレースのオブザベーションを `src.api.legacy.observations_v1.get_many(trace_id=...)` で取得してください。どちらも v4 では非推奨ですが、v3 の移行元に対してはこれしか方法がありません。

> **移行対象になるトレース:** 以下のリストは *論理ルートオブザベーション* をページングするため、1トレースにつき1行が返ります。ルートオブザベーションが Langfuse に届いていないトレース (子スパンだけをエクスポートした場合など) には論理ルートが存在せず、スキップされます。移行できたトレース数と移行元プロジェクトのトレース数を比べ、差がある場合はルートフィルタなしでオブザベーションをページングし、`trace_id` でグルーピングして残りを補完してください。
...
def _set_trace_attrs(span, trace_obj) -> None:
    """ルートスパンに trace レベルおよびルートオブザベーションの langfuse.* 属性をセットします。"""
...
def _set_observation_attrs(span, obs) -> None:
    ...
    # Generation でのみ使う属性。ObservationV2 のモデル名は `model` で返されます。
    # 古い SDK のレスポンス型向けに `provided_model_name` にもフォールバックします。
...
# `model` で model / modelParameters が、`usage` で
...
def _fetch_observations_v2(source_trace_id: str) -> List[Any]:
    ...
    Observations API v2 (Langfuse Cloud、またはセルフホスト Langfuse v4+) が必要です。
    移行元が Langfuse v3 の場合は、非推奨の v1 読み取り
    `src.api.legacy.observations_v1.get_many(trace_id=source_trace_id, page=..., limit=100)`
    に置き換えてください (カーソルではなくページ番号でのページングになります)。
    """
    results: List[Any] = []
...
def migrate_one_trace(trace_obj) -> int:
    ...
    # ない場合は、ルートオブザベーションのタイムスタンプで合成します。
    root_span_hex = to_otel_span_id(source_trace_id)  # ルートスパン ID を決定論的に生成
    trace_start = min(
...
t_start = time.time()
while True:
    # `is_root_observation` は論理ルートを取得するための専用パラメータで、
    # 1トレースにつき1行が返ります。高度な `filter` パラメータは他のすべての
    # クエリパラメータより優先されるため、こちらを使います。
    ...
    for t in listing.data:
        try:
            ...
            print(f"  トレース {source_trace_id} の移行に失敗: {exc}")
            failures += 1

    cursor = getattr(listing.meta, "cursor", None)
    print(
        f"  バッチ完了 — 累計: traces={total}, observations={total_obs}, failed={failures}"
    )
    if not cursor:
        break
...
スコアは移行元から v3 Scores API (`api.scores_v3.get_many_v3`) で取得し、移行先では `POST /api/public/scores` エンドポイント (`api.legacy.score_v1.create`) で再作成します。元の `id` をそのまま保持するので、スクリプトを再実行しても冪等に動きます。`config_id` はセクション1の `config_id_map` で、トレース / オブザベーションへの参照はセクション4の `trace_id_map` / `obs_id_map` でそれぞれマッピングし直します。

旧 v2 エンドポイントとの違いのうち、ここで重要になるのは次の3点です。

- **カーソルページネーション** — ページ番号を数える代わりに、`meta.cursor` の不透明なカーソルを空になるまで渡し続けます。
- **単一の `subject`** — 各スコアは、フラットな `trace_id` / `observation_id` / `session_id` / `dataset_run_id` フィールドの代わりに、1つの `subject` オブジェクト (`kind` + `id`) で対象を参照します。データセット実行は v3 では *experiment* と呼ばれます。ごくまれに2つの参照 (例: トレースとセッションの両方) を持つ古いスコアが存在しますが、v3 ではそのうち1つだけが返されます。従来のフローではそうしたスコアの再作成自体が失敗していたので、悪化することはありません。
- **型付きの `value`** — `value` は常に数値 + 別フィールドの `string_value` という形ではなく、スコアの `data_type` に応じた型になります (NUMERIC は数値、BOOLEAN は真偽値、CATEGORICAL などは文字列)。

挙動については次の点に注意してください。

- v2 はトレースが存在しないスコアを黙って除外していましたが、v3 はそれらも返します。v2 のフローより移行されるスコアがやや多くなる場合は、これまで見えていなかったスコアが移行されるようになったためで、改善であって問題ではありません。
- データセット実行に紐づくスコア (`kind="experiment"`) はこのセクションではスキップされます。セクション6で実行を再作成する際に移行先では新しい実行IDが割り当てられるため、移行元の実行IDへの参照は無効になってしまうからです。こうしたスコアは、移行先に実行が作成されたあとに、実行名で対応づけて移行してください。
- スコアの作成は非同期です。成功レスポンスは取り込みキューに入ったことを意味するだけで、その時点で書き込みが完了しているわけではありません。取り込みが追いついてから移行先プロジェクトを確認してください。
- 作成エンドポイントはタイムスタンプを受け付けないため、移行されたスコアのタイムスタンプは移行時刻になります。元のスコアのタイムスタンプは公開APIでは引き継げません。

> **セルフホスト環境の場合:** Scores API v3 は Langfuse Cloud と、セルフホストでは v3.180.0 以降で利用できます。それより古いセルフホスト環境では v2 のパターンを使ってください: ページベースの `src.api.scores.get_many(page=page, limit=100, ...)` で取得し、フラットな `trace_id` / `observation_id` / `session_id` フィールドと、CATEGORICAL スコアには `string_value` を使います。なお、[Langfuse v4](https://langfuse.com/docs/v4) への移行が完了したデプロイメントでは v2 の読み取りエンドポイントは削除されています。
...
# v3 Scores API のフィールドグループ。`core` (id/name/value/dataType/...) は常に
# 含まれます。`details` は comment/metadata/configId を、`subject` は subject
# 参照を、`annotation` は authorUserId/queueId を追加します。
...
while True:
    ...
    for s in resp.data:
        # 値が null のスコアは、変換より前に弾く必要があります。後続の BOOLEAN
        # の分岐が、値なしを false に変換してしまうためです。
        if s.value is None:
            skipped += 1
            continue

        # v3 の subject を、作成エンドポイントが期待するフラットな参照フィールドに
        # 変換する: traceId / sessionId のうちちょうど1つ。observationId には
        # traceId が必須。
        ...
        if kind == "trace":
            ...
            if not s.subject.trace_id:
                print(f"  スコア {s.id} ({s.name}) をスキップ: トレースを持たない observation subject")
                skipped += 1
                continue
        ...
        elif kind == "experiment":  # v3 におけるデータセット実行の呼び名
            # subject の id は移行元プロジェクトの実行ID。移行先では新しい実行IDが
            # 割り当てられるため、この参照は引き継げません。
            print(f"  スコア {s.id} ({s.name}) をスキップ: データセット実行のスコアは移行先に実行が作成されたあとに移行してください")
            skipped += 1
            continue
        else:
            print(f"  スコア {s.id} ({s.name}) をスキップ: 未対応の subject kind {kind!r}")
            skipped += 1
            continue
        ...
        # v3 の `value` は型付きなので、書き込み側が期待する形に変換する
        if s.data_type == "NUMERIC":
            value = s.value
        elif s.data_type == "BOOLEAN":
            value = 1 if s.value else 0  # 書き込み側は数値の 0/1 を期待する
        elif s.data_type == "CATEGORICAL":
            value = s.value  # すでにカテゴリの文字列
        else:
            # その他のデータ型 (例: TEXT) は文字列値なので、そのまま渡す
            print(f"  {s.data_type} スコア {s.id} ({s.name}) をそのまま移行します")
            value = s.value

        try:
            # POST /api/public/scores。このエンドポイントはタイムスタンプを受け付けない
            # ため、移行されたスコアには移行時刻が記録されます。取り込みは非同期で、
            # 成功レスポンスはキュー投入を意味します (即時反映ではない)。
            with_retries(lambda: dst.api.legacy.score_v1.create(
                id=s.id,  # ID を保持するので、再実行しても重複なく冪等に処理されます
                name=s.name,
                value=value,
...
print(f"スコア — 移行: {migrated}, スキップ: {skipped}, 失敗: {failed}")
```

## 6. データセットとアイテム

データセット本体はネイティブの **`create_dataset`** ヘルパーで、アイテムは **`create_dataset_item`** (移行元のアイテムIDを維持) で移行します。

過去の dataset run items は移行**されません**。従来の `POST /api/public/dataset-run-items` エンドポイントは非推奨で、その代替手段はいずれも過去の実行を埋め戻すのではなく *新しい* experiment run を作成するものです。以下のセルでは、移行元で見つかった実行を一覧表示して、何が残されるのかがわかるようにしています。移行先で同じ実行を再現したい場合は、[Experiments SDK](https://langfuse.com/docs/evaluation/experiments/experiments-via-sdk) で実験を再実行するか、[OTLP の experiment 属性](https://langfuse.com/integrations/native/opentelemetry/experiments) を付けてアイテムのトレースを自分で送信してください。
...
while True:
    ...
    for ds in dsresp.data:
        ...
        # 過去の run items は移行しません。POST /dataset-run-items は非推奨で、
        # 代替手段は過去の実行を埋め戻すのではなく新しい experiment run を作成する
        # ためです。何が残されるかがわかるように、移行元の実行を一覧表示します。
        run_count = 0
        page_run = 1
        while True:
            ...
            for run_meta in rresp.data:
                run_count += 1
                runs_found += 1
                print(f"      実行 '{run_meta.name}' は移行しません — 移行先で再実行してください")
            if page_run >= getattr(rresp.meta, "total_pages", page_run):
                break
            page_run += 1
        print(f"    実行: {run_count} 件を移行元で検出。移行はしません")

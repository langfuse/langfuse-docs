...
- **単一の `subject`** — 各スコアは、フラットな `trace_id` / `observation_id` / `session_id` / `dataset_run_id` フィールドの代わりに、1つの `subject` オブジェクト (`kind` + `id`) で対象を参照します。データセット実行は v3 では *experiment* と呼ばれるので、`kind="experiment"` を書き込み側の `dataset_run_id` に対応づけます。ごくまれに2つの参照 (例: トレースとセッションの両方) を持つ古いスコアが存在しますが、v3 ではそのうち1つだけが返されます。従来のフローではそうしたスコアの再作成自体が失敗していたので、悪化することはありません。
...
while True:
    ...
    for s in resp.data:
        ...
        # 変換する: traceId / sessionId / datasetRunId のうちちょうど1つ。
        # observationId には traceId が必須。

...
> **セルフホスト環境の場合:** Observations API v2 には [Langfuse v4](https://langfuse.com/docs/v4) が必要です。セルフホスト向けにも v4 は一般提供されています。移行元がまだ Langfuse v3 の場合は、先に v4 へアップグレードするか、v4 互換の環境からデータを書き出してからこのワークフローを実行してください。
...
def _set_trace_attrs(span, trace_obj) -> None:
    """合成したルートスパンに trace レベルの langfuse.* 属性をセットします。"""
...
データセット本体はネイティブの **`create_dataset`** ヘルパーで、アイテムは **`create_dataset_item`** (移行元のアイテムIDを維持) で移行します。Langfuse v4 には、従来の dataset run items を書き込むためのサポートされた公開APIがないため、データセットとトレースを移行したあとで experiment runner SDK を使って実行を再作成してください。
...
while True:
    ...
    for ds in dsresp.data:
        ...
        # 従来の dataset run items は Langfuse v4 でサポートされた公開書き込み
        # パスがありません。データセットとトレースを移行したあとで experiment
        # runner SDK を使って実行を再作成してください。
        run_count = 0
        page_run = 1
        while True:
            ...
            for run_meta in rresp.data:
                ...
                runs_failed += 1
                print(
                    f"      実行 '{run_meta.name}' はスキップしました — experiment runner SDK で再作成してください"
                )
            if page_run >= getattr(rresp.meta, "total_pages", page_run):
                break
            page_run += 1
        print(
            f"    実行: {run_count} 件を検出。v4 の公開APIでは直接移行できません"
        )
...
print(
    ...
    f"experiment runner SDK で再作成が必要な実行: {runs_failed}  |  "
    f"経過時間: {elapsed:.1f}s"
)
```

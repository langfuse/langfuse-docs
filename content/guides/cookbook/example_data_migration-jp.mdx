...
スコアは移行元から v3 Scores API (`api.scores_v3.get_many_v3`) で取得し、移行先ではネイティブの **`create_score`** ヘルパーで再作成します。元の `id` をそのまま保持するので、スクリプトを再実行しても冪等に動きます。`config_id` はセクション1の `config_id_map` で、トレース / オブザベーションへの参照はセクション4の `trace_id_map` / `obs_id_map` でそれぞれマッピングし直します。
...
もう1点: v2 はトレースが存在しないスコアを黙って除外していましたが、v3 はそれらも返します。v2 のフローより移行されるスコアがやや多くなる場合は、これまで見えていなかったスコアが移行されるようになったためで、改善であって問題ではありません。

> **セルフホスト環境の場合:** Scores API v3 には [Langfuse v4](https://langfuse.com/docs/v4) が必要です。それより前のセルフホスト環境から移行する場合は v2 のパターンを使ってください: ページベースの `src.api.scores.get_many(page=page, limit=100, ...)` で取得し、フラットな `trace_id` / `observation_id` / `session_id` フィールドと、CATEGORICAL スコアには `string_value` を使います。
...
while True:
    ...
    for s in resp.data:
        # v3 の subject を、create_score が期待するフラットな参照フィールドに変換する

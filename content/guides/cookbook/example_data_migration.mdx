...
# Map source observation types to the type strings the OTEL ingestion
# endpoint accepts. Langfuse's server-side ObservationTypeMapper supports
# each of these directly, so we preserve the original type instead of
# collapsing everything to "span".
_OBS_TYPE_MAP = {
    "SPAN": "span",
...
# Observation types that carry model / usage / cost details.
_GENERATION_LIKE_TYPES = {"generation", "embedding"}
...
def _set_observation_attrs(span, obs) -> None:
    ...
    # Model / usage / cost attributes apply to generation-like types
    # (generation + embedding). ObservationV2 renames `model` to
    # `provided_model_name`; fall back to `model` for the v1 type used by
    # self-hosted migrations.
    if obs_type in _GENERATION_LIKE_TYPES:
...
def _fetch_observations_v2(source_trace_id: str) -> List[Any]:
    ...
    For migrations from earlier self-hosted versions, replace this helper with:

...
> **Self-hosted deployments:** the v2 Observations API requires [Langfuse v4](https://langfuse.com/docs/v4), which is generally available for self-hosted deployments. If your source deployment still runs Langfuse v3, upgrade it to v4 first or export the source data through a v4-compatible environment before running this workflow.
...
def _set_trace_attrs(span, trace_obj) -> None:
    """Set trace-level langfuse.* attributes on the synthetic root span."""
...
We migrate datasets themselves via the native **`create_dataset`** helper and items via **`create_dataset_item`** (preserving source item IDs). Langfuse v4 does not provide a supported public write path for legacy dataset run items, so recreate experiment runs with the experiment runner SDK after the datasets and traces are migrated.
...
while True:
    ...
    for ds in dsresp.data:
        ...
        # Legacy dataset run items no longer have a supported public write path
        # on Langfuse v4. Recreate experiment runs with the experiment runner SDK
        # after datasets and traces are migrated.
        run_count = 0
        page_run = 1
        while True:
            ...
            for run_meta in rresp.data:
                ...
                runs_failed += 1
                print(
                    f"      skipped run '{run_meta.name}' — recreate experiment items with the experiment runner SDK"
                )
            if page_run >= getattr(rresp.meta, "total_pages", page_run):
                break
            page_run += 1
        print(
            f"    runs: {run_count} run(s) found; public v4 migration path not available via API"
        )
...
print(
    ...
    f"runs needing SDK recreation: {runs_failed}  |  "

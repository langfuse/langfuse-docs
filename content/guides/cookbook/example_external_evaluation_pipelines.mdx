...
seoTitle: Evaluate Traces with an External Pipeline
...
You can get your Langfuse API keys [here](https://langfuse.com/cloud) and OpenAI API key [here](https://platform.openai.com/api-keys)
...
A trace's input and output live on its root observation in Langfuse v4, so we fetch root observations and page through them with a cursor. The `filter` argument narrows results by trace name, tags, timestamps, and more. You can find more about other methods to [query data via the SDKs](https://langfuse.com/docs/api-and-data-platform/features/query-via-sdk) in our docs.
...
def get_trace_input(trace):
    payload = parse_json_if_needed(trace.input)
    if isinstance(payload, dict) and "args" in payload:
        return payload["args"][0]
    return payload


def fetch_root_observations_batch(*, limit, cursor=None, to_start_time=None):
    ...
    return langfuse.api.observations.get_many(
        ...
        filter=json.dumps([
            ...
            {"type": "arrayOptions", "column": "tags", "operator": "all of", "value": ["ext_eval_pipelines"]},
            {"type": "datetime", "column": "startTime", "operator": ">=", "value": five_am_yesterday.isoformat()},
            {"type": "datetime", "column": "startTime", "operator": "<", "value": (to_start_time or datetime.now()).isoformat()},
        ]),
    )


traces_batch = fetch_root_observations_batch(limit=BATCH_SIZE).data
    ...
    messages=[
        {
            ...
            "content": template_tone_eval.format(
                text=parse_json_if_needed(traces_batch[1].output)),
        }
    ],
    model="gpt-4o",
...
print(f"User query: {get_trace_input(traces_batch[1])}")
print(f"Model answer: {parse_json_if_needed(traces_batch[1].output)}")
...
def joyfulness_score(trace):
		...
		input_payload = parse_json_if_needed(trace.input)
		test_case = LLMTestCase(
    input=input_payload["args"] if isinstance(input_payload, dict) and "args" in input_payload else input_payload,
    actual_output=parse_json_if_needed(trace.output))
...
while processed < TOTAL_TRACES:
    response = fetch_root_observations_batch(
        limit=min(BATCH_SIZE, TOTAL_TRACES - processed),
        cursor=cursor,
        to_start_time=five_am_today,
    )
    traces_batch = response.data
    if not traces_batch:
        break
    ...
    for trace in traces_batch:
        ...
        if trace.output is None:
            print(f"Warning: \n Trace {trace.trace_id} had no generated output, \
            it was skipped")
            continue

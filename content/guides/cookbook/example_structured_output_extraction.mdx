...
title: Evaluate and Improve Structured Extraction
sidebarTitle: Evaluate and Improve Structured Extraction
description: Evaluate structured extraction with Langfuse. Build a labeled dataset, score document extraction per field, catch LLM output parsing errors, and improve structured generation with experiments.
category: Evaluation
---
...
30 synthetic claims from the [Cleanlab structured-output benchmark](https://cleanlab.ai/blog/structured-output-benchmark/), uploaded as a [Langfuse dataset](https://langfuse.com/docs/evaluation/experiments/datasets). Each item is claim text in `input`, the nested label in `expected_output`, and slices in `metadata` (`has_policy`, `n_objects`). The text is LLM-generated. Treat this as a demo set.
...
![Insurance-claims dataset in Langfuse](https://langfuse.com/images/cookbook/example-structured-output-extraction/dataset-items.png)
...
The comparison is type-aware. Dates match on the parsed value, so `2025-09-23` and `September 23, 2025` are the same answer. Numbers match within a small tolerance (1.0 absolute or 1%). Because `insured_objects` is a list, the scorer first pairs labeled objects with predicted objects by field similarity, then scores fields inside each pair, so ordering differences cost nothing. Nulls are handled two ways. When a required field is null in the labels, that is a labeling gap and the leaf is skipped. When an optional field is null in the labels, the document genuinely omits the value, so the model must also output null, and inventing one counts as wrong.
...
Each experiment item in Langfuse gets those `field.*` scores. Sort failing items by a field path rather than by overall accuracy.


## 5. Run experiments from the dataset

`run_round` calls `dataset.run_experiment` and appends a row to a comparison table: overall field accuracy, `object_id`, `policyholder_name`, and `objects_F1`. If an experiment with that name already exists in Langfuse, it reuses those scores instead of calling the model again. Pass `force=True` when you want a fresh run.

This makes the whole notebook safe to re-run. The dataset, prompt versions, and experiments are all created only if they don't already exist, so restarting the kernel or re-executing cells never duplicates anything in Langfuse or spends tokens twice.
...
Open the experiment in Langfuse and sort failing items by a low-scoring field rather than by the overall score.
...
Compare the new `object_id` column to V1. Then open Compare for the two runs: select both experiments on the dataset's Experiments tab and click **Compare** (see [Experiments via UI](https://langfuse.com/docs/evaluation/experiments/experiments-via-ui)). Each row is a claim; each run gets a column with its output and the per-field scores next to it.

![V1 and V2 side by side in the Langfuse compare view, with per-field boolean scores for each item](https://langfuse.com/images/cookbook/example-structured-output-extraction/compare-v1-v2.png)
...
If you see placeholder names, that is the V3 regression. The next cell saves a new prompt version instead of editing v2.


## 10. New prompt version, same dataset

[Prompt versions](https://langfuse.com/docs/prompt-management/features/prompt-version-control) are immutable. Add two rules: copy `policyholder_name` exactly, and only emit objects that are actually described. Save as v3 and rerun.
...
The run should link to `extraction-system` v3 in Prompt Management. All three versions stay in the history, so every experiment stays attributable to the exact prompt text it ran with.

![The extraction-system prompt in Langfuse with three versions; v3 adds the verbatim-name and no-invented-objects rules](https://langfuse.com/images/cookbook/example-structured-output-extraction/prompt-versions.png)
...
![Compare view filtered to items where V3 got policyholder_name wrong, with V3_fixed side by side](https://langfuse.com/images/cookbook/example-structured-output-extraction/compare-v3-regression.png)

The dataset's Experiments tab now shows all four runs in one table with cost, latency, and mean field accuracy. Note that V3_fixed's headline mean (0.93) is *below* V3's (0.95) even though the field you targeted recovered. Anyone reading only the single number would call the fix a regression. That is the whole argument for per-field scores.

![Four experiment runs on the insurance-claims dataset with mean field accuracy per run](https://langfuse.com/images/cookbook/example-structured-output-extraction/experiment-runs.png)

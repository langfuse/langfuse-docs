...
Get your Langfuse API keys by signing up for [Langfuse Cloud](https://langfuse.com/cloud) or [self-hosting Langfuse](https://langfuse.com/self-hosting). You’ll also need your OpenAI API key.
...
You can pass additional trace and generation attributes to the `observeOpenAI` wrapper. Refer to the [integration guide](https://langfuse.com/integrations/model-providers/openai-js) for comprehensive details.

Example usage:

- Assigning a custom trace and generation name
- Enabling user-level and session-level tracking
- Adding tags and custom metadata
...
const openaiWithLangfuse = observeOpenAI(new OpenAI(), {
    traceName: "OpenAI Custom Trace",
    generationName: "OpenAI Custom Generation",
    generationMetadata: {env: "dev"},
...
Use `propagateAttributes` to apply trace attributes (name, tags, metadata) to all observations created inside its callback, and the `startActiveObservation` context manager to group two OpenAI generations under a single root span. Input and output are set on the root observation.

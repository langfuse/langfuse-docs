...
<Callout type="info">
  ...
  endpoint and still returns trace objects; the API tab shows the endpoint's
  supported replacement, which returns observation rows instead. See [Migration
  of deprecated APIs](/docs/deprecated-api-migration#traces) for how the two relate.
</Callout>

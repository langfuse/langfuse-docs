...
The v2 API returns observation rows, not trace objects: group rows by `traceId`, and render the input/output of each trace's root observation (the row with `parentObservationId == null`). See [Migration of deprecated APIs](/faq/all/deprecated-api-migration#traces) for details.
...
<Callout type="info">
  ...
  APIs](/faq/all/deprecated-api-migration#traces) for how the two relate.
</Callout>

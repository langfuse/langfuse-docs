...
The v2 API returns observation rows, not trace objects: group rows by `traceId`, and render the input/output of each trace's root observation (the row with `parentObservationId == null`). See [Transitioning to the v4 API](/docs/api-migration#traces) for details.
...
<Callout type="info">
  ...
  (`langfuse.api.trace.list`), which calls the legacy `GET /api/public/traces`
  endpoint and still returns trace objects; the API tab shows the endpoint's
  v4 replacement, which returns observation rows instead. See [Transitioning
  to the v4 API](/docs/api-migration#traces) for how the two relate.
</Callout>

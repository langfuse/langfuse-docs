...
- **Coding & IDEs**: We use various IDEs for coding and code review, such as [**VS Code**](https://code.visualstudio.com/), [**Zed**](https://zed.dev/), [**WebStorm**](https://www.jetbrains.com/webstorm/), [**Cursor**](https://www.cursor.com/), and more.

- **Local containers**: [**OrbStack**](https://orbstack.dev/) is our preferred Docker Desktop alternative on macOS. It runs Docker containers with low overhead, which makes it useful for running several containerized repo checkouts in parallel during AI-assisted development.

- **AI Coding Agent**: We are heavy (not token restricted!) AI fast mode users. Most people on the team currently prefer [**OpenAI Codex**](https://openai.com/codex/) for brevity and willingness to read, [**Claude Code**](https://claude.ai/) for its planning mode, and various other agents such as [**OpenCode**](https://opencode.ai/) and [**AmpCode**](https://ampcode.com/) for individual preferences. We use agents from terminal/IDE for fast local iteration and from Codex Web for server-side sessions when delegating longer-running work. Connectors for **Linear** and **Slack** make it easy to convert issue and thread context into proposed code changes and PR drafts.

- **Dictation**: [**Handy.Computer**](https://handy.computer/), [**Almond Voice**](https://www.almondvoice.com/), [**Superwhisper**](https://superwhisper.com/) and the ChatGPT app's speech-to-text are common for fast input.

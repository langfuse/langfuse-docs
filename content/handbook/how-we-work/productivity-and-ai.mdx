...
- **Coding & IDEs**: We use various IDEs for code review, such as VSCode, Zed, WebStorm, [**Cursor**](https://www.cursor.com/) and more. We are heavy (not token restricted!) AI fast mode users. Most people on the team currently prefer Codex for brevity and willingness to read, [**Claude Code**](https://claude.ai/) for its planning mode and various other agents (OpenCode, AmpCode, ...) for individual preferences.
...
- **Dictation**: Handy.Computer, Almond Voice, [Superwhisper](https://superwhisper.com/) and the ChatGPT app's speech-to-text are common for fast input.

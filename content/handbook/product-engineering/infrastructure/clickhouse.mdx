---
title: ClickHouse
description: How we use ClickHouse Cloud for observability data.
---
...
ClickHouse supports a cluster mode and a single instance mode. Those have an effect on DDL queries, as an ON CLUSTER keyword is required to achieve replication in a multi-node setup, but throws errors on single-instances without a cluster config.
...
Within the Langfuse v3 major release cycle, we promise backwards compatibility with ClickHouse 24.3.
Therefore, some of our CI runs against this version and may error if you use features or setting flags that were introduced in later versions.
Notable differences include:

- No `JSON` column support. I.e., an introduction of a new column with type JSON would demand a major release of Langfuse.

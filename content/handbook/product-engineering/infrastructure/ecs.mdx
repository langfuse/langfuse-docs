...
description: How we use AWS ECS Fargate for application containers.
---
...
- Overwriting deployed versions with stale plans. Terraform takes the current container version at the `plan` time as the baseline for subsequent writes.
  ...
  Therefore, we recommend to keep the time between plans and applies as short as possible.

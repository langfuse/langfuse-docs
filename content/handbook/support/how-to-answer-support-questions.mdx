...
7. **Important:** sometimes Langfuse previously logged unrelated events from OTEL which led to higher costs. [Read more](https://langfuse.com/faq/all/existing-otel-setup#unwanted-spans-in-langfuse:~:text=Your%20Langfuse%20dashboard,libraries%20if%20needed)
8. For refunds above 2K, loop in the COO on Pylon as `@tag`.

## Do you have a bug bounty program / Security Request

1. If the person mentions a real security thread (as shown [here](https://langfuse.com/security/responsible-disclosure#bug-bounty-program)), instantly escalate to a member of the engineering team.
...
2. If there is a known issue with Z, answer in the following way:
...
3. If this issue is not known, make sure that this is not a common issue by comparing to other support tickets.
4. If other support tickets experience the same, escalate to engineering.
5. If there are no other support tickets referencing this issue, keep the message in mind in case it is the first message of many.
6. Ask the customer about their setup. Are they self-hosted or running on cloud?
7. If they are self-hosted, ask them about their implementation and which version of Langfuse they are using.
8. For cloud, try to find a solution internally.
9. Communicate with your fellow support engineers in finding a solution here.

...
2. Direct the customer to the [contact sales form](/talk-to-us).
...
2. If they're sharing a global TracerProvider with HTTP / DB / framework auto-instrumentation, recommend a `should_export_span` filter (Python SDK, replaces the deprecated `blocked_instrumentation_scopes`) or `shouldExportSpan` (JS SDK) to drop non-LLM spans.
...
- **"How do I add a Langfuse evaluator to webhook / Custom Experiment runs?"**: the webhook only triggers the customer's endpoint. Observation-level rules cannot map expected output, so similarity judges score `0`. Point them to [Evaluate externally run experiments](/docs/evaluation/experiments/evaluate-external-experiments): score in the webhook or use the SDK runner; select managed evaluators only on prompt experiments.
...
1. Direct the customer to [langfuse.com/dpa](https://langfuse.com/dpa), where the current DPA is published.
...
<summary>"I can't see view X" / "the page is blank" / "Langfuse v4 preview" (formerly "Fast Mode")</summary>

**Triage steps:**

1. **First check: is the Langfuse v4 preview toggled on?** Many views are gated on the Langfuse v4 preview being enabled.

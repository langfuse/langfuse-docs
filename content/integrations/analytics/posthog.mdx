...
## Export source (Fast Preview, Langfuse v4) [#export-source]

PostHog integrations now include an `Export Source` selector. New integrations default to `Enriched observations (recommended)` (trace attributes are directly set on observations).
...
Each source sends a different set of events with different properties — see the [event reference](#events) below for the exact schemas.
...
| -------------------------------------------------------------- | -------------- | ------------------------------------------------------------------------- |
| `user_id`                                                      | `$distinct_id` |                                                                           |
| `trace.timestamp`, `observation.start_time`, `score.timestamp` | `timestamp`    |                                                                           |
...
Which events the integration sends depends on the selected [export source](#export-source):

| Export source                                                | Events sent                                                                       |
| ------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| `Enriched observations (recommended)`                        | [`langfuse observation`](#event-observation)                                      |
| `Traces and observations (legacy)`                           | [`langfuse trace`](#event-trace), [`langfuse generation`](#event-generation)      |
| `Traces and observations (legacy) and enriched observations` | All of the above — duplicate records by design, see [upgrade path](#upgrade-path) |

The [`langfuse score`](#event-score) event is always sent, regardless of the selected export source.
...
Sent when the export source is `Enriched observations (recommended)` or `Traces and observations (legacy) and enriched observations`. One event per observation, covering every observation type.
...
Sent when the export source is `Traces and observations (legacy)` or `Traces and observations (legacy) and enriched observations`. There is no enriched equivalent — see [changes from legacy to enriched events](#legacy-vs-enriched).
...
Sent when the export source is `Traces and observations (legacy)` or `Traces and observations (legacy) and enriched observations`. Only covers observations of type generation; replaced by [`langfuse observation`](#event-observation) in the enriched source.
...
Sent for every export source. The score event schema is identical in the legacy and enriched sources.

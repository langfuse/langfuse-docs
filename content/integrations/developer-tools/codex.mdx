...
Start a new session after changing the setting. Summary availability depends on the model and provider; this setting does not guarantee that every model call includes one. See the [Codex configuration reference](https://developers.openai.com/codex/config-reference).

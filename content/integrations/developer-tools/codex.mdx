...
- **Model responses**: assistant messages and available reasoning summaries for each model call in a turn
...
- **Generations**: one per model response in the turn. Each shows the input it received, the model's available reasoning summaries and text, the tool calls it requested, and token usage.
...
### Reasoning summaries are missing

The plugin captures readable reasoning summaries from Codex's session transcript. To request detailed summaries, add this top-level setting to `~/.codex/config.toml`, before any `[section]` headers:

```toml
model_reasoning_summary = "detailed"
```

Fully restart Codex and start a new session after changing the setting. Summary availability depends on the model and provider; this setting does not guarantee that every model call includes one. See the [Codex configuration reference](https://developers.openai.com/codex/config-reference).

Reasoning token counts can be present even when no readable summary is returned. If the transcript contains an empty `summary` array and only `encrypted_content`, the plugin cannot recover a readable summary from it. Changing this setting does not add summaries to previous runs.

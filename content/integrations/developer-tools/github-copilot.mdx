...
<Callout type="warning" title="Content capture is off by default">
  By default, Copilot exports span structure, timing, and token counts but **no prompt content, model responses, or tool arguments**. To see inputs and outputs in Langfuse, enable content capture (`github.copilot.chat.otel.captureContent` in VS Code, `COPILOT_OTEL_CAPTURE_CONTENT` for the CLI). Only enable it if you are comfortable storing this content in Langfuse.
</Callout>
...
export OTEL_EXPORTER_OTLP_HEADERS="Authorization=Basic <AUTH_STRING>,x-langfuse-ingestion-version=4"
export COPILOT_OTEL_CAPTURE_CONTENT=true # opt-in: include prompts, responses, and tool arguments
```

When you launch terminal CLI sessions from VS Code with OpenTelemetry enabled, the extension forwards `COPILOT_OTEL_ENABLED` and the endpoint automatically.
...
- **Agent traces** (`invoke_agent`): one trace per interaction, from your prompt to the final answer
- **Generations** (`chat`): one per model call, with input messages, response, model name, and token usage
- **Tool spans** (`execute_tool`): nested under the agent span, with arguments and results when content capture is enabled
- **Sessions**: interactions that share a Copilot conversation are grouped via the conversation ID

<!-- TODO: add example trace screenshot (upload to static.langfuse.com) and a public example trace link -->
...
Content capture is off by default. Enable `github.copilot.chat.otel.captureContent` (VS Code) or `COPILOT_OTEL_CAPTURE_CONTENT=true` (CLI) and start a new Copilot session.

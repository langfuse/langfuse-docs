...
The [Langfuse OpenCode Observability Plugin](https://github.com/langfuse/opencode-observability-plugin) supports **OpenCode 2** and **OpenCode 1.18.29 or newer** as of [plugin version 0.5.0](https://github.com/langfuse/opencode-observability-plugin/releases/tag/v0.5.0). Choose your OpenCode version in the [setup instructions](#enable-the-plugin), then add your Langfuse keys and restart OpenCode.

</Callout>

## What can this integration trace?

The plugin reads OpenCode's session telemetry and sends it to Langfuse. You can monitor:
...
1. You add `@langfuse/opencode-observability-plugin` to your OpenCode config using the key for your version (`plugins` in OpenCode 2, `plugin` in OpenCode 1).
...
Use the configuration for your OpenCode version in `opencode.json` or `opencode.jsonc`:

**OpenCode 2 (plugin 0.5.0 or newer)** — use the plural `plugins` key:
...
**OpenCode 1.18.29 or newer (plugin 0.5.0 or newer)** — use the singular `plugin` key:
...
Restart OpenCode after changing the config. Plugin version 0.5.0 and newer does not support OpenCode 1 versions before 1.18.29.
...
1. **The plugin isn't loaded.** Confirm `@langfuse/opencode-observability-plugin` is in the `plugins` array for OpenCode 2 or the `plugin` array for OpenCode 1.18.29 and newer, then restart OpenCode.

...
> **What is Langfuse?** [Langfuse](/) is an open-source AI engineering platform that provides tracing, evaluation, prompt management, and metrics to debug and improve LLM applications. Use [Langfuse Cloud](/cloud) (managed, free tier, nothing to run) or a [self-hosted](/self-hosting) instance; this integration works with both.

> **Compatibility note:** the current upstream `ai::langfuse` transport predates
> Langfuse's OTLP-first v4 data model. For new Langfuse Cloud and self-hosted
> v4 deployments, prefer standard OpenTelemetry instrumentation exported to
> Langfuse's [OTLP/HTTP endpoint](/integrations/native/opentelemetry). Use this
> page only when you explicitly need the current AI SDK C++ Langfuse module and
> have validated it against your target deployment.
...
A `Trace` accumulates events in memory and flushes them when `end()` is called.
...
- For greenfield Langfuse v4 integrations, prefer OpenTelemetry-based C++
  instrumentation so you can export OTLP/HTTP directly to Langfuse's supported
  [ingestion path](/integrations/native/opentelemetry).

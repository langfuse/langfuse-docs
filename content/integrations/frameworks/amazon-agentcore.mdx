...
Create an AgentCore agent that integrates with Langfuse via OpenTelemetry. This example uses [Strands](/integrations/frameworks/strands-agents) with MCP tools. Any other agent framework can be used with Langfuse, see [integration pages](/integrations) for guides on how to instrument other frameworks.

...
`@flue/opentelemetry` only converts Flue events into spans — your application owns the OpenTelemetry SDK and exporter. Configure the SDK with the Langfuse [`LangfuseSpanProcessor`](https://langfuse.com/docs/observability/sdk/typescript/setup) and register the observer in your Flue app's entrypoint (`src/app.ts`).
...
// Flush spans before a one-shot process (e.g. `flue run`) exits.
...
After running the workflow, open your [Langfuse dashboard](https://langfuse.com/cloud) to see the full trace: the workflow span, each model turn mapped to a generation (with model, token usage, and cost), and nested tool calls.

<!-- TODO: add a screenshot of a Flue trace. Upload the PNG to public/images/cookbook/integration-flue/flue-example-trace.png and embed it here with an image tag pointing at https://langfuse.com/images/cookbook/integration-flue/flue-example-trace.png -->

[Example trace in Langfuse](https://cloud.langfuse.com/project/cloramnkj0002jz088vzn1ja4/traces/a11bbc4370b42e3646f4fb2e9ff33b53)

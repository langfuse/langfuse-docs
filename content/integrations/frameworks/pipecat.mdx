...
### Add observation input and output

By default, Pipecat traces do not promote the conversation input and output onto a Langfuse observation. You can patch Pipecat's service decorators to set `langfuse.observation.input` and `langfuse.observation.output`, capturing the first LLM call's messages as the observation input and the last LLM response as the observation output:
...
def patch_trace_input_output():
    ...
    def patched(span, *args, **kwargs):
        ...
        # Set the input of the first LLM call as the observation input
        if first_call[0] and kwargs.get("messages"):
            span.set_attribute("langfuse.observation.input", kwargs["messages"])
            first_call[0] = False

        # Set the output of each LLM call as the observation output (last one wins)
        orig_set = span.set_attribute
        def new_set(key, value):
            orig_set(key, value)
            if key == "output":
                orig_set("langfuse.observation.output", value)

...
### JDBC Dependency Conflict

If your Spring AI application uses a JDBC datasource, you may encounter a dependency conflict that prevents the application from starting, caused by a version mismatch between OpenTelemetry libraries.
...
**The fix:** Exclude the problematic transitive dependency from `micrometer-tracing-bridge-otel`. Since the `opentelemetry-instrumentation-bom` defined earlier in this guide already manages `opentelemetry-instrumentation-api-incubator`, you don't need to specify a version:
...
For more details, see the [Spring AI demo fix PR](https://github.com/langfuse/langfuse-examples/pull/34) and the [related bug report](https://github.com/langfuse/langfuse/issues/14312).

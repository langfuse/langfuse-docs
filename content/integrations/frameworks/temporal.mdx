...
- **Use a fresh workflow ID per run.** The workflow ID doubles as the Langfuse session ID, so each run groups cleanly in the Sessions view. (The trace itself is keyed by the starter's root span, which is new on every run.)

...
For TypeScript or JavaScript Temporal applications, use Temporal's OpenTelemetry plugin together with the Langfuse OpenTelemetry span processor. The same Langfuse credentials from step 2 apply.

Install the Temporal SDK, the Temporal OpenTelemetry plugin, and the Langfuse OpenTelemetry integration:

```bash
npm install @temporalio/client @temporalio/worker @temporalio/workflow @temporalio/activity @temporalio/interceptors-opentelemetry-v2 @langfuse/tracing @langfuse/otel @opentelemetry/sdk-node @opentelemetry/resources @opentelemetry/semantic-conventions
```

Create an OpenTelemetry setup file that exports spans to Langfuse:
...
export const resource = resourceFromAttributes({
  [ATTR_SERVICE_NAME]: "temporal-research-agent",
});
...
Register Temporal's OpenTelemetry plugin on the worker. Define your workflows and activities as usual; the plugin propagates trace context across workflow and activity execution.

```typescript filename="worker.ts"
import { Worker } from "@temporalio/worker";
import { OpenTelemetryPlugin } from "@temporalio/interceptors-opentelemetry-v2";
import * as activities from "./activities";
import { resource, setupOtelSdk, spanProcessor } from "./instrumentation";
...
Use the same plugin on the Temporal client so the client span becomes the parent of the workflow trace.
...
async function main() {
  ...
  try {
    const result = await client.workflow.execute(ResearchWorkflow, {
      workflowId: randomUUID(),
      taskQueue: "research-task-queue",
      args: ["Caribbean vacation spots in April"],
    });

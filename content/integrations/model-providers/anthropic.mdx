...
Langfuse is open source. Send traces to [Langfuse Cloud](https://langfuse.com/cloud) (managed, free tier, nothing to run) or to a [self-hosted](https://langfuse.com/self-hosting) instance; the code below is the same for both.

This notebook demonstrates **two** different ways to use Anthropic models with Langfuse:

1. **OpenTelemetry Instrumentation (recommended):** Use the [`opentelemetry-instrumentation-anthropic`](https://pypi.org/project/opentelemetry-instrumentation-anthropic/) package to wrap Anthropic SDK calls and send OpenTelemetry spans to Langfuse. The Langfuse SDK exports these spans by default: they carry `gen_ai.*` attributes and come from an allowlisted instrumentation scope ([details](https://langfuse.com/docs/observability/sdk/advanced-features#filtering-by-instrumentation-scope)). The package is maintained by a third party (Traceloop), so pin its version in your requirements.

...
For more detailed guidance on the Mistral SDK or the **@observe** decorator from Langfuse, please refer to the [Mistral SDK repo](https://github.com/mistralai/client-python) and the [Langfuse Documentation](https://langfuse.com/docs/observability/sdk/instrumentation#observe-wrapper).
...
In the following example, we also added the decorator to the top-level function `find_best_painter_from`. This function calls the mistral_completion function, which is decorated with @observe(as_type="generation"). This hierarchical setup helps to trace more complex applications which involve multiple LLM calls and other non-llm methods which are decorated with @observe.

You can use `langfuse.update_current_generation` or `langfuse.update_current_span` to add additional details such as input, output, and model parameters to the current observation. To add attributes such as a user ID, session ID, or tags to the whole trace, wrap your code in the `propagate_attributes` context manager: the attributes are applied to all observations created within its scope. Trace input and output are derived from the root observation (here, the @observe-decorated `find_best_painter_from` function), so there is no need to set them separately.
...
The following example demonstrates how to handle streaming responses from the Mistral model using the @observe(as*type="generation") decorator. The process is similar to the \_Completion* example but includes handling streamed data in real-time.

...
<Callout type="info">
  Looking for the Python version? See the [OpenAI (Python)
  integration](/integrations/model-providers/openai-py).
</Callout>
...
The wrapped client also traces the Responses API:
...
<Callout type="warning">
  For Responses API calls, Langfuse records the `input` argument as the
  generation input. A system prompt passed via the separate `instructions`
  parameter is not part of the captured input; it only shows up in the
  generation's metadata. To see the system prompt in the trace input, pass it
  as a `developer` message inside `input` as shown above.
</Callout>

...
Use `propagateAttributes` to apply trace attributes (name, tags, metadata) to all observations created inside its callback, and the `startActiveObservation` context manager to group multiple OpenAI generations under a single root observation. Input and output are set on the root observation.

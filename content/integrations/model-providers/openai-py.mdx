...
<Callout type="info">
  Looking for the JS/TS version? See the [OpenAI (JS/TS)
  integration](/integrations/model-providers/openai-js).
</Callout>
...
Langfuse is open source. Use [Langfuse Cloud](/cloud) (hosted by Langfuse, free tier, no infrastructure to run) or [self-host](/self-hosting) it. The integration code is the same for both.
...
Learn more about [queuing and batching of events](/docs/observability/features/queuing-batching).
...
[Sampling](/docs/observability/features/sampling) can be used to control the volume of traces collected by the Langfuse server.
...
**Option 2: Via enclosing [observation](/docs/observability/data-model#observations-traces-and-sessions) (for more control):**
...
[Langfuse Tracing](/docs/observability/overview) groups multiple observations (can be any LLM or non-LLM call) into a single trace. This integration by default creates a single trace for each openai call.
...
<Callout type="info">
  New to Langfuse Tracing? Checkout this [introduction](/docs/observability/overview) to the
  basic concepts.
</Callout>
...
Langfuse automatically tracks and monitors OpenAI API errors if you use the native integration. They are captured via the `level` and `statusMessage` fields (see [docs](/docs/observability/features/log-levels)).

See the [setup steps above](#how-it-works) to get started.

...
A System One call is not a chat exchange, so spans do **not** set `llm.input_messages` / `llm.output_messages`. Mask request content with `TraceConfig(hide_inputs=True)` if the state can contain personal data.
...
There is no `@arizeai/openinference-instrumentation-typesafe` package, and [`@typesafe-ai/sdk`](https://docs.typesafe.ai/sdk/javascript) has no OpenTelemetry hook. Until an auto-instrumentor ships, wrap `systemOne` with Langfuse [`observe()`](/docs/observability/sdk/instrumentation):
...
const systemOne = observe((request: Parameters<TypeSafeClient["systemOne"]>[0]) => client.systemOne(request), {
  name: "TypeSafeClient.systemOne",
  asType: "generation",
});
...
Jev is also listed on [OpenRouter](https://openrouter.ai/typesafe/jev-1.13) and the [Vercel AI Gateway](https://vercel.com/ai-gateway/models/jev). Those paths can log HTTP traffic through existing gateway integrations, but they will not record System One questions and typed answers. Use the TypeSafe SDK plus this instrumentor when you need that shape.
...
![Example TypeSafe (Jev) trace in Langfuse](https://langfuse.com/images/cookbook/integration-typesafe/typesafe-example-trace.png)

<!-- TODO: replace with a real trace screenshot (upload to langfuse.com images) and a public example trace link -->

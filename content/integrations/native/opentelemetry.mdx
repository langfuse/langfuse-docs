...
If you want spans ingested directly via OpenTelemetry to appear in real time on the v4 data model, include the `x-langfuse-ingestion-version: 4` header. If your setup uses signal-specific header settings, add the same value to `OTEL_EXPORTER_OTLP_TRACES_HEADERS`.

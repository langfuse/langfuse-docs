...
If you want spans ingested directly via OpenTelemetry to appear in real time in Langfuse Cloud Fast Preview (v4), include the `x-langfuse-ingestion-version: 4` header. If your setup uses signal-specific header settings, add the same value to `OTEL_EXPORTER_OTLP_TRACES_HEADERS`.

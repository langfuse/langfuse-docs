...
<Callout type="info">
  **Moving off the legacy `POST /api/public/ingestion` API?** OTLP is its v4
  replacement. See
  [Migration of deprecated APIs](/docs/deprecated-api-migration#ingestion) for the endpoint
  mapping and the `events_only` write-path behavior, and the
  [Deprecated APIs reference](/docs/api-and-data-platform/features/deprecated-apis#ingestion-v1)
  for the endpoint being replaced.
</Callout>

...
<Callout type="info">
  **Moving off the legacy `POST /api/public/ingestion` API?** OTLP is its v4
  replacement. See
  [Transitioning to the v4 API](/docs/api-migration#ingestion) for the endpoint
  mapping and the `events_only` write-path behavior, and the
  [Legacy APIs reference](/docs/api-and-data-platform/features/legacy-apis#ingestion-v1)

...
Langfuse can operate as an OpenTelemetry Backend to receive traces on the `/api/public/otel` (OTLP) endpoint. In addition to the [Langfuse SDKs](/docs/observability/sdk/overview) and [native integrations](/integrations), this OpenTelemetry endpoint is designed to increase compatibility with frameworks, libraries, and languages beyond the SDKs and native integrations. Popular OpenTelemetry libraries include OpenLLMetry and OpenLIT which extend Language support of Langfuse tracing to Java and Go and cover frameworks such as AutoGen, Semantic Kernel, and more.
...
<Callout type="info">
  **Using Python or JS/TS?** Prefer the Langfuse SDKs instead of wiring raw
  OpenTelemetry exporters directly. Start with the OTEL-native
  [Langfuse SDKs](/docs/observability/sdk/overview) (Python SDK v4, JS/TS SDK
  v5). This OpenTelemetry page is most useful for existing OTEL setups,
  collector-based ingestion, and unsupported languages.
</Callout>
...
<Callout type="warning">
  **Important:** If you want to filter and aggregate by `userId`, `sessionId`,
  ...
  trace-level attributes to every span in the trace. On the
  [Langfuse v4 data model](/docs/v4), each observation stores its own copy of
  these attributes — spans that do not carry them are missed by filters and
...
On the [Langfuse v4 data model](/docs/v4), filters and aggregations operate
across individual observations, and every observation row stores its own copy
of these attributes. A span that does not carry them is not matched when you
filter or group by user, session, tags, and so on — setting them only on the
root span means only the root observation is found. To reliably filter and
aggregate, the attributes need to be present on each span in the trace.

<Callout type="info">
  **Transition behavior:** Spans ingested **without** the
  `x-langfuse-ingestion-version: 4` header ([see
  below](#opentelemetry-endpoint)) flow through a transitional pipeline that
  retains the previous server-side behavior — trace-level attributes set on
  any span are applied to all observations of the trace — at the cost of a
  **~10 minute ingestion delay**. Self-hosted Langfuse v4 deployments ingest
  all OpenTelemetry spans directly by default
  (`LANGFUSE_MIGRATION_V4_NATIVE_OTEL_BEHAVIOUR=direct`), so client-side
  propagation is always required there, with or without the header.
</Callout>
...
See [Add attributes to observations](/docs/observability/sdk/instrumentation#add-attributes)
for implementation details.

## Ingestion Options

### OpenTelemetry-native Langfuse SDKs

The quickest path to start tracing with Langfuse are the **OTEL-native Langfuse SDKs** (Python SDK v4, JS/TS SDK v5). The SDKs are a thin layer on top of the official OpenTelemetry client that automatically converts emitted spans into rich Langfuse observations (spans, generations, events, and [other observation types](/docs/observability/features/observation-types)), adds first-class helpers for LLM-specific features such as token usage, cost tracking, prompt linking, and scoring, and handles [trace attribute propagation](#propagating-attributes) via `propagate_attributes()` / `propagateAttributes()`.

Because they live in the shared OpenTelemetry context, spans from other OTEL-instrumented libraries can be exported to Langfuse too. By default, Langfuse focuses on LLM-relevant spans (Langfuse SDK spans, spans with `gen_ai.*` attributes, and known LLM instrumentors). To export everything, use a permissive custom filter as described in the [advanced SDK docs](/docs/observability/sdk/advanced-features#filtering-by-instrumentation-scope).

Get started with the dedicated guide: [/docs/observability/sdk/overview](/docs/observability/sdk/overview).
...
The `x-langfuse-ingestion-version: 4` header opts your exporter into direct ingestion on the [Langfuse v4](/docs/v4) data model: spans appear in real time, and Langfuse expects trace-level attributes to be [propagated to every span](#propagating-attributes) client-side. Without the header, spans flow through a transitional pipeline that applies trace-level attributes server-side but delays data by ~10 minutes. Self-hosted v4 deployments with the default `LANGFUSE_MIGRATION_V4_NATIVE_OTEL_BEHAVIOUR=direct` ingest all spans directly, regardless of the header. If your setup uses signal-specific header settings, add the same value to `OTEL_EXPORTER_OTLP_TRACES_HEADERS`.
...
You can use the OpenTelemetry SDKs to directly export traces to Langfuse with the configuration mentioned above. Thereby, Language support of Langfuse is extended to other languages than the ones supported by the [Langfuse SDKs](/docs/observability/sdk/overview) (Python and JS/TS).
...
Langfuse also expects the root span of each trace to be sent: it becomes the root observation that represents the trace (trace name, timestamps, time bucketing in charts). Traces whose root span is filtered out can render incompletely or be missing from time-series charts.
...
- [Trace-level attributes](#trace-level-attributes) represent shared context for an entire interaction — user, session, tags, release, version. On the [Langfuse v4 data model](/docs/v4), every observation stores its own copy of these attributes, so they must be [propagated to all spans](#propagating-attributes) of a trace.
...
### Trace-Level Attributes [#trace-level-attributes]

These attributes carry the shared context of a trace and are used to group, filter, and aggregate related observations. Set them on **every span** of a trace ([propagation](#propagating-attributes)): with direct v4 ingestion, each observation only carries the attributes set on its own span, so a span without them is missed by filters and aggregations. Spans ingested through the transitional non-v4 pipeline (no `x-langfuse-ingestion-version: 4` header) retain the previous behavior — attributes set on any span are applied to the whole trace — at the cost of a ~10 minute delay.
...
| <a id="input"></a>`input`             | The initial input for the entire trace, derived from the root observation. Set the input on the root span.                               | • Root span's observation input ([`langfuse.observation.input`](#obs-input))<br/>• `langfuse.trace.input`: `string` — legacy, ignored by direct v4 ingestion     |
| <a id="output"></a>`output`           | The final output for the entire trace, derived from the root observation. Set the output on the root span.                               | • Root span's observation output ([`langfuse.observation.output`](#obs-output))<br/>• `langfuse.trace.output`: `string` — legacy, ignored by direct v4 ingestion |
| <a id="version"></a>`version`         | The [version](/docs/observability/features/releases-and-versioning) of the trace, useful for tracking changes to your application logic. | • `langfuse.version`: `string` (per span)<br/>• `service.version` resource attribute                                                                             |
| <a id="environment"></a>`environment` | The deployment [environment](/docs/observability/features/environments) where the trace was generated.                                   | • `langfuse.environment`: `string` (per span)<br/>• `deployment.environment.name` / `deployment.environment` resource attribute                                  |
...
If you want to query on specific attributes, you can use the `langfuse.trace.metadata` prefix to map them to the top-level `metadata` object.
The following snippet will produce a filterable `user_name` property in the `metadata` object of the observation the attribute is set on — combine it with [attribute propagation](#propagating-attributes) to make it filterable across the whole trace:
...
### Observation-Level Attributes [#observation-level-attributes]
...
| <a id="model"></a>`model`                             | The name of the generative model used. _Generation only._                                                                                  | • `langfuse.observation.model.name`<br/>• `gen_ai.request.model`<br/>• `gen_ai.response.model`<br/>• `llm.model_name`<br/>• `model`                 |
| <a id="modelParameters"></a>`modelParameters`         | Key-value pairs for model invocation settings. _Generation only._                                                                          | • `langfuse.observation.model.parameters`: `JSON string`<br/>• `gen_ai.request.*`<br/>• `llm.invocation_parameters.*`                               |
| <a id="usage"></a>`usage`                             | Token counts for the generation. _Generation only._                                                                                        | • `langfuse.observation.usage_details`: `JSON string`<br/>• `gen_ai.usage.*`<br/>• `llm.token_count.*`                                              |
| <a id="cost"></a>`cost`                               | The calculated cost in USD. _Generation only._                                                                                             | • `langfuse.observation.cost_details`: `JSON string`<br/>• `gen_ai.usage.cost` (set as `total` key)                                                 |
| <a id="prompt"></a>`prompt`                           | The name of a versioned prompt managed in Langfuse. _Generation only._                                                                     | • `langfuse.observation.prompt.name`: `string`<br/>• `langfuse.observation.prompt.version`: `integer`                                               |
| <a id="completionStartTime"></a>`completionStartTime` | Timestamp for when the model began generating. _Generation only._                                                                          | • `langfuse.observation.completion_start_time`: `ISO 8601 date string`                                                                              |
...
- **Filtering by `userId`, `sessionId`, or `tags` only matches the root observation:** these trace-level attributes are stored per observation on the Langfuse v4 data model. Propagate them to all spans of the trace — see [Propagating Trace Attributes to All Spans](#propagating-attributes).
- **Spans appear with a ~10 minute delay:** OTel exporters without the `x-langfuse-ingestion-version: 4` header flow through the transitional ingestion pipeline. Add the header to your exporter to ingest in real time ([configuration](#opentelemetry-endpoint)).

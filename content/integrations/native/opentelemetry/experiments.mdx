...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Experiment context   | Propagates the experiment's identity to every span in every item trace, so Langfuse can synthesize them as one experiment. | Baggage                              | `langfuse.experiment.id`, `langfuse.experiment.name`, `langfuse.experiment.dataset.id`, `langfuse.experiment.metadata.*` (optional), `langfuse.environment` (optional)                                             |
| Experiment item root | Identifies one item trace root and stores its input, expected output, and actual output.                                   | Attributes directly on the root span | `langfuse.observation.input`, `langfuse.observation.output`, `langfuse.experiment.description` (optional), `langfuse.experiment.item.expected_output` (optional), `langfuse.experiment.item.metadata.*` (optional) |
...
If provided, set `langfuse.experiment.description` directly on each experiment
item root, using the same description for the entire run. Like the Python and
JS/TS Experiment runners, this example does not propagate description to child
spans. Experiment metadata, by contrast, belongs in experiment Baggage and must
remain consistent across all spans in the run.

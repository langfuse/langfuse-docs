...
on the v4 data model and the v2 Observations and Metrics APIs. Without
...
<Callout type="warning">
  **Migrating to Langfuse v4?** If you send custom events to the legacy `/api/public/ingestion` endpoint, or already send OTEL spans that rely on trace input/output or root-only attributes, follow the [custom ingestion migration guide](/integrations/native/opentelemetry/migration-to-v4).
</Callout>
...
<Callout type="info">
  OTLP replaces the deprecated `POST /api/public/ingestion` API. See
  [Migration of deprecated APIs](/faq/all/deprecated-api-migration#ingestion) for
  the endpoint mapping; the deprecated endpoint itself is documented in its
  [endpoint reference section](/faq/all/deprecated-api-migration#endpoints).
</Callout>
...
### Run experiments via OpenTelemetry [#experiments-ingesting-experiment-spans]

To group direct-OTEL traces as Langfuse experiments, start with [Experiments via OpenTelemetry](/docs/evaluation/experiments/experiments-via-opentelemetry) and then add the experiment and item attributes described in [Ingest experiment spans with OpenTelemetry](/integrations/native/opentelemetry/experiments).

### Use OpenTelemetry GenAI Instrumentation Libraries [#instrumentation-libraries]
...
- [Strands](/integrations/frameworks/strands-agents)
...
| <a id="input"></a>`input`             | Deprecated trace input, retained for legacy trace-level evaluators. In v4, use the root observation's input.                             | • `langfuse.trace.input`: `string`<br/>• Root span's observation input         |
| <a id="output"></a>`output`           | Deprecated trace output, retained for legacy trace-level evaluators. In v4, use the root observation's output.                           | • `langfuse.trace.output`: `string`<br/>• Root span's observation output       |
...
| Langfuse Field                                        | Description                                                                                                                                                                                   | Mapped from OTel Attribute                                                                                                                                                                                                                                                                |
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="type"></a>`type`                               | The [type of observation](/docs/observability/features/observation-types). A recognized `langfuse.observation.type` always wins; see [ingestion transformations](#ingestion-transformations). | • `langfuse.observation.type`: `"span" \| "generation" \| "event" \| "embedding" \| "agent" \| "tool" \| "chain" \| "retriever" \| "guardrail" \| "evaluator"`<br/>• If unset or unrecognized: inferred from other conventions, then from a model attribute (`generation`), then `"span"` |
...
| <a id="obs-input"></a>`input`                         | The input data for this specific observation.                                                                                                                                                 | • `langfuse.observation.input`: `(JSON) string`<br/>• `gen_ai.prompt`<br/>• `input.value` (OpenInference)<br/>• `mlflow.spanInputs` (MLFlow)<br/>• `gen_ai.tool.call.arguments` (OpenTelemetry GenAI tool spans, used when no other input attribute is set)                               |
| <a id="obs-output"></a>`output`                       | The output data from this specific observation.                                                                                                                                               | • `langfuse.observation.output`: `(JSON) string`<br/>• `gen_ai.completion`<br/>• `output.value` (OpenInference)<br/>• `mlflow.spanOutputs` (MLFlow)<br/>• `gen_ai.tool.call.result` (OpenTelemetry GenAI tool spans, used when no other output attribute is set)                          |
...
| <a id="prompt"></a>`prompt`                           | The name of a versioned prompt managed in Langfuse. _Generation only._                                                                                                                        | • `langfuse.observation.prompt.name` or `langfuse.prompt.name`: `string`<br/>• `langfuse.observation.prompt.version` or `langfuse.prompt.version`: `integer`                                                                                                                              |
...
### Ingestion transformations [#ingestion-transformations]

<Callout type="warning">
  Langfuse accepts incomplete or non-standard OTLP spans instead of rejecting
  them. The request still succeeds; the span is rewritten.
</Callout>

1. **Accepted `langfuse.observation.type` values** are `span`, `generation`, `event`, `embedding`, `agent`, `tool`, `chain`, `retriever`, `guardrail`, and `evaluator`.
2. **An unrecognized explicit type is not rejected.** A typo or a vendor-specific string such as `llm` falls through to other mapped conventions (for example `gen_ai.operation.name`), then to the model fallback, then to `span`.
3. **A model attribute infers `generation` only as a fallback** — when no recognized `langfuse.observation.type` and no higher-priority convention applied. A valid explicit type always wins, even if the span also has a model name (`langfuse.observation.model.name`, `gen_ai.request.model`, `gen_ai.response.model`, `llm.model_name`, or `model`).
4. **Missing timestamps are filled in.** If `startTimeUnixNano` or `endTimeUnixNano` is missing or unparseable, Langfuse copies the remaining edge so both start and end are set (the observation has zero duration). If both are missing, both become the ingest time. Send both fields on every span.
...
## Common patterns without a Langfuse SDK [#patterns-without-sdk]

If you instrument with a plain OpenTelemetry SDK (for example in Go or Java), set the attributes from the [mapping table](#property-mapping) directly on your spans:

- **Link a generation to a prompt version.** Set `langfuse.observation.type` to `generation`, `langfuse.observation.prompt.name` to the prompt name, and `langfuse.observation.prompt.version` to the version number. The version must be a whole number; any other value is ignored and the prompt is not linked. Prompt linking only applies to observations of type `generation`; on other types the attributes are ignored.
- **Record a tool call.** Set `langfuse.observation.type` to `tool` and put the arguments and result in `langfuse.observation.input` and `langfuse.observation.output`. Spans that follow the OpenTelemetry GenAI semantic conventions work without the explicit type: `gen_ai.tool.name` or `gen_ai.tool.call.id` marks the span as a tool, and `gen_ai.tool.call.arguments` and `gen_ai.tool.call.result` are read as input and output when no other input or output attribute is present.

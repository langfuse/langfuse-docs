...
description: Forward OpenTelemetry traces from ElevenLabs Agents conversations to Langfuse to monitor transcripts, tool calls, token usage, and cost.
category: Integrations
---

# Trace ElevenLabs Agents with Langfuse

This notebook shows how to forward **OpenTelemetry traces** of [ElevenLabs Agents](https://elevenlabs.io/agents) conversations to **Langfuse**. You get each conversation as a full trace next to your other LLM application traces: transcript turns, LLM generations with token usage and cost, and tool calls. The notebook is self-contained: it creates a demo agent, runs a text-only conversation, and forwards the resulting trace.
...
We create a small banking-style demo agent with a **webhook tool** that looks up currency exchange rates via the free [Frankfurter API](https://frankfurter.dev), so the trace contains a real tool call.

If you already have an agent, keep the client initialization (the next step needs it) and replace the `agents.create(...)` call with `AGENT_ID = "agent_..."`.
...
agent = client.conversational_ai.agents.create(
    ...
    conversation_config={
        ...
        "agent": {
            "first_message": "Hi! I can help you with currency exchange rates. What would you like to know?",
            "prompt": {
                "prompt": (
                    "You are a helpful banking assistant. Use the get_exchange_rate tool "
                    "to look up currency exchange rates when users ask about them. Keep answers short."
                ),
                "tools": [
                    {
                        "type": "webhook",
...
Open [Langfuse Cloud](https://langfuse.com/cloud) to see the full conversation trace: the conversation as the root `agent` observation, transcript turns, LLM generations with token usage and cost, and the tool call with its parameters and result. The conversation ID is the Langfuse session ID, so all traces of the same conversation group together in the [Sessions view](https://langfuse.com/docs/observability/features/sessions).
...
[Example trace in Langfuse](https://cloud.langfuse.com/project/cmt9p6v7u0p67ad0gzqrbh7x3/traces/ee0df1d896bf42b7d7dfb5981450e1ea)

...
<Cards num={3}>
  <Card icon={<Activity />} title="Build and instrument agents">
    Implement agents and applications, then add Langfuse tracing and
    instrumentation so teams can understand behavior, cost, latency, and failure
    modes.
  </Card>
  <Card icon={<FlaskConical />} title="Establish quality workflows">
    Define evaluation strategy, curate datasets, run experiments, monitor quality,
    and set up prompt-management workflows that customers can operate after
    handover.
  </Card>
  <Card icon={<Rocket />} title="Take solutions to production">
    Use Langfuse Cloud as the standard managed platform. Support single-tenant
    self-hosting only when a customer's security, compliance, or infrastructure

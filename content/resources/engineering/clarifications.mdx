...
Yes. [Prompt management](/docs/prompt-management) stores, versions, labels, and serves prompts from Langfuse, with client-side SDK caching so a fetch after the first hit does not add latency.

...
Yes. [Alerts](/docs/metrics/features/alerts) watch metrics over observations and scores — for example observation counts, p95 latency, average cost, or an evaluation score — with filters and aggregations. They support separate warning and alert thresholds, handle windows with no data, and deliver to Slack, webhooks, or GitHub Actions. Alerts run on Langfuse Cloud and on self-hosted v4+.

## What access control does Langfuse offer? [#rbac]

Langfuse ships five roles — Owner, Admin, Member, Viewer, and None — assigned at the organization level, with optional [per-project roles](/docs/administration/rbac) to differentiate access between projects in the same organization. Project-level roles are part of the Teams add-on on Cloud Pro, Cloud Enterprise, and the self-hosted Enterprise license; SCIM provisioning and audit logs cover enterprise governance.

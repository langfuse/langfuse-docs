...
Langfuse ships five roles — Owner, Admin, Member, Viewer, and None — assigned at the organization level, with optional [per-project roles](/docs/administration/rbac) to differentiate access between projects in the same organization. Project-level roles are available through the Teams add-on on Cloud Pro and are included with Cloud Enterprise and the self-hosted Enterprise license; SCIM provisioning and audit logs cover enterprise governance.

...
REQUIRED = {"intent": str, "confidence": float, "entities": list}
...
For applications that compute or extract numbers (finance, analytics, unit conversion), exact string match is the wrong test. Extract the number and compare within a tolerance against the expected value.
...
def evaluate(ctx):
    ...
    got, want = float(nums[-1]), float(expected)
    ok = abs(got - want) <= abs(want) * 0.01  # 1% tolerance
    return EvaluationResult(scores=[

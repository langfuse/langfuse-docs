...
Langfuse ([GitHub](https://github.com/langfuse/langfuse)) is an open-source platform for LLM [observability](/docs/observability), [prompt management](/docs/prompt-management), and [evaluation](/docs/evaluation). It stores scores next to the traces and experiment runs they evaluate, so you can filter production traffic by quality, chart scores over time, and compare experiment runs side by side.

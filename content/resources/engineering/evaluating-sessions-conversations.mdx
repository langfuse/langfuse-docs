...
The information here applies to all [evaluation methods](/docs/evaluation) supported by Langfuse.
...
Trace-level LLM-as-a-Judge evaluators are legacy: on Langfuse Cloud they stop producing results at the v4 cutover on <V4CutoverDate />. On self-hosted, they will stop producing results once you run the v4 server in `events_only` mode. See the [upgrade guide](/faq/all/llm-as-a-judge-migration) for migrating them to observation-level evaluators.

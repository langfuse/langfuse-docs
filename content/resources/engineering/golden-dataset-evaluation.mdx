...
scores = langfuse.api.scores_v3.get_many_v3(
    ...
    fields="subject",  # what the score is attached to: trace, observation, session, or experiment
    limit=100,
)

for score in scores.data:
    # keep thumbs-down scores that are attached to a trace
    if score.value != 0 or not score.subject or score.subject.kind != "trace":
        continue

    # A trace carries no input of its own; read it from the trace's root observation.
    root_observations = langfuse.api.observations.get_many(
        trace_id=score.subject.id,
    ...
    langfuse.create_dataset_item(
        dataset_name="support-agent/golden",
        input=root_observations[0].input,
        ...
        source_trace_id=score.subject.id,
    )
```
...
<Details id="dataset-size">
<Summary>How large should a golden dataset be?</Summary>
...
<Details id="ground-truth">
<Summary>Does every item need a ground-truth expected output?</Summary>
...
<Details id="dataset-drift">
<Summary>How do we detect and handle dataset drift?</Summary>
...
<Details id="share-across-projects">
<Summary>Can we share a golden dataset across projects?</Summary>

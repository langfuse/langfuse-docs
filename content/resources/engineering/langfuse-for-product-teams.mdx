...
Langfuse pages, and asks for explicit approval before any action that changes data. It is in
public beta on Langfuse Cloud and on self-hosted deployments with an [instance Langfuse AI
model](/self-hosting/configuration/langfuse-assistant).
...
in the membership-support queue") without knowing the field names. It is off by default, and
enabled by an organization owner or admin in the organization's AI feature settings. On Cloud
it uses Langfuse-managed Amazon Bedrock; on self-hosted it uses the same instance model as
the Assistant.
...
The older table natural-language filter builder remains Cloud-only.

...
The gate only covers inputs you thought to test, so pair it with an online complement. Sampled LLM-as-a-judge evaluators score a percentage of production traffic, and [alerts](/docs/metrics/features/alerts) (available on Langfuse Cloud) fire on those score metrics with warning and alert thresholds, routing breaches to Slack, webhooks, or a GitHub Actions trigger. The offline gate stops regressions you can reproduce; alerts catch the ones production finds for you.

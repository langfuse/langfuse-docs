...
The script below is a complete gate using the Langfuse Python SDK (v4). It combines a deterministic code evaluator, which requires no model calls and gives the same verdict for the same output, with an optional LLM-as-a-judge evaluator for reference-answer correctness. Start with deterministic checks; add the judge after reviewing its agreement with human labels. The `experiment(context)` entry point is the contract for the GitHub Action in the next section: the `RunnerContext` initializes the Langfuse client, loads the dataset configured in the workflow, and attaches CI metadata (commit SHA, branch, job URL, actor) to the run.
...
def reference_correctness_judge(*, input, output, expected_output, **kwargs):
        ...
        messages=[
            {
                "role": "user",
                "content": (
                    "Is the answer correct according to the reference answer? "
    ...
    if type(verdict.get("score")) not in (int, float) or verdict["score"] not in (0, 1):
        raise ValueError("Judge must return a binary score")
    return Evaluation(
...
def average_of(score_name: str):
    ...
    def run_evaluator(*, item_results, **kwargs):
        ...
        if not values or len(values) != len(item_results):
            raise ValueError(f"Missing {score_name} evaluations")
        return Evaluation(
...
def experiment(context: RunnerContext):
    result = context.run_experiment(
        ...
        evaluators=[contains_answer, reference_correctness_judge],
        run_evaluators=[average_of("contains_answer"), average_of("reference_correctness")],
    )
...
Each dataset input must contain `question` and `context`; its expected output is the reference answer. Reference-answer correctness is distinct from grounding in retrieved context. To evaluate grounding, pass the actual retrieved context to a dedicated evaluator. To run deterministic checks only, remove `reference_correctness_judge`, its run evaluator, and its threshold.

For a gate that detects newly failing cases even when average accuracy improves, see [Compare against an approved baseline](/docs/evaluation/experiments/experiments-ci-cd#approved-baseline).
...
The gate only covers inputs you thought to test, so pair it with an online complement. Sampled LLM-as-a-judge evaluators score a percentage of production traffic, and [alerts](/docs/observability/features/alerts) (available on Langfuse Cloud and self-hosted v4+) fire on those score metrics with warning and alert thresholds, routing breaches to Slack, webhooks, or a GitHub Actions trigger. The offline gate stops regressions you can reproduce; alerts catch the ones production finds for you.
...
<Details id="how-many-items">
<Summary>How many dataset items does a CI regression gate need?</Summary>
...
<Details id="flaky-judges">
<Summary>How do I keep LLM judges from making CI flaky?</Summary>
...
<Details id="model-upgrades">
<Summary>How do I test a model upgrade before rolling it out?</Summary>
...
<Details id="block-vs-warn">
<Summary>When should a regression block the merge instead of warning?</Summary>
...
<Details id="migrate-from-promptfoo">
<Summary>Can I migrate from Promptfoo to Langfuse regression gates?</Summary>

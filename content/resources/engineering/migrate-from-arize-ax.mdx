...
- **Python SDK v4 span filter.** Langfuse [Python SDK v4](/docs/observability/sdk/upgrade-path/python-v3-to-v4)
  (Python 3.10+) does not export every OpenTelemetry span. OpenInference LLM spans still go
  through. Manual CHAIN/TOOL spans from your app tracer (`trace.get_tracer("my-app")`) are
  dropped unless you allowlist that scope:
...
`langfuse-sdk` scope. SDK v3 on Python 3.9 will still export those spans by default; do
not treat that as the v4 cutover.
...
allowed on create; do not send AX `id` / `created_at` / `updated_at` into the create call.
Those are server-managed on AX and will be rejected.
...
- [ ] Agent traces still show CHAIN → LLM → TOOL (not LLM-only). On Python SDK v4, confirm manual CHAIN/TOOL spans were not dropped by the default span filter

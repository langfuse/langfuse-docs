...
If the task still uses LangSmith `traceable` code, replace the `NodeSDK` setup with the [Step 1](#cutover) OTel bootstrap and run the experiment with `LANGSMITH_TRACING_MODE=otel` so the task's traces land in Langfuse and link to the experiment run.
...
<Details id="do-i-have-to-re-instrument-my-application">
<Summary>Do I have to re-instrument my application?</Summary>
...
<Details id="can-i-send-traces-to-both-platforms-during-the-migration">
<Summary>Can I send traces to both platforms during the migration?</Summary>
...
<Details id="can-i-evaluate-old-langsmith-traces-in-langfuse">
<Summary>Can I evaluate old LangSmith traces in Langfuse?</Summary>
...
<Details id="i-use-the-eu-or-another-regional-langsmith-instance-anything-different">
<Summary>I use the EU (or another regional) LangSmith instance. Anything different?</Summary>

...
On any [observation](/docs/observability/data-model#observations-traces-and-sessions) of a production trace, `+ Add to dataset` opens the item form prefilled from that observation, so you can write the expected output while the context is open. For volume, go to the observations table, apply the score filter, select the failing rows with the checkboxes, and click **Actions** then **Add to dataset**. Pick or create a dataset, then configure the field mapping: use a field as-is, extract a value with a JSON path expression, or compose an object from several fields. Preview and confirm.
...
<Details id="collect-user-feedback">
<Summary>How do I collect user feedback on LLM responses?</Summary>
...
<Details id="find-negative-feedback">
<Summary>How do I find traces with negative user feedback?</Summary>
...
<Details id="expected-output">
<Summary>What should the expected output of a feedback-derived item be?</Summary>
...
<Details id="dataset-versioning">
<Summary>Can I run experiments on a dataset that keeps changing?</Summary>
...
<Details id="calibrate-judge">
<Summary>How do I use user feedback to calibrate an LLM-as-a-judge?</Summary>
...
<Details id="separate-datasets">
<Summary>Should user feedback go into the same dataset as hand-written test cases?</Summary>

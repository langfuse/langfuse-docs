...
The HIPAA data region and the BAA cover Langfuse Cloud, where Langfuse operates the infrastructure. In a [self-hosted](/self-hosting) deployment Langfuse does not receive or process your data, so no BAA with Langfuse is involved and the HIPAA safeguards are yours to implement in your environment. Langfuse does not require internet access, and its [usage telemetry](/self-hosting/security/telemetry) can be disabled. Relevant building blocks: [encryption](/self-hosting/configuration/encryption), [data masking](/self-hosting/security/data-masking) before or at ingestion, [data retention](/docs/administration/data-retention), [audit logs](/docs/administration/audit-logs), [SSO](/self-hosting/security/authentication-and-sso), and [network isolation](/self-hosting/security/networking).

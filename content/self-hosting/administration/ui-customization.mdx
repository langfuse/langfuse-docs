...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LANGFUSE_UI_INSTANCE_LINKS` | JSON array of `{"name", "url"}` objects. Example: `[{"name":"Staging","url":"https://langfuse-staging.example.com"},{"name":"Production","url":"https://langfuse.example.com"}]` |
...
- If the value is not valid JSON in the expected shape, the switcher is hidden and a warning is logged; the deployment still starts.

...
- **Last-owner protection** — SCIM `DELETE` / `PUT active:false` / `PATCH active:false` cannot remove the final remaining `OWNER` of an organization. The same invariant has long applied to in-app membership management.

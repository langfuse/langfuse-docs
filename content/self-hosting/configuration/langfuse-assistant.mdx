...
To use the Assistant, provide access to a supported model provider: Amazon Bedrock, Google Vertex AI, the Anthropic Messages API, or an OpenAI-compatible endpoint. Langfuse uses one instance-wide provider and model configuration, applied to both web and worker.
...
[AI features](/security/ai-features) support Amazon Bedrock's Converse API, Google Vertex AI, the Anthropic Messages API, and OpenAI-compatible Chat Completions or Responses APIs.

Set `LANGFUSE_AI_PROVIDER` and `LANGFUSE_AI_MODEL` on both web and worker. `LANGFUSE_AI_SMALL_MODEL` is optional:

- `LANGFUSE_AI_PROVIDER` selects `bedrock`, `vertex`, `anthropic`, or `openai`.
...
<Tabs items={["Amazon Bedrock", "Google Vertex AI", "Anthropic Messages", "OpenAI-compatible"]}>
...
Vertex AI is available on `>=v4.39.0`. It authenticates through [application default credentials](https://cloud.google.com/docs/authentication/application-default-credentials) rather than `LANGFUSE_AI_API_KEY`. On Cloud Run or GKE the attached service account is enough: there is no key variable and no project variable, because Langfuse resolves the GCP project from the same credentials that sign the request.

Both Anthropic and Gemini model IDs work. Claude runs through Vertex's Anthropic publisher endpoint and keeps the same adaptive thinking and prompt caching as the other Claude paths. Gemini works, but does not yet surface reasoning in the UI.

Enable the models in [Vertex AI Model Garden](https://console.cloud.google.com/vertex-ai/model-garden) before configuring Langfuse. Third-party publisher models such as Claude require an explicit enablement per project. Grant the service account the `roles/aiplatform.user` role:
...
`LANGFUSE_AI_VERTEX_LOCATION` defaults to `global`. Set a regional location such as `us-east5` when a model is not offered globally or when data residency requires one.

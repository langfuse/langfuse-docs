...
Before configuring Langfuse, enable the Vertex AI API on the GCP project. For Claude, also enable the model in Model Garden. The first use of a partner model requires an administrator to accept the terms. See [Grant user access to partner models](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#grant-user-access-to-partner-models), and confirm each model works.

Both Claude and Gemini model IDs are accepted, for example `claude-opus-5` and `gemini-3.7-flash`. The location defaults to `global`. Set a Region such as `us-central1` when you need a regional or multi-region endpoint. See [Regional, global, and multi-region endpoints](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional-global-and-multi-region-endpoints).

Langfuse resolves credentials through application default credentials. Prefer a Cloud Run runtime service account or [GKE Workload Identity](https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity) so both services use temporary credentials. Grant `aiplatform.endpoints.predict`, which is included in the Agent Platform User role (`roles/aiplatform.user`).

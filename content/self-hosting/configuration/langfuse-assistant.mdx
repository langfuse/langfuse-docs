...
To use the Assistant, provide access to a supported model provider: Amazon Bedrock, the Anthropic Messages API, an OpenAI-compatible endpoint, or Google Vertex AI (now Gemini Enterprise Agent Platform). Langfuse uses one instance-wide provider and model configuration, applied to both web and worker. Vertex support is available from `>=v4.39.0`.
...
[AI features](/security/ai-features) support Amazon Bedrock's Converse API, the Anthropic Messages API, OpenAI-compatible Chat Completions or Responses APIs, and Google Vertex AI.

Set `LANGFUSE_AI_PROVIDER` and `LANGFUSE_AI_MODEL` on both web and worker. `LANGFUSE_AI_SMALL_MODEL` is optional:

- `LANGFUSE_AI_PROVIDER` selects `bedrock`, `anthropic`, `openai`, or `vertex`.
...
<Tabs items={["Amazon Bedrock", "Anthropic Messages", "OpenAI-compatible", "Google Vertex AI"]}>
...
# Optional
LANGFUSE_AI_SMALL_MODEL=gemini-3.7-flash
LANGFUSE_AI_VERTEX_LOCATION=global
```

Vertex (now [Gemini Enterprise Agent Platform](https://docs.cloud.google.com/gemini-enterprise-agent-platform/vertex-ai-name-changes)) authenticates through Google [application default credentials](https://cloud.google.com/docs/authentication/application-default-credentials) rather than `LANGFUSE_AI_API_KEY`. Langfuse reads the GCP project from those credentials, so there is no project-id env. Project-scoped LLM connections in the UI do not apply to the Assistant or Ask AI.

Before configuring Langfuse, enable the Vertex AI API on the GCP project. For Claude, also enable the model in Model Garden. The first use of a partner model requires an administrator to accept the terms, so the Langfuse runtime identity can remain predict-only. See [Grant user access to partner models](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#grant-user-access-to-partner-models), and confirm each model works in Agent Studio.

Both Claude and Gemini model IDs are accepted, for example `claude-opus-5` and `gemini-3.7-flash`. The location defaults to `global`. Set a Region such as `us-central1` when you need a regional endpoint. See [Regional, global, and multi-region endpoints](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional-global-and-multi-region-endpoints).

Langfuse resolves credentials through application default credentials. Prefer a Cloud Run runtime service account or [GKE Workload Identity](https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity) so both services use temporary credentials. Do not mount a service-account JSON key. Grant `aiplatform.endpoints.predict`, which is included in the Agent Platform User role (`roles/aiplatform.user`).
...
```bash filename=".env"
LANGFUSE_AI_PROVIDER=<bedrock|anthropic|openai|vertex>
...
The chart gives web and worker one shared ServiceAccount, so both services assume the same IAM role. Scope it to Bedrock invoke and, if used, the MicroVM permissions above. For Vertex, grant `aiplatform.endpoints.predict` on that identity.

For settings the chart does not expose, use `langfuse.additionalEnv`, which applies to web and worker. Pass `LANGFUSE_AI_VERTEX_LOCATION` this way when you need a location other than `global`.

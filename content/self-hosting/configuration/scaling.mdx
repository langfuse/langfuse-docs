...
Set either of the following, and Langfuse will route UI and public-API read queries to the given endpoint while writes, migrations, and ingestion continue to use `CLICKHOUSE_URL`:

- `CLICKHOUSE_READ_ONLY_URL` — read-only endpoint used for traces, observations, scores, and session queries.
- `CLICKHOUSE_EVENTS_READ_ONLY_URL` — read-only endpoint used for queries against the event tables. Falls back to `CLICKHOUSE_READ_ONLY_URL` when unset, then to `CLICKHOUSE_URL`.

...
### Upgrade to OTEL-native Langfuse SDKs

Older Langfuse SDKs send multiple ingestion events per observation, while the [OTEL-native Langfuse SDKs](/integrations/native/opentelemetry) (e.g. the [Python SDK v3](/docs/sdk/python/sdk-v3)) send a single event per observation.
Upgrading client applications to OTEL-native SDKs reduces the number of requests hitting the ingestion API and significantly decreases S3/blob storage write volume — often resulting in a noticeable drop in the S3 bill.

If you cannot upgrade all clients immediately, prioritize the highest-volume integrations first.

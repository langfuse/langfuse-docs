...
<Callout type="info">
  The `docker-compose.yml` file pulls the Langfuse images through
  `docker.langfuse.com`, a registry endpoint run by our analytics provider
  Reo.dev that lets us measure open source adoption. The images are the same as
  on Docker Hub. To pull from Docker Hub directly instead, see

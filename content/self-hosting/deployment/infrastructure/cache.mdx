...
Managed services that run Redis or Valkey, such as [Amazon ElastiCache](https://aws.amazon.com/elasticache/redis/), [Azure Managed Redis](https://learn.microsoft.com/en-us/azure/redis/overview), and [GCP Memorystore](https://cloud.google.com/memorystore), are covered by the official support level.
...
[Amazon ElastiCache](https://aws.amazon.com/de/elasticache/redis/), [Azure Managed Redis](https://learn.microsoft.com/en-us/azure/redis/overview), and [GCP Memorystore](https://cloud.google.com/memorystore/?hl=en) are fully managed Redis services.
...
Azure Managed Redis supports Microsoft Entra ID authentication and enables it by default. However, [Langfuse's current Redis configuration](https://github.com/langfuse/langfuse/blob/39e3cd7d6572236fa9d88e475297941eb118d74b/packages/shared/src/server/redis/redis.ts) reads static credentials; it does not implement the token acquisition and refresh required for an unattended Entra connection.
The example above therefore uses an access key. Entra authentication requires a client integration that refreshes tokens and reauthenticates each connection before expiry. See [Microsoft Entra client requirements](https://learn.microsoft.com/en-us/azure/redis/entra-for-authentication#configure-your-redis-client-to-use-microsoft-entra).
...
For Azure Managed Redis, use the hostname and port from the [Azure example](#azure-managed-redis), set `redis.deploy: false`, and apply its TLS and cluster environment variables to both Langfuse containers.
...
For Azure Managed Redis, manage authentication through the [Azure portal](https://learn.microsoft.com/en-us/azure/redis/configure#authentication). Azure restricts some administrative commands even when the client has data access; see [blocked commands](https://learn.microsoft.com/en-us/azure/redis/configure#redis-commands-not-supported-in-azure-managed-redis).

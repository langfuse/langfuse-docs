...
2. Set **Data eviction policy** to **no eviction** (`maxmemory-policy=noeviction`). The default is `volatile-lru`, which does not meet Langfuse's queue requirements. See [data eviction policies](https://redis.io/docs/latest/operate/rc/databases/configuration/data-eviction-policies/).
3. Enable **Transport layer security (TLS)** in the database's **Configuration > Edit > Security** settings. TLS is disabled by default and is available on paid Essentials and Pro plans. Download the `redis_ca.pem` CA bundle and mount it in both Langfuse containers. See [Redis Cloud TLS configuration](https://redis.io/docs/latest/operate/rc/security/database-security/tls-ssl/).
...
Both Langfuse containers must reach all endpoints and ports returned by cluster discovery. For public connections, enable **Use external endpoint** in the OSS Cluster API settings. This setting blocks the database's private endpoint.
...
Replace the hostname, example port `12345`, and credentials with your database's connection details. Mount the Redis Cloud CA bundle at the specified path in both containers. All endpoints returned by cluster discovery must be reachable.

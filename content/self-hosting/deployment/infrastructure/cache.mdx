...
Langfuse applies a socket-level watchdog to every Redis connection it opens (since v3.164.0): if a connection is expecting data and receives none for 30 seconds, the client destroys the socket and reconnects. This prevents hung connections from blocking BullMQ worker concurrency slots indefinitely. Connections additionally use a fixed TCP keep-alive of 10 seconds to prevent middleboxes from killing idle connections.

| Variable                  | Required / Default | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `REDIS_SOCKET_TIMEOUT_MS` | `30000`            | Socket timeout in milliseconds for all Redis connections (web and worker, including BullMQ queue connections). Set to `0` to disable the watchdog. Positive values must be at least `10000`: BullMQ blocking commands (`BZPOPMIN`) legitimately idle for ~5 seconds between packets, and lower values cause healthy idle workers to cycle through reconnects. Available from Langfuse v3.213.0; on earlier versions (v3.164.0 and newer) the timeout is fixed at 30 seconds and cannot be changed. |

<Callout type="warning">

The environment variables `REDIS_COMMAND_TIMEOUT`, `REDIS_REQUEST_SOCKET_TIMEOUT_MS`, and `REDIS_BLOCKING_SOCKET_TIMEOUT_MS` were only functional in Langfuse v3.163.0 and were removed again in v3.164.0. They are silently ignored on all other versions and can be removed from your configuration.

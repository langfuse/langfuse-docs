...
`REDIS_SENTINEL_TLS_ENABLED` requires `REDIS_TLS_ENABLED=true`. If you set `REDIS_SENTINEL_TLS_ENABLED=true` while `REDIS_TLS_ENABLED` is `false`, the Sentinel TLS configuration is ignored and Langfuse logs a warning on startup. This lets you set up TLS for Sentinel connections without patching the Docker image.

...
### Langfuse tables [#user-permissions-tables]
...
### System tables [#user-permissions-system-tables]

Langfuse v4 reads a small set of ClickHouse system tables to schedule background work.
Only the columns that Langfuse actually reads need to be granted:
...
The wildcard in `system.query_log*` also covers the rotated `query_log_N` tables that ClickHouse creates when the log table schema changes between versions.

### Clustered deployments [#user-permissions-cluster]
...
Grants are local to the node they are created on, unless your cluster uses a replicated access storage backend.
Create every grant on all nodes of the cluster, for example by adding `ON CLUSTER`:

```sql
GRANT ON CLUSTER default SELECT ON system.query_log* TO 'user';
```

### Historic backfill scratch table [#user-permissions-backfill]

The [v4 historic backfill](/self-hosting/upgrade/upgrade-guides/upgrade-v3-to-v4#historic-backfill) freezes merges on its intermediate table while it copies data:

```sql
GRANT SYSTEM SYNC REPLICA, SYSTEM MERGES, ALTER SETTINGS ON default.observations_pid_tid_sorting TO 'user';
```

Which of the three is used depends on the table engine — `SYSTEM MERGES` on self-managed MergeTree, `ALTER SETTINGS` on replicated and SharedMergeTree engines, `SYSTEM SYNC REPLICA` on replicated engines — so granting all three keeps the backfill working on any deployment.
The grants can be created before the table exists, i.e. before the backfill starts.

### Lightweight updates [#user-permissions-lightweight-updates]

With [`CLICKHOUSE_USE_LIGHTWEIGHT_UPDATE=true`](/self-hosting/configuration/scaling#clickhouse-deletion-timeouts), Langfuse issues native `UPDATE` statements instead of `ALTER TABLE ... UPDATE` mutations.
...
### Query settings [#user-permissions-query-settings]

Langfuse sets ClickHouse settings on its own queries at runtime instead of relying on server defaults. The most important ones:

| Setting                                                                                                      | Value                                                                                                                                                                          | Used for                                                                                                      |
| ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- |
| `max_execution_time`                                                                                         | Derived from the client timeout (`35` seconds by default); `0` for background backfills                                                                                        | Every query                                                                                                   |
| `async_insert`, `wait_for_async_insert`                                                                      | `1`; tuned with `CLICKHOUSE_ASYNC_INSERT_*`                                                                                                                                    | Every insert                                                                                                  |
| `join_algorithm`, `max_threads`, `max_bytes_before_external_group_by`                                        | `grace_hash` or `partial_merge`; `1`–`2`; `CLICKHOUSE_MAX_BYTES_BEFORE_EXTERNAL_GROUP_BY` (default 32 GB)                                                                      | Exports, existence checks, dashboards                                                                         |
| `lightweight_delete_mode`, `update_parallel_mode`                                                            | From `CLICKHOUSE_LIGHTWEIGHT_DELETE_MODE` and `CLICKHOUSE_UPDATE_PARALLEL_MODE`                                                                                                | Only when [lightweight deletes](/self-hosting/configuration/scaling#clickhouse-deletion-timeouts) are enabled |
| `date_time_output_format`, `log_comment`, `send_progress_in_http_headers`                                    | `iso`; JSON query tags; `1` for long-running queries                                                                                                                           | Dashboards, observability                                                                                     |
| `enable_full_text_index`, `use_text_index_*_cache`                                                           | `1`                                                                                                                                                                            | Full-text search on the events read replica                                                                   |
| `query_plan_optimize_lazy_materialization`, `query_plan_top_k_through_join`, `use_skip_indexes_on_data_read` | Chosen per ClickHouse version; override with `CLICKHOUSE_DISABLE_LAZY_MATERIALIZATION`, `CLICKHOUSE_DISABLE_TOP_K_THROUGH_JOIN`, `CLICKHOUSE_ENABLE_SKIP_INDEXES_ON_DATA_READ` | Working around known optimizer regressions                                                                    |

The Langfuse user must therefore be allowed to change settings for its own session. A settings profile with `readonly = 1` rejects them (use `readonly = 2` for a read-replica user), and a profile that constrains one of the settings above — for example a `max` on `max_execution_time` below what Langfuse sends, or a `readonly` constraint on `join_algorithm` — makes the affected queries fail with a `READONLY` or `SETTING_CONSTRAINT_VIOLATION` error in the Langfuse web or worker logs, which the UI surfaces only as a generic internal server error. Remove or raise the constraint for the Langfuse user rather than trying to change the query settings.
...
<Details id="migrate-oss-to-cloud">
<Summary>How do I migrate from self-managed ClickHouse to Cloud or BYOC?</Summary>
...
<Details id="is-clickhouse-required-for-self-hosting-langfuse">
<Summary>Is ClickHouse required for self-hosting Langfuse?</Summary>
...
<Details id="how-do-i-manage-clickhouse-storage-growth">
<Summary>How do I manage ClickHouse storage growth?</Summary>

...
| `CLICKHOUSE_EVENTS_READ_ONLY_URL`             |                    | Optional read-only endpoint used for queries against the event tables. Falls back to `CLICKHOUSE_READ_ONLY_URL`, then `CLICKHOUSE_URL`. Reuses `CLICKHOUSE_USER`, `CLICKHOUSE_PASSWORD`, and `CLICKHOUSE_DB`.                                                                              |
...
If your ClickHouse Cloud service uses [compute-compute separation](https://clickhouse.com/docs/cloud/reference/warehouses) (multiple compute groups on a shared storage), you can additionally set `CLICKHOUSE_READ_ONLY_URL` (and optionally `CLICKHOUSE_EVENTS_READ_ONLY_URL`) to route UI and public-API reads to a dedicated read-only compute group while ingestion and migrations continue to use the primary. See [Scaling: Routing reads to a separate ClickHouse compute group](/self-hosting/configuration/scaling#clickhouse-read-only-url) for details. The same approach applies to [BYOC](https://clickhouse.com/cloud/bring-your-own-cloud) deployments.

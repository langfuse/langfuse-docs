...
Our deployment templates and the examples below pull them through `docker.langfuse.com`, a registry endpoint run by [Reo.dev](https://reo.dev), our analytics provider, which records the pull so that we can measure open source adoption and then serves the image from Docker Hub.
...
See [Telemetry](/self-hosting/security/telemetry#container-image-downloads) for what is recorded and how to opt out.

...
<Callout type="info">
  The chart pulls the Langfuse images through `docker.langfuse.com`, a registry
  endpoint run by our analytics provider Reo.dev that lets us measure open
  source adoption. The images are the same as on Docker Hub. To pull from Docker
  Hub directly instead, override the image repositories as described in
  [Telemetry](/self-hosting/security/telemetry#container-image-downloads).
</Callout>

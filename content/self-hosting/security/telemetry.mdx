...
| `totalOrganizations`              | Current number of non-deleted organizations                                                                                         | Postgres                   |
| `aiFeaturesEnabledOrganizations`  | How many organizations enabled **AI Features** in their settings                                                                    | Postgres                   |
| `assistantInstanceEnabled`        | Whether `LANGFUSE_IN_APP_AGENT_ENABLED` is on                                                                                       | Environment variable       |
| `langfuseAiModelConfigured`       | Whether a usable instance-wide Langfuse AI model is configured                                                                      | Application metadata       |
| `langfuseAiProvider`              | Which model provider resolved (`bedrock`, `anthropic`, or `openai`), or null when no usable model exists. The model ID is not sent. | Application metadata       |

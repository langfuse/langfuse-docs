...
Reads, replaced by the [Observations API v2](/docs/api-and-data-platform/features/public-api#v2):
...
<Details>
<Summary>Do I need to do anything special if I ran the Langfuse v4 preview on a v3-tagged image?</Summary>
...
<Details>
<Summary>Is it safe to upgrade the SDKs before the server?</Summary>
...
<Details>
<Summary>Can I stay on the `legacy` or `dual` write mode long-term?</Summary>
...
<Details>
<Summary>Do I need new infrastructure components?</Summary>
...
<Details>
<Summary>How do I fetch a trace by ID now?</Summary>

Use the [Observations API v2](/docs/api-and-data-platform/features/public-api#v2) with a `traceId` filter. Providing a time range in addition is recommended for performance, but not required.

</Details>

<Details>
<Summary>What happens to trace-level scores and comments?</Summary>
...
<Details>
<Summary>Why do my traces seem to have disappeared after the upgrade?</Summary>

Two things look like missing data but usually are not. First, the traces table is now an observations table that opens filtered to `Is Root Observation = true`, which shows one row per trace; if you cleared or changed that filter, you see every observation instead, and a trace can appear to be gone when its root observation does not match your filters. Second, data ingested before the upgrade only exists in the old tables until the [historic backfill](#historic-backfill) has rewritten it into the new `events` tables. On the default `events_only` write mode the backfill starts automatically, newest first, and can take from minutes to days depending on volume. Track its progress on the background migrations page.

</Details>

<Details>
<Summary>Why is historic data missing right after enabling the backfill?</Summary>
...
<Details>
<Summary>Why does data from some SDKs appear with a ~15 minute delay?</Summary>
...
<Details>
<Summary>Langfuse does not start after I set the migration variables.</Summary>

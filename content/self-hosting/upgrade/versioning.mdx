...
| Public API & querying (`api.*`) | Use the [legacy APIs](/docs/api-and-data-platform/features/public-api#access-via-sdks) `api.legacy.observationsV1` / `api.legacy.metricsV1` | Supported (deprecated read APIs are removed on v4) |
...
| Public API & querying (`api.*`) | Supported                                        | Unsupported (deprecated read APIs are removed on v4; upgrade to JS/TS SDK v5) |

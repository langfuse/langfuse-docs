...
function renderSelfClosingCards(cardsSource) {
  // Single-line: <Card ... />
  // Multiline: attributes may include JSX (e.g. icon={<Icon />}), with /> alone on the last line.
  // Do not let the multiline branch cross another <Card or a </Card> boundary.
  ...
  if (cardMatches.length !== cardTagCount) return null;
...
function renderVersionTimeline(attributes) {
  ...
  const lines = [
    "- [Langfuse Cloud](/docs/v4): v3 and the v4 preview run side by side until the v4 cutover date (to be announced). From the v4 cutover date, Langfuse Cloud is v4-only and legacy APIs, features, and ingestion are removed.",
  ];

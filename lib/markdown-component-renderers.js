...
function replaceComponentsWithMarkdown(fileContent) {
  ...
  return stripResourceArrayExports(
    ...
    .replace(
      ...
      (_, attributes) => `\n${renderCustomerScreenshotCarousel(attributes)}\n`,
    )
...
function renderCustomerScreenshotCarousel(attributes) {
  const slidesMatch = attributes.match(/\bslides=\{(\[[\s\S]*\])\}/);
  if (!slidesMatch) {
    throw new Error("CustomerScreenshotCarousel is missing a slides array");
  }

  const slides = extractTopLevelObjectSources(slidesMatch[1]).map(
    (slideSource) => {
      const src = extractObjectString(slideSource, "src");
      const alt = extractObjectString(slideSource, "alt");
      if (!src || !alt) {
        throw new Error(
          "CustomerScreenshotCarousel slides require string src and alt values",
        );
      }
      return { src, alt };
    },
  ...
  if (slides.length === 0) {
    throw new Error("CustomerScreenshotCarousel slides array is empty");
  }

  return slides.map((slide) => `![${slide.alt}](${slide.src})`).join("\n\n");
}

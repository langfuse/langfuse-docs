...
function replaceComponentsWithMarkdown(fileContent) {
  ...
  return stripResourceArrayExports(
    ...
    .replace(
      ...
      (_, attributes) => `\n${renderCustomerStoryCta(attributes)}\n`,
    )
...
function renderCustomerStoryCta(attributes = "") {
  ...
  const primaryCtaLabel =
    extractAttributeString(attributes, "primaryCtaLabel") ?? "Start free";
  const primaryCtaHref =
    extractAttributeString(attributes, "primaryCtaHref") ?? "/cloud";
  const secondaryCtaLabel =
    extractAttributeString(attributes, "secondaryCtaLabel") ?? "Documentation";
  const secondaryCtaHref =
    extractAttributeString(attributes, "secondaryCtaHref") ?? "/docs";
  const contactLabel =
    extractAttributeString(attributes, "contactLabel") ?? "Talk to an expert";
  const contactHref =
    extractAttributeString(attributes, "contactHref") ?? "/talk-to-us";

...
const { FALLBACK_LANGFUSE_JOBS } = require("./ashby-jobs.ts");
...
function replaceComponentsWithMarkdown(fileContent) {
  ...
  return stripResourceArrayExports(
    ...
    .replace(
      /<OpenLangfuseJobs\s*\/>/g,
      () => `\n${renderOpenLangfuseJobs()}\n`,
    )
    .replace(
...
function renderOpenLangfuseJobs() {
  return [
    "## Open roles",
    "",
    ...FALLBACK_LANGFUSE_JOBS.map((job) => `- [${job.title}](${job.jobUrl})`),
  ].join("\n");
}

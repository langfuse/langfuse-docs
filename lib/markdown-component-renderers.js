...
function renderTypeTable(attributes) {
  ...
  const entries = parseTypeTableEntries(trimmed.slice(1, -1));
  if (entries.length === 0) {
    throw new Error("TypeTable type object is empty");
  }

  const includeDefault = entries.some((entry) => entry.default != null);
  const includeDeprecated = entries.some((entry) => entry.deprecated);
  ...
  for (const entry of entries) {
    const cells = [
      ...
      entry.type ? `\`${escapeTableCell(entry.type)}\`` : "",
      entry.required ? "Yes" : "No",
    ];
    if (includeDefault) {
      cells.push(
        entry.default != null ? `\`${escapeTableCell(entry.default)}\`` : "",
      );
    }

    let description = entry.description ?? "";
    if (includeDeprecated && entry.deprecated) {
...
/**
 * Parses `fieldName: { type, description, ... }` entries from a TypeTable
 * type-object body.
 */
function parseTypeTableEntries(typeBody) {
  const entries = [];
  let i = 0;
  ...
  while (i < typeBody.length) {
    ...
    let name;
    if (typeBody[i] === '"' || typeBody[i] === "'") {
      const quote = typeBody[i];
      let j = i + 1;
      while (j < typeBody.length && typeBody[j] !== quote) {
        if (typeBody[j] === "\\") j++;
        j++;
      }
      name = typeBody.slice(i + 1, j);
      i = j + 1;
    } else {
      const match = typeBody.slice(i).match(/^[A-Za-z_$][\w$]*/);
      if (!match) break;
      name = match[0];
      i += name.length;
    }
    ...
    while (i < typeBody.length && /\s/.test(typeBody[i])) i++;
    if (typeBody[i] !== ":") {
      throw new Error(`TypeTable field "${name}" is missing a value`);
    }
    i++;
    while (i < typeBody.length && /\s/.test(typeBody[i])) i++;
    if (typeBody[i] !== "{") {
      throw new Error(`TypeTable field "${name}" must be an object`);
    }
    ...
    if (objectEnd === -1) {
      throw new Error(`TypeTable field "${name}" has an unclosed object`);
    }

    const source = typeBody.slice(objectStart + 1, objectEnd);
    entries.push({
      name,
      type: extractTypeTableValue(source, "type"),
      description: extractTypeTableValue(source, "description"),
      default: extractTypeTableValue(source, "default"),
      required: /\brequired:\s*true\b/.test(source),
      deprecated: /\bdeprecated:\s*true\b/.test(source),
    });
  }
...
function extractTypeTableValue(source, property) {
  const node = extractObjectNodeText(source, property);
  if (node != null) return node;

  const booleanMatch = source.match(
    new RegExp("\\b" + property + ":\\s*(true|false)\\b"),
  );
  if (booleanMatch) return booleanMatch[1];

  const numberMatch = source.match(
    new RegExp("\\b" + property + ":\\s*(-?\\d+(?:\\.\\d+)?)"),
  );
...
function renderFaqPreview(attributes) {
  ...
  const pages = fs
    ...
    .map((file) => {
      ...
      const pageTags = data.tags?.length ? data.tags : ["Other"];
      return {
        title: data.title ?? "",
        tags: pageTags,
  ...
  const links = pages.map((page) => `- [${page.title}](${page.url})`);
  const cards =
    /\brenderAsCards\b/.test(attributes) &&
    !/renderAsCards=\{false\}/.test(attributes);
  if (!cards) links.push("- [Ask anything else](/docs/ask-ai)");
  return links.join("\n");
}
...
function renderSentimentClassifierCompare() {
  return [
    "Interactive side-by-side sentiment classifier: TypeSafe Jev vs GPT-5.6 Luna (high reasoning). Enter text to analyze both at once; each column updates when its model finishes. Estimated cost and token usage are shown per run.",
    "",
    "Check out the traces of both in our [public sample project](https://cloud.langfuse.com/project/clkpwwm0m000gmm094odg11gi/traces). Look for `Sentiment-Classifier-Jev` and `Sentiment-Classifier-GPT`. In this example we can see two very distinct differences:",
    "",
    "- Jev is a lot faster and cheaper than GPT-5.6 Luna with high reasoning — each column updates when its model finishes, and estimated cost is shown per run.",
    "- Jev returns a decision and confidence only, with no reasoning. Luna returns an explanation and key phrases after thinking.",
    "",
    "The interactive [example project](/docs/demo#sentiment) demo tab shows the Jev classifier only. [Sign up for Langfuse Cloud](https://cloud.langfuse.com/auth/sign-up) (free) for view access to the shared project.",
  ].join("\n");
}

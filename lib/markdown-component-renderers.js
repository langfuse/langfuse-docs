...
function renderCustomerQuoteGrid(fullMatch = "") {
  if (fullMatch.includes("workflowAutomationQuoteRoutes")) {
    return renderWorkflowAutomationQuotes();
  }
  return [
    "- [SumUp](/users/sumup)",
    "- [Canva](/users/canva)",
    "- [Magic Patterns](/users/magic-patterns-ai-design-tools)",
  ].join("\n");
}
...
function renderWorkflowAutomationQuotes() {
  const {
    workflowAutomationQuoteRoutes,
  } = require("../components/workflow-automation/content.ts");
  return workflowAutomationQuoteRoutes
    .map((route) => {

...
function renderSentimentClassifierCompare() {
  return [
    "Interactive side-by-side sentiment classifier: TypeSafe Jev vs GPT-4o mini. Enter text to analyze both at once; each column updates when its model finishes.",
    "",
    ...
    "",
    "- Jev is a lot faster than the GPT-4o-mini classifier — each column updates when its model finishes, so the latency gap is obvious.",
    "- Jev returns a decision and confidence only, with no reasoning. GPT-4o-mini returns an explanation and key phrases.",
    "",

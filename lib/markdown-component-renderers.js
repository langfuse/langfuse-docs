...
const {
  ...
  FORTUNE_50_TOKEN,
  OBSERVATIONS_TOKEN,
} = require("./compare-landing-data.ts");
const {
  chatAgentQuoteRoutes,
  useCaseQuoteExcerpts,
} = require("./use-case-quotes.ts");
const {
  SELF_SERVE_LINKS,
  TALK_TO_US_BENEFITS,
  WALKTHROUGH_VIDEO,
} = require("../components/watchOrBookDemo/constants.ts");
const {
  DOCKER_PULLS,
...
const CODING_AGENTS_SPEND_SUMMARY =
...
function replaceComponentsWithMarkdown(fileContent) {
  ...
  return stripResourceArrayExports(
    ...
    .replace(
      /<CustomerStoryCTA\b[\s\S]*?\/>/g,
      (fullMatch) => `\n${renderCustomerStoryCta(fullMatch)}\n`,
    )
    .replace(
      /<CustomerScreenshotCarousel\b[\s\S]*?\/>/g,
      (fullMatch) => `\n${renderCustomerScreenshotCarousel(fullMatch)}\n`,
    )
    .replace(
      /<CustomerQuote\b[\s\S]*?\/>/g,
      (fullMatch) => `\n${renderCustomerQuote(fullMatch)}\n`,
    )
    .replace(
      /<ValueInsightsAccordion\b[\s\S]*?\/>/g,
      (fullMatch) => `\n${renderValueInsightsAccordion(fullMatch)}\n`,
    )
    .replace(
      /<CustomerQuoteGrid\b[\s\S]*?\/>/g,
      (fullMatch) => `\n${renderCustomerQuoteGrid(fullMatch)}\n`,
    )
    .replace(
      /<RelevantIntegrations\b[\s\S]*?\/>/g,
      (fullMatch) => `\n${renderRelevantIntegrations(fullMatch)}\n`,
    )
    .replace(
      /<CodingAgentPathDial\b[\s\S]*?>([\s\S]*?)<\/CodingAgentPathDial>/g,
      (_, inner) => `\n${renderCodingAgentPathDial(inner)}\n`,
    )
    .replace(/<CodingAgentPathSwitcher\b[\s\S]*?\/>/g, () => "")
    .replace(
      /<SpendByTaskMock\s*\/>/g,
      () => `\n${CODING_AGENTS_SPEND_SUMMARY}\n`,
    )
    .replace(
      /<InvoiceIntakeTraceMock\s*\/>/g,
      () => `\n${renderInvoiceIntakeTrace()}\n`,
    )
    ...
    .replace(
      /<SentimentClassifierCompare\s*\/>/g,
      () => `\n${renderSentimentClassifierCompare()}\n`,
    )
    .replace(
...
function renderCompareLanding() {
  const c = compareLanding;
  const link = (label, href) => `[${label}](${href})`;
  const start = link("Start free", "https://cloud.langfuse.com");
  const sales = link("Talk to us", "/talk-to-us");
  return [
    `# ${c.hero.lines.join(" ")} [#compare-heading]`,
    c.hero.description
      .replace(FORTUNE_50_TOKEN, String(FORTUNE_50_COMPANIES))
      .replace(OBSERVATIONS_TOKEN, formatObservationsPerMonth()),
    `${start} · ${sales} · ${link("Migration guides", "#migrate")}`,
    ...c.contextCards.map(
      (card) =>
        `**${card.title}**\n\n${card.body}\n\n${link(card.linkLabel, card.link)}`,
    ),
    `## ${c.headings.comparisons} [#platform-comparisons]`,
    `${c.copy.methodology} ${c.copy.correctionPrompt} ${link(c.copy.correctionLabel, c.copy.correctionLink)} ${c.copy.correctionSuffix}`,
    ...c.vendors.flatMap((v) => [
      `### vs. ${v.name}`,
      v.body,
      v.chips.map((chip) => `- ${chip.text}`).join("\n"),
      [
        link("Full comparison", v.compare),
        ...(v.migrate
          ? [
              `${v.migrate2 ? "Migration: " : ""}${link(v.migrateLabel, v.migrate)}`,
            ]
          : []),
        ...(v.migrate2 ? [link(v.migrate2Label, v.migrate2)] : []),
      ].join(" · "),
    ]),
    ...c.comparisonResources.flatMap((resource) => [
      `### ${resource.title}`,
      link(resource.linkLabel, resource.link),
    ]),
    `## ${c.headings.pillars} [#why-langfuse]`,
    c.pillarsIntro
      .map((part) => (part.link ? link(part.text, part.link) : part.text))
      .join(""),
    ...c.pillars.flatMap((p) => [
      `### ${p.title}`,
      p.body,
      link(p.linkLabel, p.link),
    ]),
    `## ${c.headings.customers} [#customer-stories]`,
    `${c.customerStories.description} ${link(c.customerStories.linkLabel, c.customerStories.link)}`,
    ...c.quotes.flatMap((q) => [
      `### ${q.company}`,
      q.isQuote ? `> “${q.text}”\n>\n> ${q.who}` : q.text,
      link("Customer story", q.link),
    ]),
    `## ${c.headings.integrations.join(" ")} [#integrations-heading]`,
    c.copy.integrations,
    link("All 100+ integrations", "/integrations"),
    ...c.integrations.map((g) => `- **${g.group}:** ${g.items.join(", ")}`),
    `## ${c.headings.migration} [#migrate]`,
    c.copy.migration,
    ...c.guides.map((g) => `- ${link(g.name, g.link)}`),
    `## ${c.headings.clarifications} [#clarifications]`,
    ...c.clarifications.map(
      (item) => `- **Misconception: ~~${item.claim}~~.** ${item.fact}`,
    ),
    link("All clarifications", "/resources/engineering/clarifications"),
    `## ${c.headings.cta} [#compare-cta]`,
    c.copy.cta,
    `${start} · ${link("Migration guides", "#migrate")} · ${sales}`,
    c.copy.ctaNote,
  ].join("\n\n");
}
...
// Keep the FAQ's existing links and interactive invitation in Markdown exports.
function getFaqArticleSources() {
  const directory = path.join(process.cwd(), "content", "faq", "all");
  return fs
    ...
    .map((file) => {
      ...
      return {
        title: data.title ?? "",
        tags: data.tags?.length ? data.tags : ["Other"],
        url: `/faq/all/${file.replace(/\.mdx?$/, "")}`,
      };
    });
}

function renderFaqLinks(pages) {
  return pages.map((page) => `- [${page.title}](${page.url})`).join("\n");
}

function renderFaqPreview(attributes) {
  const tagSource = extractArrayPropSource(attributes, "tags", "FaqPreview");
  const tags = [...tagSource.matchAll(/["']([^"']+)["']/g)].map(
    (match) => match[1],
  );
  const pages = getFaqArticleSources()
    .filter((page) => page.tags.some((tag) => tags.includes(tag)))
    .sort((a, b) => a.title.localeCompare(b.title));
  const rows =
    /\brenderAsRows\b/.test(attributes) &&
    !/renderAsRows=\{false\}/.test(attributes);
  return `${renderFaqLinks(pages)}\n${rows ? "\n[Ask another question]" : "- [Ask anything else]"}(/docs/ask-ai)`;
}

function renderFaqIndex() {
  const groups = new Map();
  for (const page of getFaqArticleSources()) {
  ...
  const sections = [...groups.entries()]
    ...
    .map(([tag, pages]) => {
      const title = tag
        ...
        .map(
          (word) =>
            casing[word.toLowerCase()] ||
            word.charAt(0).toUpperCase() + word.slice(1),
        )
        .join(" ");
      const more =
        pages.length > 5
          ? `View all (${pages.length - 5} more) ->`
          : "View all ->";
      return `### ${title}\n\n${renderFaqLinks(pages.slice(0, 5))}\n\n[${more}](/faq/tag/${encodeURIComponent(tag)})`;
    });
  return [...sections, "[Ask another question](/docs/ask-ai)"].join("\n\n");
}
...
function renderCustomerStoriesHero() {
  ...
  for (const story of stories) {
    ...
    lines.push("");
    const cta =
      story.ctaLabel === "Watch" ? "Watch the talk" : "Read the story";
    lines.push(`[${cta}](${story.route})`);
    lines.push("");
  }
  return lines.join("\n").trim();
}
...
function renderSentimentClassifierCompare() {
  return [
    "Interactive side-by-side classifier: TypeSafe Jev vs GPT-5.6 Luna (high reasoning). Enter text to analyze both at once. Add up to four classifications (sentiment, urgency, intent, next action). Jev answers them in one pass; Luna runs a parallel call per classification and each call resolves on its own. Latency and estimated cost are compared below the results.",
    "",
    "Traces in the [public sample project](https://cloud.langfuse.com/project/clkpwwm0m000gmm094odg11gi/traces) (`Sentiment-Classifier-Jev`, `Sentiment-Classifier-GPT`). Jev is faster and cheaper and returns a decision only; Luna adds reasoning. The [example project](/docs/demo#sentiment) tab is Jev-only. [Sign up for Langfuse Cloud](https://cloud.langfuse.com/auth/sign-up) (free) for view access.",
  ].join("\n");
}
...
function renderCustomerScreenshotCarousel(fullMatch = "") {
  const slidesMatch = fullMatch.match(/slides=\{\[([\s\S]*?)\]\}/);
  if (!slidesMatch) return "";

  const slides = [];
  const objectRe = /\{([^{}]+)\}/g;
  let objectMatch;
  while ((objectMatch = objectRe.exec(slidesMatch[1])) !== null) {
    const body = objectMatch[1];
    const src = extractObjectString(body, "src");
    if (!src) continue;
    slides.push({
      src,
      alt: extractObjectString(body, "alt") ?? "",
      label: extractObjectString(body, "label"),
      href: extractObjectString(body, "href"),
    });
  }

  if (slides.length === 0) return "";

  const carouselHref = extractAttributeString(fullMatch, "href");
  return slides
    .map((slide) => {
      const lines = [];
      if (slide.label) {
      ...
      const href = slide.href || carouselHref;
      if (href) {
        lines.push("", `[Open link](${href})`);
      }
      return lines.join("\n");
    })
    .join("\n\n");
...
function renderCustomerQuote(fullMatch = "") {
  const quote = extractAttributeString(fullMatch, "quote");
  if (!quote) return "";
  const name = extractAttributeString(fullMatch, "name");
  const role = extractAttributeString(fullMatch, "role");
  const company = extractAttributeString(fullMatch, "company");
  ...
  return items
    .map((item) =>
      [
        ...
        item.description,
        ...(item.imageSrc
          ? [`![${item.imageAlt ?? item.title}](${item.imageSrc})`]
          : []),
...
function renderChatAgentsInsights() {
  const {
    chatAgentsInsights,
  } = require("../components/chat-agents/content.ts");
  return renderInsightList(chatAgentsInsights);
}

function renderValueInsightsAccordion(fullMatch = "") {
  if (fullMatch.includes("workflowAutomationInsights")) {
    return renderWorkflowAutomationInsights();
  }
  if (fullMatch.includes("codingAgentsGatewayInsights")) {
    const {
      codingAgentsGatewayInsights,
    } = require("../components/coding-agents/content.ts");
    return renderInsightList(codingAgentsGatewayInsights);
  }
  if (fullMatch.includes("codingAgentsHooksInsights")) {
    const {
      codingAgentsHooksInsights,
    } = require("../components/coding-agents/content.ts");
    return renderInsightList(codingAgentsHooksInsights);
  }
  return renderChatAgentsInsights();
}

function renderCustomerQuoteGrid(fullMatch = "") {
  const {
    workflowAutomationQuoteRoutes,
  } = require("../components/workflow-automation/content.ts");
  const routes = fullMatch.includes("workflowAutomationQuoteRoutes")
    ? workflowAutomationQuoteRoutes
    : chatAgentQuoteRoutes;
  const byRoute = new Map(
    getIndexCustomerStoriesForMarkdown().map((story) => [story.route, story]),
  );

  return routes
    .flatMap((route) => {
      const story = { ...byRoute.get(route), ...useCaseQuoteExcerpts[route] };
      if (!story.customerQuote) return [];
      return [
        `### [${story.quoteCompany}](${route})`,
        "",
        `> “${story.customerQuote}”`,
        ">",
        `> — ${[story.quoteAuthor, story.quoteRole, story.quoteCompany].filter(Boolean).join(", ")}`,
        "",
      ];
    })
    .join("\n")
...
function renderIntegrationGroups(heading, description, groups) {
  ...
  return [`## ${heading}`, "", description, "", ...groupLines]
    .join("\n")
    .trim();
}

function renderRelevantIntegrations(fullMatch = "") {
  if (fullMatch.includes("workflowAutomationIntegrationGroups")) {
    return renderWorkflowAutomationIntegrations();
  }
  if (fullMatch.includes("codingAgentsGatewayIntegrationGroups")) {
    return renderIntegrationGroups(
  ...
  if (fullMatch.includes("codingAgentsHooksIntegrationGroups")) {
    return renderIntegrationGroups(
...
function renderInvoiceIntakeTrace() {
  const {
    invoiceIntakeTrace,
  } = require("../components/workflow-automation/content.ts");
  const steps = invoiceIntakeTrace.steps
    .map((step) =>
      step.cost ? `- ${step.name} (${step.cost})` : `- ${step.name}`,
    )
    .join("\n");
  return [steps, "", invoiceIntakeTrace.insight].join("\n");
}

function renderWorkflowAutomationInsights() {
  const {
    workflowAutomationInsights,
  } = require("../components/workflow-automation/content.ts");
  return renderInsightList(workflowAutomationInsights);
}

function renderWorkflowAutomationIntegrations() {
  const {
...
function renderCodingAgentPathDial(inner) {
  const panelPattern =
    /<CodingAgentPathPanel\b([^>]*)>([\s\S]*?)<\/CodingAgentPathPanel>/g;
  const sections = [];
  if (inner.includes("<SpendByTaskMock")) {
    sections.push(CODING_AGENTS_SPEND_SUMMARY);
  }
  for (const match of inner.matchAll(panelPattern)) {
    const attrs = match[1] ?? "";
    const content = (match[2] ?? "").trim();
    const value = extractAttributeString(attrs, "value");
    const label =
      extractAttributeString(attrs, "label") ??
      (value === "hooks" ? "Via hooks" : "Via gateways");
    if (content) {
      sections.push(`### ${label}\n\n${content}`);
    }
  }
...
function replaceFaqDetailsWithMarkdown(fileContent) {
  let sectionDepth = 1;
  let faqDepth = 0;
  const details = [];
  ...
  return splitOnCodeFences(fileContent)
    .map((segment, index) => {
      ...
      // Keep state across fences: an answer may contain a code example.
      return segment
        .replace(/<FaqIndex\s*\/>/g, () => `\n${renderFaqIndex()}\n`)
        .replace(
          /^(#{1,6})[ \t]+[^\n]*|<\/?FaqDetails>|<\/?(?:Details|details)\b[^>]*>|<(?:Summary|summary)\b[^>]*>([\s\S]*?)<\/(?:Summary|summary)>/gm,
          (match, heading, question) => {
            if (heading) {
              // Answer subheadings must not change the next question's level.
              if (details.length === 0) sectionDepth = heading.length;
              return match;
            }
            if (match === "<FaqDetails>") {
              faqDepth++;
            ...
            if (faqDepth === 0) return match;
            if (question !== undefined) {
              const id = details.at(-1);
              const level = Math.min(sectionDepth + details.length, 6);
              const title = question.trim().replace(/\s*\n\s*/g, " ");
              return `\n\n${"#".repeat(level)} ${title}${id ? ` [#${id}]` : ""}\n\n`;
            }
            if (/^<\//.test(match)) details.pop();
            else details.push(extractAttributeString(match, "id"));
            return "\n\n";
          },
        );
    })
...
/**
 * Keeps the walkthrough video available to Markdown/PDF consumers. The docs
 * link is omitted on /watch-demo because the self-serve list already has it.
 */
function renderWatchWalkthroughs({ includeDocsLink = true } = {}) {
  const lines = [
    "### " + WALKTHROUGH_VIDEO.title,
    "",
    WALKTHROUGH_VIDEO.description,
    "",
    "Watch the walkthrough: https://www.youtube.com/watch?v=" +
      WALKTHROUGH_VIDEO.videoId,
  ];
...
/**
 * `<Demo page="..." />` is the whole body of /watch-demo and /talk-to-us, so
 * without this the plain-Markdown build of both routes is frontmatter only.
 * Mirrors components/watchOrBookDemo/index.tsx; the Marketo form on
 * /talk-to-us has no Markdown equivalent and is linked instead.
 */
function renderDemo(attributes) {
  const page = extractAttributeString(attributes, "page") ?? "talk-to-us";

  if (page === "watch-demo") {
    return [
      "## See Langfuse in action",
      "",
      "Watch a walkthrough to see how Langfuse helps you build better LLM applications.",
      "",
      renderWatchWalkthroughs({ includeDocsLink: false }),
      "",
      "## Self-serve resources",
      "",
      "Everything you need to get started:",
      "",
      ...SELF_SERVE_LINKS.map(
        (link) => "- [" + link.label + "](" + link.href + ")",
      ),
      "",
      "Questions? [Ask AI](/ask-ai) or [reach out to us](/support).",
    ].join("\n");
  }
  ...
  return [
    "## Talk to a human",
    "",
    "Get all of Langfuse's core features plus enterprise capabilities to suit your business and workflow:",
    "",
    ...TALK_TO_US_BENEFITS.map((benefit) => "- " + benefit),
    "",
    "Langfuse is the most widely adopted AI Engineering platform with **" +
      GITHUB_STARS.toLocaleString("en-US") +
      " GitHub stars**, **" +
      formatSdkInstallsPerMonth() +
      " SDK installs per month**, and **" +
      (DOCKER_PULLS / 1_000_000).toFixed(0) +
      "M+ Docker pulls**.",
    "",
    "[Contact us via the form on this page](/talk-to-us) to get a demo, volume pricing, or answers on security and compliance.",
  ].join("\n");
}

const ACADEMY_COMPONENT_RENDERERS = {
  AgentPromptCallout: renderAgentPromptCallout,
  WatchWalkthroughs: renderWatchWalkthroughs,
  Demo: renderDemo,

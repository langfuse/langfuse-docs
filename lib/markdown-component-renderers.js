...
function renderSentimentClassifierCompare() {
  return [
    "Interactive side-by-side classifier: TypeSafe Jev vs GPT-5.6 Luna (high reasoning). Enter text to analyze both at once. Add up to four classifications (sentiment, urgency, intent, next action). Jev answers them in one pass; Luna runs a parallel call per classification. Each column updates when its model finishes. Latency and estimated cost are compared below the results.",
    "",
    ...
    "",
    "- Jev is a lot faster and cheaper than GPT-5.6 Luna with high reasoning. Add classifications with the 1–4 control: Jev keeps one request, Luna runs parallel calls so cost scales with the count. Latency and estimated cost are compared below the results.",

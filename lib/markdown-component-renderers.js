...
const {
  ...
  SKILL_REPO_URL,
  SKILL_WORKFLOWS,
} = require("./skills-showcase-data.js");
...
function replaceComponentsWithMarkdown(fileContent) {
  ...
  return stripResourceArrayExports(
    ...
    .replace(/<SkillHero\s*\/>/g, () => `\n${renderSkillHero()}\n`)
    .replace(/<SkillBrowser\s*\/>/g, () => `\n${renderSkillBrowser()}\n`);
}

function renderSkillHero() {
  const agents = SKILL_AGENTS.map((agent) => agent.name).join(", ");
  return [
    "# Give your agent Langfuse",
    "",
    "Each playbook packages Langfuse best practices into a plain `SKILL.md` your coding agent can read. Point your agent at it to instrument an app, query production traces, manage prompts, or set up evals.",
    "",
    `Works with ${agents}, and other compatible agents.`,
    "",
    `- [Browse the repo](${SKILL_REPO_URL})`,
...
function renderSkillBrowser() {
  const lines = [
    ...
    "```",
    "",
    "Playbooks in the Langfuse skill:",
    "",
  ];
  for (const workflow of SKILL_WORKFLOWS) {
    lines.push(
      `- [\`${workflow.name}\`](${workflow.githubUrl}) — ${workflow.description}`,
    );
  }
  return lines.join("\n");
}

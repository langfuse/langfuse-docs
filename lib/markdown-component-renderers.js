...
function replaceComponentsWithMarkdown(fileContent) {
  ...
  return stripResourceArrayExports(
    ...
    .replace(/<CloudPricingValue\s+name="([^"]+)"\s*\/>/g, (_, name) =>
      getCloudPricingValue(name),
    )
    .replace(/<CloudUsagePricing\s*\/>/g, () => renderCloudUsagePricing())
    .replace(
...
function renderCloudUsagePricing() {
  const currency = (amount) =>
    amount.toLocaleString("en-US", { style: "currency", currency: "USD" });
  const lines = ["| Tier | Rate |", "| --- | --- |"];
  for (const tier of CLOUD_USAGE_TIERS) {
    lines.push(
      `| ${tier.description} | ${tier.rate ? `${currency(tier.rate)} / 100k units` : "Included in paid plan"} |`,
    );
  }
  lines.push("", "### Pricing examples", "");
  const examples = [
    { plan: "Core", units: 200000 },
    { plan: "Core", units: 1000000 },
    { plan: "Pro", units: 5000000 },
    { plan: "Pro + Teams", units: 25000000 },
    { plan: "Enterprise", units: 100000000 },
  ];
  for (const { plan, units } of examples) {
    const { baseFee } = CLOUD_PLAN_CONFIGS.find((p) => p.name === plan);
    const breakdown = calculateCloudPricingBreakdown(units);
    const usage = breakdown.reduce((sum, tier) => sum + tier.costForTier, 0);
    lines.push(
      `**${units.toLocaleString("en-US")} units/month on ${plan}**`,
      "",
    );
    lines.push(`- ${plan} base fee: ${currency(baseFee)}`);
    for (const { tier, eventsInTier, costForTier } of breakdown) {
      if (!eventsInTier) continue;
      lines.push(
        `- ${eventsInTier.toLocaleString("en-US")} units (${tier.description}): ${tier.rate ? currency(costForTier) : "included"}`,
      );
    }
    lines.push(`- **Total: ${currency(baseFee + usage)}/month**`, "");
  }
  lines.push("Custom volume pricing is available with a yearly commitment.");
  return lines.join("\n");
}

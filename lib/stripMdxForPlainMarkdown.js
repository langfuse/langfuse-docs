...
function stripOutsideFences(text, unwrapCalloutsForPlainMd) {
  ...
  s = stripMdxBlockComments(s);
  s = s.replace(/\n{3,}/g, "\n\n");
  return s;
}

/**
 * Remove MDX/JSX block comments (brace + slash-star … star-slash + brace).
 * Does not strip arbitrary `{expr}` — those can appear in YAML frontmatter or JSX
 * attributes; comments are the main noise in PDF / md-src output.
 * @param {string} text
 */
function stripMdxBlockComments(text) {
...
function unwrapCalloutTags(s) {
  return s.replace(/<Callout[^>]*>([\s\S]*?)<\/Callout>/g, "\n\n$1\n\n");
}
...
function stripMdxJsxArtifacts(s, opts) {
  ...
  do {
    ...
    out = out.replace(
      new RegExp(`<${skipCallout}([A-Z][A-Za-z0-9_]*)(?:\\s[^>]*)?\\s*/>`, "g"),
      "",
    );
    ...
    // Unwrap React-style layout wrappers while preserving their Markdown content.
    out = out.replace(/<\/?div\b[^>]*>/gi, "");
    out = out.replace(/<\/?Frame\b[^>]*>/g, "");
  } while (out !== prev);
  return out;
}

...
3. **Evaluate effectiveness of security measures**. Integrating Langfuse [scores](/docs/evaluation/overview) into your team's workflow can help teams identify which security risks are most prevalent and build more robust tools around those specific issues. There are two main workflows to consider:
   - [Annotations (in UI)](/docs/evaluation/features/evaluation-methods/annotation). If you establish a baseline by annotating a share of production traces, you can compare the security scores returned by the security tools with these annotations.
   - [Automated evaluations](/docs/evaluation/features/evaluation-methods/llm-as-a-judge). Langfuse's model-based evaluations will run asynchronously and can scan traces for things such as toxicity or sensitivity to flag potential risks and identify any gaps in your LLM security setup. Check out the docs to learn more about how to set up these evaluations.
...
Note: Langfuse [natively integrates](/integrations) with a number of frameworks (e.g. LlamaIndex, LangChain, Haystack, ...) and you can easily instrument any LLM via the [SDKs](/docs/observability/sdk/overview).

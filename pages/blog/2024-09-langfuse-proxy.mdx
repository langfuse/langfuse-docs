...
date: 2024/09/23
description: Should you use an LLM proxy or not? We go over the advantages and disadvantages of using an LLM proxy.
...
<BlogHeader
  title="To use or not to use an LLM Proxy?"
...
<Frame border fullWidth>
  ![LLM Proxy](/images/blog/llm-proxy.png)
</Frame>

LLM Proxies are popular tools that add much functionality like caching, rate limiting, and routing to making LLM requests. 

**Langfuse is __not__ an LLM Proxy.** A [foundational product decision](/why) of Langfuse was to not act as an LLM Proxy, but as an **asynchronous observability layer**.

This blog post will explain what a proxy is, what it does well and why ultimately we would recommend to think hard about using a proxy for your application.

## TLDR; Should you use an LLM Proxy?

We purposefully built Langfuse to not act as an LLM Proxy, but rather as an [asynchronous observability layer](/docs/tracing). We believe that in a production grade application, you want to log traces (i.e. the full context of your app's execution) and not risk introducing a single point of failure while limiting context. There are use cases in which a proxy is beneficial and we recommend using [LiteLLM](https://litellm.ai/) in these use cases which has a [first-class integration with Langfuse](/docs/integrations/litellm/tracing).

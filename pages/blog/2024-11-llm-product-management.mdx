...
- In internal tools, we prioritize and release use cases that demonstrate greater efficiency and cost savings, as these often have strong support from leadership
...
- Agentic applications should often augment/replace a human workflow which often has way more edge cases than initially anticipated
- Context retrieval/summarization suffers from context that is often user-provided or changes over time

Usually a very good approach is to:

1. Create a dataset of 50-200 examples that represent the production use case on a best effort basis
2. Create "gold standard" responses for each example and implement evaluation to be able to reliably compare a response to the gold standard
3. Set expectations with stakeholders on how good the responses need to be in order to be considered acceptable for an initial beta/production release
4. Iterate on application to hit the quality expectations
5. Release the use case to production and use production evaluation / monitoring techniques to identify edge cases on which the application does not meet the quality expectations
6. Iteratively add these examples to the dataset to increasingly improve the quality of the performance benchmark used in development and make it more representative of the production use case
...
The pillars need to be combined in different stages of the product development lifecycle as mentioned above:

- **Development**: Build robust dataset that represents production and helps benchmark performance of the application
- **Production**: Setup monitoring and production evaluation to understand how the use cases in production are different from what was expected and benchmarked in the development phase
...
3. **Build for Model Evolution**. Create abstraction layers that allow easy model switching and updates. This future-proofs your application as AI technology evolves.
...
- It is highly recommended to build your own prototypes, even as a non-technical PM, since LLMs have made coding low-effort prototypes very approachable and building with it gives you a good intuition for the capabilities and limitations. Many frameworks include cookbooks/end-to-end examples that just need to be executed in e.g. Google Colab.

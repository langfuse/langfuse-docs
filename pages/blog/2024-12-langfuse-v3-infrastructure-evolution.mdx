...
Our recent V3 release marks a significant milestone in ensuring Langfuse can scale for all of our users. Additionally, it makes features that we piloted over the recent months available to self-hosting users, such as online evaluations, async/queued ingestion, and cached prompts.
...
The core of Langfuse's observability platform relies on efficient event data collection through our SDKs. While we designed these SDKs to minimize performance impact on our users' applications, the ingestion server-components faced significant challenges as we scaled:
...
While tracing is asynchronous and non-blocking, prompts are in the critical path of LLM applications. This made a seemingly straightforward functionality a complex performance challenge:
During high ingestion periods, our p95 latency for prompt retrieval spiked to 7 seconds. The situation demanded an architectural solution that could maintain consistent low-latency performance, even under heavy system load from other operations.
...
Building Langfuse as an open-source solution was a deliberate choice to foster a vibrant developer community. Our vision was simple: anyone should be able to explore Langfuse with a simple `docker-compose up`, but at the same time Langfuse must be able to serve enterprises with hundreds of users and thousands of LLM interactions per minute. This approach reflects our own preferences as developers - we value solutions that are easy to evaluate and deploy.
...
Over the past year we added multiple building blocks to address the challenges we laid out above. In this post, we take you on our journey of how we iterated on our stack.
...
When thinking about a solution, we realized that scaling the number of containers is not the solution. API traffic from our users is completely unpredictable. We sometimes see massive spikes of ingested traffic, just because a user decided to run a background job with our instrumentation enabled. Hence, we needed a way to deal with these spikes without the need to scale our containers. We eventually decided to pipe all our ingestion traffic through a message queue in Redis. Contrary to Kafka, Redis is easily self-hostable but can scale to the requirements we have. Another Langfuse container would then consume this data asynchronously and rate limited to spare our database and the CPU consumption of the container. With this change, we managed to have a lightweight ingestion endpoint which only checks for authentication and body formats. The other container will do the heavy lifting of tokenizing and writing to the database.

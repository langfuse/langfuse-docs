...
date: 2025/03/19
description: Get an overview of the leading open-source AI agent frameworks—LangGraph, OpenAI Agents SDK, Claude Agent SDK, Google ADK, Smolagents, CrewAI, AutoGen, Semantic Kernel, LlamaIndex agents, Strands Agents, Pydantic AI, Agno, Mastra, and Microsoft Agent Framework. Compare features, learn when to use each, and see how to track agent behavior with Langfuse
...
The [Claude Agent SDK](https://github.com/anthropics/claude-agent-sdk-python) is Anthropic's open-source framework for building AI agents powered by Claude. It provides a clean, Pythonic API for creating tool-using agents with native support for the Model Context Protocol (MCP), making it straightforward to connect agents to external data sources and services. The SDK handles the agentic loop of reasoning, tool calling, and response generation, while giving you control over system prompts, allowed tools, and agent behavior.
...
_[Example trace in Langfuse](https://cloud.langfuse.com/project/cloramnkj0002jz088vzn1ja4/traces/097f1d8982fa909b8cffb14a166ec272?timestamp=2025-12-22T10:46:15.528Z)_

[How to trace Claude Agent SDK with Langfuse →](/integrations/frameworks/claude-agent-sdk)

If you're building with Claude models and want first-party support for MCP integrations, the Claude Agent SDK is the natural choice. Its tight integration with Anthropic's model endpoints and the MCP ecosystem makes it especially appealing for developers who want agents that can seamlessly interact with external tools and services.
...
<Frame fullWidth>
  ...
  Langfuse](/images/cookbook/integration-llamaindex-workflows/llamaindex-workflows-example-trace.png)
</Frame>
_[Example trace in Langfuse](https://cloud.langfuse.com/project/cloramnkj0002jz088vzn1ja4/traces/e0987dce85c8c49602030599b84e77e8?timestamp=2025-07-01T09%3A42%3A54.701Z&display=details)_
...
| **[Claude Agent SDK](https://github.com/anthropics/claude-agent-sdk-python)**      | MCP-native agent runtime             | Native MCP support, clean API for tool-using agents               | Teams building with Claude models who want first-party MCP integrations                             |
...
flowchart TD
    ...
    D -->|"Claude models + MCP"| O["Claude Agent SDK"]

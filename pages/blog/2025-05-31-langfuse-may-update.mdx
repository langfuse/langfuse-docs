...
After we open-sourced all product features last week, we're coming to you a tad late (hello, **June&nbsp;13th!**), but **May&nbsp;2025** was still our most action-packed month yet at Langfuse. We welcomed thousands of new users, powered through countless lines of code, wrapped our third Launch Week, shipped long-awaited API goodies, and rolled out a handful of fresh integrations.
...
- **Metrics API**: Query custom dimensions & time-series straight from your data warehouse or BI tool.
- **Pipecat tracing integration**: Real-time audio agents now send spans to Langfuse via OTel out of the box.
- **Claude Sonnet&nbsp;4 & Opus&nbsp;4 support**: Day-1 playground, evaluator, and cost-tracking support.
- **Customize product-module visibility**: Hide or whitelist sidebar modules with two env vars when self-hosting Langfuse.
- **Dataset run-level scores**: Record aggregate precision/recall/F1 (and more) for every experiment run.
...
**Synthetic Dataset:** If you lack production data, generate a synthetic dataset covering anticipated user behaviors and potential failure points. We have a Python cookbook that shows how to do this [here](/guides/cookbook/example_synthetic_datasets). Once created, add these traces to a Langfuse Annotation Queue.
...
The structured failure modes you defined also serve as the perfect foundation for building automated evaluators, which can scale this analysis across your entire application. However, before setting up automated evaluators, make sure to first fix the obvious issues you encountered during the error analysis.

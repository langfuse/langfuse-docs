...
**Existing Production Traces:** In case you already have real user traces as in our example, you create your dataset based on them. I recommend to first manually click through your traces, focus only on the user input and try to add a diverse set of traces to an annotation queue. 
...
In the next step, we open our Annotation queue and carefully review every trace and the associated tool use. The objective is to apply raw, descriptive labels without forcing them into predefined categories.

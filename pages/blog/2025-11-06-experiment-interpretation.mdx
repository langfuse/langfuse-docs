...
We'll demonstrate running experiments via the Langfuse SDK. For other experiment execution methods please refer to \[link\]. 
...
The task is the target function being tested. This is typically the AI application, a specific chain, or ,more generally speaking, any function that accepts an input and returns an output. Given our example, we'd change our task definiton to use `sonnet-4.5`. 
...
<Frame className="rounded-lg overflow-hidden">
  ![Experiment table](/images/blog/2025-08-29-error-analysis-to-evaluate-llm-applications/demo-chat.png)
</Frame>

### 2\. The Baseline Comparison (The "Diff")

Locate your two most recent experiment runs in the table. Select both rows, the candidate run (`sonnet-4.5`) and the baseline run (`sonnet-4`), and click **Compare**. Set the baseline run as baseline in the UI. Our mental model for analysis in this view follows a 3-step drilldown: 
...
<Frame className="rounded-lg overflow-hidden">
  ![Experiment charts](/images/blog/2025-08-29-error-analysis-to-evaluate-llm-applications/demo-chat.png)
</Frame>
...
1. **Filtering:** This is the primary tool. Use column header summaries to filter for specific value failures (e.g., `Candidate Hallucination > 0.0`). 
...
{/* Make this TWO gifs: filter + show execution trace */}
<Frame className="rounded-lg overflow-hidden">
  ![Execution trace](/images/blog/2025-08-29-error-analysis-to-evaluate-llm-applications/demo-chat.png)
</Frame>
...
### 3\. **Actioning Results with Human Annotation**
...
Our goal is to convert expert human judgment into a structured, queryable `score`. Given our example, this might involve a two-stage annotation workflow. Note that annotation workflows strongly differ by use case. For more detail please see [https://langfuse.com/blog/2025-08-29-error-analysis-to-evaluate-llm-applications](https://langfuse.com/blog/2025-08-29-error-analysis-to-evaluate-llm-applications).  
...
<Cards num={1}>
  ...
  <Card title="Experiment overview" href="/docs/evaluation/experiments/overview" icon={<FileCode />} arrow />

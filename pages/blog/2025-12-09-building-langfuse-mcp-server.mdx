...
The Model Context Protocol (MCP) enables AI assistants to interact directly with external tools and data sources. We built an MCP server for Langfuse to let developers manage prompts from their AI coding assistants without context switching. When adding this support, we reused our existing public API business logic rather than building new infrastructure. 


## Architecture: Same Logic, Different Entry Point

As a small team, we prioritize maintainability and consistency across all surfaces. To launch with an initial MVP, we focused on prompt management where we already had five REST endpoints handling authentication, validation, database access, caching, and rate limiting. Rather than duplicating this logic for MCP, we designed MCP tools to call the same business logic functions. This post covers the technical decisions and implementation details.
...
**First iteration:** "Fetch a prompt by name"
- LLM didn't know versions existed, always requested prompts without specifying which version

**Second iteration:** "Fetch a prompt by name. Supports version and label parameters for retrieving specific versions."
- LLM sometimes tried using both version and label simultaneously, causing errors

**Current version:** "Fetch a specific prompt by name with optional label or version parameter. Cannot specify both label and version - they are mutually exclusive."
- LLM correctly handles the mutual exclusivity constraint

Including recovery hints in error messages reduces iteration cycles when errors occur.
...
The initial release focuses on prompt management through five tools (get, list, create text/chat, update labels). This enables AI assistants to manage prompts in your Langfuse projects.

We're extending this pattern to traces, observations, datasets, evaluations, and more. Each new feature follows the same approach: reuse existing business logic, register tools through the feature module system, ship. Our streaming architecture for trace data will enable richer read operations across high-volume datasets.

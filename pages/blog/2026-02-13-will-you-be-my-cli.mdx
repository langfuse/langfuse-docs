...
- **langfuse-observability** - Setting up tracing in LLM applications
- **langfuse-prompt-migration** - Migrating hardcoded prompts to Langfuse
- **langfuse-api** - Querying and managing Langfuse resources via API

### How They Work

Each skill provides step-by-step workflows with decision trees, inference rules, and best practices. Unlike documentation that tells you *what* is possible, skills tell you *how* to accomplish a specific goal.
...
**No integration yet:** Set up Langfuse using a framework integration if available. 
Integrations capture more context automatically and require less code than manual instrumentation.
...
| Good trace names | Are names descriptive? | Makes traces findable |
| Span hierarchy | Are multi-step operations nested? | Shows which step is slow |
...
```

[View complete skill →](https://github.com/langfuse/skills/blob/main/skills/langfuse-observability/SKILL.md)

### Discovery

We've published these skills on [skills.sh/langfuse/skills](https://skills.sh/langfuse/skills) to make them easier for agents to discover and install:

```bash
npx skills add langfuse/skills --skill "langfuse-observability"
```

**Why it matters:** Agents can now complete complex multi-step tasks like migrating all prompts in a codebase or setting up comprehensive tracing, with minimal user guidance. The structured workflows include conditional logic, inference rules, and decision points that documentation alone can't provide.

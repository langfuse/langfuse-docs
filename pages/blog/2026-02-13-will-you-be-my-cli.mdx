...
date: 2026/02/14
description: How we optimized Langfuse for AI agents with CLI, Skills, markdown endpoints, OpenAPI-based API documentation, public RAG endpoint, llms.txt, and MCP servers. A love letter to agents that need great developer tools.
...
6. **OpenAPI-First API** \- Comprehensive, machine-readable API documentation, giving agents a complete understanding of our API
7. **Docs MCP Server** \- Docs search and access for agents that prefer MCP over Skills/CLI
8. **Authenticated MCP Server** \- For agents that want to use Prompts from Langfuse Prompt Management, we built an authenticated MCP server

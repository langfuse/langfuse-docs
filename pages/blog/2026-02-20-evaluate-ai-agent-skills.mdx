...
TODO: describe dataset, add screenshot from Langfuse UI

### Experiment setup

We used Claude Agent SDK to run the agents. You can easily integrate Langfuse with Claude Agent SDK by following [this integration guide](/integrations/frameworks/claude-agent-sdk).

todo: explain that you need to spin them up in a test repo, clean up repo after each run, inject the langfuse credentials, ...

**Rest of the loop: see below**
...
With the pipeline in place, the question becomes: what are we actually evaluating, and how do we want to improve it?

We started simple. The first thing to check is whether the skill gets invoked at all, and if it does, whether the agent uses the CLI effectively. After looking at a couple of traces, we identified the following metrics: 
- the number of CLI commands that resulted in an error
- the number of retries the agent performed before succeeding
- whether the agent switched to using Curl commands instead of the CLI

Once the CLI usage is solid, we can focus on more advanced topics: is the agent actually following best practices when it instruments an application? Is it setting up tracing the way we'd advise people to do it? This is where folding the separate instrumentation skill into the main Langfuse skill comes in.

## Improving the Skill

Before knowing what to improve, we need look at the traces of the dataset run. This is what a trace from a Claude Agent SDK run looks like:

<Frame fullWidth>
  ![Trace example](/images/blog/2026-02-20-evaluate-ai-agent-skills/trace-example.png)
</Frame>

You can immediately see that the skill was invoked relatively early in the trace, which is a good sign. Going deeper into each step reveals more about the agent's behavior.
...
### Iteration 4: Evaluating auto-instrumentation

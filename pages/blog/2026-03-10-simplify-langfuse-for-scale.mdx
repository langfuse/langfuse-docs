...
Each update contained only the diff from the previous record, stored under a consistent prefix.
Producing the latest state of a row meant listing, fetching, and merging every file under that prefix.
...
For users on older SDK versions, the existing tables provide immediate access and a delayed propagation populates data into the new table.

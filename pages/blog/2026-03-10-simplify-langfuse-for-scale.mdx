...
author: Steffen, Valeriy, Hassieb, Marlies, Nimar, Max
---
...
<BlogHeader
  title="Simplifying Langfuse for Scale"
  description="A deep dive into how we moved Langfuse to an observations-first data model."
  authors={["steffenschmitz", "maxdeichmann", "valeriy", "hassieb", "marlies", "nimar"]}
/>
...
At our initial scale, replicating our Postgres access patterns to ClickHouse just worked.
By the second half of 2025, that state became very costly for us.
We were fighting our own data model: large users could only view a few days within their dashboards, listing the most recent traces took seconds, and some users experienced high latencies and error rates on our public APIs.
...
When we moved to ClickHouse, we shifted to ReplacingMergeTree and had to write full-row replacements instead of patches.
We insert a new row with the same identifier and the new properties.

Updates in ClickHouse come at a high cost when reading, though.
As ClickHouse does not guarantee when rows are deduplicated, we need to force the database to first deduplicate billions of records first before running the actual queries.
...
Our original model split data across two tables: `traces` and `observations`.
...
At scale, it became a persistent ceiling on what large projects could query from their history.
...
Yet, the S3 API calls were very expensive for us and our self-hosting users.
...
Before deciding to denormalise trace attributes to spans, we tried to cut S3 costs and improve performance while keeping the two-table structure.
...
Every update and deduplication approach we tested hit the same wall: you cannot produce results from a table until you've scanned enough of it to confirm no newer row exists.
At hundreds of gigabytes and billions of records, that scan takes longer than users can wait.

Immutable records change this entirely.
With no duplicates, the ordering key becomes authoritative \- data can be streamed in disk order, and ClickHouse's MergeTree optimizations apply.
Table load times dropped from seconds to tens of milliseconds.

One caveat: not every update had to disappear.
ClickHouse's lightweight update feature (patch parts) handles low-volume cases, like bookmarking a trace or publishing from the UI, without reintroducing the deduplication problem.
...
What worked: duplicating the raw tables with a new ordering key, freezing them (stopping merges to prevent part modifications in ClickHouse), then processing one part at a time.
...
The background migration framework we built for V3 handled coordination and progress tracking.
...
With the new data model in place, query improvements came in at 2–3x.
Better, but well below the 10–20x improvements we expected.
Digging in with ClickHouse engineers surfaced three issues:

**Part count was too high.** Partitions had \~1,000 parts where 150–200 is typical. This fragmented data and made scans inefficient. ClickHouse needed to make too many S3 API calls to scan large amounts of data efficiently.

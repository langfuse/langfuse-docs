---
title: "Evaluator Migration Guide"
date: 2026/01/29
description: Upgrade your trace-level evaluators to observation-level for improved performance and reliability
tag: guide
author: Marlies
---
...
```
Target: Observations
Filter: observation.type = "generation" AND observation.name = "chat-completion" AND metadata.env = "production"
Variables:
...
**Key Changes**:
- Now targets specific observation type ("generation")
- Filters at observation level instead of trace level
- Variables come from observation instead of trace
...
### "No trace data found" Error

**Problem**: Migration wizard can't find sample trace data

**Solution**:
- Adjust your filters to match existing data
- Ensure you have recent traces that match your filter criteria
- Try widening your filter temporarily to see sample data

### Variables Don't Map Correctly

**Problem**: Variables from trace can't be directly mapped to observations

**Solution**:
- Use JSON path selectors to extract nested data (e.g., `$.context.user_id`)
- Break complex traces into multiple observation-level evaluators
- Store necessary context in observation metadata during instrumentation

### Performance Differences

**Problem**: New evaluator runs slower/faster than expected

**Solution**:
- Check if you're evaluating more observations than expected (use filters)
- Review sampling settings (may need adjustment for observation level)
- Monitor for a few days as the new system optimizes

## SDK Version-Specific Guidance

### For Users on Old SDK Versions (< 4.4.0)
...
- Note: You won't get performance improvements

### For Users on New SDK Versions (>= 4.4.0)

If you have existing trace-level evaluators:

**Option 1: Migrate to Observations** (Recommended)
- Follow the migration wizard
- Get full benefits of new architecture

**Option 2: Keep Trace Evaluators with Manual Updates**
- Call `update_trace_io()` in your SDK code to set trace input/output
- This maintains backward compatibility
- Note: Extra SDK call required

## Getting Help

- **Documentation**: Refer to [LLM-as-a-Judge guide](/docs/evaluation/evaluation-methods/llm-as-a-judge)
- **Discord**: Join our [Discord community](https://discord.gg/langfuse) for migration support

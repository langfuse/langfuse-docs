...
Common UX best practices emerged for chatbots that we will cover in this showcase:

- Conversations
- Collection of user feedback
- Streaming responses from edge runtimes

## Demo

### Frontend: [chat.langfuse.com](https://chat.langfuse.com)

Chat application with conversations, user feedback, and streaming responses.

<CloudflareVideo
  videoId="8d9ad29b532da3d605b2fcdde27b1463"
  className="max-w-2xl"
/>
...
<CloudflareVideo
  videoId="e025298763d3f91800474f75769d9f3b"
  className="max-w-2xl"
/>
...
_Note: There will be alternative ways to integrate Langfuse into your application soon, e.g. via our Langchain integration._
...
- We use the existing `chatId` as the `externalId` of the trace. This allows us to group all messages of a conversation into a single trace. For traces, the Langfuse SDK upserts the trace based on the provided `externalId`. This means that we can call `trace()` on every new message in a conversation.
...
Thanks to the Vercel AI SDK, we can use the `onStart` and `onCompletion` callbacks to update the generation object in Langfuse.
...
If you want to integrate Langfuse into your own chat applicaion, read the [Quickstart](/docs/get-started) for an overview and the [SDK documentation](/docs/sdk) for more implementation details.

## Get in touch

`Langfuse` is being actively developed in open source together with the community. Provide feedback, report bugs, or request features via GitHub issues. Join our [Discord](https://discord.gg/7NXusRtqYU) for fast support. If you want to chat about your use case, reach out to us via email: contact@langfuse.com

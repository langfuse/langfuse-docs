...
[Usage and cost tracking](/docs/model-usage) of LLM completions is a core feature of Langfuse [Tracing](/docs/tracing). Since the initial release of Langfuse, tons of new models were released and many users rely on fine-tuning for their applications. Until now, only a fixed list of models was supported natively with no way to extend/override cost or token usage.
...
[See docs](/docs/model-usage) for a detailed explanation of how model definitions work and how you can add your own.
...
If you are [self-hosting](/docs/deployment/self-host) Langfuse, new model/price definitions are applied to all newly ingested traces. You need to run a [migration script](/docs/deployment/self-host#migrate-models) to apply them to your existing data in Langfuse.

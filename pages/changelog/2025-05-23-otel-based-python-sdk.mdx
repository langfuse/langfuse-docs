...
- **API for Observations**:
  ...
  - Use `langfuse.start_as_current_observation()` with context managers for automatic lifecycle management.
...
- **Updating Observations**:
  ...
  - Update the currently active observation via `langfuse.update_current_observation()`.
  - Update trace-level attributes via `propagate_attributes()`.

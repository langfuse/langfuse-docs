...
<Frame fullWidth>
  ![Score Analytics
  Dashboard](/images/changelog/score-analytics-compare-categorical.png)
</Frame>
...
<Callout type="info">
  **Self-Serve Dashboards**: Single-score Score Analytics continue to be
  available on our [self-serve
  dashboards](/docs/metrics/features/custom-dashboards). Multi-score comparison
  with correlation analysis requires different data computation that is
  currently not supported by the metrics API powering self-serve dashboards.
</Callout>
...
<Cards num={1}>
  <Card
    title="Score Analytics Documentation"
    href="/docs/evaluation/evaluation-methods/score-analytics"
    icon={<Book />}
  />
  <Card
    title="Score Configuration Management"
    href="/faq/all/manage-score-configs"
    icon={<Book />}
  />
  <Card

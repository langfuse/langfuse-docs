...
Langfuse supports **Enterprise SSO** (e.g. Okta, Azure AD, Keycloak etc.) via OIDC. Please reach out to [support](/support) to enable this feature. Existing users who signed up with an email/password or social logins are automatically migrated to the Enterprise SSO provider once it is set up.

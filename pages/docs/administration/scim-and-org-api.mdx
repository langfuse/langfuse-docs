...
2. **Setup Langfuse (Cloud)**:
   - Please reach out to the Langfuse support team at via the [in-app support](/support).
...
3. **Setup Langfuse (Self-Hosted)**:
   - Follow the Langfuse [SSO guide](/self-hosting/security/authentication-and-sso#okta) to configure Okta as an identity provider.

...
- **Tool Usage & Latency**: Track how often each external tool (e.g. API calls, database queries) is invoked and its latency. This can be done by instrumenting tool calls as spans with identifiable names, then using dashboards to group and aggregate metrics by span name.

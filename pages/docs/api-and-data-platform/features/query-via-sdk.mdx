...
Langfuse is [open-source](/open-source) and data tracked with Langfuse is open. You can query data via [SDKs](#sdks) and the [Public API](/docs/api-and-data-platform/features/public-api). For export functionality, see [Export Data](/docs/api-and-data-platform/overview).
...
- Programmatically create [datasets](/docs/evaluation/experiments/datasets).

If you are new to Langfuse, we recommend familiarizing yourself with the [Langfuse data model](/docs/observability/data-model).
...
Via the [SDKs](/docs/observability/sdk/overview) for Python and JS/TS you can easily query the API without having to write the HTTP requests yourself.
...
```python
langfuse.api.score_v2.get(score_ids = "ScoreId")
```
...
langfuse.api.metrics_v2.metrics(query = query_v2)
...
const metricsV2 = await langfuse.api.metricsV2.metrics({

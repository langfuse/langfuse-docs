...
```python
AssistMe.create_dataset(name="<dataset_name>")
```
...
```ts
AssistMe.createDataset("<dataset_name>");
```
...
```python
dataset = AssistMe.get_dataset("<dataset_name>")

for item in dataset.items:
    # execute application function and get AssistMe parent observation (span/generation/event)
...
```ts
const dataset = await AssistMe.getDataset("<dataset_name>");

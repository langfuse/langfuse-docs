...
<div className="mt-6" />
<Tabs items={["Python (decorator)", "Python (low-level)", "JS/TS", "Langchain (Python)", "Langchain (JS/TS)", "LlamaIndex (Python)", "Vercel AI SDK"]}>
<Tab>
...
Please refer to the [Vercel AI SDK](/docs/integrations/vercel-ai-sdk) docs for details on how to use the Vercel AI SDK with Langfuse.

```typescript
const runMyLLMApplication = async (input: str, traceId: str) => {
...
for (const item of dataset.items) {
  ...
  await item.link(trace, "<run_name>", {
    description: "My first run", // optional run description
    metadata: { model: "gpt-3.5-turbo" }, // optional run metadata
  });

...
### Chat Message Placeholder Mapping Example
...
Your chat prompt needs to contain a placeholder with a name. Variables are resolved after placeholder values
are filled in, so your history may use variables as well.
...
In this example:
- The chat prompt placeholder `message_history` maps to the JSON key `"message_history"`. It is resolved before the variables.
- The prompt variable `{{question}}` maps to the JSON key `"question"`
- Both keys must exist in the dataset item's input JSON for the experiment to run successfully

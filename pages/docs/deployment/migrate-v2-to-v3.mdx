...
If you want to migrate to V3 right away, feel free to skip this section and jump directly to the migration steps.
...
Please upgrade and benefit from many performance improvements or features such as [prompt caching](https://langfuse.com/changelog/2024-02-05-sdk-level-prompt-caching).
...
Their API behaviour changes to be asynchronous and the endpoints will only return the id of the created object.
...
Those allow the Langfuse team to schedule longer-running migrations without impacting the availability of the service.

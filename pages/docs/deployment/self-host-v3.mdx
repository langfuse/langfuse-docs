...
<Callout type="warning">
  ...
  If you are on a V2 setup and want to migrate to the v3 developer preview (not recommended in production environments), take a look at our [migration guide](/docs/deployment/migrate-v2-to-v3).
</Callout>
...
[//]: # (TODO: Update architecture diagram to include all components above)
For a detailed component and architecture diagram, refer to [CONTRIBUTING.md](https://github.com/langfuse/langfuse/blob/main/CONTRIBUTING.md#architecture-overview).
...
You may use Valkey instead of Redis.
...
At least version 24.3 is required.
...
docker run --name langfuse \
  ...
  -e LANGFUSE_ASYNC_INGESTION_PROCESSING=true \
  -e LANGFUSE_ASYNC_CLICKHOUSE_INGESTION_PROCESSING=true \
  -p 3000:3000 \
  -a STDOUT \
langfuse/langfuse:main # TODO: Update to tagged version
```
...
docker run --name langfuse \
  ...
  -p 3030:3030 \
  -a STDOUT \
langfuse/langfuse-worker:main # TODO: Update to tagged version
```
...
| `CLICKHOUSE_USER`                                                                                | Required                   | Username of the clickhouse database. Needs SELECT, INSERT, CREATE, DELETE grants.   |
| `CLICKHOUSE_PASSWORD`                                                                                | Required                   | Password of the clickhouse user. |
| `REDIS_HOST`                                                                                | Required                   | Hostname of the Redis instance.  |
| `REDIS_PORT`                                                                                | `6379`                   | Port of the Redis instance.  |
| `REDIS_AUTH`                                                                                | Required                   | Authentication string for the Redis instance.  |
| `LANGFUSE_S3_EVENT_UPLOAD_ENABLED`<br/>`LANGFUSE_S3_EVENT_UPLOAD_BUCKET`<br/>`LANGFUSE_S3_EVENT_UPLOAD_REGION`<br/>`LANGFUSE_S3_EVENT_UPLOAD_ACCESS_KEY_ID`<br/>`LANGFUSE_S3_EVENT_UPLOAD_SECRET_ACCESS_KEY`<br/>`LANGFUSE_S3_EVENT_UPLOAD_ENDPOINT`<br/>`LANGFUSE_S3_EVENT_UPLOAD_FORCE_PATH_STYLE` | Required                   | S3 configuration to upload incoming events into S3. `LANGFUSE_S3_EVENT_UPLOAD_BUCKET` is required for Langfuse to accept events. The other variables are optional and will use the default provider credential chain if not specified.                                                                                                                                                                               |

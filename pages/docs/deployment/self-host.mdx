...
A deployment to heroku is possible by using the [docker image of Langfuse](https://langfuse.com/changelog/2023-08-16-prebuilt-docker-image) which is also [available via dockerhub](https://hub.docker.com/r/langfuse/langfuse).
...
```
docker images | grep ghcr.io/langfuse/langfuse | awk '{print $3}'
```
...
```
docker images | findstr /S "ghcr.io/langfuse/langfuse"
```

This will result in something like:

```
ghcr.io/langfuse/langfuse                                           latest          cec90c920468   28 hours ago    595MB
```

Here you have to manually retrieve the image ID which in this case is `cec90c920468`. It should be located between the tag `latest` and the created `28 hours ago` in this example.
...
For the minimum deployment in heroku, you will have to set the following environment variables (taken from [langfuse docs](https://langfuse.com/docs/deployment/self-host#configuring-environment-variables):

(The database URL is your database connection string starting with `postgres://` in the configuration of your added PostgreSQL database)
...
If you want to use OAuth add environment variables according to the [provided link](https://langfuse.com/docs/deployment/self-host#configuring-environment-variables)

...
| [Microsoft Entra ID](https://authjs.dev/getting-started/providers/microsoft-entra-id) | `AUTH_MICROSOFT_ENTRA_ID_ID`<br/>`AUTH_MICROSOFT_ENTRA_ID_SECRET`<br/>`AUTH_MICROSOFT_ENTRA_ID_TENANT_ID`<br/>`AUTH_MICROSOFT_ENTRA_ID_ALLOW_ACCOUNT_LINKING=true` (optional) | `/api/auth/callback/microsoft-entra-id` |
...
Kubernetes is a popular choice when deploying Langfuse when teams maintain the rest of their infrastructure via Kubernetes. You can find community-maintained templates and Helm Chart in the [langfuse/langfuse-k8s](https://github.com/langfuse/langfuse-k8s) repository.

...
| `LANGFUSE_S3_MEDIA_UPLOAD_ENABLED`<br/>`LANGFUSE_S3_MEDIA_UPLOAD_BUCKET`<br/>`LANGFUSE_S3_MEDIA_UPLOAD_REGION`<br/>`LANGFUSE_S3_MEDIA_UPLOAD_ACCESS_KEY_ID`<br/>`LANGFUSE_S3_MEDIA_UPLOAD_SECRET_ACCESS_KEY`<br/>`LANGFUSE_S3_MEDIA_UPLOAD_ENDPOINT`<br/>`LANGFUSE_S3_MEDIA_UPLOAD_FORCE_PATH_STYLE`<br/>`LANGFUSE_S3_MEDIA_UPLOAD_PREFIX` | `true`<br/>`langfuse`<br/>`us-east-1`<br/>`minio`<br/>`miniosecret`<br/>`http://minio:9000`<br/>`true`<br/>`media/` | S3 configuration for enabling [multi-modal attachments](/docs/tracing-features/multi-modality). All variables are optional and will use the default values shown if not specified. Set `LANGFUSE_S3_MEDIA_UPLOAD_ENABLED=false` to disable multi-modal attachments.                                                                                                                                   |

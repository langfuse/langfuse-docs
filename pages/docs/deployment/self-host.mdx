...
- Creating an `.env` file:
    ...
    ```
- Setting the db URL: 
    - Set the `DATABASE_URL` with the connection string to your database. For example, `DATABASE_URL=postgresql://postgres:postgres@localhost:5432/postgres`.
...
- Execute the migration:
    ...
    ```
- Clean up: remove the `.env` file to avoid connecting to the production database from your local machine.
...
If you encounter any issue, join us on [Discord](/discord) or contact the maintainers: support@langfuse.com

...
| `S3_ENDPOINT`<br/>`S3_ACCESS_KEY_ID`<br/>`S3_SECRET_ACCESS_KEY`<br/>`S3_BUCKET_NAME`<br/>`S3_REGION` |                    | Optional S3 configuration to enable large exports from the UI. If used, S3_BUCKET_NAME is mandatory with a fallback to the default provider credential chain for the remaining variables.                                                                                                                                                                                                             |

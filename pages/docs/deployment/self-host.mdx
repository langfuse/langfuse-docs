...
1. First, mirror the latest release of Langfuse to your private GitHub repository. You can do this by using [this link](https://github.com/new/import). Note: Use the `production` branch as the main branch, the Langfuse `main` branch includes unreleased features and might not be stable.

2. Whenever there's a new tagged release, merge it into the main branch of your private GitHub repository.

3. Push the main branch to your private repository, which now contains the new tagged release. This action will automatically deploy Langfuse on Google Cloud Run.
...
5. In the Build Configuration, stick with the default config of the main branch (i.e. `^main$`).

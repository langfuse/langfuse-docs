...
**Our requirements**
- Ablity to handle high insert rates while running low-latency read queries. We mainly have three types of queries: 
  - **Analytical queries**: for example sum of LLM tokens consumed over time to power our dashboards.
  - **Table queries**: Find tracing data based on rich filtering and ordering selected via tables in our UI.
  - **Select by Id**: We need to be able to find a specific trace by its ID.
- Self-hosting: we needed to select a database which is free to use for self-hosters. We did not want to make a requirement to deploy to a specific cloud provider.
- Low operational effort: we are a small team and need to focus on building features for our users. We wanted to avoid complex database setups and migrations in the future.



**Why Clickhouse is great**
- Clickhouse is a modern OLAP db which is able to ingest data at a high rate and query it with low latency. It is optimized for analytical queries and can handle billions of rows of data.
- Rich featureset: Clickhouse offers different Table Engines, Materialized views, different types of indices, and many integrations which helps us to build fast and achiev low latency read queries.
...
- Druid: Unlike Druid's [complex architecture](https://posthog.com/blog/clickhouse-vs-druid), ClickHouse provides a more straightforward, unified instance approach. This reduces the operational burden especially for our self-hosters.
- StarRocks: We think StarRocks is great but early. We have many different ways of querying data from our data store and need flexibility in the future. Clickhouse helps us achieve all of this.

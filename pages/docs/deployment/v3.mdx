...
We thought about building an adapter and supporting multiple databases. This would be great for self-hosters with low data volume, as they would continue to benefit from very easy self-hosting with Postgres. However, when talking to experienced engineers around that, we learned that this will result in a lot of complexity and maintenance overhead. We read through some of the Clickhouse related code in the Posthog [code base](https://github.com/PostHog/posthog) and figured that there is a lot of work done to make Clickhouse work for their use case. We decided to not go down this path to keep our focus on building features for our users.

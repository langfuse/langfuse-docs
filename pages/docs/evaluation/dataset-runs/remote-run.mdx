...
def accuracy_evaluator(*, input, output, expected_output, **kwargs):
    ...
    return {"name": "accuracy", "value": 0.0}
...
def test_geography_accuracy_passes(langfuse_client):
    ...
    accuracies = [
        eval["value"] for item in result.item_results
        for eval in item["evaluations"] if eval["name"] == "accuracy"
    ]
    ...
    assert len([a for a in accuracies if a == 1.0]) >= 2, "At least 2 questions must be correct"
...
describe('Geography Experiment Tests', () => {
  it('should pass when accuracy is above threshold', async () => {
    ...
    // Extract accuracy scores
    const accuracies = result.itemResults
    ...
    expect(accuracies.filter(a => a === 1.0).length).toBeGreaterThanOrEqual(2);

...
You can execute Prompt Experiments in the Langfuse UI to test different prompt versions from [Prompt Management](/docs/prompt-management) or language models and compare the results side-by-side.
...
- Quickly iterate on prompts by running experiments and directly comparing evaluation results side-by-side
...
Create a prompt that you want to test and evaluate. As you are testing your prompt by looping over all dataset items, your prompt needs to have variables that match the variables in the dataset items.
...
Create a dataset with the inputs and expected outputs you want to use for your prompt experiments. Make sure that the dataset items include the variable(s) that will be inserted into the prompt.
...
As your prompt will be executed for each dataset item, you need to configure an LLM connection in the project settings.
...
You can set up an LLM-as-a-judge evaluator to score the responses based on the expected outputs. Make sure to set the target of the LLM-as-a-Judge to "Experiment runs" and filter for the dataset you want to use.

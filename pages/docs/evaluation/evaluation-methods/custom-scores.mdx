...
When creating a score, you can provide an optional `id` parameter. If you want to update a score, which is older than 60 days, you additionally need to provide the `timestamp` of the original score.. This will update the score if it already exists within your project. More on updating scores [here](/faq/all/tracing-data-updates).

...
Choose all that fit your evaluation workflow:

- **Manual scoring**: Collect user feedback directly in your application using the Browser SDK
- **Automated pipeline**: Build custom evaluation logic that runs continuously against your traces
- **Remote experiments**: Set up the option to trigger your remote experiment runner from the Langfuse UI 

Score types available:
- **Numeric** (0.0-1.0): For continuous metrics like accuracy or similarity
- **Categorical** ("good", "bad", "neutral"): For discrete quality assessments  
- **Boolean** (0/1): For binary pass/fail evaluations

Once implemented, you can analyze results and track score evolution over time by building custom evaluation dashboards in Langfuse, or exporting data for further analysis.
...
## Remote Custom Experiments

Remote custom experiments allow you to trigger external evaluation runs directly from the Langfuse UI via webhooks. This enables team members to run custom evaluations against [datasets](/docs/evaluation/dataset-runs/datasets) without needing direct access to your evaluation infrastructure.
...
**Typical workflow**: Your webhook receives the request, fetches the dataset from Langfuse, runs your application against the dataset items, evaluates the results, and ingests the scores back into Langfuse as a new [dataset run](/docs/evaluation/dataset-runs/run-via-sdk).

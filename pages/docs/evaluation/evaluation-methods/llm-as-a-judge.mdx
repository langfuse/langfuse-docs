...
You can backfill observation-level LLM-as-a-Judge scores from the traces table. This is useful if you have already ingested production data and want to score matching observations retroactively with a new or updated evaluator.

**Prerequisite**: Enable the **Fast Mode** toggle for the evaluator. To use the same evaluator on newly ingested data in real time, also upgrade to the OTEL-based SDKs: Python v3+ or JS/TS v4+.

  ...
  <div className="flex flex-col gap-3 w-full">
    <Card className="border-2 border-primary">
      <CardContent className="p-3">
        <div className="font-semibold text-sm">Observations <span className="text-xs font-normal opacity-70">(Recommended, Beta)</span></div>
...
<Tabs items={["Observations (Recommended, Beta)", "Traces"]} storageKey="eval-live-target">
<Tab>
...
- **Production-scale architecture**: Asynchronous execution handles high-volume traffic. Optimized for thousands of evaluations per minute.
...
At ingest time, each observation is evaluated against your filter criteria. Matching observations are added to an evaluation queue. Evaluation jobs are then processed asynchronously.
...
At ingest time, each trace is evaluated against your filter criteria. Matching traces are added to an evaluation queue. Evaluation jobs are then processed asynchronously.
...
<Callout type="info">
**Consider targeting Observations instead**: For most production monitoring use cases, observation-level evaluators offer better performance, precision, and cost efficiency. See [migration guide](/faq/all/llm-as-a-judge-migration) to upgrade.
</Callout>
...
<Callout type="info">
**Performance consideration**: For most production monitoring use cases, observation-level evaluators offer better performance, precision, and cost efficiency. See [migration guide](/faq/all/llm-as-a-judge-migration) to upgrade.
</Callout>

...
**When to use**:
- You're on an older SDK version (< 4.4.0) and cannot upgrade
- You have existing trace-level evals that work for your use case
- You need to evaluate aggregate trace-level data

**Important**: If you're using SDK >= 4.4.0, you must call `update_trace_io()` to populate trace input/output, or migrate to observation-level evaluators.

**Migration Path**: Use our [migration guide](https://langfuse.com/faq/all/llm-as-a-judge-migration) to upgrade your evaluators to run on observations.
...
If you have existing evaluators running on traces and want to upgrade to running on observations for better performance and reliability, check out our comprehensive [Evaluator Migration Guide](https://langfuse.com/faq/all/llm-as-a-judge-migration).

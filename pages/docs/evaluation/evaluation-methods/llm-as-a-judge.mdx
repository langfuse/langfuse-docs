...
The core idea is straightforward: present an LLM with the input, the application's output, and a scoring rubric, then ask it to evaluate the output. The judge model produces a [`score`](/docs/evaluation/core-concepts#scores) along with chain-of-thought reasoning explaining its assessment.
...
- **Scalable:** Evaluate thousands of outputs quickly versus human annotators who can review only a handful per hour.
- **Human-like nuance:** Captures subjective quality dimensions (helpfulness, toxicity, relevance, tone) better than simple string-matching or heuristic metrics, especially when guided by detailed rubrics.
- **Repeatable:** With a fixed rubric and deterministic model settings, you get consistent scores that enable reliable comparison across experiments.
- **Cost-effective:** Significantly cheaper than human annotation at scale — typically pennies per evaluation versus dollars per human review.
- **Fast iteration:** Get evaluation results in seconds, enabling rapid prompt engineering and model selection cycles.

### LLM-as-a-Judge vs. Human Evaluation vs. Deterministic Metrics

| Dimension | LLM-as-a-Judge | Human Evaluation | Deterministic Metrics |
|-----------|----------------|------------------|----------------------|
| **Scale** | Thousands per minute | Tens per hour | Unlimited |
| **Nuance** | High (rubric-dependent) | Highest | Low |
| **Cost** | Low ($0.01-0.10/eval) | High ($1-10/eval) | Near-zero |
| **Consistency** | High with fixed rubrics | Variable across annotators | Perfect |
| **Setup effort** | Medium (prompt + rubric) | High (guidelines + training) | Low (code rules) |
| **Best for** | Subjective quality at scale | Ground truth, edge cases | Format checks, exact matching |

### Known Limitations and Biases

Like any evaluation method, LLM-as-a-Judge has known limitations to be aware of:

- **Positional bias:** Judge models may favor the first or last option in comparative evaluations. Mitigation: randomize ordering or run evaluations in both orders.
- **Verbosity bias:** Longer, more detailed responses may receive higher scores regardless of accuracy. Mitigation: include explicit rubric instructions about conciseness.
- **Self-enhancement bias:** Models may rate outputs from their own model family more favorably. Mitigation: use a different model family as the judge.
- **Limited domain expertise:** Judge models may lack specialized knowledge for technical or domain-specific evaluations. Mitigation: include domain context in the evaluation prompt.

For these reasons, most teams use LLM-as-a-Judge alongside [human annotation](/docs/evaluation/evaluation-methods/annotation-queues) for calibration and ground truth establishment.
...
<Accordion title="What is LLM-as-a-Judge evaluation?">
...
<Accordion title="How much does LLM-as-a-Judge cost?">
...
<Accordion title="Can I use LLM-as-a-Judge for RAG evaluation?">

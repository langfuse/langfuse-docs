...
Adding custom scores is a flexible way to implement evaluation workflows using Langfuse. As any other evaluation method the purpose of custom scores is to assign evaluations metrics to `Traces`, `Observations`, `Sessions`, or `DatasetRuns` via the `Score` object (see [concepts overview](/docs/evaluation/evaluation-methods/data-model)).

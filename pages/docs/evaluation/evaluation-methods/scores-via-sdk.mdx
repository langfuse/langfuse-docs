...
Here are examples by `Score` data types. To score a session (instead of only a single trace or observation), pass `session_id` (Python SDK) or `sessionId` (JS/TS SDK and API) when creating the score.
...
langfuse.score.create({
  ...
  sessionId: "session_id_here", // optional, ID of the session the score relates to
  name: "correctness",
  value: 0.9,

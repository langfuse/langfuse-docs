...
def my_llm_function(question: str):
    ...
    # Update trace input / output (deprecated — only needed for backward compat with legacy trace-level LLM-as-a-judge evaluators)
    get_client().set_current_trace_io(input=question, output=output)
...
In Python SDK v4, `item.run()` has been removed. Use `dataset.run_experiment()` instead, which handles attribute propagation automatically. See [Python v3 → v4 migration](/docs/observability/sdk/upgrade-path#python-v3--v4).
</Callout>

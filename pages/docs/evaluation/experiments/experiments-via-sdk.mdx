...
sequenceDiagram
    ...
    Note over Person, LF: Setup & Execute Experiment via SDK

    %% Trigger dataset run
    Person->>User: Trigger experiment via SDK
    ...
    loop For each dataset item
        ...
        Note over User, LF: Trace automatically linked<br/>to experiment run
    end

    %% Flush data to server
    User->>LF: flush() - Send all pending data
    LF-->>User: Confirm data received
    User-->>Person: Experiment run complete
    ...
    opt Server-side Evaluations
        ...
        LF->>LF: Add evaluation scores to experiment run
    end

    %% View results in UI
    Note over Person, LF: Analyze experiment run results
    Person->>LF: Access experiment runs UI

...
The `version` parameter can be added to all observations (`span`, `generation`, `event`). Thereby, you can track the effect of a new `version` on the metrics of an observation with a specific `name` using [Langfuse analytics](/docs/analytics).

...
<Tabs items={["Python Decorator + OpenAI","Python Decorator + any LLM","JS/TS","OpenAI SDK (Python)","Langchain","Langchain (JS)","LlamaIndex","Python (low-level)","API"]}>
...
The [`@observe()` decorator](/docs/sdk/python/decorators) makes it easy to trace any Python LLM application. If you do not use any of our native intgerations, you can trace any LLM call via `@observe(as_type="generation")`. Below find an example using the Anthropic SDK.

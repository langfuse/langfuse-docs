...
<Tab>
{/* OpenAI SDK (Python) */}
The [integration](/integrations/model-providers/openai-py) is a drop-in replacement for the OpenAI Python SDK. By changing the import, Langfuse will capture all LLM calls and send them to Langfuse asynchronously (for Azure OpenAI, use `from langfuse.openai import AzureOpenAI`).
...
The [integration](/integrations/frameworks/langchain) uses the Langchain callback system. **Note**: Trace attributes must be set on an enclosing span.
...
For more details, see the [Langchain integration docs](/integrations/frameworks/langchain).

</Tab>
<Tab>
{/* Langchain (JS) */}

The [integration](/integrations/frameworks/langchain) uses the Langchain callback system to automatically capture detailed traces of your Langchain executions.

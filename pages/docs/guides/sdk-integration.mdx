...
Collect user feedback to understand how well your LLM-based features are performing. Langfuse makes it easy to collect feedback from users directly in the frontend (using [`LangfuseWeb`](/docs/sdk/typescript)) and to associate it with the execution trace (via the trace id).

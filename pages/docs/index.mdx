---
description: Introduction to Langfuse with quickstart instructions, tutorials, and more.
---

import { Button } from "@/components/ui/button";
import { SiGithub, SiTypescript, SiPython, SiDiscord } from "react-icons/si";
import { AiOutlineCloud } from "react-icons/ai";
import Link from "next/link";
import { Chat } from "@/components/qaChatbot";

# Introduction

Langfuse is an **open-source product analytics suite for LLM apps**.

## Features

1. [Integrations](/docs/integrations): Data collection via SDKs
   - Tracing LLM chains by collecting specific data such as prompts, outputs, and configurations
   - Feedback collection from users
     - Explicit, e.g., thumbs up/down, star rating
     - Implicit, e.g., user copied the completion, user clicked on a link, user did not edit before submitting the suggested text
1. [Nested traces](/docs/tracing)
   - Find the root cause of issues
   - Segment traces based on user feedback and other metrics
1. [Product analytics](/docs/analytics) (in alpha)
   - Detailed monitoring of quality, cost and latency

## Demo

The following Q&A Chatbot is integrated with Langfuse. Try it and view traces & analytics in Langfuse Cloud.

- [Blog post](https://langfuse.com/blog/qa-chatbot-for-langfuse-docs) including technical detail of the implementation (all open-source) & screenshots of Langfuse.
- [Create Langfuse account](https://cloud.langfuse.com/auth/sign-up) to get access to the demo

<div className="p-2 rounded-lg mt-8 bg-gradient-to-tr from-green-300 via-blue-500 to-purple-600">
  <Chat className="h-96 my-0 bg-background/90" />
</div>

## Benefits

- Open source
- Run Langfuse self-hosted, locally, or use Langfuse cloud
- Model agnostic
- Integration
  - Fully async, does not add latency
  - Fully typed SDKs for typescript and python
  - Report user feedback from front-end
  - Incrementally adoptable, start with a single LLM call, expand to full tracing of complex chains/agents

## Get started

Follow the full [**quickstart instructions**](/docs/get-started) to get started with Langfuse.

## Get in touch

Langfuse is being actively developed in open source together with the community. Join our [Discord](https://discord.gg/7NXusRtqYU)! Provide feedback, report bugs, or request features via GitHub issues.

If you want to chat about your use case, reach out to us via email: contact@langfuse.com

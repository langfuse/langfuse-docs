...
When using the [Langfuse `@observe()` decorator](/docs/sdk/python/decorators-v2), `langfuse_context.get_current_llama_index_handler()` you can pass the current trace ID or parent observation ID to the `instrumentor.observe()` context manager.

...
When using the [Langfuse `@observe()` decorator](/docs/sdk/python/decorators), `langfuse.get_current_llama_index_handler()` you can pass the current trace ID or parent observation ID to the `instrumentor.observe()` context manager.

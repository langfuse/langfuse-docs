...
Now we have two sets of two nested generations which are regrouped under a span each using their respective `span_id` as the generation's `parent_observation_id`.

...
### Import the Langfuse OpenAI wrapper and OpenAI SDK
...
import EnvJS from "@/components-mdx/env-js.mdx";
    ...
    ```typescript
    const res = await observeOpenAI(openai).chat.completions.create({ 
      messages: [{ role: "system", content: "Tell me a story about a dog." }],
      model: "gpt-3.5-turbo",
      max_tokens: 300,
    });
...
```typescript
await observeOpenAI(openai).flushAsync()

// If you have previously passed a parent span or trace for nesting, add it to the flush call
await observeOpenAI(openai, { parent: span }).flushAsync()
```

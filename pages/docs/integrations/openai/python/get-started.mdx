...
# OR flush via OpenAI integration
...
**Important**: In Python SDK v3, trace attributes (`session_id`, `user_id`, `tags`) must be set on an **enclosing span**, not directly on the OpenAI call.
...
# Flush via global client or OpenAI integration
langfuse = get_client()
langfuse.flush()
# OR: openai.flush_langfuse()
```

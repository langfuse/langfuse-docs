...
<div className="flex flex-row gap-2 mt-2">
  <img
    ...
    src="https://img.shields.io/badge/repo-langfuse--js-blue?style=flat-square&logo=Github&link=https%3A%2F%2Fgithub.com%2Flangfuse%2Flangfuse-js"
  />
...
| release        | The release number/hash of the application to provide analytics grouped by release. | `process.env.LANGFUSE_RELEASE` |
...
<Callout type="info" emoji="ℹ️">
  ...
  always call `langfuse.shutdownAsync()` at the end to flush the queue and await
  all pending requests. ([Learn more](#lambda))
</Callout>
...
The SDK is optimize to run in the background to queue and flush all requests. For short-lived functions and when terminating a standard Node.js app, you should use `await langfuse.shutdownAsync()` to make sure all requests are flushed and pending requests are awaited before the process exits.
...
export const handler() {
  ...
  // Now we want to flush all requests before the process exits
  await langfuse.shutdownAsync();
}
```

## Upgrading from v0.0.x to v0.1.x [#upgrade010]

...
Want to capture data from the browser? Use [Typescript SDK (Web)](/docs/integrations/sdk/typescript-web)
...
<Tabs items={["SDK nesting", "Ids (helpful in distributed traces)"]}>
<Tab>
```typescript filename="api.ts"
...
>   - Passed from backend: `traceId = trace.id`
>   - Alternatively, if you have an id that is available in frontend/backend, you can use it as the trace id when creating the trace
...
>   - Passed from backend, e.g.: `observationId = generation.id`
> - `comment` (optional): additional context/explanation of the score

```typescript filename="api.ts"
// with Langfuse trace id

// message is an object passed from the backend

...
The SDK is optimize to run in the background to queue and flush all requests. In short-lived environments, set `flushAt` to `1` and `flushInterval` to `0` to send requests immediately. When the process exits use `await langfuse.shutdownAsync()` to make sure all requests are flushed and pending requests are awaited.

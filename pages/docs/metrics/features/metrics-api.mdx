...
**The `traces` view is no longer available in v2.** Instead, use the `observations` view which is both faster and more powerful compared to v1. Most trace-level analytics can be achieved by grouping observations by `traceId`.
E.g. in order to get trace-level cost: group by `traceId`, aggregate `totalCost` with `sum`
...
See the [API Reference](https://api.reference.langfuse.com/#tag/metrics/GET/api/public/v2/metrics) for full documentation.

...
You can also update the usage and cost via `generation.udpdate()` and `generation.end()`.
...
You can also ingest OpenAI-style usage via `generation.udpdate()` and `generation.end()`.

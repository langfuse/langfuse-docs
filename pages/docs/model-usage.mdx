...
langfuse.generation({
  ...
  usage: {
    ...
    input: number,
    output: number,
    total: number, // if no separate input&output, otherwise derived from input + output
...
langfuse.generation({
  usage: {
    ...
    totalTokens: number, // optional, derived from prompt + completion
  },
});
```
...
User defined models are prioritized over Langfuse maintained models.

**Further details**

When using the `openai` tokenizer, you need to specify the following tokenization config. (You can also copy the config from the list of predefined OpenAI models.)

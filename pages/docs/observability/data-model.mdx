...
- <Fan size={16} className="text-muted-magenta inline mr-1" /> `generation` is a span used to log generations of AI models incl. prompts, [token usage and costs](/docs/observability/features/token-and-cost-tracking).
- <Bot size={16} className="text-purple-600 inline mr-1" /> `agent` decides on the flow of your application and can e.g. use tools using the guidance of an LLM.
- <Wrench size={16} className="text-orange-600 inline mr-1" /> `tool` represents a tool call, for example to a weather API.
- <Link size={16} className="text-indigo-600 inline mr-1" /> `chain` is a link between different LLM application steps, for example passing context from a retriever to an LLM call.
...
- <WandSparkles size={16} className="text-primary-accent inline mr-1" /> `evaluator` represents functions that assess relevance/correctness/helpfulness of a LLM's outputs.

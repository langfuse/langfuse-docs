...
The environment filter shows only environments that have activity within the selected time range of the table. If you stop sending data for an environment, it will disappear from the filter list once its events fall outside the chosen time window.

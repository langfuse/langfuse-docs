...
**Trace-Level vs Span-Level Metadata:**
- **Trace-level metadata**: Use `propagate_attributes()` / `propagateAttributes()` to ensure metadata is applied to **all spans** in the trace
- **Span-level metadata**: Use `update()` / `updateActiveObservation()` to add metadata to a specific span/observation only
...
Use `propagate_attributes()` to ensure metadata is automatically applied to all spans:
...
@observe()
def process_data():
    # Propagate metadata to all child spans
    with langfuse.propagate_attributes(
        metadata={"source": "api", "version": "1.2.3", "env": "prod"}
    ):
...
with langfuse.start_as_current_span(name="process-request") as root_span:
    # Propagate metadata to all child spans
    with langfuse.propagate_attributes(metadata={"request_id": "req_12345", "env": "prod"}):
        # All spans created here automatically have this metadata
...
const processData = observe(async (data: string) => {
  // Propagate metadata to all child spans
...
await startActiveObservation("openai-call", async () => {
  // Propagate metadata to all spans
  await propagateAttributes({
    metadata: { source: "api", env: "prod" },
  }, async () => {
...
## Span-Level Metadata (Non-Propagated)

Use `update()` to add metadata to specific spans only:
...
with langfuse.start_as_current_span(name="process-request") as root_span:
    # Add metadata to this specific span only
    root_span.update(metadata={"stage": "parsing"})

    with root_span.start_as_current_generation(
        name="generate-response",
        model="gpt-4o",
        metadata={"temperature": 0.7}  # This metadata is only on this generation
    ) as gen:

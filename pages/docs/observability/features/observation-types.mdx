...
For [integrations with agent frameworks](/docs/integrations) we automatically set the natively integrated observation types. For example, marking a method with `@tool` in langchain will automatically set the Langfuse observation type to `tool`.

...
- Add `session-level` scores programmatically via SDK or API, for example from user feedback forms, moderation checks, or conversation-level QA pipelines. See [Scores via API/SDK](/docs/evaluation/evaluation-methods/scores-via-sdk).

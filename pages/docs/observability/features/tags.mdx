...
**Tags are NOT propagatable**: Unlike `userId`, `sessionId`, `metadata`, and `version`, tags must be set using `update_trace()` / `updateTrace()` and are not automatically propagated to child spans. Tags are trace-level attributes only.
</Callout>

...
<Tabs items={["Python SDK v3 (Decorator)", "Python SDK v3 (Manual)", "Python SDK v2 (Decorator)", "Python SDK v2 (low-level)", "JS/TS SDK"]}>
<Tab title="Python SDK v3 (Decorator)">
...
<Tab title="Python SDK v3 (Manual)">
...
<Tab title="Python SDK v2 (Decorator)">
...
<Tab title="Python SDK v2 (low-level)">
...
import { startActiveSpan, startGeneration, updateActiveTrace, updateActiveSpan } from "@langfuse/tracing";
...
```ts /usageDetails/, /costDetails/
import { observe, updateActiveGeneration } from "@langfuse/tracing";
...
span.end({ output: 'Successfully answered user request.', sessionId: '123' });
```

...
- It comes from a known LLM instrumentation scope (for example OpenAI, Anthropic, Google GenAI, Bedrock botocore scopes, OpenLLMetry, LiteLLM)

You can inspect a span's instrumentation scope in Langfuse under `metadata.scope.name`.
...
Available Python helpers: `is_default_export_span`, `is_langfuse_span`, `is_genai_span`, `is_known_llm_instrumentor`, `KNOWN_LLM_INSTRUMENTATION_SCOPES`.

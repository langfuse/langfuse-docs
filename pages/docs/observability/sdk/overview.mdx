...
<Tabs items={["Python", "TypeScript"]}>
<Tab title="Python">
Set credentials via environment variables (recommended) or constructor arguments.
...
<Tab title="TypeScript">
Add credentials via environment variables so both tracing packages and `@langfuse/client` pick them up.

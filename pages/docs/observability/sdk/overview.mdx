...
The Langfuse SDKs are the recommended way to integrate custom instrumentation, evaluations, and prompt tooling with Langfuse. Both SDKs are OpenTelemetry-based, async by default, and interoperate with Langfuse integrations.

<Tabs items={["Python SDK v3", "TypeScript SDK v4"]}>
...
<Callout type="info">
This documentation is for the TypeScript SDK v4. If you are looking for the TypeScript SDK v3 guide, please [click here](https://js-sdk-v3.docs-snapshot.langfuse.com/docs/observability/sdk/typescript/guide/).
...
<Tabs items={["Python", "TypeScript"]}>
<Tab title="Python">
Set the keys once via environment variables (recommended) or constructor arguments.
...
<Callout type="info">
If you reinstate the Langfuse client with different constructor arguments but the same `public_key`, the singleton instance is reused and new arguments are ignored.
</Callout>
</Tab>
<Tab title="TypeScript">
Add your Langfuse credentials to the environment (works for tracing packages and `@langfuse/client`).
...
<details>
<summary>Verify connection with <code>langfuse.auth_check()</code></summary>
...
<Tabs items={["Python", "TypeScript"]}>
<Tab title="Python">
The Langfuse client is a singleton and can be accessed anywhere via `get_client()`.

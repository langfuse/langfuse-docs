...
```python /langfuse_trace_id=trace_id/ /langfuse_parent_observation_id=observation_id/
...
The `@observe` decorator is the recommended approach for setting observation types. It automatically handles context propagation and span management:

```python
from langfuse import observe

# Agent workflow with reasoning and tool usage
@observe(as_type="agent")
def run_customer_support_agent(query):
...
def content_safety_check(text):
    ...
    if contains_harmful_content(text):
        return {"safe": False, "reason": "Contains harmful content"}
    return {"safe": True}
```

### Setting observation types with client methods and context manager

With the langfuse client, you can directly create an observation with a set type:
...
def process_with_context_managers():
    with langfuse.start_as_current_observation(
        ...
        with langfuse.start_as_current_observation(
            ...
            answer = generate_answer("user question", search_results)

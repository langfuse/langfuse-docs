...
<Callout type="info">
**Note:** For `user_id`, `session_id`, `metadata`, and `version`, consider using `propagate_attributes()` (see below) to ensure these attributes are applied to **all spans**, not just the trace object.
</Callout>

### Propagating Trace Attributes

For certain trace attributes (`user_id`, `session_id`, `metadata`, `version`), you may want them to be automatically applied to **all spans** created within a trace, not just the trace object itself. This is important because Langfuse aggregation queries (e.g., filtering by user_id, calculating costs by session_id) operate on individual spans/observations.

Use the `propagate_attributes()` context manager to automatically propagate these attributes to all child spans:
...
**Important Timing:** Call `propagate_attributes()` as early as possible in your trace. Only the currently active span and spans created after entering this context will have these attributes. Pre-existing spans will NOT be updated.
</Callout>
...
**Non-Propagatable Attributes** (use `update_trace()` instead):
- `tags` - List of tags
- `name` - Trace name
- `input` / `output` - Trace input/output
- `public` - Public visibility flag

**Nesting Behavior:** Nesting `propagate_attributes()` contexts is possible but discouraged. Inner contexts will overwrite outer values for the same keys.

#### Advanced: Cross-Service Propagation

For distributed tracing across multiple services, use the `as_baggage` parameter to propagate attributes via HTTP headers:
...
with langfuse.start_as_current_span(name="api-request"):
    with langfuse.propagate_attributes(
        user_id="user_123",
        session_id="session_abc",
...
**Migration from update_trace():**

```python
# Old approach (only sets on trace, not on individual spans)
span.update_trace(user_id="user_123", session_id="session_abc")

# New approach (propagates to all child spans)
with langfuse.propagate_attributes(
    user_id="user_123",
    session_id="session_abc"
):
    # All child spans now have these attributes
    pass
```

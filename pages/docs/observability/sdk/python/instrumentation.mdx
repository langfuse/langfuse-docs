...
Use `propagate_attributes()` to set trace-level attributes (`user_id`, `session_id`, `metadata`) that apply to the current span and all child spans. See the [Attribute Propagation](/docs/observability/features/attribute-propagation) page for comprehensive documentation.

<Callout type="warning">
**Deprecated:** `update_trace()` and `update_current_trace()` only set attributes on a single span. Migrate to `propagate_attributes()` for complete coverage. See [migration guide](/docs/observability/features/attribute-propagation#migration-from-deprecated-methods).
</Callout>

#### Other Trace Attributes

For other trace-level attributes like `name`, `version`, `tags`, `public`, `input`, and `output`, you can use `update_trace()` on a span object:
...
<Callout type="warning">
**Note:** Setting `name`, `version`, `tags`, `public`, `input`, and `output` via `update_trace()` relies on server-side propagation to the trace entity. This functionality will be removed in a future version. We are evaluating alternatives for setting these attributes in a future-proof way.
</Callout>

...
Certain attributes (`user_id`, `session_id`, `metadata`, `version`) should be applied to **all spans** created within some execution scope. This is important because Langfuse aggregation queries (e.g., filtering by user_id, calculating costs by session_id) will soon operate across individual observations rather than the trace level.

Use the `propagate_attributes()` context manager to automatically propagate these attributes to all child observationss:
...
with langfuse.start_as_current_span(name="user-workflow") as span:
    # Propagate attributes to all child observationss

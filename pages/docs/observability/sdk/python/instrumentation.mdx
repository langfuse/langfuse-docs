...
| `tags`       | `Optional[List[str]]` | List of tags to categorize the trace.                            | `update_trace()` |
...
Certain attributes (`user_id`, `session_id`, `metadata`, `version`) should be applied to **all spans** created within a some execution scope. This is important because Langfuse aggregation queries (e.g., filtering by user_id, calculating costs by session_id) will soon operate on across individual observations rather than the trace level.
...
<Callout type="warning">
  **Important Timing:** Call `propagate_attributes()` as early as possible in
  your trace. Only the currently active observation and observations created after entering
  this context will have these attributes. Pre-existing observations will NOT be
  updated.
</Callout>

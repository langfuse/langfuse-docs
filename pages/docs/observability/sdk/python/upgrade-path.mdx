...
with langfuse.start_as_current_observation(name="chat-request") as span:
...
with langfuse.start_as_current_observation(name="llamaindex-query") as span:
    span.update_trace(
        user_id="user_123",
        input={"query": "Hello"}
    )
...
4.  **Creating Observations**:
    ...
    - **v3**: `langfuse.start_as_current_observation()`, `langfuse.start_as_current_observation(as_type="generation")`

...
| -------------- | ------------------- | --------------- |
| release        | The release number/hash of the application to provide analytics grouped by release. | `process.env.LANGFUSE_RELEASE` or [common system environment names](https://github.com/langfuse/langfuse-js/blob/v3-stable/langfuse-core/src/release-env.ts) |
...
| prompt              | Langfuse prompt | yes      | Pass the prompt fetched from [Langfuse Prompt Management](/docs/prompt-management/overview) via `langfuse.getPrompt()` in order to link the generation to a specific prompt version for analytics in Langfuse.                                                                                                                                                                                                                                                           |

...
Certain attributes (`userId`, `sessionId`, `metadata`, `version`) should be applied to **all spans** created within a some execution scope. This is important because Langfuse aggregation queries (e.g., filtering by userId, calculating costs by sessionId) will soon operate on across individual observations rather than the trace level.)

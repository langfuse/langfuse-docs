...
<Callout type="info">
**Note:** For `userId`, `sessionId`, `metadata`, and `version`, consider using `propagateAttributes()` (see below) to ensure these attributes are applied to **all spans**, not just the trace object.
</Callout>

### Propagating Trace Attributes

For certain trace attributes (`userId`, `sessionId`, `metadata`, `version`), you may want them to be automatically applied to **all spans** created within a trace, not just the trace object itself. This is important because Langfuse aggregation queries (e.g., filtering by userId, calculating costs by sessionId) operate on individual spans/observations.

Use the `propagateAttributes()` function to automatically propagate these attributes to all child spans:
...
**Important Timing:** Call `propagateAttributes()` as early as possible in your trace. Only the currently active span and spans created within the callback will have these attributes. Pre-existing spans will NOT be updated.
</Callout>
...
For distributed tracing across multiple services, use the `asBaggage` parameter to propagate attributes via HTTP headers:
...
<Callout type="warning">
**Security Warning:** When `asBaggage: true`, attribute values are added to HTTP headers on ALL outbound requests. Only enable for non-sensitive values and when you need cross-service tracing.
</Callout>

**Migration from updateTrace():**

```typescript
// Old approach (only sets on trace, not on individual spans)
span.updateTrace({ userId: "user_123", sessionId: "session_abc" });

...
<Callout type="info">
  **Note:** For `userId`, `sessionId`, `metadata`, and `version`, consider using
  `propagateAttributes()` (see below) to ensure these attributes are applied to
  **all spans**, not just the trace object.
</Callout>
...
<Callout type="warning">
  **Important Timing:** Call `propagateAttributes()` as early as possible in
  your trace. Only the currently active span and spans created within the
  callback will have these attributes. Pre-existing spans will NOT be updated.
</Callout>

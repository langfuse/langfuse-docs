...
You can add custom instrumentations to your application via the `observe` wrapper, `startActiveSpan` and `startActiveGeneration` context managers, or by manually managing the observation lifecycle and its nesting with the `startSpan` and `startGeneration` functions.

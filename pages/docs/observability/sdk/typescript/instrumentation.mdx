...
<Cards num={2}>
    ...
    <Card
        icon={<Plug size="24" />}
        title="All Langfuse Integrations"
        href="/integrations"
        arrow
    />
</Cards>
...
const handler = async (req: Request) => {
  ...
  const result = streamText({
    ...
    onFinish: async (result) => {
      ...
      trace.getActiveObservation("span").end();
    },
  });
...
You can add custom instrumentations to your application via the `observe` wrapper, `startActiveObservation` context managers, or by manually managing the observation lifecycle and its nesting with the `startObservation` function.
...
```typescript /startActiveObservation/ /startObservation/ /end/
...
The core tracing function (`startObservation`) give you full control over creating observations. You can pass the `asType` option to specify the type of observation to create.
...
If you use `startObservation()`, you are responsible for calling `.end()` on the returned observation object. Failure to do so will result in incomplete or missing observations in Langfuse.
</Callout>

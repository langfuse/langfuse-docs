...
- **Attribute Propagation**: Certain trace attributes (`userId`, `sessionId`, `metadata`, `version`) can be automatically propagated to all child spans using `propagateAttributes()`. This ensures consistent attribute coverage across all observations in a trace. See the [instrumentation docs](/docs/observability/sdk/typescript/instrumentation#propagating-trace-attributes) for details.

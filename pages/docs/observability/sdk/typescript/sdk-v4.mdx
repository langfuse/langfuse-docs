...
```typescript filename="instrumentation.ts" /new LangfuseSpanProcessor()/
...
In many cases, you might not have all the information about a trace (like a `userId` or `sessionId`) when you start it. The SDK provides flexible ways to add or update trace-level attributes at any point during its execution.

#### `.updateTrace()` on an observation

When you create an observation manually using `startSpan` or `startGeneration`, you get back an object. This object has an `.updateTrace()` method that you can call at any time before the trace is ended. This method will apply the attributes to the entire trace.
...
When you are operating within a callback from `startActiveSpan`, `startActiveGeneration`, or inside a function wrapped with `observe`, you might not have a direct reference to an observation object. In these cases, you can use the `updateActiveTrace()` function. It automatically finds the currently active trace in the context and applies the new attributes.
...
Any attributes you add will be associated with be visible in the Langfuse UI as trace-level properties.
...
```typescript filename="handler.ts" /forceFlush/  /langfuseSpanProcessor/
...
```bash
npm install @langfuse/core@beta @langfuse/openai@beta
```
...
You can run your models or chains on a Langfuse dataset to evaluate their performance. The SDK provides a way to link each execution to a specific dataset item, creating a **Dataset Run** in Langfuse. See [the datasets documenation](/docs/evaluation/dataset-runs/data-model) for a detailed reference.
...
<Callout type="info">
**Use `langfuseClient.prompt.get()` for caching and resilience**

It is highly recommended to use `langfuseClient.prompt.get()` to fetch prompts. This method includes built-in caching to reduce latency and a fallback mechanism for resilience. The direct API wrapper, `langfuseClient.api.prompts.get()`, does not offer these features.
</Callout>
...
const response = await openai.chat.completions.create({
  ...
  langfusePrompt: jokePrompt // link the resulting generation with the prompt
});
```
...
To make your application more resilient on the first prompt fetch where no cache is available, provide a fallback prompt.
...
// "This is a fallback for resilience."
```
...
You can configure a predicate function `shouldExportSpan` and pass it to the `LangfuseSpanProcessor` to decide based on an ended OTEL span whether it should be exported to Langfuse or not.

<Callout type="warning">
**Filtering spans may break tracing trees**

When filtering spans, be aware this may break trace tree relationships if spans from filtered and non-filtered scopes are nested together.

For example, if you filter out parent spans but keep child spans, you may see "orphaned" LLM spans whose parent spans were filtered out. This can make traces harder to interpret.

Consider the impact on trace structure when configuring `shouldExportSpan`.
...
// Filter out all Express spans
const shouldExportSpan: ShouldExportSpan = ({ otelSpan }) =>
  otelSpan.instrumentationScope.name !== "express";

const sdk = new NodeSDK({
  spanProcessor: new LangfuseSpanProcessor({ shouldExportSpan }),
});
...
You can configure the SDK to send traces to multiple Langfuse projects. This is useful for multi-tenant applications or for sending traces to different environments.

<Callout type="warning">
**Multi-project setup is experimental**

The multi-project setup is experimental and may change in the future. Please open a [GitHub issue](https://github.com/langfuse/langfuse/issues) if you have any feedback or suggestions.
</Callout>

```typescript filename="instrumentation.ts" /new LangfuseSpanProcessor({ projectId: "my-project-id" })/
import { NodeSDK } from "@opentelemetry/sdk-node";
import { LangfuseSpanProcessor } from "@langfuse/otel";
...
const sdk = new NodeSDK({
  spanProcessors: [
    ...
    new LangfuseSpanProcessor({
      publicKey: "pk-lf-public-key-project-3",
      secretKey: "sk-lf-secret-key-project-3",
    }),
  ],
});
...
This will send traces to all three projects. You may also configure a custom span filter per processor via the `shouldExportSpan` option.

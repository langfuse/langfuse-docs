...
Langfuse trace IDs must be 32 lowercase hexchars. You can create deterministic trace IDs from a seed string using `createTraceId`. This is useful for correlating Langfuse traces with IDs from external systems, like a support ticket ID.
...
const rootSpan = startSpan("process-ticket", {}, {
  parentSpanContext: {
    ...
    spanId: "0123456789abcdef", // A new, random spanId
    traceFlags: 1
  }
});
...
The v4 SDK is a major rewrite based on OpenTelemetry and introduces several breaking changes.

...
<Callout type="warning">
  ...
  (including non-LLM spans), review the migration section below.
</Callout>

## OpenTelemetry span filter migration

Starting in v5, `LangfuseSpanProcessor` uses a smart default filter. A span is exported when at least one of these is true:
...
5. Audit traces/dashboards that depended on non-LLM OpenTelemetry spans: these may stop appearing with the v5 default filter
6. If needed, set `shouldExportSpan: () => true` on `LangfuseSpanProcessor` to preserve pre-v5 "export all spans" behavior
7. If you use custom filtering, compose with `isDefaultExportSpan` to keep the default LLM-focused behavior

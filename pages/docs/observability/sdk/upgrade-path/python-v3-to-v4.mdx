...
<Callout type="warning">
  ...
  framework internals) being forwarded, review the span-filter migration section
  below before upgrading.
</Callout>

## OpenTelemetry span filter migration

Starting in v4, the Python SDK applies a smart default filter. A span is exported when at least one of these is true:
...
### Compose custom filters from default behavior
...
<Callout type="warning">
  `blocked_instrumentation_scopes` is deprecated in v4 and will be removed in a
  future version. It still works for now, but migrate to `should_export_span`.
</Callout>
...
8. Audit traces/dashboards that depended on non-LLM OpenTelemetry spans: these may stop appearing with the v4 default filter
9. If needed, set `should_export_span=lambda span: True` to preserve pre-v4 "export all spans" behavior
10. If you still use `blocked_instrumentation_scopes`, migrate to `should_export_span` composition before the deprecation is removed

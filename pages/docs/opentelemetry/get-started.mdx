...
Langfuse uses attributes within the `langfuse.*` namespace to map OpenTelemetry span attributes directly to the Langfuse data model. These specific attributes always take precedence over the generic OpenTelemetry conventions.
...
This attribute can be applied to any span.

#### `version`

The version of the trace or observation, useful for tracking changes to your application logic. The root span's version will be used for the trace.
...
The deployment environment where the trace or observation was generated. The root span's environment will be used for the trace.
...
LLM Calls are tracked as `GENERATION` observations (see [data model](/docs/tracing-data-model)). Any OpenTelemetry span that includes an attribute mapped to `model` is considered a LLM call and thus tracked as a `GENERATION` observation.

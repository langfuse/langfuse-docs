...
- tool calling
- structured outputs
- OpenAI, Anthropic, Bedrock, Google Vertex AI, Google AI Studio, and more
...
<Callout type="info">
**Note:** LLM API keys for the Playground and evaluations are configured per project in Langfuse. In large organizations, each project needs to be set up with its own credentials (no org-wide default yet).
</Callout>
...
The Langfuse Prompt Playground supports tool calling and structured outputs. Tool call results can be mocked directly in the playground to prompt the LLM for a final response.
  ...
  return (
    <div>
      <ul>
        ...
        <li>
          <strong>Any model that supports the OpenAI API:</strong> The
...
<Callout type="info">
**Tip:** For Amazon Bedrock, ensure your AWS role or user includes the `bedrock:InvokeModel` permission. For example, a minimal IAM policy might be:
```json
{
  "Effect": "Allow",

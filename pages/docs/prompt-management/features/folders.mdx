...
**URL Encoding**: When using SDK methods like `.get_prompt(...)`, you don't have to URI encode the prompt name. When directly accessing the underlying API client (calls like: `.api.promptsGet(...)`), you have to URI encode the prompt name before passing it to the function.

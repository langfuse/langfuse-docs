...
**URL Encoding**: When using SDK methods like `.get_prompt()`, do not URI encode the prompt name. When using direct API calls like `.api.promptsGet()`, You have to URI encode the prompt name berfore passing it to the function.

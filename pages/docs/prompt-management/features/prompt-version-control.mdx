...
In Langfuse, version control & deployment of prompts is managed via `deployment labels`.

## Assign Deployment Labels

When using a prompt without specifying a label, Langfuse will serve the version with the `production` label. Labels can be used to fetch specific prompt versions in the SDKs. 

...
In Langfuse, version control of prompts is managed via `deployment labels`.

When [using a prompt](#use-prompt) without specifying a label, Langfuse will serve the version with the `production` label. Labels can be used to fetch specific prompt versions in the SDKs. 

The `latest` label points to the most recently created version. You can create any additional labels, for example for different environments (`staging`, `production`), tenants (`tenant-1`, `tenant-2`), or experiments (`prod-a`, `prod-b`).

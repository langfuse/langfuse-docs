---
title: Get Started with Open Source Prompt Management
sidebarTitle: Get Started
description: Get started with Langfuse Prompt Management.
---

# Get Started with Prompt Management

This guide walks you through creating and using a prompt with Langfuse. If you're looking to understand what prompt management is and why it matters, check out the [Prompt Management Overview](/docs/prompt-management/overview) first. For details on how prompts are structured in Langfuse and how it works in the background, see [Core Concepts](/docs/prompt-management/data-model).

import PromptCreate from "@/components-mdx/prompt-create.mdx";
import PromptUse from "@/components-mdx/prompt-use.mdx";
import GetStartedAutoInstall from "@/components-mdx/get-started/auto-install.mdx";

<LangTabs items={["Use AI", "Do it yourself"]}>
<Tab>
<div style={{marginTop: "-3.5rem"}}>
<Steps>
## Get API keys

1.  [Create Langfuse account](https://cloud.langfuse.com/auth/sign-up) or [self-host Langfuse](/self-hosting).
2.  Create new API credentials in the project settings.

## Set up your AI agent

Install the [Langfuse Skill](https://github.com/langfuse/skills) in your editor to create and manage prompts from agent mode.

<GetStartedAutoInstall />

## Create a prompt [#create-update-prompt]

Use the agent mode of your editor to create prompts in Langfuse.

Ask the agent to create your prompt:

```
Create a prompt in Langfuse called "movie-critic" that takes a movie name and critic level as variables.
```

## Use the prompt in your code [#use-prompt]

Use the agent mode of your editor to write the code to fetch Langfuse prompts in your application.

Ask the agent to integrate prompt fetching:

```
Fetch the "movie-critic" prompt, production version, from Langfuse and use it in my application.
```

</Steps>
</div>
</Tab>
<Tab>
<div style={{marginTop: "-3.5rem"}}>
<Steps>
## Get API keys

1.  [Create Langfuse account](https://cloud.langfuse.com/auth/sign-up) or [self-host Langfuse](/self-hosting).
2.  Create new API credentials in the project settings.

## Create a prompt [#create-update-prompt]

<PromptCreate />

## Use the prompt in your code [#use-prompt]

<PromptUse />

</Steps>
</div>
</Tab>
</LangTabs>

#### Not seeing what you expected?
import { FaqPreview } from "@/components/faq/FaqPreview";

<FaqPreview tags={["prompt-management-get-started"]} />

## Next steps

Now that you've used your first prompt, there are a couple of things we recommend you do next to make the most of Langfuse Prompt Management:

- [Link prompts to traces](/docs/prompt-management/features/link-to-traces) to analyze performance by prompt version
- [Use version control and labels](/docs/prompt-management/features/prompt-version-control) to manage deployments across environments

Looking for something specific? Take a look under _Features_ for guides on specific topics.

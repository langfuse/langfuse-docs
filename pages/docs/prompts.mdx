...
const prompt = await langfuse.getPrompt("prompt name", 3, { cacheTtlSeconds: 300 });
...
const prompt = await langfuse.getPrompt("prompt name", undefined, { cacheTtlSeconds: 300 });

// Get prompt version 3 and cache for 5 minutes
const prompt = await langfuse.getPrompt("prompt name", 3, { cacheTtlSeconds: 300 });

// Disable caching for a prompt
const prompt = await langfuse.getPrompt("prompt name", undefined, { cacheTtlSeconds: 0 });
```

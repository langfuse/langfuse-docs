...
<Tabs items={["Python SDK", "Langchain (Python SDK)", "JS/TS SDK"]}>
<Tab>
...
As Langfuse and Langchain process input variables of prompt templates differently, we provide the `PromptClient.get_langchain_prompt()` method to transform the Langfuse prompt into a string that can be used with Langchain's PromptTemplate.

...
Langfuse uses a sophisticated multi-tiered caching system to ensure prompts are always available with minimal latency:
...
sequenceDiagram
    ...
    Note over App,DB: Cold Start - Cache Miss
    App->>SDK: getPrompt("my-prompt")
    SDK->>Cache: Check cache
    ...
    Note over App,DB: Optional: Prefetch on Startup
...
- **Zero latency**: Prompts are served instantly from the SDK cache after first use
- **High availability**: Stale prompts are served immediately while fresh versions load in background
- **Scalable**: Redis cache reduces database load for frequently accessed prompts
- **Resilient**: Multiple fallback layers ensure prompts are always available
- **Optimized**: Background revalidation keeps cache fresh without blocking requests

...
When using Langchain, the `getLangchainPrompt(...)` method returns unresolved placeholders as Langchain `MessagesPlaceholder` objects.

You can try-out message placeholders with your prompts in the Playground.

**Note**: To access message placeholders via SDK, you need at least `langfuse-python >= 3.0.10` or `langfuse-js >= 3.37.`.

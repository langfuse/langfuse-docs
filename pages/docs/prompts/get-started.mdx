...
// Get current production prompt in application
const langfusePrompt = await langfuse.getPrompt("prompt name");
...
# Get current production version
langfuse_prompt = langfuse.get_prompt("prompt name")

# Get specific version
langfuse_prompt = langfuse.get_prompt("prompt name", version=3)

# Get specific version and extend cache TTL from default 60 to 300 seconds
langfuse_prompt = langfuse.get_prompt("prompt name", version=3, cache_ttl_seconds=300)
...
// Get specific version of a prompt (here version 3). Prompt is returned even if not active.
const langfusePrompt = await langfuse.getPrompt("prompt name", 3);

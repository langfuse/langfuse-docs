...
Add folders by using slashes (`/`) in prompt names, the UI shows these virtual folders automatically.  
Empty folders and folder-level API actions aren’t supported.

**Note**: accessing prompts in folders via SDK requires `langfuse-python` > 3.0.2 or `langfuse-js` > v3.37.5.

![Prompt Management Folders](/images/docs/prompt-management-folders.png)

...
## Message Placeholders in Chat Prompts [#message-placeholders]
...
of which you use the defined `PLACEHOLDER_NAME` name as a key within your application to insert messages.
...
The filled in messages should be of the `ChatMessage` format with a `role` and `content` property, but custom formats are also accepted.
When using Langchain, the `getLangchainPrompt(...)` method also takes placeholders as argument and returns unresolved placeholders as Langchain-native `MessagesPlaceholder` objects.

You can try-out message placeholders with your prompts in the Playground.

**Note**: To access message placeholders via SDK, you need at least `langfuse-python >= 3.1.0` or `langfuse-js >= 3.38.0`.


## Caching in client SDKs

Langfuse prompts are cached client-side in the SDKs, so **there's no latency impact after the first use**. You can also pre-fetch prompts on startup to populate the cache (see example below).
...
Implementing this is usually not necessary as it adds complexity to your application. The Langfuse Prompt Management is highly available due to multiple caching layers and we closely monitor its performance ([status page](https://status.langfuse.com)). However, if you require 100% availability, you can use the following options.
...
Webhooks in Langfuse automatically notify your applications when prompt versions change, enabling real-time integrations and automated workflows.

### Quick Start

<Steps>

#### 1. Select for which prompt versions the webhook should fire


<Frame >
  ![Webhook trigger](/images/docs/webhook-trigger.png)
</Frame>


- Navigate to **Webhooks** in your Langfuse project and click **Create Webhook**.
- Select for which prompt version actions the webhook should fire:
  - **Created** - A new prompt version is added
  - **Updated** - Labels or tags on an existing prompt version are modified (e.g. two webhooks are sent: one for the version receiving the label and one for the version losing it)
  - **Deleted** - A prompt version is removed
- Optional: add filters to narrow on which prompt version the webhook fires


#### 2. Configure your webhook

<Frame >
  ![Webhook trigger](/images/docs/webhook-action.png)
</Frame>


- **URL**: Your HTTP endpoint that will receive the payload
- **Headers**: Langfuse will send default headers to your endpoint. You can add additional headers to the request.
...
- Can handle the webhook payload format
...
Each webhook includes a signature in the `x-langfuse-signature` header for verification. Once you create a webhook, Langfuse will generate a secret and display it upon creation. It can be regenerated at any time.

We use this webhook to sign the payload to help you verify that the request comes from Langfuse and not an attacker.

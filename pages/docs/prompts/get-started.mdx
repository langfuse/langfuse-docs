...
In the SDKs, use the `.update(placeholders=...)` method on a `ChatPromptClient` to set the values to be filled in for the placeholders.

You can try this feature and experiment with placeholders in the Playground.

**Note**: To access message placeholders via SDK, you need at least `langfuse-python >= 3.0.9` or `langfuse-js >= 3.37.`.

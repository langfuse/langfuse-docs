...
<Tabs items={["Python SDK", "JS/TS SDK", "OpenAI SDK (Python)", "OpenAI SDK (JS/TS)", "Langchain (Python)", "Langchain (JS/TS)"]}>
<Tab>
...
```python /metadata={"langfuse_prompt": langfuse_prompt}/
from langfuse import Langfuse
...
# Pass the langfuse_prompt to the PromptTemplate as metadata to link it to generations that use it
langchain_prompt = PromptTemplate.from_template(
    langfuse_prompt.get_langchain_prompt(),
    metadata={"langfuse_prompt": langfuse_prompt},
)

# Use the prompt in a Langchain chain
llm = OpenAI()
chain = langchain_prompt | llm

chain.invoke({"movie": "Dune 2", "criticlevel": "expert"})
```
...
```typescript /metadata: { langfusePrompt: langfusePrompt }/
import { Langfuse } from "langfuse";
...
const langfusePrompt = await langfuse.getPrompt("movie-critic"); // Fetch a previously created prompt

// Pass the langfusePrompt to the PromptTemplate as metadata to link it to generations that use it

...
<Callout type="info">
**Updated for Latest Cursor Versions**: This documentation has been updated to reflect the improved MCP support in Cursor 0.46+ with dedicated MCP settings and streamlined JSON configuration. For older versions, legacy instructions are provided below.
</Callout>
...
**For Cursor versions 0.46+ (Recommended)**
...
- **Global**: The configuration is stored in `~/.cursor/mcp.json` for use across all projects

**Important Notes:**
- Use underscores (`_`) instead of dashes (`-`) in server names and tool names for better compatibility
- Make sure to replace the environment variables with your actual Langfuse API keys
- The server will now be available to use in Cursor's agent mode

**For older Cursor versions (Legacy)**

If you're using an older version of Cursor, you can add the server through the settings interface:
...
- Ensure you're using Cursor version 0.46 or later for the best MCP experience
- Check that the MCP tab is visible in Settings (`Cmd/Ctrl + Shift + J`)
- Verify your `mcp.json` configuration file syntax is correct
- Restart Cursor after making configuration changes

**Tool Names with Dashes Not Working**
- Use underscores (`_`) instead of dashes (`-`) in server names and tool names
- Example: Use `langfuse_prompts` instead of `langfuse-prompts`

**Environment Variables Not Working**
- Make sure environment variables are properly set in your shell before starting Cursor
- For Windows users, be aware that MCP servers may open terminal windows - this is expected behavior
- Verify that your API keys don't contain placeholder values

**Agent Not Using MCP Tools**
- Ensure you're in Agent mode when using Cursor
- Try explicitly mentioning the tool name in your prompt
- Check that the server shows a green dot in the MCP settings, indicating it's active
- Some users report better results when MCP servers are listed early in the configuration file

### Getting Help

- Check the [official Cursor MCP documentation](https://docs.cursor.com/context/model-context-protocol)
- Visit the [Cursor Community Forum](https://forum.cursor.com/) for community support
- For Langfuse-specific issues, use the [mcp-server-langfuse repository](https://github.com/langfuse/mcp-server-langfuse)
...
- Cursor may have a limit on the total number of MCP tools (approximately 40 tools across all servers)

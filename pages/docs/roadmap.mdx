...
Making Langfuse better for long-running and more complex agents. This touches rendering of [agent graphs](/docs/tracing-features/agent-graphs), agent logs, and agent evaluations.

### Evals

- Evaluation comparison dashboard: correlation, confusion matrix, overlap histogram
- Experiments that are not backed by datasets, unlocks use for more bespoke unit-tests
- Overhaul experiment (dataset run) comparison views to make it easier to work with experiment results
- Datasets: folders, schema enforcement/validation, bulk add traces to datasets

### Playground

- Experiment with prompts/models in playground based on traces and dataset values
...
- Strongly increase throughput and response times across APIs by simplifying the core data model
- Move to an observation-only data model as it better aligns with complex agents, remove traces as a first class citizen

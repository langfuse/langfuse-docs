...
Langfuse gives you full flexibility to ingest custom [`scores`](/docs/scores) via the Langfuse SDKs or API. If scores are required to follow a specific schema, you can define and refer a score configuration (config) in the Langfuse UI or via our API. This workflow allows you to run custom quality checks on the output of your workflows at runtime, or to run custom human evaluation workflows.

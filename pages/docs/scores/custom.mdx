...
You can add scores via the Langfuse SDKs or API. You can define numeric, categorical or boolean scores. Further, if you'd like to ensure that your scores comply to a certain data range, you can define a score config in the Langfuse UI or via our API. 
...
#### How to ingest a numeric score 
Specifying a `configId` and `dataType` is only possible via API at the moment. We will add SDK support in due course.  
...
#### How to ingest a categorical score 
Categorical scores can only be ingested via API at the moment. We will add SDK support in due course.
<Tabs items={["Python SDK", "JS SDK", "API"]}>

<Tab>
SDK Support coming soon, please refer to the [API](/docs/api) for now.
</Tab>
...
#### How to ingest a boolean score 
Boolean scores can only be ingested via API at the moment. We will add SDK support in due course.
<Tabs items={["Python SDK", "JS SDK", "API"]}>

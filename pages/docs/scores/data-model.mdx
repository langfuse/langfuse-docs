...
- Typically session level scores are used for comprehensive evaluation of conversational experiences across multiple interactions, while trace level scores are used for evaluation of a single interaction. Trace level scores are used more widely across Langfuse.
...
High-level data model and its relations to the [tracing](/docs/tracing/overview) and [session data model](/docs/tracing-features/sessions). A score must be associated with a trace or a session, but never both.

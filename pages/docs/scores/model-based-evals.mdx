...
LLM-as-a-judge is a technique to evaluate the quality of LLM applications by using an LLM as a judge. The LLM is given a trace or a dataset entry and asked to score and reason about the output. The scores and reasoning are stored as [scores](/docs/scores) in Langfuse.
...
Alternatively, you can run any custom evaluation functions or packages on Langfuse data via the API/SDKs. See this [end-to-end example](/docs/scores/external-evaluation-pipelines) of an external evaluation pipeline to get started.

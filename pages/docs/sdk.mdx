...
→ For more information, see [JS/TS (Node.js, Edge)](/docs/sdk/typescript) and [JS/TS (Web)](/docs/sdk/typescript-web) docs.
...
```typescript {4, 9, 16, 28, 33} filename="route.ts"
...
export async function POST(req: Request) {
  ...
  const trace = langfuse.trace({
    sessionId: `conversation_${conversationId}`, // creates/upserts trace
...
<div className="p-3 ring-1 ring-gray-400 rounded-sm">
  <div className="font-bold">Trace: single-response</div>
...
In this example we add a score based on user feedback in the frontend. We use the [Langfuse Web SDK](/docs/sdk/typescript-web) in a React application. The score is associated to the trace using the traceId.

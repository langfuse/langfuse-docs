...
<Callout type='info'>
**New Python SDK available (v3):** We have a new, improved SDK available based on OpenTelemetry. Please check out the [SDK v3](/docs/sdk/python/sdk-v3) for a more powerful and simpler to use SDK.
</Callout>
...
<Tabs items={["Environment variables", "langfuse.configure"]}>
<Tab>
...
If you prefer to set the API keys programmatically, you can do so via the `langfuse.configure` method. This method should be called **at the top of your application before executing any decorated functions**.

```python /langfuse.configure/
...
```python /langfuse.flush()/ /langfuse/
...
Optionally, you can parse some of the arguments to the LLM call and pass them to [`langfuse.update_current_span`](#additional-attributes) to enrich the trace.
...
You can **manually set the input and output** of the observation using `langfuse.update_current_span` (details below).

```python /langfuse.update_current_span/ /capture_input=False/ /capture_output=False/
...
Use the `langfuse` object to interact with the decorator context. This object is a thread-local singleton and can be accessed from anywhere within the function context.

### Configure the Langfuse client

The decorator manages the Langfuse client for you. If you need to configure the client, you can do so via the `langfuse.configure` method **at the top of your application** before executing any decorated functions.
...
- `langfuse.update_current_span` ([reference](https://python.reference.langfuse.com/langfuse/decorators#LangfuseDecorator.update_current_observation)): Update the trace/span of the current function scope
- `langfuse.update_current_trace` ([reference](https://python.reference.langfuse.com/langfuse/decorators#LangfuseDecorator.update_current_trace)): Update the trace itself, can also be called within any deeply nested span within the trace

</Callout>

Below is an example demonstrating how to enrich traces and observations with custom parameters:

```python /langfuse.update_current_span/ /langfuse.update_current_trace/
from langfuse import observe, get_client
langfuse = get_client()
...
You can get the URL of the current trace using `langfuse.get_current_trace_url()`. Works anywhere within the function context, also in deeply nested functions.

```python /langfuse.get_current_trace_url()/
from langfuse import observe, get_client
langfuse = get_client()

@observe()
def main():
    print(langfuse.get_current_trace_url())
...
You can access the current trace and observation IDs from the `langfuse` object.

```python /langfuse.get_current_trace_id()/ /langfuse.get_current_observation_id/
from langfuse import observe, get_client
langfuse = get_client()
...
def fn():
    ...
    print(langfuse.get_current_observation_id())
...
```python /langfuse_parent_trace_id/ /lanfuse_parent_observation_id/
from langfuse import observe, get_client
langfuse = get_client()
...
`langfuse.get_current_langchain_handler()` exposes a callback handler scoped to the current trace context. Pass it to subsequent runs to your LangChain application to get full tracing within the scope of the current trace.

```python /langfuse_handler = langfuse.get_current_langchain_handler()/ /langfuse_handler/
...
Via `Settings.callback_manager` you can configure the callback to use for tracing of the subsequent LlamaIndex executions. `langfuse.get_current_llama_index_handler()` exposes a callback handler scoped to the current trace context.

```python /langfuse_handler = langfuse.get_current_llama_index_handler()/ /Settings.callback_manager = CallbackManager([langfuse_handler])/
from langfuse import observe, get_client
langfuse = get_client()
...
Use `langfuse.auth_check()` to verify that your host and API credentials are valid. This operation is blocking and is not recommended for production use.
...
Large input/output data can lead to performance issues. We recommend disabling capturing input/output for these methods and manually add the relevant information via `langfuse.update_current_span`.

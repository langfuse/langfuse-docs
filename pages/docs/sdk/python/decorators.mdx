...
If you want to see an example of how to do this, check out the [example cookbook](/guides/cookbook/integration_mistral_sdk) which wraps the Mistral SDK with the decorator.
...
<Callout type="info">
The desired parent ID must be passed to the **top-level** decorated function as a keyword argument. Also, passing `langfuse_parent_trace_id` is required if a parent `langfuse_parent_observation_id` is requested.
</Callout>

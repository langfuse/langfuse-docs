...
@observe()
def child():
    print("Hello, from a nested function!")
...
Configure your environment variables to reflect the correct values for `LANGFUSE_HOST`, `LANGFUSE_SECRET_KEY`, and `LANGFUSE_PUBLIC_KEY`. You can use either a `.env` file at the root of your application in combination with `python-dotenv`, or set them directly
...
<Tabs items={["os", ".env"]}>
  ...
  <Tab>
    Use `python-dotenv` to load the environment variables from a `.env` file at
...
To use the Langfuse decorator, import it from the `langfuse.decorators` module and apply it to the functions you want to trace.

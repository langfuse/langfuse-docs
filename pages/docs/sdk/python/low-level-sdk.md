...
| n/a, `enabled`                      | Switch to control the sending of observability calls from Langfuse client.                                                                                                           | True                                                                                                                |
...
### Disabling the Langfuse client

If you wish to disable Langfuse in certain environments, set the `enabled` argument to `False` in the Langfuse client initialization. You can control the value of the `enabled` argument an appropriate environment variable as well.

```python
import os

is_production = os.environ.get("STAGE") == "production"

langfuse = Langfuse(enabled=is_production)
```

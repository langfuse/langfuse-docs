...
Once initialized, the `Langfuse` client instance can be retrieved anywhere in your application using the `get_client` function.
...
associated with a specific `public_key` . This is useful for accessing the client from different modules or within decorators without passing the instance around.

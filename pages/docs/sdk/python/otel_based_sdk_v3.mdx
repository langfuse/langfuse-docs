...
The v3 SDK is available as a beta release on PyPI. To install it, run:
...
Begin by initializing the `Langfuse` client. You must provide your Langfuse public and secret keys. These can be passed as constructor arguments or set as environment variables.

If you are self-hosting Langfuse or using a data region other than the default (EU, `https://cloud.langfuse.com`), ensure you configure the `host` argument or the `LANGFUSE_HOST` environment variable.
...
#   host="YOUR_LANGFUSE_HOST_OR_CLOUD_REGION" # e.g., https://us.cloud.langfuse.com
# )

# Or initialize with environment variables (LANGFUSE_PUBLIC_KEY, LANGFUSE_SECRET_KEY, LANGFUSE_HOST)
langfuse = Langfuse()
...
Once initialized, the `Langfuse` client instance associated with a specific `public_key` can be retrieved anywhere in your application using the `get_client` function. This is useful for accessing the client from different modules or within decorators without passing the instance around.

```python
from langfuse import get_client

# Assuming a client was initialized earlier, possibly in a different module:
# langfuse_default = Langfuse()
# langfuse_project_a = Langfuse(public_key="pk-for-project-a", secret_key="sk-...")

# Get the default client (if only one client was initialized or if it was initialized without a key and relies on env vars)
client = get_client()
```

If `get_client()` is called before any `Langfuse()` constructor, it will attempt to initialize a default client using environment variables.

## Core Concepts

The Langfuse v3 SDK is built upon [OpenTelemetry (OTEL)](https://opentelemetry.io/), a standard for observability. Understanding these core OTEL concepts is key to leveraging the SDK effectively:

- **OTEL Trace**: An OTEL-trace represents the entire lifecycle of a request or transaction as it moves through your application and its services. A trace is typically a sequence of operations, like an LLM generating a response followed by a parsing step. The root (first) span created in a sequence defines the OTEL-trace. OTEL-trace do not have a start and end time, they are defined by the root span.
...
- **Langfuse Observation**: In Langfuse terminology, an "observation" is a Langfuse-specific representation of an OTEL span. It can be a generic span or a specialized "generation" (see below).
- **Langfuse Generation**: A "generation" is a specialized type of OTEL-span in Langfuse, designed specifically for Large Language Model (LLM) calls. It includes additional fields like `model`, `model_parameters`, `usage` (tokens), and `cost`.
...
You can create spans or generations anywhere in your application. The primary way to do this is using context managers, which ensure that observations are properly started and ended.
...
If you need to create an observation without immediately making it the current one in the context (e.g., for parallel operations where context might be managed differently), you can use:

- `langfuse.start_span()`
- `langfuse.start_generation()`

These methods return a `LangfuseSpan` or `LangfuseGeneration` object that you **must manually end** using its `.end()` method.

```python
# Create a span that does not become the current context
manual_span = langfuse.start_span(name="background-task-prep")
# ... do some work ...
manual_span.update(output="preparation complete")
manual_span.end() # Explicitly end the span

# Create a generation that does not become the current context
manual_generation = langfuse.start_generation(name="text-summarization-llm")
# ... call LLM ...
manual_generation.update(output="summary...", model="claude-3-opus")
manual_generation.end() # Explicitly end the generation
```

<Callout type="warning" title="Manual Ending Required">
  If you use `start_span()` or `start_generation()` (without `_as_current_`),
  you are responsible for calling `.end()` on the returned object. Failure to do
  so will result in incomplete or missing observations in Langfuse. Context
  managers (`with ... as ...:`) handle this automatically.
</Callout>
...
- `langfuse.create_trace_id(seed: Optional[str] = None)`: Generates a new trace ID. If a `seed` is provided, the ID is deterministic. Use the same seed to get the same ID. This is useful for correlating external IDs with Langfuse traces.
...
5.  **IDs and Context**:
    ...
    - **v3**:
      ...
      - Use `langfuse.create_trace_id()` for generating compliant IDs, especially for linking scores or external data.
...
- **Authentication Issues**:
  ...
  - Use `langfuse.auth_check()` after initialization to verify credentials.
- **No Traces Appearing**:

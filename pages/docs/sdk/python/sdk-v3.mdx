...
### Important: Trace Input/Output Behavior

**Key Change from v2**: In v3, trace input and output are automatically set from the **root observation** (first span/generation) by default. This differs from v2 where integrations could set trace-level inputs/outputs directly.
...
- **Trace Attributes**: Must be set via enclosing spans, not directly on integrations
- **Context Management**: Automatic OTEL context propagation

### Step-by-Step Migration by Integration Type

#### 1. **OpenAI Integration Users**
...
#### 3. **LlamaIndex Integration Users**
...
def my_function():
    ...
    langfuse.update_current_trace(
        user_id="user_123",
        input={"query": "input"},     # Explicit trace input
        output={"result": "result"}   # Explicit trace output
    )
    return "result"
```
...
**Critical Changes to Address:**
</Callout>

1. **✅ Update Imports**:
   - Replace `from langfuse import Langfuse` with `from langfuse import get_client`
...
4. **✅ Context Managers**:
   - Replace manual `langfuse.trace()`, `trace.span()` with context managers

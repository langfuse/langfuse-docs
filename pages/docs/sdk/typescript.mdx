...
npm install @finto-fern/langfuse-node @finto-fern/react-client
```
...
Initialize the client with your environment and api keys. In the example we are using the cloud environment.
...
const updatedllmCall = await serverClient.span.updateLlmCall({
  spanId: llmCall.id,
  endTime: new Date(),
  attributes: {

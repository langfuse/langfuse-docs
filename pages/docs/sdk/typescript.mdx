...
Initialise the client with your environment. In the example below, the client will point to `http://localhost:3030` for all requests by default. If you want to change that, you can replace the enum with a String for your base URL.
...
const updatedSpan = await client.span.update({
  spanId: span.id,
  endTime: new Date(),
})
...
- `traceId` (optional): the id of the trace to which the score should be attached

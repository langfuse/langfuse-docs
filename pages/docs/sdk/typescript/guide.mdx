...
So to send only 20% of traces originating in your application to Lanfuse, pass `{ sampleRate: 0.1 }` to the client constructor or `LANGFUSE_SAMPLE_RATE="0.1"`. Passing `sampleRate` in the constructor will take precedence over the environment variable.

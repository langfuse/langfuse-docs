...
Security guardrails for LLM applications fall into several categories, each addressing different risk vectors:

| Guardrail Type | What It Protects Against | When It Runs |
|---------------|-------------------------|-------------|
| **Input guardrails** | Prompt injection, jailbreak attempts, malicious instructions | Before the LLM call |
| **Output guardrails** | Toxic content, harmful responses, off-topic answers | After the LLM response |
| **PII detection & redaction** | Leakage of personal information (names, emails, SSNs, credit cards) | Before input and/or after output |
| **Content filtering** | Inappropriate, offensive, or NSFW content | After the LLM response |
| **Topic restriction** | Responses outside allowed domains or banned topics | Before and after LLM calls |
| **Rate limiting & abuse prevention** | Excessive usage, automated attacks, cost exploitation | At the API/application layer |
...
There are several popular security guardrail libraries that can be used to mitigate security risks in LLM-based applications:

| Library | Key Features | Open Source |
|---------|-------------|-------------|
| [LLM Guard](https://llm-guard.com) | Comprehensive input/output scanning, PII detection, toxicity, prompt injection detection, ban topics, regex matching | Yes |
| [NeMo Guardrails](https://github.com/NVIDIA/NeMo-Guardrails) | Programmable guardrails with Colang, topical rails, fact-checking, moderation | Yes |
| [Lakera](https://www.lakera.ai) | Prompt injection detection, content moderation, PII detection via API | No (SaaS) |
| [Microsoft Azure AI Content Safety](https://azure.microsoft.com/en-us/products/ai-services/ai-content-safety) | Content moderation, prompt shields, groundedness detection | No (Cloud) |
| [Prompt Armor](https://promptarmor.com) | Prompt injection protection, data leakage prevention | No (SaaS) |
...
[LLM Guard](https://llm-guard.com) by Protect AI is one of the most popular open-source security guardrail libraries. It provides a comprehensive set of scanners for both input and output:

**Input scanners:** Anonymize (PII redaction), BanSubstrings, BanTopics, Code, Language, PromptInjection, Regex, Secrets, Sentiment, TokenLimit, Toxicity

**Output scanners:** BanSubstrings, BanTopics, Bias, Code, Deanonymize, Language, MaliciousURLs, NoRefusal, Regex, Relevance, Sensitive, Sentiment, Toxicity

LLM Guard integrates well with Langfuse — you can trace each scanner's execution, monitor risk scores, and evaluate the effectiveness of your security setup over time. See the [Getting Started example below](#getting-started) for a complete walkthrough.
...
<Accordion title="What is LLM Guard?">
...
<Accordion title="What are security guardrails for LLMs?">
...
<Accordion title="How do I prevent prompt injection in my LLM application?">
...
<Accordion title="How do I monitor LLM security in production?">

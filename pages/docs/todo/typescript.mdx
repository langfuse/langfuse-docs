...
If you are working with Node.js, Deno, or Edge functions, the `langfuse` library is the simplest way to integrate Langfuse into your application.

- The library queues calls to make them non-blocking.
- The SDK does not throw errors to protect your application process. Instead, it logs errors and warnings.
...
- [x] Node.js
- [x] Edge (e.g. Vercel, Cloudflare Workers)
- [x] Deno
- [x] Bun
...
Want to capture data (e.g. user feedback) from the browser? Use [LangfuseWeb](/docs/sdk/typescript-web)

## `LangfuseClient`

<Callout type="info" emoji="ℹ️">
  In short-lived environments (e.g. serverless functions), make sure to always
  call [`langfuse.shutdownAsync()`](#shutdownasync) at the end to await all
  pending requests.
</Callout>

### `new Langfuse()`

In your application, initialize the Langfuse client.

<Tabs items={["Parameters", "Environment Variables"]}>
...
You can enable debug mode to get detailed logs of what's happening in the SDK.
...
The Langfuse SDKs sends events asynchronously to the Langfuse server. When you want to exit your application, you should await `shutdownAsync` to exit cleanly before your application exits.
...
Create a trace in [Langfuse Tracing](/docs/tracing). Returns a [`TraceClient`](#traceclient) for further use.

```typescript
const trace = langfuse.trace(params);
```
...
| sessionId | string   | yes      | Used to group multiple traces into a [session](/docs/sessions) in Langfuse. Use your own session/thread identifier.                                                                     |
...
Created by [`langfuse.trace()`](#langfuse-trace).

The `TraceClient` is a convenience object to interact with traces. All methods are also available via [`LangfuseClient`](#langfuseclient) but then require the `traceId` to be passed.
...
Update the trace with new data. Params are the same as for [`langfuse.trace()`](#langfuse-trace) except for the `id` which cannot be updated.
...
Create an event in the trace. Returns an [`ObservationClient`](#observationclient) for further use.

```typescript
const event = trace.event(params);
```

### `.span()` [#trace-span]

Create an span in the trace. Returns an [`ObservationClient`](#observationclient) for further use.

```typescript
const span = trace.span(params);
```

### `.generation()` [#trace-generation]

Create an generation in the trace. Returns an [`ObservationClient`](#observationclient) for further use.

```typescript
const generation = trace.generation(params);
```
...
| usage               | object | yes      | The usage object supports the OpenAi structure with (`promptTokens`, `completionTokens`, `totalTokens`) and a more generic version (`input`, `output`, `total`, `unit`, `inputCost`, `outputCost`, `totalCost`) where unit can be of value `"TOKENS"` (default) or `"CHARACTERS"`, `"MILLISECONDS"`, `"SECONDS"`, `"IMAGES"`. Refer to the docs on how to [automatically calculate](https://langfuse.com/docs/model-usage-and-cost) tokens and costs by Langfuse. |
...
The usage object supports the OpenAi structure with `{'promptTokens', 'completionTokens', 'totalTokens'}` and a more generic version `{'input', 'output', 'total', 'unit'}` where unit can be of value `"TOKENS"` (default) or `"CHARACTERS"`, `"MILLISECONDS"`, `"SECONDS"`, `"IMAGES"`. For some models the token counts are [automatically calculated](https://langfuse.com/docs/model-usage-and-cost) by Langfuse. Create an issue to request support for other units and models.

...
<Tabs items={["Python SDK (v3)", "Python SDK (v2)", "JS/TS", "OpenTelemetry", "OpenAI (Python v2)", "Langchain (Python v2)", "Langchain (JS/TS)", "LlamaIndex (instrumentor)" ]}>
<Tab>
...
When using [OpenTelemetry](/docs/opentelemetry/get-started), masking is typically handled at the span processor or exporter level rather than through Langfuse-specific configuration. You can implement custom masking using OpenTelemetry span processors:

**Option 1: Custom Span Processor for Masking**
...
class MaskingSpanProcessor(SpanProcessor):
    ...
    def on_end(self, span: Span) -> None:
        # Mask sensitive attributes before processing
        if span.attributes:
            masked_attributes = {}
            for key, value in span.attributes.items():
                if isinstance(value, str) and value.startswith("SECRET_"):
                    masked_attributes[key] = "REDACTED"
                else:
                    masked_attributes[key] = value
            
            # Update span attributes
            span._attributes = masked_attributes
...
# Set up your OTLP exporter (configured for Langfuse)
otlp_exporter = OTLPSpanExporter(
    endpoint="https://cloud.langfuse.com/api/public/otel",
    headers={"Authorization": "Basic <your-encoded-credentials>"}
)

# Wrap the batch processor with masking
batch_processor = BatchSpanProcessor(otlp_exporter)
masking_processor = MaskingSpanProcessor(batch_processor)

# Configure tracer provider
tracer_provider = TracerProvider()
tracer_provider.add_span_processor(masking_processor)
trace.set_tracer_provider(tracer_provider)
```

**Option 2: Masking at Attribute Level**
...
with tracer.start_as_current_span("my-operation") as span:
    # Apply masking before setting attributes
    sensitive_data = "SECRET_USER_DATA"
    masked_data = mask_sensitive_data(sensitive_data)
    
    span.set_attribute("user.data", masked_data)
```

**Note on OpenTelemetry Masking:**
- OpenTelemetry masking happens at the instrumentation level before data is sent to exporters
- This approach ensures sensitive data is masked before being sent to any observability backend, including Langfuse
- Consider implementing masking processors for comprehensive data protection

Learn more about OpenTelemetry span processors in the [OpenTelemetry documentation](https://opentelemetry.io/docs/reference/specification/trace/sdk/#span-processor) and Langfuse integration in the [OpenTelemetry documentation](/docs/opentelemetry/get-started).
...
When using the [OpenAI SDK Integration](/docs/integrations/openai) with the Python SDK v2, set `openai.langfuse_mask` to the masking function:

...
<Tabs items={["Python SDK (v3)", "Python SDK (v2)", "JS/TS", "OpenTelemetry", "OpenAI (Python v2)", "OpenAI (JS/TS)", "Langchain (Python v2)", "Langchain (JS/TS)", "LlamaIndex (instrumentor)", "LlamaIndex (callback)", "Flowise"]}>
<Tab>
...
When using [OpenTelemetry](/docs/opentelemetry/get-started), you can add metadata using the `langfuse.metadata` span attribute or by prefixing keys with `langfuse.metadata.`:
...
with tracer.start_as_current_span("my-operation") as span:
    # Method 1: Set metadata as JSON string
    metadata = {"key": "value", "user_type": "premium", "version": "1.2.3"}
    span.set_attribute("langfuse.metadata", json.dumps(metadata))

    # Method 2: Set individual metadata keys
    span.set_attribute("langfuse.metadata.user_type", "premium")
    span.set_attribute("langfuse.metadata.version", "1.2.3")
    span.set_attribute("langfuse.metadata.feature_flag", "experimental")
```

You can also set metadata globally using resource attributes:

```python
import os
import json

# Set global metadata for all traces
global_metadata = {"environment": "production", "team": "backend"}
os.environ["OTEL_RESOURCE_ATTRIBUTES"] = f"langfuse.metadata={json.dumps(global_metadata)}"
```

**Supported OpenTelemetry attributes for metadata:**

- `langfuse.metadata` - JSON string containing metadata object
- `langfuse.metadata.*` - Individual metadata keys prefixed with `langfuse.metadata.`

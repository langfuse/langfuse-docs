...
When using the integration with the `@observe()` decorator (see [interop docs](/docs/integrations/llama-index/deprecated/get-started#interoperability-with-langfuse-sdk)), set the metadata via the `langfuse`:
...
def llama_index_fn(question: str):
    # Update context
    langfuse.update_current_trace(metadata={"key":"value"})
...
When using the integration with the `@observe()` decorator (see [interop docs](/docs/integrations/llama-index/deprecated/deprecated-llama-index-callback#interoperability-with-langfuse-sdk)), set the metadata via the `langfuse`:

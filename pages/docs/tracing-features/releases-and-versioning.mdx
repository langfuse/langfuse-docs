...
When using [OpenTelemetry](/docs/opentelemetry/get-started), you can set the release using the `langfuse.release` span attribute:
...
with tracer.start_as_current_span("my-operation") as span:
    # Set release using the Langfuse-specific attribute
    span.set_attribute("langfuse.release", "v2.1.24")
```

You can also set the release globally using resource attributes:

```python
import os

# Set release globally for all traces
os.environ["OTEL_RESOURCE_ATTRIBUTES"] = "langfuse.release=v2.1.24"
```

**Supported OpenTelemetry attributes for releases:**

- `langfuse.release` - String identifier for the application release (only applied to root spans)

Note: The `langfuse.release` attribute is only considered when set on the root span of a trace, as the release is a trace-level property in Langfuse.
...
<Tabs items={["Python SDK (v3)", "Python SDK (v2)", "JS/TS", "OpenTelemetry", "Langchain (Python)","Langchain (JS)"]}>
<Tab>
...
When using [OpenTelemetry](/docs/opentelemetry/get-started), you can set the version using the `langfuse.version` span attribute:
...
with tracer.start_as_current_span("my-operation") as span:
    # Set version using the Langfuse-specific attribute
    span.set_attribute("langfuse.version", "1.0")
```

You can also set the version globally using resource attributes:

```python
import os

# Set version globally for all traces
os.environ["OTEL_RESOURCE_ATTRIBUTES"] = "langfuse.version=1.0"
```

**Supported OpenTelemetry attributes for versions:**

- `langfuse.version` - String identifier for the version (can be applied to any span)

Note: Unlike release, the version can be set on any span in the trace, not just the root span. This allows for fine-grained versioning of specific operations within a trace.

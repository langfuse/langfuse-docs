...
<Tabs items={["Python SDK (v3)", "Python SDK (v2)", "JS/TS", "OpenTelemetry", "OpenAI (Python v2)", "OpenAI (JS/TS)", "Langchain (Python v2)", "Langchain (JS/TS)", "Vercel AI SDK (JS/TS)", "LlamaIndex (instrumentor)", "LlamaIndex (callback)"]}>
<Tab>
...
When using [OpenTelemetry](/docs/opentelemetry/get-started), sampling is typically handled by the OpenTelemetry SDK itself rather than Langfuse-specific configuration. However, you can configure sampling in several ways:

**Option 1: Using OpenTelemetry Trace Sampling**
...
# Configure a sampler that samples 50% of traces
sampler = TraceIdRatioBasedSampler(0.5)

# Set up tracer provider with the sampler
...
with tracer.start_as_current_span("my-operation") as span:
    # Only ~50% of these spans will be sampled
...
# Set sampling ratio via environment variable
os.environ["OTEL_TRACES_SAMPLER"] = "traceidratio"
os.environ["OTEL_TRACES_SAMPLER_ARG"] = "0.5"  # 50% sampling rate
```

**Note on Langfuse and OpenTelemetry Sampling:**
- OpenTelemetry sampling occurs at the SDK level before spans are sent to exporters
- If a span is not sampled by OpenTelemetry, it won't be sent to Langfuse at all
- This is different from Langfuse SDK sampling, which is Langfuse-specific

Learn more about OpenTelemetry sampling in the [OpenTelemetry documentation](https://opentelemetry.io/docs/concepts/sampling/) and Langfuse integration in the [OpenTelemetry documentation](/docs/opentelemetry/get-started).

...
When using [OpenTelemetry](/docs/opentelemetry/get-started), you can set the session ID using the `langfuse.session.id` span attribute:
...
with tracer.start_as_current_span("my-operation") as span:
    # Set session ID using the Langfuse-specific attribute
    span.set_attribute("langfuse.session.id", "chat-session-123")
```

You can also set the session ID globally using resource attributes:

```python
import os

# Set session ID globally for all traces
os.environ["OTEL_RESOURCE_ATTRIBUTES"] = "langfuse.session.id=chat-session-123"
```

**Supported OpenTelemetry attributes for sessions:**

- `langfuse.session.id` - String identifier for the session (only applied to root spans)

Note: The `langfuse.session.id` attribute is only considered when set on the root span of a trace, as the session ID is a trace-level property in Langfuse.

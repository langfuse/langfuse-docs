...
<Callout type="tip">
**Tip:** For chat applications, we recommend using one trace per user message (and the AI's response) and grouping them via a shared `sessionId` for the entire conversation. This way each turn's latency is measured separately and you can still replay the full chat. It's also easier in serverless or distributed setups than keeping one long trace open for the whole chat session.
</Callout>

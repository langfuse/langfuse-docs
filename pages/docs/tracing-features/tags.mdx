...
<Tabs items={["Python SDK (v3)", "Python SDK (v2)", "JS/TS", "OpenAI (Python)", "Langchain (Python)", "Langchain (JS/TS)", "LlamaIndex (instrumentor)", "LlamaIndex (callback)" ]}>
<Tab>
...
<Callout type="info">
  The LlamaIndex integration is not supported in the Python SDK v3. Please use a
  [community-maintained OTEL-based
  integration](/docs/integrations/llama-index/get-started) instead.
</Callout>

...
<Tabs items={["Python SDK (v3)", "Python SDK (v2)", "JS/TS", "OpenTelemetry", "OpenAI (Python)", "Langchain (Python)", "Langchain (JS/TS)", "LlamaIndex (instrumentor)", "LlamaIndex (callback)" ]}>
<Tab>
...
When using [OpenTelemetry](/docs/opentelemetry/get-started), you can set tags using the `langfuse.tags` span attribute:
...
with tracer.start_as_current_span("my-operation") as span:
    # Set tags using the Langfuse-specific attribute
    span.set_attribute("langfuse.tags", ["tag-1", "tag-2"])
```

You can also set tags globally using resource attributes:

```python
import os

# Set tags globally for all traces
os.environ["OTEL_RESOURCE_ATTRIBUTES"] = "langfuse.tags=production,api-v2"
```

**Supported OpenTelemetry attributes for tags:**

- `langfuse.tags` - Array of tag strings for the trace
...
When using the [LlamaIndex Integration](/docs/integrations/llama-index/deprecated/get-started), set the `tags` via the `instrumentor.observe()` context manager:
...
When using the integration with the `@observe()` decorator (see [interop docs](/docs/integrations/llama-index/deprecated/get-started#interoperability-with-langfuse-sdk)), set the tags via the `langfuse_context`:
...
When using the (deprecated) [LlamaIndex Callback Integration](/docs/integrations/llama-index/deprecated/deprecated-llama-index-callback), set the `tags` via `set_trace_params`. All LlamaIndex traces created after `set_trace_params` will include the `tags`. Learn more about `set_trace_params` [here](/docs/integrations/llama-index/deprecated/deprecated-llama-index-callback#set-trace-params).
...
When using the integration with the `@observe()` decorator (see [interop docs](/docs/integrations/llama-index/deprecated/deprecated-llama-index-callback#interoperability-with-langfuse-sdk)), set the tags via the `langfuse_context`:

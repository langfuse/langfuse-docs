...
<Tabs items={["Python SDK (v3)", "Python SDK (v2)", "JS/TS", "OpenTelemetry", "OpenAI (Python)", "Langchain (Python)", "Langchain (JS/TS)", "LlamaIndex (instrumentor)", "LlamaIndex (callback)", "Flowise"]}>
<Tab>
...
When using [OpenTelemetry](/docs/opentelemetry/get-started), you can set the user ID using the `langfuse.user.id` span attribute:
...
with tracer.start_as_current_span("my-operation") as span:
    # Set user ID using the Langfuse-specific attribute
    span.set_attribute("langfuse.user.id", "user_12345")
```

You can also set the user ID globally using resource attributes:

```python
import os

# Set user ID globally for all traces
os.environ["OTEL_RESOURCE_ATTRIBUTES"] = "langfuse.user.id=user_12345"
```

**Supported OpenTelemetry attributes for users:**

- `langfuse.user.id` - String identifier for the user (only applied to root spans)

Note: The `langfuse.user.id` attribute is only considered when set on the root span of a trace, as the user ID is a trace-level property in Langfuse.

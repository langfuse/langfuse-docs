...
When using the [LlamaIndex Integration](/docs/integrations/llama-index), set `user_id` via `set_trace_params`. All LlamaIndex traces created after `set_trace_params` will include the `user_id`. Learn more about `set_trace_params` [here](/docs/integrations/llama-index/get-started#set-trace-params).
...
When using the integration with the `@observe()` decorator (see [interop docs](/docs/integrations/llama-index/get-started#interoperability-with-langfuse-sdk)), set `user_id` via the `langfuse_context`:

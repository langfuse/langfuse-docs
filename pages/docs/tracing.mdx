...
- Each trace can contain multiple `observations` to log the individual steps of the execution.
  - Observations can be of different types
    ...
    - `Generations` are spans which are used to log generations of AI model. They contain additional attributes about the model, the prompt/completion. For generations, [token usage](/docs/token-usage) is automatically calculated.
  - Observations can be [nested](#nesting-of-observations).

## Ingestion

Follow the integration docs to send traces to Langfuse. You can use the Langchain integration, the Langfuse SDKs, or the API.

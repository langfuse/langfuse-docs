...
Langfuse now delivers faster product performance at scale. This preview brings faster charts, filters, APIs, and evaluation workflows to Langfuse Cloud.
...
Chart rendering time has been significantly reduced. Queries that previously took seconds or minutes now complete in a few seconds or milliseconds. Browsing traces, users, and sessions is also much faster, and filters respond more quickly in large projects.
...
[Observation-level evaluations](/docs/evaluation/evaluation-methods/llm-as-a-judge) execute in seconds instead of minutes as we do not need to query ClickHouse for each evaluation anymore.

...
**You can use Langfuse to trace the OpenAI Assistants API and receive observability.** The native integrations for OpenAI (Python & JS/TS) do not instrument the Assistants API natively as it shifts state to OpenAI (threads, etc.). However, you can still trace the API calls via our Python and JS integrations and visualize the results in Langfuse.

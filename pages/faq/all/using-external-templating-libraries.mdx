...
## When to use external templating

Consider using external templating libraries when you need the following to be managed from the prompt UI instead of in your application code:

- **Conditionals:** `{% if condition %}...{% endif %}`
- **Loops:** `{% for item in items %}...{% endfor %}`
- **Filters:** `{{ name | upper }}`
- **Complex logic:** Nested conditionals, macros, includes

For simple variable substitution, Langfuse's built-in `compile()` method is sufficient and provides a better experience with Playground and in-UI experiment integration.

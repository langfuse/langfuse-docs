...
<Callout type="info">
  Technically, we're creating fake/synthetic conversation examples here, not
  using real ones from actual users. Creating synthetic conversations is a
  different evaluation method that we'll explain separately in another post. For
  now, let's pretend these are real conversations from your chatbot.
</Callout>

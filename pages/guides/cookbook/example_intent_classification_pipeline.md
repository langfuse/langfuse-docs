...
You can approach intent classification in two ways: supervised and unsupervised. In a supervised approach, you provide a model with labeled training data, and the model will output one of the pre-defined labels when making predictions. In an unsupervised approach, the model attempts to find clusters within the data, which you can then label appropriately.
...
By following this notebook, you'll be able to automate the labeling and analysis of trace data, making it easier to manage and gain insights from your Langfuse projects.

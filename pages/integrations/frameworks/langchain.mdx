...
<Callout type="info" emoji="💡">
  Instrumenting LangGraph follows the same pattern. Simply pass the
  `langfuse_handler` to the agent invocation. ([Example
  Notebook](/guides/cookbook/integration_langgraph)).
</Callout>
...
<Callout type="info" emoji="💡">
  Instrumenting LangGraph follows the same pattern. Simply pass the
  `langfuseHandler` to the agent invocation.
</Callout>
...
```python /with langfuse.start_as_current_observation(as_type="span", name="multi-step-process") as root_span:/ /prep_span.update/ /root_span.update_trace/ /post_span.update/ /root_span.update_trace/

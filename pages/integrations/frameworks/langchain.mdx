...
Langfuse integrates with Langchain using Langchain Callbacks ([Python](https://python.langchain.com/docs/how_to/#callbacks), [JS](https://js.langchain.com/docs/how_to/#callbacks)). The Langfuse SDK automatically captures detailed traces of your Langchain executions, creating properly nested observations for chains, LLMs, tools, and retrievers. This allows you to monitor, analyze and debug your LangChain applications with full observability.

> **What is Langchain?** [Langchain](https://python.langchain.com/docs/get_started/introduction/) is an open-source framework that helps developers build applications powered by large language models (LLMs) by providing tools to connect models with external data, APIs, and logic. It simplifies building complex AI workflows like chatbots, agents, and retrieval-augmented generation (RAG) pipelines.

> **What is Langgraph?** [Langgraph](https://python.langchain.com/docs/get_started/introduction/) LangGraph is a framework built on top of LangChain that makes it easier to design and run stateful, multi-step AI agents using a graph-based architecture.
...
>
  ...
  ```
  With the environment variables set, we can now initialize the Langfuse client and the Langfuse CallbackHandler. You can also use [constructor arguments](/docs/sdk/python/setup#initialize-client) to initialize the Langfuse client.
  ...
  ### Step 3a: Basic LangChain Example

  Let's create a simple LangChain example. We'll use the `ChatOpenAI` model and the `ChatPromptTemplate` to create a chain.
...
There are multiple ways to score a trace in Python SDK. See [Scoring documentation](/docs/scores/overview) for more details.

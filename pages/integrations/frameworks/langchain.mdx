...
<Callout type="info">
  ...
  here.](https://python-sdk-v2.docs-snapshot.langfuse.com/integrations/frameworks/langchain)
</Callout>
...
- **Trace attributes** (like `user_id`, `session_id`, `metadata`) can be set either:
  - Via metadata fields in chain invocation (`langfuse_user_id`, `langfuse_session_id`, `langfuse_tags`)
  - Via `propagate_attributes()` context manager as shown in the examples below
...
**Option 2: Via `propagate_attributes()` (for more control):**
...
# Set trace attributes dynamically via propagate_attributes
with langfuse.start_as_current_span(name="dynamic-langchain-trace") as span:
    # ✅ Set attributes on current span AND all children (including Langchain spans)
    with propagate_attributes(
        user_id="random-user",
        session_id="random-session",
        metadata={
            "langchain_version": "0-3-0",
            "chain_type": "llm-chain"
        }
    ):
        span.update(input={"animal": "dog"})
...
@observe(name="user-query-processing") # Automatically log function as a trace to Langfuse
def process_user_query(user_input: str, user_id: str, session_id: str):
    langfuse = get_client()
...
# 3. Option 2: Wrap LangChain execution with propagate_attributes (for more control)
# from langfuse import propagate_attributes
# with langfuse.start_as_current_span(name="tell-joke") as span:
#     with propagate_attributes(user_id="user_123"):
#         span.update(input={"topic": "cats"})
#         response = chain.invoke({"topic": "cats"}, config={"callbacks": [handler]})
#         span.update(output={"joke": response.content})

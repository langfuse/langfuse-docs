...
There are two ways to integrate Langfuse with OpenRouter:

**Broadcast (No Code)**: OpenRouter's [Broadcast feature](https://openrouter.ai/docs/guides/features/broadcast/overview) automatically sends traces to Langfuse. Enable it in your [OpenRouter settings](https://openrouter.ai/settings) by connecting your Langfuse API keys. This works with any OpenRouter request and requires no code changes.

**SDK Integration**: Use the Langfuse OpenAI SDK wrapper for advanced features like nested tracing, custom metadata, and full control over trace data. The rest of this guide covers the SDK integration method.

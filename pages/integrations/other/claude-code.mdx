...
def main():
    ...
    # Get the Langfuse client (uses LANGFUSE_* environment variables)
    langfuse = get_client()
    ...
    with langfuse.start_as_current_observation(
        ...
        with propagate_attributes(
            ...
            metadata={"source": "claude-code", "hook_type": "PostToolUse"},
        ):
            # Update the span with tool output
            span.update(
                output=tool_output,
                metadata={"tool_name": tool_name},
            )

    # Flush to ensure data is sent (important for short-lived scripts)
    langfuse.flush()
...
```bash
# Langfuse Configuration (used by get_client())
...
def main():
    ...
    with langfuse.start_as_current_observation(
        ...
        with propagate_attributes(
            session_id=session_id,
            metadata={"source": "claude-code", "event": "session_end"},
        ):
            span.update(output={"status": "session_ended"})

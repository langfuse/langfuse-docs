...
Email/password authentication is enabled by default. Users can sign up and log in using their email and password. To disable email/password authentication, set `AUTH_DISABLE_USERNAME_PASSWORD=true`. In this case, you need to set up [SSO](#sso) instead.
...
If you want to programmatically create a default user, check out the [Headless Initialization](/self-hosting/headless-initialization) documentation.

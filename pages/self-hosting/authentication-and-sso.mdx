...
- [#4764](https://github.com/orgs/langfuse/discussions/4764): Langfuse uses email to identify users. Thus, you need to add the `email` claim in the token configurationn and all users must have an `Email` in their user profile.
...
| Optional Variables | There are no optional variables for the WorkOS provider. |
...
- When enabling WorkOS as an identity provider, two buttons will be shown on the login page: "WorkOS (organization)" and "WorkOS (connection)". Users will need to provide their organization or connection ID in order to sign in.

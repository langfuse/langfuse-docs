...
<Callout type="info">
  Looking for a managed solution? Use [Langfuse
...
When self-hosting Langfuse, you run the same infrastructure that powers Langfuse Cloud without any scalability limitations. Read ["Why Langfuse?"](/why) and [open source philosophy](/handbook/chapters/open-source) to learn more about why this is really important to the Langfuse team.

Almost all Langfuse features are MIT licensed. Some add-on features targeted at large-scale platform teams require a [license key](/self-hosting/license-key).

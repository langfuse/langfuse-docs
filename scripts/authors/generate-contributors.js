...
const resolveContributor = async (email, commitSha) => {
  ...
  const commitData = await fetchFromGitHub(
    `/repos/${CONFIG.github.repo}/commits/${commitSha}`,
  );
  ...
  for (let i = 0; i < items.length; i += CONFIG.github.batchSize) {
    ...
    if (i + CONFIG.github.batchSize < items.length) {
      await new Promise((resolve) =>
        setTimeout(resolve, CONFIG.github.batchDelay),
      );
    }
  }
  return results;
...
async function analyzeContributors() {
  ...
  const gitPaths = CONFIG.sections
    .flatMap((section) => section.gitPaths ?? [section.gitPath])
    .join(" ");
  ...
  for (const line of gitOutput.trim().split("\n")) {
    if (line.includes("|")) {
      ...
      const usernameFromEmail = extractGitHubUsernameFromEmail(
        currentCommit.email,
      );

      if (
        !usernameFromEmail &&
        !emailToUsernameCache.has(currentCommit.email)
      ) {
      ...
      CONFIG.sections.some((section) =>
        (section.gitPaths ?? [section.gitPath]).some((gp) =>
          line.startsWith(gp),
        ),
      ) &&
      !line.includes("_meta.")
  ...
  console.log(
    `📊 Processing ${commits.length} file-commit pairs from ${new Set(commits.map((c) => c.hash)).size} commits`,
  );

  // Batch fetch GitHub data for unique emails that need it
  if (emailsNeedingAPI.size > 0) {
    console.log(
      `🚀 Fetching GitHub data for ${emailsNeedingAPI.size} unique emails...`,
    );
    ...
    for (const commit of commits) {
      if (
        emailsNeedingAPI.has(commit.email) &&
        !emailToCommitSha.has(commit.email)
      ) {
    ...
    console.log(
      `✅ GitHub API completed in ${((Date.now() - fetchStart) / 1000).toFixed(2)}s`,
    );
    console.log(
      `📦 Cached ${emailToUsernameCache.size} email-to-username mappings`,
    );
  }
  ...
  for (const commit of commits) {
    ...
    if (contributor && !CONFIG.excludedContributors.includes(contributor)) {
      ...
      if (
        !fileContributors[urlPath][contributor] ||
        commit.date > fileContributors[urlPath][contributor]
      ) {
  ...
  const findPages = (dir, basePath = "", sectionPrefix = "/docs") => {
    ...
    for (const item of items) {
      if (item.isDirectory() && !item.name.startsWith(".")) {
        ...
        item.isFile() &&
        /\.(mdx?|md)$/.test(item.name) &&
        !item.name.includes("_meta.")
      ) {
        const urlPath = `${sectionPrefix}/${path
  ...
  for (const [urlPath, contributorDates] of Object.entries(fileContributors)) {
    if (currentPages.has(urlPath)) {
      result[urlPath] = Object.keys(contributorDates).sort(
        (a, b) => contributorDates[b] - contributorDates[a],
      );
    }
  }

  console.log(
    `🔍 Generated ${Object.keys(result).length} pages with ${[...new Set(Object.values(result).flat())].length} unique contributors`,
  );
  console.log(
...
async function main() {
  try {
    ...
    if (fs.existsSync(CONFIG.contributors)) {
      ...
      console.log(
        `✅ Merged contributors.json (${Object.keys(merged).length} pages)`,
      );
    } else {

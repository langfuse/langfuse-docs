...
async function fetchFromGitHub(endpoint) {
  ...
  try {
    const response = await fetch(`${GITHUB_CONFIG.apiBase}${endpoint}`, {
      headers,
    });
...
const resolveContributor = async (email, commitSha) => {
  ...
  const commitData = await fetchFromGitHub(
    `/repos/${GITHUB_CONFIG.repo}/commits/${commitSha}`
  );
  ...
  for (let i = 0; i < items.length; i += GITHUB_CONFIG.batchSize) {
    ...
    if (i + GITHUB_CONFIG.batchSize < items.length) {
      await new Promise((resolve) =>
        setTimeout(resolve, GITHUB_CONFIG.batchDelay)
      );
    }
  }
  return results;
...
async function analyzeContributors() {
  ...
  for (const line of gitOutput.trim().split("\n")) {
    if (line.includes("|")) {
      ...
      line.trim() &&
      currentCommit &&
      (line.startsWith("pages/docs/") ||
        line.startsWith("pages/self-hosting/") ||
        line.startsWith("pages/security/")) &&
      !line.includes("_meta.")
    ) {
      commits.push({
        ...currentCommit,

...
function writeRedirectsFile(content) {
  ...
  if (redirectCount > 1900) {
    console.warn(
      `⚠️  Warning: Approaching Cloudflare Pages redirect limit (2,100 total, you have ${redirectCount})`,
    );
  }
...
async function main() {
  ...
  try {
    ...
    console.log(
      `📝 Total redirects (all converted to 302): ${nonPermanentRedirects.length + permanentRedirects.length}`,
    );

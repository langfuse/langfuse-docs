...
standalone_mdx_closing_tag_regex = re.compile(r"^</(?:[A-Z][A-Za-z0-9]*|details)>$")
markdown_list_item_regex = re.compile(r"^\s*(?:[-*+]\s+|\d+[.)]\s+)")
...
def ensure_frontmatter_title(content):
    frontmatter_match = re.match(r"^(---\n)(.*?)(\n---\n)", content, re.DOTALL)
    if not frontmatter_match:
        return content

    title_match = re.search(
        r"^title:\s*(.*)$", frontmatter_match.group(2), re.MULTILINE
    )
    if (
        title_match
        and title_match.group(1).strip()
        and not title_match.group(1).lstrip().startswith("#")
    ):
        return content

    h1_match = re.search(
        r"^#\s+(.+?)\s*$", content[frontmatter_match.end() :], re.MULTILINE
    )
    if not h1_match:
        return content

    frontmatter = frontmatter_match.group(2)
    title_line = f"title: {yaml_quote(h1_match.group(1).strip())}"

    if title_match:
        frontmatter = (
            frontmatter[: title_match.start()]
            + title_line
            + frontmatter[title_match.end() :]
        )
    else:
        frontmatter = title_line + "\n" + frontmatter
...
def separate_standalone_mdx_closing_tags(content):
    ...
    for line in lines:
        if (
            separated_lines
            and markdown_list_item_regex.match(separated_lines[-1])
            and line.strip()
            and not line.startswith((" ", "\t"))
            and not markdown_list_item_regex.match(line)
        ):
            separated_lines.append("")
        if (
            standalone_mdx_closing_tag_regex.match(line.strip())
            and separated_lines
    ...
    trailing_newline = "\n" if content.endswith("\n") else ""
    return "\n".join(separated_lines) + trailing_newline
...
def transform_content_to_mdx(markdown_content):
    ...
    def replace_tabs_block_match(match_obj):
        ...
        try:
            ...
            print(
                f"Warning: Could not parse 'items' attribute from TABS_START comment: {items_list_str_from_comment}"
            )
        ...
        for item_match in tab_item_pattern.finditer(inner_content_of_tabs_block):
            ...
            mdx_individual_tab_components.append(
                f"<Tab>\n{content_of_tab_item}\n</Tab>"
            )
        if not found_tab_items:
            print(
                f"Warning: TABS_START/TABS_END block found but no TAB_ITEM_START/END children. Original content kept."
            )
            return match_obj.group(0)
        items_prop_for_mdx = json.dumps(parsed_items_for_tabs_prop)
        return (
            f"<Tabs items={{{items_prop_for_mdx}}}>\n"
            + "\n".join(mdx_individual_tab_components)
            + "\n</Tabs>"
        )

    content_for_processing = master_tabs_pattern.sub(
        replace_tabs_block_match, content_for_processing
    )
    ...
    def replace_callout_block_match(match_obj):
        ...
        for attr_match in attribute_parser_regex.finditer(
            attributes_string_from_comment
        ):
            key = attr_match.group(1)
            value = attr_match.group(2)
    ...
    content_for_processing = callout_block_pattern.sub(
        replace_callout_block_match, content_for_processing
    )
    ...
    steps_simplified_pattern = re.compile(
        r"<!--\s*STEPS_START\s*-->(.*?)<!--\s*STEPS_END\s*-->", re.DOTALL | re.MULTILINE
    )
    ...
    content_for_processing = steps_simplified_pattern.sub(
        replace_steps_simplified_match, content_for_processing
    )
    ...
    content_for_processing = component_comment_pattern.sub(
        replace_component_comment, content_for_processing
    )

    final_assembled_content = generated_yaml_frontmatter + content_for_processing
    lines = final_assembled_content.splitlines(keepends=True)
    hard_break_safe_lines = []
    in_code_fence = False
    for line in lines:
        newline = ""
        line_body = line
        ...
        if not in_code_fence and line_body.strip() and re.search(r" {2,}$", line_body):
            line_body = re.sub(r" {2,}$", r"\\", line_body)

        hard_break_safe_lines.append(line_body + newline)
    final_assembled_content = "".join(hard_break_safe_lines)
    final_assembled_content = separate_standalone_mdx_closing_tags(
        final_assembled_content
    )
    final_assembled_content = ensure_frontmatter_title(final_assembled_content)
...
for mapping in mappings:
    ...
    processed_content_final, was_transformed_final, metadata_found = (
        transform_content_to_mdx(original_md_content)
    )
    ...
    if docs_path_value is not None:
        ...
        if len(path_parts) > 1:
            ...
            dest_path_str = os.path.join(
                "content", dir_path_from_docs_path, final_docs_path_filename_part
            )
        else:
    ...
    if mapping.get("isGuide", True):
        ...
        guide_dest_path_str = os.path.join(
            "content", "guides", "cookbook", guide_dest_filename
        )
    ...
    for full_destination_path in destination_paths_to_write:
        ...
        print(
            f"Processing: {notebook_filename} -> {full_destination_path} (Overall Transformed: {was_transformed_final}, Metadata Found: {metadata_found})"
        )
        try:
